...
static void say(const char *s) { (void)Schrijven(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *systeemidentiteit, int pass)
{
...
int main(void)
{
    ...
    report("recvfrom", recvfrom(server, overdrachtsbuffer_2, sizeof(overdrachtsbuffer_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       overdrachtsbuffer_2[0] == 'p' && overdrachtsbuffer_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, overdrachtsbuffer_2, sizeof(overdrachtsbuffer_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Sluiten(client) == 0);
    report("close-server", Sluiten(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
