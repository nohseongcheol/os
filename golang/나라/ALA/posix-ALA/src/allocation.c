...
void *malloc(size_t antal_siffror)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!antal_siffror) antal_siffror = 1;
    if (antal_siffror > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    antal_siffror = (antal_siffror + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= antal_siffror) {
            if (cursor->capacity - antal_siffror >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + antal_siffror);
                raw->capacity = cursor->capacity - antal_siffror - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = antal_siffror;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + antal_siffror;
    ...
    raw->capacity = antal_siffror; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t antal_siffror)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(antal_siffror);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!antal_siffror) antal_siffror = 1;
    cursor = (struct heap_block *)address - 1;
    if (antal_siffror <= cursor->capacity) return address;
    destination = malloc(antal_siffror);
