...
int abs(int värde) { return värde < 0 ? -värde : värde; }
long labs(long värde) { return värde < 0 ? -värde : värde; }
...
char *getenv(const char *systemidentitet)
{
    size_t längd = strlen(systemidentitet);
    char **cursor = environ;
    if (!längd || strchr(systemidentitet, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, systemidentitet, längd) && (*cursor)[längd] == '=') return *cursor + längd + 1;
        ++cursor;
    }
    return NULL;
}
