...
static void say(const char *s) { (void)Skriv(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *systemidentitet, int pass)
{
...
int main(void)
{
    ...
    report("recvfrom", recvfrom(server, överföringsbuffert_2, sizeof(överföringsbuffert_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       överföringsbuffert_2[0] == 'p' && överföringsbuffert_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, överföringsbuffert_2, sizeof(överföringsbuffert_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Stäng(client) == 0);
    report("close-server", Stäng(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
