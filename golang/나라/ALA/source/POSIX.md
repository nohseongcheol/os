...
freestanding library in `./användarutrymme/posix`.
...
```sh
cd ./användarutrymme/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./användarutrymme/posix/deklarationer` and link the archive into an EngOS i386 user
...
`./användarutrymme/posix/TEST_RESULTS.md`. Build all runtime test executables
