...
ssize_t Leximi(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Leximi, fd, buf, count));
}

ssize_t Shkrimi(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Shkrimi, fd, buf, count));
}

int Mbyll(int fd)
{
    return (int)__syscall_result(SC1(SYS_Mbyll, fd));
}
