...
ssize_t Lectura(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Lectura, fd, buf, count));
}

ssize_t Escriptura(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Escriptura, fd, buf, count));
}

int Tanca(int fd)
{
    return (int)__syscall_result(SC1(SYS_Tanca, fd));
}
