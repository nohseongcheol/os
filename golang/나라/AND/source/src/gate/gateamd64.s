...
GLOBL ·identificadortDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·identificadort<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·installidt(SB),NOSPLIT,$0
	LEAQ ·identificadortDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·identificadort<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·GestorInterrupció(SB),NOSPLIT,$0-10
	...
	LEAQ ·interrupciógateentrada(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	...
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchInterrupció(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchInterrupció(SB),NOSPLIT,$0
...
TEXT ·interrupciógateentrada(SB),NOSPLIT,$0
