...
void *memcpy(void *destination, const void *source, size_t longitud)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    while (longitud--) *cursor++ = *input++;
    return destination;
}

void *memmove(void *destination, const void *source, size_t longitud)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    if ((uintptr_t)destination <= (uintptr_t)source)
        return memcpy(destination, source, longitud);
    while (longitud) { --longitud; cursor[longitud] = input[longitud]; }
    return destination;
}

void *memset(void *destination, int character, size_t longitud)
{
    unsigned char *cursor = destination;
    while (longitud--) *cursor++ = (unsigned char)character;
    return destination;
}

int memcmp(const void *left, const void *right, size_t longitud)
{
...
void *memchr(const void *source, int character, size_t longitud)
{
...
{
    size_t longitud = 0;
    while (longitud < limit && text[longitud]) ++longitud;
    return longitud;
}
...
{
    size_t longitud = strnlen(source, limit);
    memcpy(destination, source, longitud);
    memset(destination + longitud, 0, limit - longitud);
    return destination + longitud;
}
...
{
    size_t longitud = strlen(source);
    if (limit) {
        size_t copy_length = longitud < limit ? longitud : limit;
        memcpy(destination, source, copy_length);
        if (longitud < limit) destination[longitud] = 0;
    }
    return longitud;
}
...
{
    size_t longitud = strlen(source);
    if (!longitud) return (char *)text;
    while (*text) {
        if (strncmp(text, source, longitud) == 0) return (char *)text;
        ++text;
    }
    return NULL;
}
...
{
    size_t longitud = 0;
    while (text[longitud] && strchr(delimiters, text[longitud])) ++longitud;
    return longitud;
}
...
{
    size_t longitud = strnlen(text, limit);
    char *destination = malloc(longitud + 1);
    if (destination) { memcpy(destination, text, longitud); destination[longitud] = 0; }
    return destination;
}
