...
ssize_t Ընթերցում(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Ընթերցում, fd, buf, count));
}

ssize_t Գրել(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Գրել, fd, buf, count));
}

int Փակել(int fd)
{
    return (int)__syscall_result(SC1(SYS_Փակել, fd));
}
