...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Tusi(0xA0)
	} else {
		self.deviceport.Tusi(0xB0)
	}
	self.controlport.Tusi(0)
	self.deviceport.Tusi(0xA0)
	...
	self.sectorcountport.Tusi(0)
	self.lbalowport.Tusi(0)
	self.lbamidport.Tusi(0)
	self.lbahiport.Tusi(0)
	self.commandport.Tusi(0xEC)
...
func (self *TAdvancedtechnologyattachment) Faitau28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.deviceport.Tusi(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Tusi(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Tusi(0)
	self.sectorcountport.Tusi(1)

	self.lbalowport.Tusi(uint8(sector & 0x000000FF))
	self.lbamidport.Tusi(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Tusi(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Tusi(0x20)
...
func (self *TAdvancedtechnologyattachment) Tusi28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Tusi(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Tusi(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Tusi(0)
	self.sectorcountport.Tusi(1)
	self.lbalowport.Tusi(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Tusi(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Tusi(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Tusi(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Tusi(0xE0)
	} else {
		self.deviceport.Tusi(0xF0)
	}
	self.commandport.Tusi(0xE7)
