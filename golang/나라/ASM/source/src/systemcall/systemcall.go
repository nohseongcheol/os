...
func Sysprintstr(buffer string) {
	...
	Syscall(Systusi, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Systusi, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfaitau:
		...
		return esp
	case Systusi:
		cpu.Eax = uint32(systusi(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|otusionly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: otusionly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: otusionly}
}
...
func systusi(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == otusionly || accessmode == ofaitautusi || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func Stdinputbyte(c byte) {
	next := (stdintusi + 1) % uint32(len(stdinbuffer))
	if next == stdinfaitau {
		return
	}
	stdinbuffer[stdintusi] = c
	stdintusi = next
}

func stdingetblocking() byte {
	for stdinfaitau == stdintusi {
