...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	autūthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if autūthread != nil {
		autūthread.Pid = process.id
		autūthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(autūthread)))
	}
