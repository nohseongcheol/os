...
var attributecontrollertusiport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Tusiregister(register []byte) {
	var regindex uint16 = 0

	Porttusibyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Porttusibyte(sequencerindexport, i)
		Porttusibyte(sequencerdataport, register[regindex])
		regindex++
	}

	Porttusibyte(crtcindexport, 0x03)

	Porttusibyte(crtcdataport, (Portfaitaubyte(crtcdataport) | 0x80))
	Porttusibyte(crtcindexport, 0x11)
	Porttusibyte(crtcdataport, (Portfaitaubyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Porttusibyte(crtcindexport, i)
		Porttusibyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Porttusibyte(graphicscontrollerindexport, i)
		Porttusibyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portfaitaubyte(attributecontrollerresetport)
		Porttusibyte(attributecontrollerindexport, i)
		Porttusibyte(attributecontrollertusiport, register[regindex])
		regindex++
	}

	Portfaitaubyte(attributecontrollerresetport)
	Porttusibyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Porttusibyte(graphicscontrollerindexport, 0x06)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Tusiregister(g320x200x256)
