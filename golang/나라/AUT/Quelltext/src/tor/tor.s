...
GLOBL ·kennungtDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·kennungt<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
GLOBL ·torHandlers<>(SB), NOPTR, $NUM_IDT_ENTRIES*8
...
TEXT ·installierenidt(SB),NOSPLIT,$0
	LEAQ ·kennungtDescriptor<>(SB), AX
...
TEXT ·GriffUnterbrechung(SB),NOSPLIT,$0-10
	...
	LEAQ ·unterbrechungTorEintrag(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	SUBQ $16, SP;                          \
	MOVQ R15, 0(SP);                       \
	MOVQ ·torHandlers<>+8*num(SB), R15;   \
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchUnterbrechung(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchUnterbrechung(SB),NOSPLIT,$0
...
TEXT ·unterbrechungTorEintrag(SB),NOSPLIT,$0
