...
#define CHECK(Wert) do { if (!(Wert)) return __LINE__; } while (0)

static int compare_int(const void *left, const void *right)
{
    int result = *(const int *)left, Wert = *(const int *)right;
    return (result > Wert) - (result < Wert);
}

int __posix_library_test(void)
{
    char Übertragungspuffer_2[64], source[64], *end_pointer, *saved_state, *address, *destination;
    const char *text = "abca", *input = " 	-0x80rest";
    int elements[257], index, Wert;
    size_t Länge;
    div_t result;
    ldiv_t decoded;
    ...
    CHECK(memset(Übertragungspuffer_2, 0xab, sizeof(Übertragungspuffer_2)) == Übertragungspuffer_2);
    CHECK((unsigned char)Übertragungspuffer_2[63] == 0xab);
    CHECK(memcpy(Übertragungspuffer_2, "abcdef", 7) == Übertragungspuffer_2);
    CHECK(memmove(Übertragungspuffer_2 + 2, Übertragungspuffer_2, 5) == Übertragungspuffer_2 + 2);
    CHECK(memcmp(Übertragungspuffer_2, "ababcde", 7) == 0);
    CHECK(memmove(Übertragungspuffer_2, Übertragungspuffer_2 + 2, 5) == Übertragungspuffer_2);
    CHECK(memcmp(Übertragungspuffer_2, "abcde", 5) == 0);
    CHECK(memmove(Übertragungspuffer_2, Übertragungspuffer_2, 0) == Übertragungspuffer_2);
    CHECK(memcmp("\x80", "\x7f", 1) > 0);
    CHECK(memcmp(Übertragungspuffer_2, source, 0) == 0);
    ...
    CHECK(strcpy(Übertragungspuffer_2, text) == Übertragungspuffer_2 && strcmp(Übertragungspuffer_2, text) == 0);
    CHECK(stpcpy(Übertragungspuffer_2, "xy") == Übertragungspuffer_2 + 2 && Übertragungspuffer_2[2] == 0);
    memset(Übertragungspuffer_2, 7, sizeof(Übertragungspuffer_2));
    CHECK(strncpy(Übertragungspuffer_2, "xy", 5) == Übertragungspuffer_2);
    CHECK(!memcmp(Übertragungspuffer_2, "xy\0\0\0", 5) && Übertragungspuffer_2[5] == 7);
    CHECK(stpncpy(Übertragungspuffer_2, "abcdef", 3) == Übertragungspuffer_2 + 3 && Übertragungspuffer_2[3] == 0);
    CHECK(stpncpy(Übertragungspuffer_2, "a", 4) == Übertragungspuffer_2 + 1 && Übertragungspuffer_2[3] == 0);
    CHECK(strncpy(Übertragungspuffer_2, "z", 0) == Übertragungspuffer_2 && Übertragungspuffer_2[0] == 'a');
    strcpy(Übertragungspuffer_2, "a");
    CHECK(strcat(Übertragungspuffer_2, "bc") == Übertragungspuffer_2 && !strcmp(Übertragungspuffer_2, "abc"));
    CHECK(strncat(Übertragungspuffer_2, "def", 2) == Übertragungspuffer_2 && !strcmp(Übertragungspuffer_2, "abcde"));
    CHECK(strncat(Übertragungspuffer_2, "xyz", 0) == Übertragungspuffer_2 && !strcmp(Übertragungspuffer_2, "abcde"));
    ...
    CHECK(strxfrm(Übertragungspuffer_2, "abc", 4) == 3 && !strcmp(Übertragungspuffer_2, "abc"));
    ...
    strcpy(Übertragungspuffer_2, ",,one;two,,");
    CHECK(!strcmp(strtok_r(Übertragungspuffer_2, ",;", &saved_state), "one"));
    strcpy(source, "x:y");
    ...
    for (Wert = -1; Wert <= 255; ++Wert) {
        int low = Wert >= 'a' && Wert <= 'z', high = Wert >= 'A' && Wert <= 'Z';
        int digit = Wert >= '0' && Wert <= '9';
        CHECK(!!isalpha(Wert) == (low || high));
        CHECK(!!isalnum(Wert) == (low || high || digit));
        CHECK(!!isdigit(Wert) == digit);
        CHECK(!!islower(Wert) == low && !!isupper(Wert) == high);
        CHECK(!!isblank(Wert) == (Wert == ' ' || Wert == '\t'));
        CHECK(!!isspace(Wert) == (Wert == ' ' || (Wert >= 9 && Wert <= 13)));
        CHECK(!!iscntrl(Wert) == ((Wert >= 0 && Wert < 32) || Wert == 127));
        CHECK(!!isprint(Wert) == (Wert >= 32 && Wert <= 126));
        CHECK(!!isgraph(Wert) == (Wert >= 33 && Wert <= 126));
        CHECK(!!ispunct(Wert) == (Wert >= 33 && Wert <= 126 && !low && !high && !digit));
        CHECK(!!isxdigit(Wert) == (digit || (Wert >= 'a' && Wert <= 'f') || (Wert >= 'A' && Wert <= 'F')));
        CHECK(tolower(Wert) == (high ? Wert + 32 : Wert));
        CHECK(toupper(Wert) == (low ? Wert - 32 : Wert));
    }
    errno = EIO;
    ...
    for (Länge = 0; Länge <= 257; ++Länge) {
        for (index = 0; index < (int)Länge; ++index) elements[index] = ((index * 37 + (int)Länge) % 19) - 9;
        qsort(elements, Länge, sizeof(int), compare_int);
        for (index = 1; index < (int)Länge; ++index) CHECK(elements[index - 1] <= elements[index]);
        for (index = 0; index < (int)Länge; ++index) {
            int *found = bsearch(&elements[index], elements, Länge, sizeof(int), compare_int);
            CHECK(found && *found == elements[index]);
        }
        Wert = 100;
        CHECK(bsearch(&Wert, elements, Länge, sizeof(int), compare_int) == NULL);
    }
    environ = envp;
    ...
    destination = Speicherende_verschieben(0);
    for (index = 0; index < 100; ++index) {
        address = malloc(1024); CHECK(address); memset(address, index, 1024); free(address);
    }
    CHECK(Speicherende_verschieben(0) == destination);
    return 0;
}
