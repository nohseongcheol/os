...
ssize_t Oxuma(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Oxuma, fd, buf, count));
}

ssize_t Yazma(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Yazma, fd, buf, count));
}

int Bağla(int fd)
{
    return (int)__syscall_result(SC1(SYS_Bağla, fd));
}
