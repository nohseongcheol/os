...
static void test_open_and_io(void)
{
    ...
    fd = Aç("/USER2", O_RDONLY);
    report("open-readonly", fd >= 3);
    if (fd >= 0) {
        report("read-bytes", Oxuma(fd, bytes, 2) == 2 &&
        ...
        report("close-file", Bağla(fd) == 0);
        errno = 0;
        report("closed-fd", failed_with(Bağla(fd), EBADF));
    }
    ...
    if (root >= 0) {
        ...
        errno = 0;
        report("read-directory", Oxuma(root, bytes, 1) == -1 && errno == EISDIR);
        (void)Bağla(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", Yazma(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", Oxuma(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", Oxuma(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("close-ebadf", failed_with(Bağla(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", Oxuma(fd, &first, 1) == 1 && Oxuma(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Bağla(fd) == 0 && fstat(copy, &st) == 0);
        fd = copy;
    }
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &status) == child) {
        report("fork-fd-isolation", Oxuma(fd, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
