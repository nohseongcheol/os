...
type TMasterAçılışrecord struct {
	açılışloader	[440]byte
	signature	uint32
	unused		uint16
...
type Tmsdospartitiontable struct {
	Mbr TMasterAçılışrecord
}

func (self *Tmsdospartitiontable) Oxumapartition(hd *TƏtraflıtechnologyattachment) {

	console_2 := TConsole{}
	console_2.MÇapEt(([]byte)("Reading MBR"))

	var partitionBayt [512]byte
	var buffer_2 = partitionBayt[:]
	hd.Oxuma28(0, &buffer_2, 512)

	self.Mbr = TMasterAçılışrecord{}
	var i int = 0
	for ; i < 440; i++ {
		self.Mbr.açılışloader[i] = partitionBayt[i]
	}
	...
	if self.Mbr.magicnumber != 0xAA55 {
		console_2.MÇapEt(([]byte)("illegal MBR"))
		return
	}
