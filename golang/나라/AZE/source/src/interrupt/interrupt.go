...
func TestÇapEt(position uint8, data uint8)
func setds(dssegment uint32)
func setgs(gssegment uint32)
func interruptÇıxloop()
...
func Yeniinterrupthandler(Interruptmanager uintptr, Interruptnumber uint8) *TInterrupthandler {
...
var Fəalinterruptmanager uintptr = 0

const interruptXətaHəlli = false
...
var PrimarypicƏmrioQapı uint16 = 0x20
var PrimarypicdataioQapı uint16 = 0x21
var SecondarypicƏmrioQapı uint16 = 0xA0
var SecondarypicdataioQapı uint16 = 0xA1
...
func (self *TInterruptmanager) Init(hardwareinterruptoffset uint16, globaldescriptortable *TShareddescriptortable, taskmanager *TTaskmanager) {
	...
	QapıYazmabyte(PrimarypicƏmrioQapı, 0x11)
	QapıYazmabyte(SecondarypicƏmrioQapı, 0x11)

	QapıYazmabyte(PrimarypicdataioQapı, 0x20)
	QapıYazmabyte(SecondarypicdataioQapı, 0x28)

	QapıYazmabyte(PrimarypicdataioQapı, 0x04)
	QapıYazmabyte(SecondarypicdataioQapı, 0x02)

	QapıYazmabyte(PrimarypicdataioQapı, 0x01)
	QapıYazmabyte(SecondarypicdataioQapı, 0x01)

	QapıYazmabyte(PrimarypicdataioQapı, 0xF8)
	QapıYazmabyte(SecondarypicdataioQapı, 0xEF)

	idtpointer := [6]uint8{0, 0, 0, 0, 0, 0}
	böyüklük := (*uint16)(Pointer(&idtpointer[0]))
	(*böyüklük) = (uint16)(Sizeof(idtdata) - 1)
...
func (self *TInterruptmanager) Interruptdescriptortableentryset(interrupt int,
	...
	handleraddressAlçaqbits := (*uint16)(Pointer(&idtdata[interrupt*8+0]))
	(*handleraddressAlçaqbits) = uint16(handler & 0xFFFF)
	...
	(*access) = (Idtdescriptorpresent | DescriptorNöv | ((Descriptorprivilegelevel & 3) << 5))
...
func (self *TInterruptmanager) Dohandleinterrupt(interrupt uint8, esp uint32) uint32 {
	...
	if interruptXətaHəlli {
		console_2.MÇapEtxy("[esp:", 1, 20)
		console_2.MUnsignedinteger32ÇapEt(uint32(interrupt))
		console_2.MÇapEt(":")
	...
	if !handlerİcraEt && interrupt == uint8(self.hardwareinterruptoffset) && self.taskmanager != nil {
		esp = uint32(uintptr(Pointer(self.taskmanager.Schedule((*Tcpustate)(Pointer(uintptr(esp)))))))

	}
	if !handlerİcraEt && interrupt == 0x80 {
	...
	if 0x20 <= interrupt && interrupt < 0x30 {
		if 0x28 <= interrupt {
			QapıYazmabyte(SecondarypicƏmrioQapı, 0x20)
		}
		QapıYazmabyte(PrimarypicƏmrioQapı, 0x20)
	}
	return esp
}
...
func Handleinterrupt(esp uint32, interrupt uint32) uint32 {
	...
	if interruptXətaHəlli && interrupt != 0x80 && interrupt != 0x20 {
		console_2.MÇapEtxy("[esp:", 1, 21)
		console_2.MUnsignedinteger32ÇapEt(uint32(interrupt))
		console_2.MÇapEt(":")
		console_2.MUnsignedinteger32ÇapEt(esp)
	}

	if Fəalinterruptmanager != 0 {
		p := (*TInterruptmanager)(Pointer(Fəalinterruptmanager))
...
func handleunhandledsyscall(esp uint32) uint32 {
	...
	if cpu.Eax == 1 || cpu.Eax == 252 {
		cpu.Eip = uint32(ValueOf(interruptÇıxloop).Pointer())
...
func exceptionhasXətacode(interrupt uint32) bool {
	switch interrupt {
...
func exceptionAd(interrupt uint32) string {
	switch interrupt {
	case 0x00:
...
func exceptionframeQiymət(frame uint32, offset uint32) uint32 {
...
func çapEtSəhifəfaultinfo(err uint32) {
...
func çapEtexceptionselectorinfo(err uint32) {
...
func Handleexception(esp uint32, interrupt uint32) uint32 {
	...
	if exceptionhasXətacode(interrupt) {
		err = exceptionframeQiymət(esp, 0)
		eipoffset = 4
	}
	eip := exceptionframeQiymət(esp, eipoffset)
	cs := exceptionframeQiymət(esp, eipoffset+4)
	eflags := exceptionframeQiymət(esp, eipoffset+8)
	...
	if interrupt == 0x0E {
		...
		çapEtSəhifəfaultinfo(err)
	}

	if (cs & 0x03) != 0 {
		MEmergencylogstring(" useresp=")
		MEmergencylogunsignedinteger32(exceptionframeQiymət(esp, eipoffset+12))
		MEmergencylogstring(" ss=")
		MEmergencylogunsignedinteger32(exceptionframeQiymət(esp, eipoffset+16))
	}

	if exceptionhasXətacode(interrupt) {
		çapEtexceptionselectorinfo(err)
	}
	MEmergencylogstring("\n=== ENGOS HALTED AFTER EXCEPTION ===\n")
	except++
...
func interrupthandler_2(interrupt uint8, esp uint32) uint32 {
	...
	console_2 := TConsole{}
	console_2.MÇapEt(buffer)
	console_2.MHexadecimalÇapEt(0x40)
	return esp
}
func çapEtesp(esp uint32) {
	console_2 := TConsole{}
	console_2.MUnsignedinteger32ÇapEtxy(esp, 20, 21)
}
func gettls() uint32
func ÇapEttls() {
