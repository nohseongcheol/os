...
func ÇapEtBayt(datapointer uintptr, böyüklük int) {
	var buffer_2 [4 * 1024]byte = *(*([4 * 1024]byte))(Pointer(datapointer))
	var i int
	utilconsole.MÇapEt([]byte("util["))
	for i = 0; i < böyüklük; i++ {
		utilconsole.MHexadecimalÇapEt(buffer_2[i])
	}
	utilconsole.MÇapEt([]byte("]"))
}

func ÇapEttest(params ...interface{}) {
	for _, param := range params {
		utilconsole.MÇapEt([]byte(TypeOf(param).Name()))

	}
}
func EqualBayt(a, b []byte) bool {
	if len(a) != len(b) {
		return false
	}
	for i, v := range a {
...
func Bayttostring(b []byte) string {
...
func GetBaytfrompointer(pointer_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		address	uintptr
		len	int
		cap	int
