...
type TWidgetSiçaneventhandler struct {
}

var siçanwidget TWidget
var siçanvga TVideographicsarray
var previousx int16 = 0
var previousy int16 = 0
var xposition int16 = 0
var yposition int16 = 0

func (self *TWidgetSiçaneventhandler) Init(widget TWidget, vga TVideographicsarray) {
	siçanwidget = widget
	siçanvga = vga
}
func (self *TWidgetSiçaneventhandler) OnSiçandown(button int8) {

	siçanwidget.Set_local_coordinates(uint32(previousx), uint32(previousy))
	siçanwidget.Setcolor(0xA8, 0x00, 0x00)
	siçanwidget.Draw(&siçanvga)

	siçanwidget.Draw(&siçanvga)

}

func (self *TWidgetSiçaneventhandler) OnSiçanYuxarı(button int8) {
}

func (self *TWidgetSiçaneventhandler) OnSiçanDaşı(x int8, y int8) {
	xposition += int16(x)
	if xposition < 0 {
		xposition = 0
	}
	...
	siçanwidget.Set_local_coordinates(uint32(previousx), uint32(previousy))
	siçanwidget.Setcolor(0x00, 0x00, 0x00)
	siçanwidget.Draw(&siçanvga)

	siçanwidget.Set_local_coordinates(uint32(xposition), uint32(yposition))
	siçanwidget.Setcolor(0x00, 0x00, 0xA8)
	siçanwidget.Draw(&siçanvga)
