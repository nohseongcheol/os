...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portgusomaword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portgusomaword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portgusomaword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portgusomaword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portgusomaword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portgusomaword(Macaddress4port) / 256)
	...
	Portkwandikaword(registeraddressport, 20)
	Portkwandikaword(buscontrolregisterdataport, 0x102)

	Portkwandikaword(registeraddressport, 0)
	Portkwandikaword(registerdataport, 0x04)
	...
	Portkwandikaword(registeraddressport, 1)
	Portkwandikaword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portkwandikaword(registeraddressport, 2)
	Portkwandikaword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portkwandikaword(registeraddressport, 0)
	Portkwandikaword(registerdataport, 0x41)

	Portkwandikaword(registeraddressport, 4)
	temporary := Portgusomaword(registerdataport)
	Portkwandikaword(registeraddressport, 4)
	Portkwandikaword(registerdataport, temporary|0xC00)

	Portkwandikaword(registeraddressport, 0)
	Portkwandikaword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portgusomaword(resetport)
	Portkwandikaword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portkwandikaword(registeraddressport, 0)
	temporary := uint32(Portgusomaword(registerdataport))
	...
	Portkwandikaword(registeraddressport, 0)
	Portkwandikaword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portkwandikaword(registeraddressport, 0)
	Portkwandikaword(registerdataport, 0x48)
