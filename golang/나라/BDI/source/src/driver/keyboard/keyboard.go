...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portgusomabyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portgusomabyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func kwandikaps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portkwandikabyte(commandport_2, value)
	return true
}

func kwandikaps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portkwandikabyte(dataport_2, value)
	return true
}
...
func gusomaps2data() (uint8, bool) {
	...
	return Portgusomabyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portgusomabyte(commandport_2)&0x01) != 0; i++ {
		Portgusomabyte(dataport_2)
	}

	if !kwandikaps2command(0xAE) || !kwandikaps2command(0x20) {
		return
	}
	status, ok := gusomaps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !kwandikaps2command(0x60) || !kwandikaps2data(status) {
		return
	}
...
var keyboardqueuegusoma uint8
var keyboardqueuekwandika uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuekwandika + 1) % keyboardqueuesize
	if next == keyboardqueuegusoma {
		return
	}
	keyboardqueue[keyboardqueuekwandika] = key
	keyboardqueuekwandika = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuegusoma != keyboardqueuekwandika {
		key := keyboardqueue[keyboardqueuegusoma]
		keyboardqueuegusoma = (keyboardqueuegusoma + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portgusomabyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portgusomabyte(dataport_2)
	if scancode == 0xE0 {
