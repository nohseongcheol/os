...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysopen:
		...
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|okwandikaonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: ogusomaonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: okwandikaonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: okwandikaonly}
}
...
func sysgusoma(fd int32, address uint32, count uint32) int32 {
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return gusomavfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == okwandikaonly || accessmode == ogusomakwandika || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func gusomavfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	gusomafile(entry.name[:entry.namelen], tmp)
...
func Stdinputbyte(c byte) {
	next := (stdinkwandika + 1) % uint32(len(stdinbuffer))
	if next == stdingusoma {
		return
	}
...
func pollkeyboardscancode() byte {
	for (Portgusomabyte(0x64) & 0x01) == 0 {
	}
	sc := Portgusomabyte(0x60)
...
func gusomafile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Gusoma(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
