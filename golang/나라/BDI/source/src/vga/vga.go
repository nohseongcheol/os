...
var attributecontrollergusomaport uint16 = 0x3c1
var attributecontrollerkwandikaport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Kwandikaregister(register []byte) {
	var regindex uint16 = 0

	Portkwandikabyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portkwandikabyte(sequencerindexport, i)
		Portkwandikabyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portkwandikabyte(crtcindexport, 0x03)

	Portkwandikabyte(crtcdataport, (Portgusomabyte(crtcdataport) | 0x80))
	Portkwandikabyte(crtcindexport, 0x11)
	Portkwandikabyte(crtcdataport, (Portgusomabyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portkwandikabyte(crtcindexport, i)
		Portkwandikabyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portkwandikabyte(graphicscontrollerindexport, i)
		Portkwandikabyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portgusomabyte(attributecontrollerresetport)
		Portkwandikabyte(attributecontrollerindexport, i)
		Portkwandikabyte(attributecontrollerkwandikaport, register[regindex])
		regindex++
	}

	Portgusomabyte(attributecontrollerresetport)
	Portkwandikabyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portkwandikabyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portgusomabyte(graphicscontrollerdataport) >> 2) & 0x03)
