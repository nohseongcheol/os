...
void *malloc(size_t aantal_cijfers)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!aantal_cijfers) aantal_cijfers = 1;
    if (aantal_cijfers > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    aantal_cijfers = (aantal_cijfers + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= aantal_cijfers) {
            if (cursor->capacity - aantal_cijfers >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + aantal_cijfers);
                raw->capacity = cursor->capacity - aantal_cijfers - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = aantal_cijfers;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + aantal_cijfers;
    ...
    raw->capacity = aantal_cijfers; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t aantal_cijfers)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(aantal_cijfers);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!aantal_cijfers) aantal_cijfers = 1;
    cursor = (struct heap_block *)address - 1;
    if (aantal_cijfers <= cursor->capacity) return address;
    destination = malloc(aantal_cijfers);
