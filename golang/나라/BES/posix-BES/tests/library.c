...
#define CHECK(waarde) do { if (!(waarde)) return __LINE__; } while (0)

static int compare_int(const void *left, const void *right)
{
    int result = *(const int *)left, waarde = *(const int *)right;
    return (result > waarde) - (result < waarde);
}

int __posix_library_test(void)
{
    char overdrachtsbuffer_2[64], source[64], *end_pointer, *saved_state, *address, *destination;
    const char *text = "abca", *input = " 	-0x80rest";
    int elements[257], index, waarde;
    size_t lengte;
    div_t result;
    ldiv_t decoded;
    ...
    CHECK(memset(overdrachtsbuffer_2, 0xab, sizeof(overdrachtsbuffer_2)) == overdrachtsbuffer_2);
    CHECK((unsigned char)overdrachtsbuffer_2[63] == 0xab);
    CHECK(memcpy(overdrachtsbuffer_2, "abcdef", 7) == overdrachtsbuffer_2);
    CHECK(memmove(overdrachtsbuffer_2 + 2, overdrachtsbuffer_2, 5) == overdrachtsbuffer_2 + 2);
    CHECK(memcmp(overdrachtsbuffer_2, "ababcde", 7) == 0);
    CHECK(memmove(overdrachtsbuffer_2, overdrachtsbuffer_2 + 2, 5) == overdrachtsbuffer_2);
    CHECK(memcmp(overdrachtsbuffer_2, "abcde", 5) == 0);
    CHECK(memmove(overdrachtsbuffer_2, overdrachtsbuffer_2, 0) == overdrachtsbuffer_2);
    CHECK(memcmp("\x80", "\x7f", 1) > 0);
    CHECK(memcmp(overdrachtsbuffer_2, source, 0) == 0);
    ...
    CHECK(strcpy(overdrachtsbuffer_2, text) == overdrachtsbuffer_2 && strcmp(overdrachtsbuffer_2, text) == 0);
    CHECK(stpcpy(overdrachtsbuffer_2, "xy") == overdrachtsbuffer_2 + 2 && overdrachtsbuffer_2[2] == 0);
    memset(overdrachtsbuffer_2, 7, sizeof(overdrachtsbuffer_2));
    CHECK(strncpy(overdrachtsbuffer_2, "xy", 5) == overdrachtsbuffer_2);
    CHECK(!memcmp(overdrachtsbuffer_2, "xy\0\0\0", 5) && overdrachtsbuffer_2[5] == 7);
    CHECK(stpncpy(overdrachtsbuffer_2, "abcdef", 3) == overdrachtsbuffer_2 + 3 && overdrachtsbuffer_2[3] == 0);
    CHECK(stpncpy(overdrachtsbuffer_2, "a", 4) == overdrachtsbuffer_2 + 1 && overdrachtsbuffer_2[3] == 0);
    CHECK(strncpy(overdrachtsbuffer_2, "z", 0) == overdrachtsbuffer_2 && overdrachtsbuffer_2[0] == 'a');
    strcpy(overdrachtsbuffer_2, "a");
    CHECK(strcat(overdrachtsbuffer_2, "bc") == overdrachtsbuffer_2 && !strcmp(overdrachtsbuffer_2, "abc"));
    CHECK(strncat(overdrachtsbuffer_2, "def", 2) == overdrachtsbuffer_2 && !strcmp(overdrachtsbuffer_2, "abcde"));
    CHECK(strncat(overdrachtsbuffer_2, "xyz", 0) == overdrachtsbuffer_2 && !strcmp(overdrachtsbuffer_2, "abcde"));
    ...
    CHECK(strxfrm(overdrachtsbuffer_2, "abc", 4) == 3 && !strcmp(overdrachtsbuffer_2, "abc"));
    ...
    strcpy(overdrachtsbuffer_2, ",,one;two,,");
    CHECK(!strcmp(strtok_r(overdrachtsbuffer_2, ",;", &saved_state), "one"));
    strcpy(source, "x:y");
    ...
    for (waarde = -1; waarde <= 255; ++waarde) {
        int low = waarde >= 'a' && waarde <= 'z', high = waarde >= 'A' && waarde <= 'Z';
        int digit = waarde >= '0' && waarde <= '9';
        CHECK(!!isalpha(waarde) == (low || high));
        CHECK(!!isalnum(waarde) == (low || high || digit));
        CHECK(!!isdigit(waarde) == digit);
        CHECK(!!islower(waarde) == low && !!isupper(waarde) == high);
        CHECK(!!isblank(waarde) == (waarde == ' ' || waarde == '\t'));
        CHECK(!!isspace(waarde) == (waarde == ' ' || (waarde >= 9 && waarde <= 13)));
        CHECK(!!iscntrl(waarde) == ((waarde >= 0 && waarde < 32) || waarde == 127));
        CHECK(!!isprint(waarde) == (waarde >= 32 && waarde <= 126));
        CHECK(!!isgraph(waarde) == (waarde >= 33 && waarde <= 126));
        CHECK(!!ispunct(waarde) == (waarde >= 33 && waarde <= 126 && !low && !high && !digit));
        CHECK(!!isxdigit(waarde) == (digit || (waarde >= 'a' && waarde <= 'f') || (waarde >= 'A' && waarde <= 'F')));
        CHECK(tolower(waarde) == (high ? waarde + 32 : waarde));
        CHECK(toupper(waarde) == (low ? waarde - 32 : waarde));
    }
    errno = EIO;
    ...
    for (lengte = 0; lengte <= 257; ++lengte) {
        for (index = 0; index < (int)lengte; ++index) elements[index] = ((index * 37 + (int)lengte) % 19) - 9;
        qsort(elements, lengte, sizeof(int), compare_int);
        for (index = 1; index < (int)lengte; ++index) CHECK(elements[index - 1] <= elements[index]);
        for (index = 0; index < (int)lengte; ++index) {
            int *found = bsearch(&elements[index], elements, lengte, sizeof(int), compare_int);
            CHECK(found && *found == elements[index]);
        }
        waarde = 100;
        CHECK(bsearch(&waarde, elements, lengte, sizeof(int), compare_int) == NULL);
    }
    environ = envp;
