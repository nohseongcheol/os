...
void *malloc(size_t অঙ্কের_সংখ্যা)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!অঙ্কের_সংখ্যা) অঙ্কের_সংখ্যা = 1;
    if (অঙ্কের_সংখ্যা > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    অঙ্কের_সংখ্যা = (অঙ্কের_সংখ্যা + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= অঙ্কের_সংখ্যা) {
            if (cursor->capacity - অঙ্কের_সংখ্যা >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + অঙ্কের_সংখ্যা);
                raw->capacity = cursor->capacity - অঙ্কের_সংখ্যা - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = অঙ্কের_সংখ্যা;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + অঙ্কের_সংখ্যা;
    ...
    raw->capacity = অঙ্কের_সংখ্যা; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t অঙ্কের_সংখ্যা)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(অঙ্কের_সংখ্যা);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!অঙ্কের_সংখ্যা) অঙ্কের_সংখ্যা = 1;
    cursor = (struct heap_block *)address - 1;
    if (অঙ্কের_সংখ্যা <= cursor->capacity) return address;
    destination = malloc(অঙ্কের_সংখ্যা);
