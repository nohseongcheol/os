...
{
    unsigned int দৈর্ঘ্য = 0;
    while (text[দৈর্ঘ্য] != 0)
        দৈর্ঘ্য++;
    return দৈর্ঘ্য;
}
...
{
    (void)lekha(STDOUT_FILENO, text, text_length(text));
}

static void report(const char *ব্যবস্থার_পরিচয়, int passed)
{
    checks++;
    if (passed) {
        say("PTEST:PASS:");
...
static pid_t reap_nohang(pid_t child, int *অবস্থা)
{
    unsigned int spins;
    pid_t result;

    for (spins = 0; spins < 2000000U; spins++) {
        result = waitpid(child, অবস্থা, WNOHANG);
        if (result != 0)
            return result;
    }
    return 0;
...
static void test_open_and_io(void)
{
    ...
    report("open-readonly", নথি_নির্দেশক >= 3);
    if (নথি_নির্দেশক >= 0) {
        report("read-bytes", pora(নথি_নির্দেশক, bytes, 2) == 2 &&
               (unsigned char)bytes[0] == 0x7f && bytes[1] == 'E');
        report("lseek-set", lseek(নথি_নির্দেশক, 0, SEEK_SET) == 0);
        report("lseek-cur", lseek(নথি_নির্দেশক, 1, SEEK_CUR) == 1);
        report("lseek-end", lseek(নথি_নির্দেশক, -1, SEEK_END) > 0);
        errno = 0;
        report("lseek-negative", lseek(নথি_নির্দেশক, -1, SEEK_SET) == -1 && errno == EINVAL);
        errno = 0;
        report("lseek-whence", lseek(নথি_নির্দেশক, 0, 99) == -1 && errno == EINVAL);
        report("fstat-file", fstat(নথি_নির্দেশক, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 2);
        errno = 0;
        report("fstat-efault", failed_with(fstat(নথি_নির্দেশক, (struct stat *)0), EFAULT));
        report("fsync-file", fsync(নথি_নির্দেশক) == 0);
        report("close-file", close(নথি_নির্দেশক) == 0);
        errno = 0;
        report("closed-fd", failed_with(close(নথি_নির্দেশক), EBADF));
    }
    ...
    if (root >= 0) {
        ...
        errno = 0;
        report("read-directory", pora(root, bytes, 1) == -1 && errno == EISDIR);
        (void)close(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", lekha(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", pora(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", pora(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", lekha(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
...
static void test_dup_and_fcntl(void)
{
    ...
    copy = dup(নথি_নির্দেশক);
    report("dup", copy >= 0 && copy != নথি_নির্দেশক);
    if (copy >= 0) {
        report("dup-shared-offset", pora(নথি_নির্দেশক, &first, 1) == 1 && pora(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", close(নথি_নির্দেশক) == 0 && fstat(copy, &st) == 0);
        নথি_নির্দেশক = copy;
    }

    target = dup2(নথি_নির্দেশক, 20);
    report("dup2", target == 20 && fstat(20, &st) == 0);
    report("dup2-same", dup2(নথি_নির্দেশক, নথি_নির্দেশক) == নথি_নির্দেশক);
    if (target == 20)
        (void)close(20);
    errno = 0;
    report("dup2-old-ebadf", failed_with(dup2(-1, 10), EBADF));
    errno = 0;
    report("dup2-new-ebadf", failed_with(dup2(নথি_নির্দেশক, 99), EBADF));

    report("fcntl-getfd", fcntl(নথি_নির্দেশক, F_GETFD) == 0);
    report("fcntl-setfd", fcntl(নথি_নির্দেশক, F_SETFD, FD_CLOEXEC) == 0 &&
           fcntl(নথি_নির্দেশক, F_GETFD) == FD_CLOEXEC);
    high = fcntl(নথি_নির্দেশক, F_DUPFD, 10);
    report("fcntl-dupfd", high >= 10);
    if (high >= 0) {
        report("fcntl-fd-flags-independent", fcntl(high, F_GETFD) == 0);
        (void)close(high);
    }
    report("fcntl-getfl", (fcntl(নথি_নির্দেশক, F_GETFL) & O_ACCMODE) == O_RDONLY);
    report("fcntl-setfl", fcntl(নথি_নির্দেশক, F_SETFL, O_APPEND) == 0 &&
           (fcntl(নথি_নির্দেশক, F_GETFL) & O_APPEND) != 0);
    errno = 0;
    report("fcntl-command-einval", failed_with(fcntl(নথি_নির্দেশক, 999), EINVAL));
    errno = 0;
...
static void test_terminal_and_uname(void)
{
    ...
    if (নথি_নির্দেশক >= 0) {
        errno = 0;
        report("isatty-enotty", isatty(নথি_নির্দেশক) == 0 && errno == ENOTTY);
        (void)close(নথি_নির্দেশক);
    } else {
    ...
    report("uname", uname(&ব্যবস্থার_পরিচয়) == 0 && ব্যবস্থার_পরিচয়.sysname[0] != 0 &&
           ব্যবস্থার_পরিচয়.nodename[0] != 0 && text_equal(ব্যবস্থার_পরিচয়.machine, "i386"));
    errno = 0;
...
void posix_test_process(void)
{
    ...
    report("fork", child > 0);
    if (child > 0) {
        waited = reap_nohang(child, &অবস্থা);
        report("waitpid", waited == child && WIFEXITED(অবস্থা) && WEXITSTATUS(অবস্থা) == 23);
        errno = 0;
        report("waitpid-echild", waitpid(child, &অবস্থা, WNOHANG) == -1 && errno == ECHILD);
    }
    ...
    if (child > 0 && reap_nohang(child, &অবস্থা) == child) {
        report("fork-fd-isolation", pora(নথি_নির্দেশক, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
    ...
    if (child > 0) {
        waited = reap_nohang(child, &অবস্থা);
        report("execve", waited == child && WIFEXITED(অবস্থা) && WEXITSTATUS(অবস্থা) == 37);
    }
    ...
    if (child > 0) {
        ...
        for (spin = 0; spin < 2000000U; spin++)
            (void)getpid();
        waited = wait(&অবস্থা);
        report("wait", waited == child && WIFEXITED(অবস্থা) && WEXITSTATUS(অবস্থা) == 29);
    }
}
