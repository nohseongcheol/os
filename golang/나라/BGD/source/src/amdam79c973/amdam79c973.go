...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portporaword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portporaword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portporaword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portporaword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portporaword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portporaword(Macaddress4port) / 256)
	...
	Portlekhaword(registeraddressport, 20)
	Portlekhaword(buscontrolregisterdataport, 0x102)

	Portlekhaword(registeraddressport, 0)
	Portlekhaword(registerdataport, 0x04)
	...
	Portlekhaword(registeraddressport, 1)
	Portlekhaword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portlekhaword(registeraddressport, 2)
	Portlekhaword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portlekhaword(registeraddressport, 0)
	Portlekhaword(registerdataport, 0x41)

	Portlekhaword(registeraddressport, 4)
	temporary := Portporaword(registerdataport)
	Portlekhaword(registeraddressport, 4)
	Portlekhaword(registerdataport, temporary|0xC00)

	Portlekhaword(registeraddressport, 0)
	Portlekhaword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portporaword(resetport)
	Portlekhaword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portlekhaword(registeraddressport, 0)
	temporary := uint32(Portporaword(registerdataport))
	...
	Portlekhaword(registeraddressport, 0)
	Portlekhaword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	for i := 0; i < int(size); i++ {
		...
		*(*byte)(Pointer(uintptr(destination_2))) = উৎস_2[int(size)-i-1]
	...
	Portlekhaword(registeraddressport, 0)
	Portlekhaword(registerdataport, 0x48)
