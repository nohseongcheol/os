...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Lekha(0xA0)
	} else {
		self.deviceport.Lekha(0xB0)
	}
	self.controlport.Lekha(0)
	self.deviceport.Lekha(0xA0)

	var status uint8 = self.commandport.Pora()
	if status == 0xFF {
	...
	self.sectorcountport.Lekha(0)
	self.lbalowport.Lekha(0)
	self.lbamidport.Lekha(0)
	self.lbahiport.Lekha(0)
	self.commandport.Lekha(0xEC)

	status = self.commandport.Pora()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Pora28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Lekha(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Lekha(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Lekha(0)
	self.sectorcountport.Lekha(1)

	self.lbalowport.Lekha(uint8(sector & 0x000000FF))
	self.lbamidport.Lekha(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Lekha(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Lekha(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Pora()
...
func (self *TAdvancedtechnologyattachment) Lekha28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Lekha(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Lekha(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Lekha(0)
	self.sectorcountport.Lekha(1)
	self.lbalowport.Lekha(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Lekha(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Lekha(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Lekha(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Lekha(0xE0)
	} else {
		self.deviceport.Lekha(0xF0)
	}
	self.commandport.Lekha(0xE7)
