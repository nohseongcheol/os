...
func (self *TDefaultkeyboardeventhandler) Onkeyউপর(key byte) {
}
...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portporabyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portporabyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func lekhaps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portlekhabyte(commandport_2, value)
	return true
}

func lekhaps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portlekhabyte(dataport_2, value)
	return true
}

func poraps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portporabyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portporabyte(commandport_2)&0x01) != 0; i++ {
		Portporabyte(dataport_2)
	}

	if !lekhaps2command(0xAE) || !lekhaps2command(0x20) {
		return
	}
	status, ok := poraps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !lekhaps2command(0x60) || !lekhaps2data(status) {
		return
	}

	if !lekhaps2data(0xF4) {
		return
	}
	ack, ok := poraps2data()
	if !ok || ack != 0xFA {
...
var keyboardqueuepora uint8
var keyboardqueuelekha uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuelekha + 1) % keyboardqueuesize
	if next == keyboardqueuepora {
		return
	}
	keyboardqueue[keyboardqueuelekha] = key
	keyboardqueuelekha = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuepora != keyboardqueuelekha {
		key := keyboardqueue[keyboardqueuepora]
		keyboardqueuepora = (keyboardqueuepora + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portporabyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portporabyte(dataport_2)
	if scancode == 0xE0 {
