...
func (self TDefaultmouseeventhandler) Onmouseউপর(button int8)	{}
...
func sendmousecommand(value uint8) bool {
	if !lekhaps2command(0xD4) || !lekhaps2data(value) {
		return false
	}
	ack, ok := poraps2data()
	return ok && ack == 0xFA
...
func (self *TMousedriver) Initdriver(manager *TInterruptmanager, mouseeventhandler IMouseeventhandler) {
	...
	if !lekhaps2command(0xA8) || !lekhaps2command(0x20) {
		return
	}
	...
	for i = 0; i < 3; i++ {
		...
		if (newbutton & mask) != (oldbutton & mask) {
			if (newbutton & mask) != 0 {
				...
				imouseeventhandler.Onmouseউপর(int8(i + 1))
			}
		}
	}
	button_2 = newbutton
