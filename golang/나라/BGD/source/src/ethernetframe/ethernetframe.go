...
func (self *TEthernetframeheader) Init(buffer_2 TEthernetframeheaderbuffer) {
	...
	self.উৎসmacbe = (Arraytounsignedinteger48(buffer_2.উৎসmacbe))
	self.ethernetধরণbe = (Arraytounsignedinteger16(buffer_2.ethernetধরণbe))
...
func (self *TEthernetframeheader) Setbuffer(buffer_2 *TEthernetframeheaderbuffer) {
	...
	buffer_2.উৎসmacbe = Unsignedinteger48toarray(Unsignedinteger48r(self.উৎসmacbe))
	buffer_2.ethernetধরণbe = Unsignedinteger16toarray(Unsignedinteger16r(self.ethernetধরণbe))
}

type IEthernetframehandler interface {
	Init(backend TEthernetframeprovider)
	Sethandler(handler IEthernetframehandler, ethernetধরণ uint16)
	Ethernetframereceivewhen(datapointer uintptr, size int) bool
	Send(destinationmacbe uint64, datapointer uintptr, size uint32)
	Framesend(destinationmacbe uint64, ethernetধরণbe uint16, datapointer uintptr, size uint32)
...
func (self *TEthernetframehandler) Sethandler(handler IEthernetframehandler, pethernetধরণ uint16) {
	handler_2[pethernetধরণ] = handler
}
...
func (self *TEthernetframehandler) Send(destinationmacbe uint64, datapointer uintptr, size uint32) {
	Backend.Framesend(destinationmacbe, frame.ethernetধরণbe, datapointer, size)
}
func (self *TEthernetframehandler) Framesend(destinationmacbe uint64, ethernetধরণbe uint16, datapointer uintptr, size uint32) {
	Backend.Framesend(destinationmacbe, ethernetধরণbe, datapointer, size)
}
...
func (self *TEthernetframeprovider) Onrawdatareceive(datapointer uintptr, size int) bool {
	...
	if frame.destinationmacbe == 0x0000FFFFFFFFFFFF || Unsignedinteger48r(frame.destinationmacbe) == self.Getmacaddress() {
		if handler_2[frame.ethernetধরণbe] != nil {
			ethernetconsole.MPrint(([]byte)("provider\n"))

			var pointer uintptr = uintptr(Pointer(datapointer)) + uintptr(frameheadersize)
			reply = handler_2[frame.ethernetধরণbe].Ethernetframereceivewhen(pointer, size-frameheadersize)

		}
	}

	if reply {
		frame.destinationmacbe = frame.উৎসmacbe
		frame.উৎসmacbe = Unsignedinteger48r(self.Getmacaddress())
		frame.Setbuffer(buffer_2)

	}

	ethernetconsole.MPrintxy(([]byte)("spro["), 0, 1)
	ethernetconsole.MUnsignedinteger64print(frame.উৎসmacbe)
	...
	ethernetconsole.MPrint(([]byte)(":"))
	ethernetconsole.MUnsignedinteger16print(frame.ethernetধরণbe)
	ethernetconsole.MPrint(([]byte)("]"))
...
func (self *TEthernetframeprovider) Framesend(destinationmacbe uint64, ethernetধরণbe uint16, datapointer uintptr, size uint32) {
	...
	frame.উৎসmacbe = Unsignedinteger48r(self.networkcard.Getmacaddress())
	frame.ethernetধরণbe = Unsignedinteger16r(ethernetধরণbe)

	frame.Setbuffer(buffer_2)
	var উৎস_2 [4096]byte = *(*([4096]byte))(Pointer(datapointer))

	var i uint32 = 0
	for i = 0; i < size; i++ {
		buffer2_2[uint32(frameheadersize)+i] = উৎস_2[i]

	}

	var pointer uintptr = uintptr(Pointer(&buffer2_2))

	self.networkcard.Send(pointer, size+uint32(frameheadersize))
