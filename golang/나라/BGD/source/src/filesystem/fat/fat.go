...
func (self *TBiosparameterblock32) Init(data []byte) {
	...
	copy(self.fatধরণlabel[:8], data[82:90])
...
func (self *TBiosparameterblock32) Len(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte) uint32 {
	...
	hd.Pora28(partitionoffset, &bpbbytes, 90)
	...
	hd.Pora28(rootstart, &direntbytes, 512)
...
func (self *TBiosparameterblock32) Pora(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte, data []byte) {
		...
		for Size > 0 {
			...
			for ; Size > 0; Size -= 512 {
				...
				if dirent[i].size > 512 {
					...
					hd.Pora28(filesector+uint32(sectoroffset), &buffer3, 512)

				} else {
					buffer3 = buffer_2[:dirent[i].size]
					hd.Pora28(filesector+uint32(sectoroffset), &buffer3, int(dirent[i].size))
				}
			...
			hd.Pora28(fatstart+fatsectorforcurrentcluster, &fatbuf, 512)
...
func (self *TDirectoryentryfat32) Init(data [32]byte) {
	...
	self.cসময়tenth = data[13]
	self.cসময় = uint16(data[14]) | uint16(data[15])<<8
	self.cdate = uint16(data[16]) | uint16(data[17])<<8
	self.aসময় = uint16(data[18]) | uint16(data[19])<<8
	self.firstclusterhi = uint16(data[20]) | uint16(data[21])<<8
	self.wসময় = uint16(data[22]) | uint16(data[23])<<8
