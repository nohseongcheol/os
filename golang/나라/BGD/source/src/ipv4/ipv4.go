...
func (self *TInternetprotocolv4message) Init(buffer_2 TInternetprotocolv4messagebuffer) {
	...
	self.সময়tolive = buffer_2.সময়tolive
...
func (self *TInternetprotocolv4message) Setbuffer(buffer_2 *TInternetprotocolv4messagebuffer) {
	...
	buffer_2.সময়tolive = self.সময়tolive
...
type IInternetprotocolhandler interface {
	...
	Internetprotocolreceivewhen(উৎসipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, datapointer uintptr, size uint32) bool
...
func (self *TInternetprotocolhandler) Internetprotocolreceivewhen(উৎসipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, datapointer uintptr, size uint32) bool {
...
func (self *Ipethernetframehandler) Send(destinationipaddressnetworkbyteorder uint64, datapointer uintptr, size uint32) {
	...
	var ethernetধরণbe = Unsignedinteger16r(0x0800)
	self.TEthernetframehandler.Framesend(destinationipaddressnetworkbyteorder, ethernetধরণbe, datapointer, size)
...
func (self *TInternetprotocolprovider) Ethernetframereceivewhen(ethernetframepayload uintptr, size uint32) bool {
	...
	if internetprotocolmessage.destinationipaddress == uint32(efhandler.Getipaddress()) {
		...
		if handler_2[internetprotocolmessage.protocol] != nil {
			reply = handler_2[internetprotocolmessage.protocol].Internetprotocolreceivewhen(internetprotocolmessage.উৎসipaddress, internetprotocolmessage.destinationipaddress, ethernetframepayload+uintptr(4*internetprotocolmessage.headerlength), uint32(length-uint32(4*internetprotocolmessage.headerlength)))
	...
	if reply {
		...
		internetprotocolmessage.destinationipaddress = internetprotocolmessage.উৎসipaddress
		internetprotocolmessage.উৎসipaddress = temporary

		internetprotocolmessage.সময়tolive = 0x40
	...
	ipconsole.MUnsignedinteger32print(internetprotocolmessage.উৎসipaddress)
...
func (self *TInternetprotocolprovider) Send(destinationipaddressnetworkbyteorder uint32, protocol uint8, datapointer uintptr, size uint32) {
	...
	message.সময়tolive = 0x40
	message.protocol = protocol

	message.destinationipaddress = destinationipaddressnetworkbyteorder

	message.উৎসipaddress = uint32(efhandler.Getipaddress())
	...
	if (destinationipaddressnetworkbyteorder & self.Subnetmask) != (message.উৎসipaddress & self.Subnetmask) {
	...
	var ethernetধরণbe = Unsignedinteger16r(0x0800)
	efhandler.Framesend(self.arpprovider.Resolve(nexthopipaddressnetworkbyteorder), ethernetধরণbe, senddatapointer, uint32(ipsize)+uint32(size))
