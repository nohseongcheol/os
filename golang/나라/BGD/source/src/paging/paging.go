...
func handlepaginginterrupt(esp uint32) uint32 {
	if Resolvecopyonlekhafault() {
		return esp
	}
	return Handlefatalinterruptframe(esp, 0x0E)
}

func Cloneaddressspacecow(উৎসpagedirectory uint32) uint32 {
	if Activememorymanager == nil || উৎসpagedirectory == 0 {
		return 0
	}
	...
	for pde := uint32(0); pde < 1024; pde++ {
		উৎসpdeaddress := উৎসpagedirectory + pde*4
		উৎসpde := Getvalue(উৎসpdeaddress)
		if (উৎসpde & Pagepresent) == 0 {
			continue
		}
		if issharedpde(pde) {
			Setunsignedinteger32ataddress(উৎসpde, destinationpagedirectory+pde*4)
			continue
		}
		...
		উৎসpt := উৎসpde & Pageframe
		destinationpt := uint32(uintptr(destinationptpointer))
		Setunsignedinteger32ataddress((destinationpt | (উৎসpde & 0xFFF)), destinationpagedirectory+pde*4)

		for pte := uint32(0); pte < 1024; pte++ {
			pteaddress := উৎসpt + pte*4
...
func Resolvecopyonlekhafault() bool {
	...
	উৎস_2 := Getbytesfrompointer(uintptr(faultaddress&Pageframe), 0x1000, 0x1000)
	destination_2 := Getbytesfrompointer(uintptr(newframe), 0x1000, 0x1000)
	copy(destination_2, উৎস_2)
...
func Setblockinpagedirectory(উৎস_2 []byte, destination_2 []byte, size uint32, pagedirectory uint32) {
...
func makepageprivatewritablecurrent(pagedirectory uint32, virtualaddress uint32) bool {
	...
	newframe := uint32(uintptr(newpointer)) & Pageframe
	উৎস_2 := Getbytesfrompointer(uintptr(virtualaddress&Pageframe), 0x1000, 0x1000)
	destination_2 := Getbytesfrompointer(uintptr(newframe), 0x1000, 0x1000)
	copy(destination_2, উৎস_2)
