...
var stdinpora uint32
var stdinlekha uint32
...
func Syspora_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Syspora, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Syslekha, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Syslekha, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Syspora:
		cpu.Eax = uint32(syspora(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Syslekha:
		cpu.Eax = uint32(syslekha(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|olekhaonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: oporaonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: olekhaonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: olekhaonly}
}
...
func syslekha(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func syspora(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return porastdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return poravfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == olekhaonly || accessmode == oporalekha || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func poravfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	porafile(entry.name[:entry.namelen], tmp)
...
func allocatesocket(domain uint32, socketধরণ uint32, protocol uint32) int32 {
	if domain != afinet {
		return Eafnosupport
	}
	if socketধরণ != sockdatagram {
...
func socketsendto(fd int32, bufferaddress_2 uint32, length uint32, destinationaddress uint32, destinationlength uint32) int32 {
	...
	*packet = socketpacket{used: true, size: length, উৎস: socket.local}
	if length != 0 {
		উৎস := Getbytesfrompointer(uintptr(bufferaddress_2), int(length), int(length))
		copy(packet.data[:length], উৎস)
	}
...
func socketreceivefrom(fd int32, bufferaddress_2 uint32, length uint32, উৎসaddress uint32, উৎসlengthaddress uint32) int32 {
	socket, err := socketforfd(fd)
	if err != 0 {
		return err
	}
	...
	if উৎসaddress != 0 {
		if উৎসlengthaddress == 0 {
			return Efault
		}
		providedlength := (*uint32)(Pointer(uintptr(উৎসlengthaddress)))
		if *providedlength >= 16 {
			*(*socketaddressipv4)(Pointer(uintptr(উৎসaddress))) = packet.উৎস
		}
		*providedlength = 16
	}
	*packet = socketpacket{}
...
func porastdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinlekha + 1) % uint32(len(stdinbuffer))
	if next == stdinpora {
		return
	}
	stdinbuffer[stdinlekha] = c
	stdinlekha = next
}

func stdingetblocking() byte {
	for stdinpora == stdinlekha {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinpora]
	stdinpora = (stdinpora + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portporabyte(0x64) & 0x01) == 0 {
	}
	sc := Portporabyte(0x60)
...
func sysexit(status uint32) {
	...
	for i := 0; i < len(processtable); i++ {
		if processtable[i].used && processtable[i].pid == pid {
			closeসবprocessfds(&processtable[i])
...
func syswaitpid(pid int32, statusaddress uint32, অপশন uint32) int32 {
	if (অপশন & ^uint32(1)) != 0 {
		return Einval
	}
	parentpid := Currentpid()
...
func closeসবprocessfds(process *processentry) {
	if process == nil {
		return
	}
	for fd := int32(0); fd < maxfd; fd++ {
...
func porafile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Pora(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
