...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	prodhanthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if prodhanthread != nil {
		prodhanthread.Pid = process.id
		prodhanthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(prodhanthread)))
	}
...
func (self *Processhelper) copypagedirectory(উৎসentry uintptr, destinationentry uintptr) {
	উৎস_2 := Getunsignedinteger32arrayfrompointer(উৎসentry, 1024, 1024)
