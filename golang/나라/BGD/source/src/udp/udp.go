...
func (self *TUserdatagramprotocolheader) Init(buffer_2 *TUserdatagramprotocolheaderbuffer) {
	self.উৎসportnumber = Arraytounsignedinteger16(buffer_2.উৎসportnumber)
...
func (self *TUserdatagramprotocolheader) Setbuffer(buffer_2 *TUserdatagramprotocolheaderbuffer) {
	...
	buffer_2.উৎসportnumber = Unsignedinteger16toarray(self.উৎসportnumber)
...
func (self *TUserdatagramprotocolprovider) Internetprotocolreceivewhen(উৎসipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, internetprotocolpayload uintptr, size uint32) bool {
	...
	for i := 0; i < numbersockets && socket == nil; i++ {
		if sockets[i].localportnumber == msg.destinationportnumber && sockets[i].localip == destinationipaddressnetworkbyteorder && sockets[i].listening == true {
			...
			socket.remoteportnumber = msg.উৎসportnumber
			socket.remoteip = উৎসipaddressnetworkbyteorder
		} else if sockets[i].localportnumber == msg.destinationportnumber && sockets[i].localip == destinationipaddressnetworkbyteorder && sockets[i].remoteportnumber == msg.উৎসportnumber && sockets[i].remoteip == উৎসipaddressnetworkbyteorder {
			socket = &sockets[i]
...
func (self *TUserdatagramprotocolprovider) Send(socket *TUserdatagramprotocolsocket, pdata uintptr, size uint16) {
	...
	msg.উৎসportnumber = socket.localportnumber
