...
var attributecontrollerporaport uint16 = 0x3c1
var attributecontrollerlekhaport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Lekharegister(register []byte) {
	var regindex uint16 = 0

	Portlekhabyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portlekhabyte(sequencerindexport, i)
		Portlekhabyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portlekhabyte(crtcindexport, 0x03)

	Portlekhabyte(crtcdataport, (Portporabyte(crtcdataport) | 0x80))
	Portlekhabyte(crtcindexport, 0x11)
	Portlekhabyte(crtcdataport, (Portporabyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portlekhabyte(crtcindexport, i)
		Portlekhabyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portlekhabyte(graphicscontrollerindexport, i)
		Portlekhabyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portporabyte(attributecontrollerresetport)
		Portlekhabyte(attributecontrollerindexport, i)
		Portlekhabyte(attributecontrollerlekhaport, register[regindex])
		regindex++
	}

	Portporabyte(attributecontrollerresetport)
	Portlekhabyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portlekhabyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portporabyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Lekharegister(g320x200x256)
