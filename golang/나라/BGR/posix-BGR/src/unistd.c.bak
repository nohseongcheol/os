...
{
    return (ssize_t)__syscall_result(SC3(SYS_Четене, fd, buf, count));
}

ssize_t Писане(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Писане, fd, buf, count));
}

int Затваряне(int fd)
{
    return (int)__syscall_result(SC1(SYS_Затваряне, fd));
}
