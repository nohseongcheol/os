...
static void say(const char *s) { (void)Писане(STDOUT_FILENO, s, length_of(s)); }
...
int main(void)
{
    ...
    report("recvfrom", recvfrom(server, buffer, sizeof(buffer), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       buffer[0] == 'p' && buffer[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, buffer, sizeof(buffer), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Затваряне(client) == 0);
    report("close-server", Затваряне(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
