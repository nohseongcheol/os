...
TEXT ·инсталиранеidt(SB),NOSPLIT,$0
...
// used).
TEXT ·РъкохваткаПрекъсване(SB),NOSPLIT,$0-10
	...
	LEAQ ·прекъсванеgateзапис(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	...
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchПрекъсване(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchПрекъсване(SB),NOSPLIT,$0
...
TEXT ·прекъсванеgateзапис(SB),NOSPLIT,$0
