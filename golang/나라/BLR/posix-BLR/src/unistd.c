...
ssize_t Чытанне(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Чытанне, fd, buf, count));
}

ssize_t Запіс(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Запіс, fd, buf, count));
}

int Закрыць(int fd)
{
    return (int)__syscall_result(SC1(SYS_Закрыць, fd));
}
