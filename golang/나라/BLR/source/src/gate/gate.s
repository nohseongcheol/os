...
GLOBL ·іДЭНТЫФІКАТАРtDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·іДЭНТЫФІКАТАРt<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·устанавіцьidt(SB),NOSPLIT,$0
	LEAQ ·іДЭНТЫФІКАТАРtDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·іДЭНТЫФІКАТАРt<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·HandleПерарыванне(SB),NOSPLIT,$0-10
	...
	LEAQ ·перарываннеgateentry(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	...
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchПерарыванне(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchПерарыванне(SB),NOSPLIT,$0
...
TEXT ·перарываннеgateentry(SB),NOSPLIT,$0
