...
ssize_t Baca(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Baca, fd, buf, count));
}

ssize_t Tulis(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Tulis, fd, buf, count));
}

int Tutup(int fd)
{
    return (int)__syscall_result(SC1(SYS_Tutup, fd));
}
