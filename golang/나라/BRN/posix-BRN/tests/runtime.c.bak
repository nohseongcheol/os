...
static void test_open_and_io(void)
{
    ...
    fd = Buka("/USER2", O_RDONLY);
    report("open-readonly", fd >= 3);
    if (fd >= 0) {
        report("read-bytes", Baca(fd, bytes, 2) == 2 &&
        ...
        report("close-file", Tutup(fd) == 0);
        errno = 0;
        report("closed-fd", failed_with(Tutup(fd), EBADF));
    }
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", Baca(fd, &first, 1) == 1 && Baca(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Tutup(fd) == 0 && fstat(copy, &st) == 0);
        fd = copy;
    }
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &status) == child) {
        report("fork-fd-isolation", Baca(fd, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
