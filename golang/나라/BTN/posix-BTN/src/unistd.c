...
ssize_t log(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_log, fd, buf, count));
}

ssize_t bri(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_bri, fd, buf, count));
}
