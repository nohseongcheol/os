...
func Getfileཚད(filename []byte) uint32 {
	...
	var ཚད uint32 = bios.Len(&ata0s, partition.Mbr.Primarypartition[0], filename)
	ata0s.Flush()
...
func KKernelEntry(Pagedirectoryentry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memorymanager := &TMemorymanager{}
	memorymanager.Init(0, Maxqueueཚད)
	...
	ཚད = Getfileཚད(linkerfile)
	linkeraddress := memorymanager.Malloc(ཚད)
	linkerdata := Getbytesfrompointer(uintptr(linkeraddress), int(ཚད), int(ཚད))
	Logfile(linkerfile, linkerdata)
	...
	ཚད = Getfileཚད(lib1file)

	lib1address := memorymanager.Malloc(ཚད)
	lib1data := Getbytesfrompointer(uintptr(lib1address), int(ཚད), int(ཚད))
	Logfile(lib1file, lib1data)
	...
	ཚད = Getfileཚད(lib2file)

	lib2address := memorymanager.Malloc(ཚད)
	lib2data := Getbytesfrompointer(uintptr(lib2address), int(ཚད), int(ཚད))
	Logfile(lib2file, lib2data)
	...
	ཚད = Getfileཚད(user1file)
	user1address := memorymanager.Malloc(ཚད)
	user1data := Getbytesfrompointer(uintptr(user1address), int(ཚད), int(ཚད))
	Logfile(user1file, user1data)
	...
	*code1pointer = uintptr(linkerentry)
	...
	ཚད = Getfileཚད(user2file)
	user2address := memorymanager.Malloc(ཚད)
	user2data := Getbytesfrompointer(uintptr(user2address), int(ཚད), int(ཚད))
	Logfile(user2file, user2data)
	...
	*code2pointer = uintptr(linkerentry)
	...
	ཚད = Getfileཚད(user3file)
	user3address := memorymanager.Malloc(ཚད)
	user3data := Getbytesfrompointer(uintptr(user3address), int(ཚད), int(ཚད))
	Logfile(user3file, user3data)
