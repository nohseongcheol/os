...
type IRawdatahandler interface {
	Onrawdatareceive(datapointer uintptr, ཚད int) bool
	Send(datapointer uintptr, ཚད uint32)
}
...
func (self *TRawdatahandler) Onrawdatareceive(datapointer uintptr, ཚད int) bool {
	networkcardconsole.MPrintxy(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawdatahandler) Send(datapointer uintptr, ཚད uint32) {
	networkcardconsole.MPrintxy(([]byte)("TRawDataSend"), 10, 10)
	rawdatabackend.Send(datapointer, ཚད)
}
...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portlogword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portlogword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portlogword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portlogword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portlogword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portlogword(Macaddress4port) / 256)
	...
	Portbriword(registeraddressport, 20)
	Portbriword(buscontrolregisterdataport, 0x102)

	Portbriword(registeraddressport, 0)
	Portbriword(registerdataport, 0x04)
	...
	Portbriword(registeraddressport, 1)
	Portbriword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portbriword(registeraddressport, 2)
	Portbriword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portbriword(registeraddressport, 0)
	Portbriword(registerdataport, 0x41)

	Portbriword(registeraddressport, 4)
	temporary := Portlogword(registerdataport)
	Portbriword(registeraddressport, 4)
	Portbriword(registerdataport, temporary|0xC00)

	Portbriword(registeraddressport, 0)
	Portbriword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portlogword(resetport)
	Portbriword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portbriword(registeraddressport, 0)
	temporary := uint32(Portlogword(registerdataport))
	...
	Portbriword(registeraddressport, 0)
	Portbriword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, ཚད uint32) {
	...
	var destination_2 uint32 = sendbufferdescription[senddescriptor].address_2 + ཚད - 1

	for i := 0; i < int(ཚད); i++ {

		*(*byte)(Pointer(uintptr(destination_2))) = source_2[int(ཚད)-i-1]
	...
	sendbufferdescription[senddescriptor].flags = 0x8300F000 | uint32((-ཚད)&0xFFF)

	Portbriword(registeraddressport, 0)
	Portbriword(registerdataport, 0x48)
...
func (self *Tamdam79c973) Receive() {
	...
	for ; (recvbufferdescription[currentrecvbuffer].flags & 0x80000000) == 0; currentrecvbuffer = (currentrecvbuffer + 1) % 8 {
		...
		if !(recvbufferdescription[currentrecvbuffer].flags&0x40000000 != 0) && ((recvbufferdescription[currentrecvbuffer].flags & 0x03000000) == 0x03000000) {
			var ཚད uint32 = recvbufferdescription[currentrecvbuffer].flags & 0xFFF
			if ཚད > 64 {
				ཚད -= 4
			}

			console_2.MPrint([]byte(" size : ["))
			console_2.MUnsignedinteger32print(ཚད)
			console_2.MPrint([]byte("]"))
			...
			if irawdatahandler != nil {
				if irawdatahandler.Onrawdatareceive(pointer, int(ཚད)) {

					console_2.MPrintxy(([]byte)("self.Send"), 0, 22)

					self.Send(pointer, ཚད)
				}
			}
