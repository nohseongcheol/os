...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Bri(0xA0)
	} else {
		self.deviceport.Bri(0xB0)
	}
	self.controlport.Bri(0)
	self.deviceport.Bri(0xA0)

	var status uint8 = self.commandport.Log()
	if status == 0xFF {
	...
	self.sectorcountport.Bri(0)
	self.lbalowport.Bri(0)
	self.lbamidport.Bri(0)
	self.lbahiport.Bri(0)
	self.commandport.Bri(0xEC)

	status = self.commandport.Log()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Log28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Bri(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Bri(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Bri(0)
	self.sectorcountport.Bri(1)

	self.lbalowport.Bri(uint8(sector & 0x000000FF))
	self.lbamidport.Bri(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Bri(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Bri(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Log()
...
func (self *TAdvancedtechnologyattachment) Bri28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Bri(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Bri(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Bri(0)
	self.sectorcountport.Bri(1)
	self.lbalowport.Bri(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Bri(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Bri(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Bri(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Bri(0xE0)
	} else {
		self.deviceport.Bri(0xF0)
	}
	self.commandport.Bri(0xE7)
