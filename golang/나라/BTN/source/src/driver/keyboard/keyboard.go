...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portlogbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portlogbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func brips2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portbribyte(commandport_2, value)
	return true
}

func brips2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portbribyte(dataport_2, value)
	return true
}

func logps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portlogbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portlogbyte(commandport_2)&0x01) != 0; i++ {
		Portlogbyte(dataport_2)
	}

	if !brips2command(0xAE) || !brips2command(0x20) {
		return
	}
	status, ok := logps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !brips2command(0x60) || !brips2data(status) {
		return
	}
...
const keyboardqueueཚད = 64

var keyboardqueue [keyboardqueueཚད]byte
var keyboardqueuelog uint8
var keyboardqueuebri uint8
var leftshift bool
var གཡསshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuebri + 1) % keyboardqueueཚད
	if next == keyboardqueuelog {
		return
	}
	keyboardqueue[keyboardqueuebri] = key
	keyboardqueuebri = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuelog != keyboardqueuebri {
		key := keyboardqueue[keyboardqueuelog]
		keyboardqueuelog = (keyboardqueuelog + 1) % keyboardqueueཚད
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portlogbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portlogbyte(dataport_2)
	if scancode == 0xE0 {
