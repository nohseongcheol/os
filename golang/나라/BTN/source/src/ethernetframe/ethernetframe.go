...
type IEthernetframehandler interface {
	...
	Ethernetframereceivewhen(datapointer uintptr, ཚད int) bool
	Send(destinationmacbe uint64, datapointer uintptr, ཚད uint32)
	Framesend(destinationmacbe uint64, ethernettypebe uint16, datapointer uintptr, ཚད uint32)
...
func (self *TEthernetframehandler) Ethernetframereceivewhen(datapointer uintptr, ཚད int) bool {
	ethernetconsole.MPrint(([]byte)("OnEtherFrameReceived"))
	return false
}
func (self *TEthernetframehandler) Send(destinationmacbe uint64, datapointer uintptr, ཚད uint32) {
	Backend.Framesend(destinationmacbe, frame.ethernettypebe, datapointer, ཚད)
}
func (self *TEthernetframehandler) Framesend(destinationmacbe uint64, ethernettypebe uint16, datapointer uintptr, ཚད uint32) {
	Backend.Framesend(destinationmacbe, ethernettypebe, datapointer, ཚད)
}
...
func (self *TEthernetframerawdatahandler) Onrawdatareceive(datapointer uintptr, ཚད int) bool {
	return provider.Onrawdatareceive(datapointer, ཚད)
}
func (self *TEthernetframerawdatahandler) Send(datapointer uintptr, ཚད uint32) {
	provider.Send(datapointer, ཚད)
}
...
func (self *TEthernetframeprovider) Onrawdatareceive(datapointer uintptr, ཚད int) bool {
	...
	if frame.destinationmacbe == 0x0000FFFFFFFFFFFF || Unsignedinteger48r(frame.destinationmacbe) == self.Getmacaddress() {
		if handler_2[frame.ethernettypebe] != nil {
			...
			var pointer uintptr = uintptr(Pointer(datapointer)) + uintptr(frameheaderཚད)
			reply = handler_2[frame.ethernettypebe].Ethernetframereceivewhen(pointer, ཚད-frameheaderཚད)
...
func (self *TEthernetframeprovider) Send(datapointer uintptr, ཚད uint32) {
	self.networkcard.Send(datapointer, ཚད)
}
func (self *TEthernetframeprovider) Framesend(destinationmacbe uint64, ethernettypebe uint16, datapointer uintptr, ཚད uint32) {
	...
	var i uint32 = 0
	for i = 0; i < ཚད; i++ {
		buffer2_2[uint32(frameheaderཚད)+i] = source_2[i]

	}

	var pointer uintptr = uintptr(Pointer(&buffer2_2))

	self.networkcard.Send(pointer, ཚད+uint32(frameheaderཚད))
