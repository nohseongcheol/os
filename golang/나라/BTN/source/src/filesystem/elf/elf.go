...
func (self *Elf) Parse(data []byte, Pagedirectoryentry uint32) {
	...
	if elfheader.eshnum != 0 {
		...
		sectheaderཚད := uint32(Sizeof(Elfsectionheader{}))

		for i := uint32(0); i < uint32(elfheader.eshnum); i++ {
			sectheader := (*Elfsectionheader)(Pointer(&data[elfheader.eshoff+sectheaderཚད*i]))
			...
			if sectheader.shtype != 8 {
				endoffset := sectheader.shoffset + sectheader.shཚད
			...
			if sectheader.shaddress > 0x1000 {
				ཚད := sectheader.shཚད
				if sectheader.shtype == 8 {
					Zeroblockinpagedirectory(sectheader.shaddress, ཚད, Pagedirectoryentry)
				} else {
					destination_2 := Getbytesfrompointer(uintptr(sectheader.shaddress), int(ཚད), int(ཚད))
					Setblockinpagedirectory(sectvalue, destination_2, ཚད, Pagedirectoryentry)
				}
			}
			...
			if Equalbytes(sectname, ([]byte)(".text")) {
				...
				console_2.MPrint(":")
				console_2.MUnsignedinteger32print(sectheader.shཚད)
				console_2.MPrint("]")
				copy(self.text[:sectheader.shཚད], sectvalue[:sectheader.shཚད])
				self.textlen = sectheader.shཚད
			}
			if Equalbytes(sectname, ([]byte)(".rel.text")) {
				...
				console_2.MUnsignedinteger32print(sectheader.shཚད)
				console_2.MPrint("]")
				for rt := uint32(0); rt < sectheader.shཚད/8; rt++ {
			...
			if Equalbytes(sectname, ([]byte)(".strtab")) {
				...
				for st := uint32(1); st < sectheader.shཚད; st++ {
