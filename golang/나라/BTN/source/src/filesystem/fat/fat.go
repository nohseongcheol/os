...
func (self *TBiosparameterblock32) Init(data []byte) {
	...
	self.tableཚད = Unsignedinteger32r(Arraytounsignedinteger32(buffer1))
...
func (self *TBiosparameterblock32) Len(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte) uint32 {
	...
	hd.Log28(partitionoffset, &bpbbytes, 90)

	var bpb = TBiosparameterblock32{}
	bpb.Init(bpbbytes)

	var fatstart = partitionoffset + uint32(bpb.reservedsectors)
	var fatཚད = bpb.tableཚད

	var datastart = fatstart + fatཚད*uint32(bpb.fatcopy)
	...
	hd.Log28(rootstart, &direntbytes, 512)
...
func (self *TBiosparameterblock32) Log(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte, data []byte) {
		...
		for Sཚད > 0 {
			...
			for ; Sཚད > 0; Sཚད -= 512 {
				...
				if dirent[i].ཚད > 512 {
					...
					hd.Log28(filesector+uint32(sectoroffset), &buffer3, 512)

				} else {
					buffer3 = buffer_2[:dirent[i].ཚད]
					hd.Log28(filesector+uint32(sectoroffset), &buffer3, int(dirent[i].ཚད))
				}

				copy(data[int32(dirent[i].ཚད)-Sཚད:], buffer3)
			...
			hd.Log28(fatstart+fatsectorforcurrentcluster, &fatbuf, 512)
...
func (self *TDirectoryentryfat32) Init(data [32]byte) {
	...
	self.ཚད = Unsignedinteger32r(Arraytounsignedinteger32(buffer))
}
