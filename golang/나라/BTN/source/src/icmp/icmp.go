...
func (self *Icmphandler) Internetprotocolreceivewhen(sourceipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, datapointer uintptr, ཚད uint32) bool {
	return icmp.Internetprotocolreceivewhen(sourceipaddressnetworkbyteorder, destinationipaddressnetworkbyteorder, datapointer, ཚད)
}
...
func (self *TInternetcontrolmessageprotocol) Internetprotocolreceivewhen(sourceipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, datapointer uintptr, ཚད uint32) bool {
	if ཚད < uint32(icmpཚད) {
		return false
	}
	...
	case 8:
		...
		msg.checksum = iphandler.Providerget().Checksum((*([4096]uint16))(Pointer(datapointer)), uint32(icmpཚད))
...
func (self *TInternetcontrolmessageprotocol) Echorequestsend(ipnetworkbyteorder uint32) bool {
	...
	icmp.checksum = iphandler.Providerget().Checksum((*([4096]uint16))(Pointer(&buffer_2)), uint32(icmpཚད))
	icmp.Setbuffer(buffer_2)

	var datapointer uintptr = uintptr(Pointer(buffer_2))
	iphandler.Send(ipnetworkbyteorder, 0x01, datapointer, uint32(icmpཚད))
