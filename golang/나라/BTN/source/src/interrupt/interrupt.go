...
func (self *TInterruptmanager) Init(hardwareinterruptoffset uint16, globaldescriptortable *TShareddescriptortable, taskmanager *TTaskmanager) {
	...
	Portbribyte(Primarypiccommandioport, 0x11)
	Portbribyte(Secondarypiccommandioport, 0x11)

	Portbribyte(Primarypicdataioport, 0x20)
	Portbribyte(Secondarypicdataioport, 0x28)

	Portbribyte(Primarypicdataioport, 0x04)
	Portbribyte(Secondarypicdataioport, 0x02)

	Portbribyte(Primarypicdataioport, 0x01)
	Portbribyte(Secondarypicdataioport, 0x01)

	Portbribyte(Primarypicdataioport, 0xF8)
	Portbribyte(Secondarypicdataioport, 0xEF)

	idtpointer := [6]uint8{0, 0, 0, 0, 0, 0}
	ཚད := (*uint16)(Pointer(&idtpointer[0]))
	(*ཚད) = (uint16)(Sizeof(idtdata) - 1)
...
func (self *TInterruptmanager) Dohandleinterrupt(interrupt uint8, esp uint32) uint32 {
	...
	if 0x20 <= interrupt && interrupt < 0x30 {
		if 0x28 <= interrupt {
			Portbribyte(Secondarypiccommandioport, 0x20)
		}
		Portbribyte(Primarypiccommandioport, 0x20)
	}
	return esp
}
