...
type IInternetprotocolhandler interface {
	...
	Internetprotocolreceivewhen(sourceipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, datapointer uintptr, ཚད uint32) bool
	Send(destinationipaddressnetworkbyteorder uint32, pprotocol uint8, datapointer uintptr, ཚད uint32)
...
func (self *TInternetprotocolhandler) Internetprotocolreceivewhen(sourceipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, datapointer uintptr, ཚད uint32) bool {
	ipconsole.MPrint(([]byte)("ipHandler:OnInternet"))
	return false
}
func (self *TInternetprotocolhandler) Send(destinationipaddressnetworkbyteorder uint32, pprotocol uint8, datapointer uintptr, ཚད uint32) {

	ipprovider.Send(destinationipaddressnetworkbyteorder, pprotocol, datapointer, ཚད)
}
...
func (self *Ipethernetframehandler) Ethernetframereceivewhen(datapointer uintptr, ཚད int) bool {
	ipconsole.MPrint(([]byte)("iphandler:onEtherfameRecv\n"))
	return ipprovider.Ethernetframereceivewhen(datapointer, uint32(ཚད))

}

func (self *Ipethernetframehandler) Send(destinationipaddressnetworkbyteorder uint64, datapointer uintptr, ཚད uint32) {
	ipconsole.MPrint(([]byte)("ipefhandler:send\n"))
	var ethernettypebe = Unsignedinteger16r(0x0800)
	self.TEthernetframehandler.Framesend(destinationipaddressnetworkbyteorder, ethernettypebe, datapointer, ཚད)
...
func (self *TInternetprotocolprovider) Ethernetframereceivewhen(ethernetframepayload uintptr, ཚད uint32) bool {
	if ཚད < uint32(ipཚད) {
		return false
	}
...
func (self *TInternetprotocolprovider) Send(destinationipaddressnetworkbyteorder uint32, protocol uint8, datapointer uintptr, ཚད uint32) {
	...
	message.version = 4
	message.headerlength = ipཚད / 4
	message.tos = 0
	message.totallength = Unsignedinteger16r(uint16(ཚད + uint32(ipཚད)))
	...
	message.checksum = self.Checksum((*([4096]uint16))(Pointer(&buffer1_2)), uint32(ipཚད))
	message.Setbuffer(buffer_2)
	...
	for i := 0; i < int(ཚད); i++ {
		...
		buffer1_2[i+int(ipཚད)] = databuffer_2[i]
	}

	ipconsole.MPrintxy(([]byte)("ipprovider:send["), 1, 18)
	for i := 0; i < int(ཚད)+int(ipཚད); i++ {
	...
	var ethernettypebe = Unsignedinteger16r(0x0800)
	efhandler.Framesend(self.arpprovider.Resolve(nexthopipaddressnetworkbyteorder), ethernettypebe, senddatapointer, uint32(ipཚད)+uint32(ཚད))
