...
func Resolvecopyonbrifault() bool {
...
func Setblockinpagedirectory(source_2 []byte, destination_2 []byte, ཚད uint32, pagedirectory uint32) {
	if ཚད == 0 || pagedirectory == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(pagedirectory)
	makerangeprivatewritablecurrent(pagedirectory, uint32(uintptr(unsafe.Pointer(&destination_2[0]))), ཚད)

	for i := uint32(0); i < ཚད; i++ {
		destination_2[i] = source_2[i]
	}
	setcr3(oldcr3)
}

func Zeroblockinpagedirectory(address uint32, ཚད uint32, pagedirectory uint32) {
	if ཚད == 0 || pagedirectory == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(pagedirectory)
	makerangeprivatewritablecurrent(pagedirectory, address, ཚད)
	destination_2 := Getbytesfrompointer(uintptr(address), int(ཚད), int(ཚད))
	for i := uint32(0); i < ཚད; i++ {
		destination_2[i] = 0
	}
	setcr3(oldcr3)
...
func makerangeprivatewritablecurrent(pagedirectory uint32, address uint32, ཚད uint32) bool {
	if ཚད == 0 {
		return true
	}
	last := address + ཚད - 1
...
func Makerangeprivatewritable(pagedirectory uint32, address uint32, ཚད uint32) bool {
	if pagedirectory == 0 {
		return false
	}
	oldcr3 := getcr3()
	setcr3(pagedirectory)
	ok := makerangeprivatewritablecurrent(pagedirectory, address, ཚད)
	setcr3(oldcr3)
	return ok
}
