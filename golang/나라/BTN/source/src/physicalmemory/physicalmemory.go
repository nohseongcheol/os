...
func (self *Physicalmemorymanager) Amountofmemory() uint32 {
	return self.memoryཚད
}
func (self *Physicalmemorymanager) Getbitmaspཚད() uint32 {
	return self.memoryཚད / Blockཚད / Blockperbyte
}
...
func (self *Physicalmemorymanager) Firstfreeཚད(ཚད uint32) uint32 {
	if ཚད == 0 {
		return 0xffffffff
	}
	if ཚད == 1 {
		...
		if self.memoryarray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryarray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= ཚད; count++ {
...
func (self *Physicalmemorymanager) Init(ཚད uint32, bitmap []uint32) {
	self.memoryཚད = ཚད
	self.memoryarray = bitmap
	self.maximumblock = ཚད / Blockཚད
