...
func Syslog_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Syslog, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysbri, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysbri, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysexit:
		sysexit(cpu.Ebx)
		return uint32(uintptr(Pointer(Sབཀགcurrentthread(cpu))))
	case Sysrtexit:
		sysexit(cpu.Ebx)
		return uint32(uintptr(Pointer(Sབཀགcurrentthread(cpu))))
	case Sysfork:
		...
		return esp
	case Syslog:
		cpu.Eax = uint32(syslog(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysbri:
		cpu.Eax = uint32(sysbri(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|obrionly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: ologonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: obrionly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: obrionly}
}
...
func sysbri(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func syslog(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.position >= entry.ཚད {
		return 0
	}
	remaining := entry.ཚད - entry.position
	if count > remaining {
		count = remaining
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return logvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == obrionly || accessmode == ologbri || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func logvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	memorymanager := &mem.TMemorymanager{}
	tmppointer := memorymanager.Malloc(entry.ཚད)
	if tmppointer == nil {
		return Einval
	}
	tmp := Getbytesfrompointer(uintptr(tmppointer), int(entry.ཚད), int(entry.ཚད))
	logfile(entry.name[:entry.namelen], tmp)
...
func sysaccess(pathaddress uint32, mode uint32) int32 {
	...
	if !exists {
		namelen, name := copypath(pathaddress)
		exists = namelen != 0 && fileཚད(name[:namelen]) != 0
	}
	if !exists {
		return Enoent
	}
...
func sysgetcwd(bufferaddress uint32, ཚད uint32) int32 {
	if bufferaddress == 0 {
		return Efault
	}
	if ཚད < 2 {
	...
	buffer_2 := Getbytesfrompointer(uintptr(bufferaddress), int(ཚད), int(ཚད))
	buffer_2[0] = '/'
	buffer_2[1] = 0
	return 2
}

func fillposixstat(stataddress uint32, mode uint32, ཚད uint32, inode uint32) int32 {
	if stataddress == 0 {
		return Efault
	}
...
func sysstat(pathaddress uint32, stataddress uint32) int32 {
	...
	return fillposixstat(stataddress, sifreg|0444, ཚད, inode)
}
...
func sysfstat(fd int32, stataddress uint32) int32 {
	...
	case fdkindrootdirectory:
		return fillposixstat(stataddress, sifdir|0555, 0, 1)
	case fdkindfat:
		return fillposixstat(stataddress, sifreg|0444, entry.ཚད, uint32(fd+2))
	case fdkindsocket:
...
func socketsendto(fd int32, bufferaddress_2 uint32, length uint32, destinationaddress uint32, destinationlength uint32) int32 {
	...
	*packet = socketpacket{used: true, ཚད: length, source: socket.local}
	if length != 0 {
...
func logstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinbri + 1) % uint32(len(stdinbuffer))
	if next == stdinlog {
		return
	}
	stdinbuffer[stdinbri] = c
	stdinbri = next
}

func stdingetblocking() byte {
	for stdinlog == stdinbri {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinlog]
	stdinlog = (stdinlog + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portlogbyte(0x64) & 0x01) == 0 {
	}
	sc := Portlogbyte(0x60)
...
func sysexecve(cpu *Tcpustate, pathaddress uint32) int32 {
	...
	memorymanager := &mem.TMemorymanager{}
	filepointer := memorymanager.Malloc(ཚད)
	if filepointer == nil {
		return Einval
	}
	data := Getbytesfrompointer(uintptr(filepointer), int(ཚད), int(ཚད))
	logfile(name[:namelen], data)
	if ཚད < 52 || data[0] != 0x7F || data[1] != 'E' || data[2] != 'L' || data[3] != 'F' {
...
func sysfork(cpu *Tcpustate) int32 {
	...
	stackpointer := memorymanager.Malloc(Threadstackཚད)
	...
	child.Cpustate = (*Tcpustate)(Pointer(uintptr(stackpointer) + Threadstackཚད - Sizeof(Tcpustate{})))
	*child.Cpustate = *cpu
	child.Cpustate.Eax = 0
	child.Userstack_2 = cpu.Esp
	child.Userstackཚད_2 = 0
	child.Pid = pid
...
func fileཚད(filename []byte) uint32 {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()

	partition := Tmsdospartitiontable{}
	partition.Logpartition(&ata0s)

	bios := TBiosparameterblock32{}
	ཚད := bios.Len(&ata0s, partition.Mbr.Primarypartition[0], filename)
	ata0s.Flush()
	return ཚད
}

func logfile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Log(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
