...
const Procuserheapཚད = 1 * 1024 * 1024
...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	gtsowothread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if gtsowothread != nil {
		gtsowothread.Pid = process.id
		gtsowothread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(gtsowothread)))
	}
...
func (self *Processhelper) Spawn(entrypoint func(), threadhelper *TThreadhelper, scheduler *Scheduler, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	if process.Threads != nil && process.Threads.Sཚད_2 > 0 {
