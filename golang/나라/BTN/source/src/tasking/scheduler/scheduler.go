...
func (self *Schedulerdata) Getnextreadythread() *TThread {
	...
	for checked := 0; checked < list.Sཚད_2; checked++ {
		currentthreadindex++
		if currentthreadindex >= list.Sཚད_2 {
			currentthreadindex = 0
		}
...
func initpit(frequency uint32) {
	...
	Portbribyte(0x43, 0x36)
	Portbribyte(0x40, uint8(divisor&0xFF))
	Portbribyte(0x40, uint8((divisor>>8)&0xFF))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	if schedata.tickcount == schedata.frequency {
		...
		if list.Sཚད_2 > 0 && schedata.Enabled == true {
			...
			if nextthread.Stack != 0 {
				schedata.tss.Setstack(Segkerneldata, nextthread.Stack+Threadstackཚད)
			}
...
func Sབཀགcurrentthread(cpustate *Tcpustate) *Tcpustate {
	if schedata.currentthread == nil {
...
func Initialthreaduserjump(thread *TThread) {
	...
	schedata.tss.Setstack(Segkerneldata, thread.Stack+Threadstackཚད)
	...
	useresp := thread.Userstack_2 + thread.Userstackཚད_2
