...
const Threadstackཚད = 32 * 1024

type TThread struct {
	Cpustate	*Tcpustate
	Stack		uint32
	Userstack_2	uint32
	Userstackཚད_2	uint32
	Pid		uint32
	Parentpid	uint32
...
func (self *TThreadhelper) Createfromfunction(entrypoint_2 func(), Pagedirectoryentry uint32, iskernel bool) TThread {
	...
	result.Stack = uint32(uintptr(self.mem.Malloc(Threadstackཚད)))
	if result.Stack == 0 {
		return result
	}
	...
	result.Cpustate = (*Tcpustate)(Pointer(uintptr(result.Stack) + Threadstackཚད - Sizeof(Tcpustate{})))
	result.Cpustate.Esp = result.Stack + Threadstackཚད
	...
	result.Userstackཚད_2 = Userstackཚད
	result.Pid = 0
	result.Parentpid = 0
