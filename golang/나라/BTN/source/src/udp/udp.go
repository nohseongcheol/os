...
type IUserdatagramprotocolhandler interface {
	Handleuserdatagramprotocolmessage(socket *TUserdatagramprotocolsocket, data uintptr, ཚད uint16)
}
...
func (self *TUserdatagramprotocolhandler) Handleuserdatagramprotocolmessage(socket *TUserdatagramprotocolsocket, data uintptr, ཚད uint16) {
}

type IUserdatagramprotocolsocket interface {
	Handleuserdatagramprotocolmessage(data uintptr, ཚད uint16)
}
...
func (self *TUserdatagramprotocolsocket) Handleuserdatagramprotocolmessage(data uintptr, ཚད uint16) {
	if udphandler != nil {
		udphandler.Handleuserdatagramprotocolmessage(self, data, ཚད)
	}
}
func (self *TUserdatagramprotocolsocket) Send(pdata []byte, ཚད uint16) {
	var buffer_2 [4096]byte
	for i := 0; i < int(ཚད); i++ {
		buffer_2[i] = pdata[i]
	}
	var data = uintptr(Pointer(&buffer_2))
	udpprovider.Send(self, data, ཚད)
}
...
func (self *TUserdatagramprotocolprovider) Internetprotocolreceivewhen(sourceipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, internetprotocolpayload uintptr, ཚད uint32) bool {
	if ཚད < udpheaderཚད {
		return false
	}
	...
	if socket != nil {
		socket.Handleuserdatagramprotocolmessage(internetprotocolpayload+uintptr(udpheaderཚད), uint16(ཚད-udpheaderཚད))
	}
...
func (self *TUserdatagramprotocolprovider) Send(socket *TUserdatagramprotocolsocket, pdata uintptr, ཚད uint16) {
	var totallength = uint32(ཚད) + udpheaderཚད
	...
	for i := 0; i < int(ཚད); i++ {
		buffer_2[int(udpheaderཚད)+i] = databytes[i]
	}
