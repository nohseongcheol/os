...
var attributecontrollerlogport uint16 = 0x3c1
var attributecontrollerbriport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Briregister(register []byte) {
	var regindex uint16 = 0

	Portbribyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portbribyte(sequencerindexport, i)
		Portbribyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portbribyte(crtcindexport, 0x03)

	Portbribyte(crtcdataport, (Portlogbyte(crtcdataport) | 0x80))
	Portbribyte(crtcindexport, 0x11)
	Portbribyte(crtcdataport, (Portlogbyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portbribyte(crtcindexport, i)
		Portbribyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portbribyte(graphicscontrollerindexport, i)
		Portbribyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portlogbyte(attributecontrollerresetport)
		Portbribyte(attributecontrollerindexport, i)
		Portbribyte(attributecontrollerbriport, register[regindex])
		regindex++
	}

	Portlogbyte(attributecontrollerresetport)
	Portbribyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portbribyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portlogbyte(graphicscontrollerdataport) >> 2) & 0x03)
