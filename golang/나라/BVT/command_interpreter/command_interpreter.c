...
static size_t text_byte_length(const char *text)
{
    size_t length = 0;
    while (text[length] != '\0')
        length++;
...
{
    size_t length = text_byte_length(text);
    while (length > 0U) {
        ssize_t written_byte_count = write(STDOUT_FILENO, text, length);
        if (written_byte_count <= 0)
            return;
        text += written_byte_count;
        length -= (size_t)written_byte_count;
    }
}
...
static void write_integer(int value)
{
    ...
    while (digit_count > 0U) {
        digit_count--;
        (void)write(STDOUT_FILENO, &digit_characters[digit_count], 1);
    }
}
...
static int read_input_line(struct input_stream *input, char *input_line, size_t capacity)
{
    size_t position = 0;
    ...
    for (;;) {
        if (input->position == input->length) {
            bytes_read = read(input->input_descriptor, input->transfer_buffer, sizeof(input->transfer_buffer));
            if (bytes_read < 0) {
                if (errno == EINTR)
                    continue;
                return -1;
        ...
        character = input->transfer_buffer[input->position++];
        if (character == '\n')
            break;
...
static void display_file_contents(const char *file_name)
{
    ...
    while ((bytes_read = read(file_descriptor, transfer_buffer, sizeof(transfer_buffer))) > 0)
        (void)write(STDOUT_FILENO, transfer_buffer, (size_t)bytes_read);
    if (bytes_read < 0)
...
static void show_system_identity(void)
{
    ...
    write_text(system_identity.sysname);
    write_text(" ");
    write_text(system_identity.release);
    write_text(" ");
    write_text(system_identity.machine);
    write_text("\n");
}

static void run_program(int argument_count, char **arguments)
{
    pid_t child_process_identifier;
    ...
    write_integer(WEXITSTATUS(child_termination_status));
    write_text("\n");
}

static void test_datagram_loopback(const char *message)
{
    struct sockaddr_in receiving_endpoint_address = {0};
    struct sockaddr_in sender_endpoint_address = {0};
    socklen_t sender_address_length = sizeof(sender_endpoint_address);
    char received_data[96];
    size_t message_byte_length = text_byte_length(message);
    ...
    receiving_endpoint_address.sin_family = AF_INET;
    receiving_endpoint_address.sin_port = htons(40404);
    receiving_endpoint_address.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
    if (bind(receiving_socket, (const struct sockaddr *)&receiving_endpoint_address, sizeof(receiving_endpoint_address)) < 0) {
        report_error("bind");
        goto close_sockets;
    }
    if (connect(sending_socket, (const struct sockaddr *)&receiving_endpoint_address, sizeof(receiving_endpoint_address)) < 0) {
        report_error("connect");
        goto close_sockets;
    }
    if (send(sending_socket, message, message_byte_length, 0) != (ssize_t)message_byte_length) {
        report_error("send");
        goto close_sockets;
    }
    received_byte_count = recvfrom(receiving_socket, received_data, sizeof(received_data) - 1U, 0,
                         (struct sockaddr *)&sender_endpoint_address, &sender_address_length);
...
int main(void)
{
    ...
    (void)interpret_input(STDIN_FILENO);
