...
type TFile_system_parameters32 struct {
	jmp			[3]uint8
	softname		[8]byte
...
func (self *TFile_system_parameters32) Init(data []byte) {
...
func (self *TFile_system_parameters32) Len(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte) uint32 {
	...
	bpbpointer := memorymanager.Allocate_memory(90)
	bpbbytes := Getbytesfrompointer(uintptr(bpbpointer), 90, 90)
	var partitionoffset = partentry.Startlba

	hd.Read28(partitionoffset, &bpbbytes, 90)

	var file_system_parameters = TFile_system_parameters32{}
	file_system_parameters.Init(bpbbytes)

	var fatstart = partitionoffset + uint32(file_system_parameters.reservedsectors)
	var fatsize = file_system_parameters.tablesize

	var datastart = fatstart + fatsize*uint32(file_system_parameters.fatcopy)

	var rootstart = datastart + uint32(file_system_parameters.sectorspercluster)*(file_system_parameters.rootcluster-2)

	direntpointer := memorymanager.Allocate_memory(512)
	direntbytes := Getbytesfrompointer(uintptr(direntpointer), 512, 512)
	hd.Read28(rootstart, &direntbytes, 512)
...
func (self *TFile_system_parameters32) Read(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		for Size > 0 {
			var filesector = datastart + uint32(file_system_parameters.sectorspercluster)*uint32(nextfilecluster-2)
			...
			for ; Size > 0; Size -= 512 {
				...
				if dirent[i].size > 512 {
					...
					hd.Read28(filesector+uint32(sectoroffset), &buffer3, 512)

				} else {
					buffer3 = buffer_2[:dirent[i].size]
					hd.Read28(filesector+uint32(sectoroffset), &buffer3, int(dirent[i].size))
				}
				...
				if sectoroffset > int(file_system_parameters.sectorspercluster) {
					break
				}
			...
			hd.Read28(fatstart+fatsectorforcurrentcluster, &fatbuf, 512)
