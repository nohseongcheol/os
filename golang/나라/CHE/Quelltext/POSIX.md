...
freestanding library in `./benutzerbereich/posix`.
...
```sh
cd ./benutzerbereich/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./benutzerbereich/posix/deklarationen` and link the archive into an EngOS i386 user
...
`./benutzerbereich/posix/TEST_RESULTS.md`. Build all runtime test executables
