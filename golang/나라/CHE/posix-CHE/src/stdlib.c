...
int abs(int Wert) { return Wert < 0 ? -Wert : Wert; }
long labs(long Wert) { return Wert < 0 ? -Wert : Wert; }
...
char *getenv(const char *Systemidentität)
{
    size_t Länge = strlen(Systemidentität);
    char **cursor = environ;
    if (!Länge || strchr(Systemidentität, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, Systemidentität, Länge) && (*cursor)[Länge] == '=') return *cursor + Länge + 1;
        ++cursor;
    }
    return NULL;
}
