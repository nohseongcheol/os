...
GLOBL ·identifianttDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·identifiantt<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
GLOBL ·porteHandlers<>(SB), NOPTR, $NUM_IDT_ENTRIES*8
...
TEXT ·installeridt(SB),NOSPLIT,$0
	LEAQ ·identifianttDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·identifiantt<>(SB), BX
	MOVQ BX, 2(AX)
...
TEXT ·Poignéeinterruption(SB),NOSPLIT,$0-10
	...
	LEAQ ·interruptionporteélément(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	SUBQ $16, SP;                          \
	MOVQ R15, 0(SP);                       \
	MOVQ ·porteHandlers<>+8*num(SB), R15;   \
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchinterruption(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchinterruption(SB),NOSPLIT,$0
...
TEXT ·interruptionporteélément(SB),NOSPLIT,$0
