...
int abs(int valeur) { return valeur < 0 ? -valeur : valeur; }
long labs(long valeur) { return valeur < 0 ? -valeur : valeur; }
...
{
    size_t longueur = strlen(identité_du_système);
    char **cursor = environ;
    if (!longueur || strchr(identité_du_système, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, identité_du_système, longueur) && (*cursor)[longueur] == '=') return *cursor + longueur + 1;
        ++cursor;
    }
    return NULL;
}
