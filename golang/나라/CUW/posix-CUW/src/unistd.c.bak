...
ssize_t lesa(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_lesa, fd, buf, count));
}

ssize_t skirbi(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_skirbi, fd, buf, count));
}
