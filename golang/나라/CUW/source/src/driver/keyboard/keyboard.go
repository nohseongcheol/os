...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portlesabyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portlesabyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func skirbips2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portskirbibyte(commandport_2, value)
	return true
}

func skirbips2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portskirbibyte(dataport_2, value)
	return true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portlesabyte(commandport_2)&0x01) != 0; i++ {
		Portlesabyte(dataport_2)
	}

	if !skirbips2command(0xAE) || !skirbips2command(0x20) {
		return
	}
	...
	status |= 0x01
	status &^= 0x10
	if !skirbips2command(0x60) || !skirbips2data(status) {
		return
	}

	if !skirbips2data(0xF4) {
		return
	}
	ack, ok := lesaps2data()
	if !ok || ack != 0xFA {
...
var keyboardqueuelesa uint8
var keyboardqueueskirbi uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueueskirbi + 1) % keyboardqueuesize
	if next == keyboardqueuelesa {
		return
	}
	keyboardqueue[keyboardqueueskirbi] = key
	keyboardqueueskirbi = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuelesa != keyboardqueueskirbi {
		key := keyboardqueue[keyboardqueuelesa]
		keyboardqueuelesa = (keyboardqueuelesa + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portlesabyte(commandport_2)
