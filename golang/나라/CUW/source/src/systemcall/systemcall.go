...
const (
	...
	olesaonly	uint32	= 0
	oskirbionly	uint32	= 1
	olesaskirbi	uint32	= 2
	ocreate		uint32	= 0x40
...
func Sysprintstr(buffer string) {
	...
	Syscall(Sysskirbi, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysskirbi, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Syslesa:
		...
		return esp
	case Sysskirbi:
		cpu.Eax = uint32(sysskirbi(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|oskirbionly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: olesaonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: oskirbionly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: oskirbionly}
}
...
func sysskirbi(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func syslesa(fd int32, address uint32, count uint32) int32 {
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return lesavfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == oskirbionly || accessmode == olesaskirbi || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func lesavfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	lesafile(entry.name[:entry.namelen], tmp)
...
func Stdinputbyte(c byte) {
	next := (stdinskirbi + 1) % uint32(len(stdinbuffer))
	if next == stdinlesa {
		return
	}
	stdinbuffer[stdinskirbi] = c
	stdinskirbi = next
}

func stdingetblocking() byte {
	for stdinlesa == stdinskirbi {
...
func lesafile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
