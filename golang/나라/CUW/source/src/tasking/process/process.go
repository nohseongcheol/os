...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	prinsipalthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if prinsipalthread != nil {
		prinsipalthread.Pid = process.id
		prinsipalthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(prinsipalthread)))
	}
