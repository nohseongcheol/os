...
ssize_t Ανάγνωση(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Ανάγνωση, fd, buf, count));
}

ssize_t Εγγραφή(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Εγγραφή, fd, buf, count));
}

int Κλείσιμο(int fd)
{
    return (int)__syscall_result(SC1(SYS_Κλείσιμο, fd));
}
