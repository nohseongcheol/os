...
GLOBL ·τΑΥΤΌΤΗΤΑtDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·τΑΥΤΌΤΗΤΑt<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·εγκατάστασηidt(SB),NOSPLIT,$0
	LEAQ ·τΑΥΤΌΤΗΤΑtDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·τΑΥΤΌΤΗΤΑt<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·ΧειρολαβήΔιακοπή(SB),NOSPLIT,$0-10
	...
	LEAQ ·διακοπήgateκαταχώρηση(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
//--------------------------- -----------------------------------------
TEXT ·dispatchΔιακοπή(SB),NOSPLIT,$0
...
TEXT ·διακοπήgateκαταχώρηση(SB),NOSPLIT,$0
