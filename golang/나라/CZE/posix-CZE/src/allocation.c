...
void *malloc(size_t počet_číslic)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!počet_číslic) počet_číslic = 1;
    if (počet_číslic > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    počet_číslic = (počet_číslic + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= počet_číslic) {
            if (cursor->capacity - počet_číslic >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + počet_číslic);
                raw->capacity = cursor->capacity - počet_číslic - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = počet_číslic;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + počet_číslic;
    ...
    raw->capacity = počet_číslic; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t počet_číslic)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(počet_číslic);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!počet_číslic) počet_číslic = 1;
    cursor = (struct heap_block *)address - 1;
    if (počet_číslic <= cursor->capacity) return address;
    destination = malloc(počet_číslic);
