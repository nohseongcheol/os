...
int abs(int hodnota) { return hodnota < 0 ? -hodnota : hodnota; }
long labs(long hodnota) { return hodnota < 0 ? -hodnota : hodnota; }
...
char *getenv(const char *identita_systému)
{
    size_t délka = strlen(identita_systému);
    char **cursor = environ;
    if (!délka || strchr(identita_systému, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, identita_systému, délka) && (*cursor)[délka] == '=') return *cursor + délka + 1;
        ++cursor;
    }
    return NULL;
}
