...
{
    (void)Zápis(STDOUT_FILENO, text, text_length(text));
}

static void report(const char *identita_systému, int passed)
{
    checks++;
    if (passed) {
        say("PTEST:PASS:");
...
static pid_t reap_nohang(pid_t child, int *stav)
{
    unsigned int spins;
    pid_t result;

    for (spins = 0; spins < 2000000U; spins++) {
        result = waitpid(child, stav, WNOHANG);
        if (result != 0)
            return result;
    }
    return 0;
...
static void test_open_and_io(void)
{
    ...
    report("open-readonly", deskriptor_souboru >= 3);
    if (deskriptor_souboru >= 0) {
        report("read-bytes", Čtení(deskriptor_souboru, bytes, 2) == 2 &&
               (unsigned char)bytes[0] == 0x7f && bytes[1] == 'E');
        report("lseek-set", lseek(deskriptor_souboru, 0, SEEK_SET) == 0);
        report("lseek-cur", lseek(deskriptor_souboru, 1, SEEK_CUR) == 1);
        report("lseek-end", lseek(deskriptor_souboru, -1, SEEK_END) > 0);
        errno = 0;
        report("lseek-negative", lseek(deskriptor_souboru, -1, SEEK_SET) == -1 && errno == EINVAL);
        errno = 0;
        report("lseek-whence", lseek(deskriptor_souboru, 0, 99) == -1 && errno == EINVAL);
        report("fstat-file", fstat(deskriptor_souboru, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 2);
        errno = 0;
        report("fstat-efault", failed_with(fstat(deskriptor_souboru, (struct stat *)0), EFAULT));
        report("fsync-file", fsync(deskriptor_souboru) == 0);
        report("close-file", Zavřít(deskriptor_souboru) == 0);
        errno = 0;
        report("closed-fd", failed_with(Zavřít(deskriptor_souboru), EBADF));
    }

    root = Otevřít("/", O_RDONLY | O_DIRECTORY);
    report("open-directory", root >= 3);
    if (root >= 0) {
        report("fstat-directory", fstat(root, &st) == 0 && S_ISDIR(st.st_mode));
        errno = 0;
        report("read-directory", Čtení(root, bytes, 1) == -1 && errno == EISDIR);
        (void)Zavřít(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", Zápis(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", Čtení(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", Čtení(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", Zápis(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("open-enoent", failed_with(Otevřít("/NOFILE", O_RDONLY), ENOENT));
    errno = 0;
    report("open-efault", failed_with(Otevřít((const char *)0, O_RDONLY), EFAULT));
    errno = 0;
    report("open-write-erofs", failed_with(Otevřít("/USER2", O_WRONLY), EROFS));
    errno = 0;
    report("open-rdwr-erofs", failed_with(Otevřít("/USER2", O_RDWR), EROFS));
    errno = 0;
    report("open-create-erofs", failed_with(Otevřít("/NEWFILE", O_CREAT | O_WRONLY, 0600), EROFS));
    errno = 0;
    report("open-trunc-erofs", failed_with(Otevřít("/USER2", O_TRUNC | O_RDONLY), EROFS));
    errno = 0;
    report("open-append-erofs", failed_with(Otevřít("/USER2", O_APPEND | O_RDONLY), EROFS));
    errno = 0;
    report("open-enotdir", failed_with(Otevřít("/USER2", O_RDONLY | O_DIRECTORY), ENOTDIR));
    errno = 0;
    report("creat-erofs", failed_with(creat("/NEWFILE", 0600), EROFS));
    errno = 0;
    report("close-ebadf", failed_with(Zavřít(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    copy = dup(deskriptor_souboru);
    report("dup", copy >= 0 && copy != deskriptor_souboru);
    if (copy >= 0) {
        report("dup-shared-offset", Čtení(deskriptor_souboru, &first, 1) == 1 && Čtení(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Zavřít(deskriptor_souboru) == 0 && fstat(copy, &st) == 0);
        deskriptor_souboru = copy;
    }

    target = dup2(deskriptor_souboru, 20);
    report("dup2", target == 20 && fstat(20, &st) == 0);
    report("dup2-same", dup2(deskriptor_souboru, deskriptor_souboru) == deskriptor_souboru);
    if (target == 20)
        (void)Zavřít(20);
    errno = 0;
    report("dup2-old-ebadf", failed_with(dup2(-1, 10), EBADF));
    errno = 0;
    report("dup2-new-ebadf", failed_with(dup2(deskriptor_souboru, 99), EBADF));

    report("fcntl-getfd", fcntl(deskriptor_souboru, F_GETFD) == 0);
    report("fcntl-setfd", fcntl(deskriptor_souboru, F_SETFD, FD_CLOEXEC) == 0 &&
           fcntl(deskriptor_souboru, F_GETFD) == FD_CLOEXEC);
    high = fcntl(deskriptor_souboru, F_DUPFD, 10);
    report("fcntl-dupfd", high >= 10);
    if (high >= 0) {
        report("fcntl-fd-flags-independent", fcntl(high, F_GETFD) == 0);
        (void)Zavřít(high);
    }
    report("fcntl-getfl", (fcntl(deskriptor_souboru, F_GETFL) & O_ACCMODE) == O_RDONLY);
    report("fcntl-setfl", fcntl(deskriptor_souboru, F_SETFL, O_APPEND) == 0 &&
           (fcntl(deskriptor_souboru, F_GETFL) & O_APPEND) != 0);
    errno = 0;
    report("fcntl-command-einval", failed_with(fcntl(deskriptor_souboru, 999), EINVAL));
    errno = 0;
...
static void test_terminal_and_uname(void)
{
    ...
    if (deskriptor_souboru >= 0) {
        errno = 0;
        report("isatty-enotty", isatty(deskriptor_souboru) == 0 && errno == ENOTTY);
        (void)Zavřít(deskriptor_souboru);
    } else {
    ...
    report("uname", uname(&identita_systému) == 0 && identita_systému.sysname[0] != 0 &&
           identita_systému.nodename[0] != 0 && text_equal(identita_systému.machine, "i386"));
    errno = 0;
...
void posix_test_process(void)
{
    ...
    report("fork", child > 0);
    if (child > 0) {
        waited = reap_nohang(child, &stav);
        report("waitpid", waited == child && WIFEXITED(stav) && WEXITSTATUS(stav) == 23);
        errno = 0;
        report("waitpid-echild", waitpid(child, &stav, WNOHANG) == -1 && errno == ECHILD);
    }
    ...
    if (child > 0 && reap_nohang(child, &stav) == child) {
        report("fork-fd-isolation", Čtení(deskriptor_souboru, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
    ...
    if (child > 0) {
        waited = reap_nohang(child, &stav);
        report("execve", waited == child && WIFEXITED(stav) && WEXITSTATUS(stav) == 37);
    }
    ...
    if (child > 0) {
        ...
        for (spin = 0; spin < 2000000U; spin++)
            (void)getpid();
        waited = wait(&stav);
        report("wait", waited == child && WIFEXITED(stav) && WEXITSTATUS(stav) == 29);
    }
}
