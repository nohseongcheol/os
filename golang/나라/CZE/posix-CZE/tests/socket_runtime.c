...
static void say(const char *s) { (void)Zápis(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *identita_systému, int pass)
{
...
int main(void)
{
    ...
    char vyrovnávací_paměť_přenosu_2[8] = {0};
    ...
    report("recvfrom", recvfrom(server, vyrovnávací_paměť_přenosu_2, sizeof(vyrovnávací_paměť_přenosu_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       vyrovnávací_paměť_přenosu_2[0] == 'p' && vyrovnávací_paměť_přenosu_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, vyrovnávací_paměť_přenosu_2, sizeof(vyrovnávací_paměť_přenosu_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Zavřít(client) == 0);
    report("close-server", Zavřít(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
