...
ssize_t Læse(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Læse, fd, buf, count));
}

ssize_t Skrive(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Skrive, fd, buf, count));
}

int Luk(int fd)
{
    return (int)__syscall_result(SC1(SYS_Luk, fd));
}
