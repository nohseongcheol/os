...
ssize_t anbeb(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_anbeb, fd, buf, count));
}

ssize_t tsahaf(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_tsahaf, fd, buf, count));
}
