...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portanbebword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portanbebword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portanbebword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portanbebword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portanbebword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portanbebword(Macaddress4port) / 256)
	...
	Porttsahafword(registeraddressport, 20)
	Porttsahafword(buscontrolregisterdataport, 0x102)

	Porttsahafword(registeraddressport, 0)
	Porttsahafword(registerdataport, 0x04)
	...
	Porttsahafword(registeraddressport, 1)
	Porttsahafword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Porttsahafword(registeraddressport, 2)
	Porttsahafword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Porttsahafword(registeraddressport, 0)
	Porttsahafword(registerdataport, 0x41)

	Porttsahafword(registeraddressport, 4)
	temporary := Portanbebword(registerdataport)
	Porttsahafword(registeraddressport, 4)
	Porttsahafword(registerdataport, temporary|0xC00)

	Porttsahafword(registeraddressport, 0)
	Porttsahafword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portanbebword(resetport)
	Porttsahafword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Porttsahafword(registeraddressport, 0)
	temporary := uint32(Portanbebword(registerdataport))
	...
	Porttsahafword(registeraddressport, 0)
	Porttsahafword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Porttsahafword(registeraddressport, 0)
	Porttsahafword(registerdataport, 0x48)
