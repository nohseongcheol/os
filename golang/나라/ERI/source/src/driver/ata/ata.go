...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Tsahaf(0xA0)
	} else {
		self.deviceport.Tsahaf(0xB0)
	}
	self.controlport.Tsahaf(0)
	self.deviceport.Tsahaf(0xA0)

	var status uint8 = self.commandport.Anbeb()
	if status == 0xFF {
	...
	self.sectorcountport.Tsahaf(0)
	self.lbalowport.Tsahaf(0)
	self.lbamidport.Tsahaf(0)
	self.lbahiport.Tsahaf(0)
	self.commandport.Tsahaf(0xEC)

	status = self.commandport.Anbeb()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Anbeb28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Tsahaf(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Tsahaf(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Tsahaf(0)
	self.sectorcountport.Tsahaf(1)

	self.lbalowport.Tsahaf(uint8(sector & 0x000000FF))
	self.lbamidport.Tsahaf(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Tsahaf(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Tsahaf(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Anbeb()
...
func (self *TAdvancedtechnologyattachment) Tsahaf28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Tsahaf(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Tsahaf(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Tsahaf(0)
	self.sectorcountport.Tsahaf(1)
	self.lbalowport.Tsahaf(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Tsahaf(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Tsahaf(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Tsahaf(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Tsahaf(0xE0)
	} else {
		self.deviceport.Tsahaf(0xF0)
	}
	self.commandport.Tsahaf(0xE7)
