...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portanbebbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portanbebbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func tsahafps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Porttsahafbyte(commandport_2, value)
	return true
}

func tsahafps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Porttsahafbyte(dataport_2, value)
	return true
}

func anbebps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portanbebbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portanbebbyte(commandport_2)&0x01) != 0; i++ {
		Portanbebbyte(dataport_2)
	}

	if !tsahafps2command(0xAE) || !tsahafps2command(0x20) {
		return
	}
	status, ok := anbebps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !tsahafps2command(0x60) || !tsahafps2data(status) {
		return
	}

	if !tsahafps2data(0xF4) {
		return
	}
	ack, ok := anbebps2data()
	if !ok || ack != 0xFA {
...
var keyboardqueueanbeb uint8
var keyboardqueuetsahaf uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuetsahaf + 1) % keyboardqueuesize
	if next == keyboardqueueanbeb {
		return
	}
	keyboardqueue[keyboardqueuetsahaf] = key
	keyboardqueuetsahaf = next
}

func Processpendingkeyboardevents() {
	for keyboardqueueanbeb != keyboardqueuetsahaf {
		key := keyboardqueue[keyboardqueueanbeb]
		keyboardqueueanbeb = (keyboardqueueanbeb + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portanbebbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portanbebbyte(dataport_2)
	if scancode == 0xE0 {
