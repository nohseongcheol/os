...
func (self *TPeripheralcomponentinterconnectcontroller) Anbeb(bus uint16, device_2 uint16, function uint16, registeroffset uint32) uint32 {
	var id uint32 = 0
	id = 0x1<<31 | (uint32(bus&0xFF) << 16) | (uint32(device_2&0x1f) << 11) | (uint32(function&0x07) << 8) | uint32(registeroffset&0xFC)

	Porttsahafdword(self.commandport, id)

	result1 := Portanbebdword(self.dataport)
	result2 := (result1 >> (8 * (registeroffset % 4)))

	return result2
}

func (self *TPeripheralcomponentinterconnectcontroller) Tsahaf(bus uint16, device_2 uint16, function uint16, registeroffset uint32, value uint32) {
	var id uint32
	id = 0x1<<31 | uint32((bus&0xFF)<<16) | uint32((device_2&0x1f)<<11) | uint32((function&0x07)<<8) | uint32(registeroffset&0xFC)
	Porttsahafdword(self.commandport, id)
	Porttsahafdword(self.dataport, value)
}
func (self *TPeripheralcomponentinterconnectcontroller) Devicehasfunctions(bus uint16, device_2 uint16) bool {
	result := self.Anbeb(bus, device_2, 0, 0x0E)
	if (result & (1 << 7)) != 0 {
		return true
	} else {
		return false
...
func (self *TPeripheralcomponentinterconnectcontroller) Getdevicedescriptor(bus uint16, device_2 uint16, function uint16) TPeripheralcomponentinterconnectdevicedescriptor {
	...
	result.Vendorid = uint16(self.Anbeb(bus, device_2, function, 0x00))
	result.Deviceid = uint16(self.Anbeb(bus, device_2, function, 0x02))

	result.classid = uint8(self.Anbeb(bus, device_2, function, 0x0b))
	result.subclassid = uint8(self.Anbeb(bus, device_2, function, 0x0a))
	result.interfaceid = uint8(self.Anbeb(bus, device_2, function, 0x09))

	result.revision = uint8(self.Anbeb(bus, device_2, function, 0x08))
	result.Interrupt = uint32(self.Anbeb(bus, device_2, function, 0x3C))
...
func (self *TPeripheralcomponentinterconnectcontroller) Getbaseaddressregister(bus uint16, device_2 uint16, function uint16, bar uint16) TBaseaddressregister {
	...
	headertype := self.Anbeb(bus, device_2, function, 0x0E) & 0x7F
	var maxbars int = int(6 - (4 * headertype))
	if bar >= uint16(maxbars) {
		return result
	}

	barvalue := self.Anbeb(bus, device_2, function, uint32(0x10+4*bar))
