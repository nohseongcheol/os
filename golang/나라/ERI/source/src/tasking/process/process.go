...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	ዋናthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if ዋናthread != nil {
		ዋናthread.Pid = process.id
		ዋናthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(ዋናthread)))
	}
