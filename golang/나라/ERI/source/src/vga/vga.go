...
var attributecontrolleranbebport uint16 = 0x3c1
var attributecontrollertsahafport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Tsahafregister(register []byte) {
	var regindex uint16 = 0

	Porttsahafbyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Porttsahafbyte(sequencerindexport, i)
		Porttsahafbyte(sequencerdataport, register[regindex])
		regindex++
	}

	Porttsahafbyte(crtcindexport, 0x03)

	Porttsahafbyte(crtcdataport, (Portanbebbyte(crtcdataport) | 0x80))
	Porttsahafbyte(crtcindexport, 0x11)
	Porttsahafbyte(crtcdataport, (Portanbebbyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Porttsahafbyte(crtcindexport, i)
		Porttsahafbyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Porttsahafbyte(graphicscontrollerindexport, i)
		Porttsahafbyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portanbebbyte(attributecontrollerresetport)
		Porttsahafbyte(attributecontrollerindexport, i)
		Porttsahafbyte(attributecontrollertsahafport, register[regindex])
		regindex++
	}

	Portanbebbyte(attributecontrollerresetport)
	Porttsahafbyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Porttsahafbyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portanbebbyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Tsahafregister(g320x200x256)
