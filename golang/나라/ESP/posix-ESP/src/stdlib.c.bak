...
{
    size_t longitud = strlen(identidad_del_sistema);
    char **cursor = environ;
    if (!longitud || strchr(identidad_del_sistema, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, identidad_del_sistema, longitud) && (*cursor)[longitud] == '=') return *cursor + longitud + 1;
        ++cursor;
    }
    return NULL;
}
