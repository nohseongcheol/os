...
ssize_t Lugemine(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Lugemine, fd, buf, count));
}

ssize_t Kirjutamine(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Kirjutamine, fd, buf, count));
}

int Sulge(int fd)
{
    return (int)__syscall_result(SC1(SYS_Sulge, fd));
}
