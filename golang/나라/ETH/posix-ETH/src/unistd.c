...
ssize_t ማንበቢያ(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ማንበቢያ, fd, buf, count));
}

ssize_t መጻፊያ(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_መጻፊያ, fd, buf, count));
}

int መዝጊያ(int fd)
{
    return (int)__syscall_result(SC1(SYS_መዝጊያ, fd));
}
