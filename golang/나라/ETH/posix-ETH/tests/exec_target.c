...
int main(int argc, char **argv, char **envp)
{
    ...
    (void)መጻፊያ(STDOUT_FILENO, loaded, sizeof(loaded) - 1);
    if (argc == 2 && argv != (char **)0 && envp != (char **)0 &&
        ...
        (void)መጻፊያ(STDOUT_FILENO, args_pass, sizeof(args_pass) - 1);
    } else {
        (void)መጻፊያ(STDOUT_FILENO, args_fail, sizeof(args_fail) - 1);
        _exit(38);
    }
    errno = 0;
    if (fcntl(10, F_GETFD) == -1 && errno == EBADF) {
        static const char cloexec_pass[] = "PTEST:PASS:cloexec\n";
        (void)መጻፊያ(STDOUT_FILENO, cloexec_pass, sizeof(cloexec_pass) - 1);
        _exit(37);
    }
    {
        static const char cloexec_fail[] = "PTEST:FAIL:cloexec\n";
        (void)መጻፊያ(STDOUT_FILENO, cloexec_fail, sizeof(cloexec_fail) - 1);
    }
    _exit(39);
}
