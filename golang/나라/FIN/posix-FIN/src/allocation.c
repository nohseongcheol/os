...
void *malloc(size_t numeroiden_määrä)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!numeroiden_määrä) numeroiden_määrä = 1;
    if (numeroiden_määrä > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    numeroiden_määrä = (numeroiden_määrä + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= numeroiden_määrä) {
            if (cursor->capacity - numeroiden_määrä >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + numeroiden_määrä);
                raw->capacity = cursor->capacity - numeroiden_määrä - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = numeroiden_määrä;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + numeroiden_määrä;
    ...
    raw->capacity = numeroiden_määrä; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t numeroiden_määrä)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(numeroiden_määrä);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!numeroiden_määrä) numeroiden_määrä = 1;
    cursor = (struct heap_block *)address - 1;
    if (numeroiden_määrä <= cursor->capacity) return address;
    destination = malloc(numeroiden_määrä);
