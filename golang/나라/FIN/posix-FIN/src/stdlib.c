...
int abs(int arvo) { return arvo < 0 ? -arvo : arvo; }
long labs(long arvo) { return arvo < 0 ? -arvo : arvo; }
...
char *getenv(const char *järjestelmätiedot)
{
    size_t pituus = strlen(järjestelmätiedot);
    char **cursor = environ;
    if (!pituus || strchr(järjestelmätiedot, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, järjestelmätiedot, pituus) && (*cursor)[pituus] == '=') return *cursor + pituus + 1;
        ++cursor;
    }
    return NULL;
}
