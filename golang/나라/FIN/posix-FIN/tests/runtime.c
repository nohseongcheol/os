...
{
    (void)Kirjoitus(STDOUT_FILENO, text, text_length(text));
}

static void report(const char *järjestelmätiedot, int passed)
{
    checks++;
    if (passed) {
        say("PTEST:PASS:");
...
static pid_t reap_nohang(pid_t child, int *tila)
{
    unsigned int spins;
    pid_t result;

    for (spins = 0; spins < 2000000U; spins++) {
        result = waitpid(child, tila, WNOHANG);
        if (result != 0)
            return result;
    }
    return 0;
...
static void test_open_and_io(void)
{
    ...
    report("open-readonly", tiedostokuvaaja >= 3);
    if (tiedostokuvaaja >= 0) {
        report("read-bytes", Luku(tiedostokuvaaja, bytes, 2) == 2 &&
               (unsigned char)bytes[0] == 0x7f && bytes[1] == 'E');
        report("lseek-set", lseek(tiedostokuvaaja, 0, SEEK_SET) == 0);
        report("lseek-cur", lseek(tiedostokuvaaja, 1, SEEK_CUR) == 1);
        report("lseek-end", lseek(tiedostokuvaaja, -1, SEEK_END) > 0);
        errno = 0;
        report("lseek-negative", lseek(tiedostokuvaaja, -1, SEEK_SET) == -1 && errno == EINVAL);
        errno = 0;
        report("lseek-whence", lseek(tiedostokuvaaja, 0, 99) == -1 && errno == EINVAL);
        report("fstat-file", fstat(tiedostokuvaaja, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 2);
        errno = 0;
        report("fstat-efault", failed_with(fstat(tiedostokuvaaja, (struct stat *)0), EFAULT));
        report("fsync-file", fsync(tiedostokuvaaja) == 0);
        report("close-file", Sulje(tiedostokuvaaja) == 0);
        errno = 0;
        report("closed-fd", failed_with(Sulje(tiedostokuvaaja), EBADF));
    }

    root = Avaa("/", O_RDONLY | O_DIRECTORY);
    report("open-directory", root >= 3);
    if (root >= 0) {
        report("fstat-directory", fstat(root, &st) == 0 && S_ISDIR(st.st_mode));
        errno = 0;
        report("read-directory", Luku(root, bytes, 1) == -1 && errno == EISDIR);
        (void)Sulje(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", Kirjoitus(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", Luku(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", Luku(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", Kirjoitus(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("open-enoent", failed_with(Avaa("/NOFILE", O_RDONLY), ENOENT));
    errno = 0;
    report("open-efault", failed_with(Avaa((const char *)0, O_RDONLY), EFAULT));
    errno = 0;
    report("open-write-erofs", failed_with(Avaa("/USER2", O_WRONLY), EROFS));
    errno = 0;
    report("open-rdwr-erofs", failed_with(Avaa("/USER2", O_RDWR), EROFS));
    errno = 0;
    report("open-create-erofs", failed_with(Avaa("/NEWFILE", O_CREAT | O_WRONLY, 0600), EROFS));
    errno = 0;
    report("open-trunc-erofs", failed_with(Avaa("/USER2", O_TRUNC | O_RDONLY), EROFS));
    errno = 0;
    report("open-append-erofs", failed_with(Avaa("/USER2", O_APPEND | O_RDONLY), EROFS));
    errno = 0;
    report("open-enotdir", failed_with(Avaa("/USER2", O_RDONLY | O_DIRECTORY), ENOTDIR));
    errno = 0;
    report("creat-erofs", failed_with(creat("/NEWFILE", 0600), EROFS));
    errno = 0;
    report("close-ebadf", failed_with(Sulje(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    copy = dup(tiedostokuvaaja);
    report("dup", copy >= 0 && copy != tiedostokuvaaja);
    if (copy >= 0) {
        report("dup-shared-offset", Luku(tiedostokuvaaja, &first, 1) == 1 && Luku(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Sulje(tiedostokuvaaja) == 0 && fstat(copy, &st) == 0);
        tiedostokuvaaja = copy;
    }

    target = dup2(tiedostokuvaaja, 20);
    report("dup2", target == 20 && fstat(20, &st) == 0);
    report("dup2-same", dup2(tiedostokuvaaja, tiedostokuvaaja) == tiedostokuvaaja);
    if (target == 20)
        (void)Sulje(20);
    errno = 0;
    report("dup2-old-ebadf", failed_with(dup2(-1, 10), EBADF));
    errno = 0;
    report("dup2-new-ebadf", failed_with(dup2(tiedostokuvaaja, 99), EBADF));

    report("fcntl-getfd", fcntl(tiedostokuvaaja, F_GETFD) == 0);
    report("fcntl-setfd", fcntl(tiedostokuvaaja, F_SETFD, FD_CLOEXEC) == 0 &&
           fcntl(tiedostokuvaaja, F_GETFD) == FD_CLOEXEC);
    high = fcntl(tiedostokuvaaja, F_DUPFD, 10);
    report("fcntl-dupfd", high >= 10);
    if (high >= 0) {
        report("fcntl-fd-flags-independent", fcntl(high, F_GETFD) == 0);
        (void)Sulje(high);
    }
    report("fcntl-getfl", (fcntl(tiedostokuvaaja, F_GETFL) & O_ACCMODE) == O_RDONLY);
    report("fcntl-setfl", fcntl(tiedostokuvaaja, F_SETFL, O_APPEND) == 0 &&
           (fcntl(tiedostokuvaaja, F_GETFL) & O_APPEND) != 0);
    errno = 0;
    report("fcntl-command-einval", failed_with(fcntl(tiedostokuvaaja, 999), EINVAL));
    errno = 0;
...
static void test_terminal_and_uname(void)
{
    ...
    if (tiedostokuvaaja >= 0) {
        errno = 0;
        report("isatty-enotty", isatty(tiedostokuvaaja) == 0 && errno == ENOTTY);
        (void)Sulje(tiedostokuvaaja);
    } else {
    ...
    report("uname", uname(&järjestelmätiedot) == 0 && järjestelmätiedot.sysname[0] != 0 &&
           järjestelmätiedot.nodename[0] != 0 && text_equal(järjestelmätiedot.machine, "i386"));
    errno = 0;
...
void posix_test_process(void)
{
    ...
    report("fork", child > 0);
    if (child > 0) {
        waited = reap_nohang(child, &tila);
        report("waitpid", waited == child && WIFEXITED(tila) && WEXITSTATUS(tila) == 23);
        errno = 0;
        report("waitpid-echild", waitpid(child, &tila, WNOHANG) == -1 && errno == ECHILD);
    }
    ...
    if (child > 0 && reap_nohang(child, &tila) == child) {
        report("fork-fd-isolation", Luku(tiedostokuvaaja, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
    ...
    if (child > 0) {
        waited = reap_nohang(child, &tila);
        report("execve", waited == child && WIFEXITED(tila) && WEXITSTATUS(tila) == 37);
    }
    ...
    if (child > 0) {
        ...
        for (spin = 0; spin < 2000000U; spin++)
            (void)getpid();
        waited = wait(&tila);
        report("wait", waited == child && WIFEXITED(tila) && WEXITSTATUS(tila) == 29);
    }
}
