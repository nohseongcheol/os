...
static void say(const char *s) { (void)Kirjoitus(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *järjestelmätiedot, int pass)
{
...
int main(void)
{
    ...
    report("recvfrom", recvfrom(server, siirtopuskuri_2, sizeof(siirtopuskuri_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       siirtopuskuri_2[0] == 'p' && siirtopuskuri_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, siirtopuskuri_2, sizeof(siirtopuskuri_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Sulje(client) == 0);
    report("close-server", Sulje(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
