...
freestanding library in `./käyttäjätila/posix`.
...
```sh
cd ./käyttäjätila/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./käyttäjätila/posix/esittelyt` and link the archive into an EngOS i386 user
...
`./käyttäjätila/posix/TEST_RESULTS.md`. Build all runtime test executables
