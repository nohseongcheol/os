...
GLOBL ·tUNNISTEtDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·tUNNISTEt<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·asennaidt(SB),NOSPLIT,$0
	LEAQ ·tUNNISTEtDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·tUNNISTEt<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·KahvaKeskeytys(SB),NOSPLIT,$0-10
	...
	LEAQ ·keskeytysgatehakusana(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
//--------------------------- -----------------------------------------
TEXT ·dispatchKeskeytys(SB),NOSPLIT,$0
...
TEXT ·keskeytysgatehakusana(SB),NOSPLIT,$0
