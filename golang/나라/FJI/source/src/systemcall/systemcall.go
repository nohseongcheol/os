...
func Sysread_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysread, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Syswrite, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Syswrite, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func syswrite(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysread(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return readstdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return readvfsfile(entry, buffer, count)
}
...
func readvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	memorymanager := &mem.TMemorymanager{}
	tmppointer := memorymanager.Allocate_memory(entry.size)
	if tmppointer == nil {
		return Einval
	}
...
func readstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func pushexecunsignedinteger32(stack_storage *uint32, value uint32) {
	*stack_storage -= 4
	*(*uint32)(Pointer(uintptr(*stack_storage))) = value
}
...
func setupexecstack(cpu *Tcpustate, arguments_2 *execvector, environment *execvector) int32 {
	...
	for i := int(environment.count) - 1; i >= 0; i-- {
		...
		stack_storage -= length
		destination := Getbytesfrompointer(uintptr(stack_storage), int(length), int(length))
		copy(destination, environment.values[i][:length])
		environmentpointers[i] = stack_storage
	}
	for i := int(arguments_2.count) - 1; i >= 0; i-- {
		...
		argumentpointers[i] = stack_storage
	}
	stack_storage &= ^uint32(3)
	pushexecunsignedinteger32(&stack_storage, 0)
	for i := int(environment.count) - 1; i >= 0; i-- {
		pushexecunsignedinteger32(&stack_storage, environmentpointers[i])
	}
	pushexecunsignedinteger32(&stack_storage, 0)
	for i := int(arguments_2.count) - 1; i >= 0; i-- {
		pushexecunsignedinteger32(&stack_storage, argumentpointers[i])
	}
	pushexecunsignedinteger32(&stack_storage, arguments_2.count)
	cpu.Esp = stack_storage
	cpu.Ebp = 0
	return 0
}
...
func sysexecve(cpu *Tcpustate, pathaddress uint32) int32 {
	...
	memorymanager := &mem.TMemorymanager{}
	filepointer := memorymanager.Allocate_memory(size)
	if filepointer == nil {
		return Einval
	}
...
func sysfork(cpu *Tcpustate) int32 {
	...
	memorymanager := &mem.TMemorymanager{}
	threadpointer := memorymanager.Allocate_memory(uint32(Sizeof(TThread{})))
	stackpointer := memorymanager.Allocate_memory(Threadstacksize)
