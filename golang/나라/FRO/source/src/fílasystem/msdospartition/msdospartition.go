...
func (self *TPartitiontableentry) Init(data [16]byte) {
	...
	self.longd = Unsignedinteger32r(Arraytounsignedinteger32(buffer2))
}
...
func (self *Tmsdospartitiontable) Lesapartition(hd *TFramkomiðtechnologyattachment) {

	console_2 := TConsole{}
	console_2.MPrint(([]byte)("Reading MBR"))

	var partitionbýt [512]byte
	var buffer_2 = partitionbýt[:]
	hd.Lesa28(0, &buffer_2, 512)

	self.Mbr = TMasterbootrecord{}
	var i int = 0
	for ; i < 440; i++ {
		self.Mbr.bootloader[i] = partitionbýt[i]
	}

	var buffer1 [4]byte
	copy(buffer1[:4], partitionbýt[i:i+4])
	self.Mbr.signature = Unsignedinteger32r(Arraytounsignedinteger32(buffer1))
	i += 4

	var buffer2 [2]byte
	copy(buffer2[:2], partitionbýt[i:i+2])
	self.Mbr.unused = Unsignedinteger16r(Arraytounsignedinteger16(buffer2))
	i += 2

	var buffer3 [16]byte
	copy(buffer3[:16], partitionbýt[i:i+16])
	...
	var buffer6 [2]byte
	copy(buffer6[:2], partitionbýt[i:i+2])
