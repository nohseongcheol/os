...
var PrimarypicStýriboðioport uint16 = 0x20
var Primarypicdataioport uint16 = 0x21
var SecondarypicStýriboðioport uint16 = 0xA0
...
func (self *TInterruptmanager) Init(hardwareinterruptoffset uint16, globaldescriptortable *TShareddescriptortable, taskmanager *TTaskmanager) {
	...
	Portskrivabyte(PrimarypicStýriboðioport, 0x11)
	Portskrivabyte(SecondarypicStýriboðioport, 0x11)

	Portskrivabyte(Primarypicdataioport, 0x20)
	Portskrivabyte(Secondarypicdataioport, 0x28)

	Portskrivabyte(Primarypicdataioport, 0x04)
	Portskrivabyte(Secondarypicdataioport, 0x02)

	Portskrivabyte(Primarypicdataioport, 0x01)
	Portskrivabyte(Secondarypicdataioport, 0x01)

	Portskrivabyte(Primarypicdataioport, 0xF8)
	Portskrivabyte(Secondarypicdataioport, 0xEF)

	idtpointer := [6]uint8{0, 0, 0, 0, 0, 0}
	stødd := (*uint16)(Pointer(&idtpointer[0]))
	(*stødd) = (uint16)(Sizeof(idtdata) - 1)
...
func (self *TInterruptmanager) Interruptdescriptortableentryset(interrupt int,
	...
	handleraddressLágurbits := (*uint16)(Pointer(&idtdata[interrupt*8+0]))
	(*handleraddressLágurbits) = uint16(handler & 0xFFFF)
	...
	handleraddressHøgtbits := (*uint16)(Pointer(&idtdata[interrupt*8+6]))
	(*handleraddressHøgtbits) = uint16((handler >> 16) & 0xFFFF)
...
func (self *TInterruptmanager) Dohandleinterrupt(interrupt uint8, esp uint32) uint32 {
	...
	if !handlerKoyr && interrupt == uint8(self.hardwareinterruptoffset) && self.taskmanager != nil {
		esp = uint32(uintptr(Pointer(self.taskmanager.Schedule((*TcpuStøða)(Pointer(uintptr(esp)))))))

	}
	if !handlerKoyr && interrupt == 0x80 {
	...
	if 0x20 <= interrupt && interrupt < 0x30 {
		if 0x28 <= interrupt {
			Portskrivabyte(SecondarypicStýriboðioport, 0x20)
		}
		Portskrivabyte(PrimarypicStýriboðioport, 0x20)
	}
	return esp
}
...
func handleunhandledsyscall(esp uint32) uint32 {
	cpu := (*TcpuStøða)(Pointer(uintptr(esp)))
...
func exceptionhasBrekcode(interrupt uint32) bool {
	switch interrupt {
