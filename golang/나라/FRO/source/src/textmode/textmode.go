...
func (e textmodewriter) Skriva(p []byte) (int, error) {
	textmodeprintbýt(p)
	return len(p), nil
}

type textmodeBrekwriter struct {
}

func (e textmodeBrekwriter) Skriva(p []byte) (int, error) {
	textmodeprintBrekbýt(p)
	return len(p), nil
}
...
func textmodeÓvirkjacursor() {
	Outb(0x3D4, 0x0A)
	Outb(0x3D5, 0x20)
}

func textmodeDagførcursor(x int, y int) {
...
func textmodeprintbýt(a []byte) {
	for _, b := range a {
		textmodeprintchar(uint8(b))
	}
}

func textmodeprintBrekbýt(a []byte) {
	for _, b := range a {
...
func textmodeprintBrek(s string) {
	for _, b := range s {
		textmodeprintcharcol(uint8(b), 4<<4|0xf)
	}
}
