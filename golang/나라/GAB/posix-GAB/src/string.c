...
void *memmove(void *destination, const void *source, size_t longueur)
{
    ...
    if ((uintptr_t)destination <= (uintptr_t)source)
        return memcpy(destination, source, longueur);
    while (longueur) { --longueur; cursor[longueur] = input[longueur]; }
    return destination;
}

void *memset(void *destination, int character, size_t longueur)
{
    unsigned char *cursor = destination;
    while (longueur--) *cursor++ = (unsigned char)character;
    return destination;
}

int memcmp(const void *left, const void *right, size_t longueur)
{
...
void *memchr(const void *source, int character, size_t longueur)
{
...
{
    size_t longueur = 0;
    while (longueur < limit && text[longueur]) ++longueur;
    return longueur;
}
...
{
    size_t longueur = strnlen(source, limit);
    memcpy(destination, source, longueur);
    memset(destination + longueur, 0, limit - longueur);
    return destination + longueur;
}
...
size_t strxfrm(char *destination, const char *source, size_t limit)
{
    ...
    if (limit) {
        size_t copy_length = longueur < limit ? longueur : limit;
        memcpy(destination, source, copy_length);
        if (longueur < limit) destination[longueur] = 0;
    }
    return longueur;
}
...
{
    size_t longueur = strlen(source);
    if (!longueur) return (char *)text;
    while (*text) {
        if (strncmp(text, source, longueur) == 0) return (char *)text;
        ++text;
    }
    return NULL;
}
...
{
    size_t longueur = 0;
    while (text[longueur] && strchr(delimiters, text[longueur])) ++longueur;
    return longueur;
}
...
{
    size_t longueur = strnlen(text, limit);
    char *destination = malloc(longueur + 1);
    if (destination) { memcpy(destination, text, longueur); destination[longueur] = 0; }
    return destination;
}
