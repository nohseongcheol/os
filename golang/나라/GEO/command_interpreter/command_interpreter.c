...
static void write_text(const char *text)
{
    ...
    while (length > 0U) {
        ssize_t bytes_written = ჩაწერა(STDOUT_FILENO, text, length);
        if (bytes_written <= 0)
            return;
        text += bytes_written;
...
static void write_integer(int value)
{
    ...
    while (size > 0U) {
        size--;
        (void)ჩაწერა(STDOUT_FILENO, &digit_text[size], 1);
    }
}
...
static int read_line(struct input_stream *input, char *input_line, size_t capacity)
{
    ...
    for (;;) {
        if (input->position == input->length) {
            bytes_read = კითხვა(input->input_descriptor, input->buffer, sizeof(input->buffer));
            if (bytes_read < 0) {
                if (errno == EINTR)
                    continue;
                return -1;
...
static int command_source(const char *file_name)
{
    ...
    file_descriptor = გახსნა(file_name, O_RDONLY);
...
{
    char buffer[128];
    int file_descriptor = გახსნა(file_name, O_RDONLY);
    ssize_t bytes_read;
    ...
    while ((bytes_read = კითხვა(file_descriptor, buffer, sizeof(buffer))) > 0)
        (void)ჩაწერა(STDOUT_FILENO, buffer, (size_t)bytes_read);
    if (bytes_read < 0)
...
cleanup:
    if (send_socket >= 0)
        (void)დახურვა(send_socket);
    if (receive_socket >= 0)
        (void)დახურვა(receive_socket);
}
