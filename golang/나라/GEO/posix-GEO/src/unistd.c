...
ssize_t კითხვა(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_კითხვა, fd, buf, count));
}

ssize_t ჩაწერა(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ჩაწერა, fd, buf, count));
}

int დახურვა(int fd)
{
    return (int)__syscall_result(SC1(SYS_დახურვა, fd));
}
