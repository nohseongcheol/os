...
int abs(int valor) { return valor < 0 ? -valor : valor; }
long labs(long valor) { return valor < 0 ? -valor : valor; }
...
char *getenv(const char *identidade_do_sistema)
{
    size_t comprimento = strlen(identidade_do_sistema);
    char **cursor = environ;
    if (!comprimento || strchr(identidade_do_sistema, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, identidade_do_sistema, comprimento) && (*cursor)[comprimento] == '=') return *cursor + comprimento + 1;
        ++cursor;
    }
    return NULL;
}
