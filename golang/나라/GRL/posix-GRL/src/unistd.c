...
ssize_t atuarneq(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_atuarneq, fd, buf, count));
}

ssize_t allanneq(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_allanneq, fd, buf, count));
}
