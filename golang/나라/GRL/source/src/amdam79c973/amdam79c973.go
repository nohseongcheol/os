...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portatuarneqword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portatuarneqword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portatuarneqword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portatuarneqword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portatuarneqword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portatuarneqword(Macaddress4port) / 256)
	...
	Portallanneqword(registeraddressport, 20)
	Portallanneqword(buscontrolregisterdataport, 0x102)

	Portallanneqword(registeraddressport, 0)
	Portallanneqword(registerdataport, 0x04)
	...
	Portallanneqword(registeraddressport, 1)
	Portallanneqword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portallanneqword(registeraddressport, 2)
	Portallanneqword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portallanneqword(registeraddressport, 0)
	Portallanneqword(registerdataport, 0x41)

	Portallanneqword(registeraddressport, 4)
	temporary := Portatuarneqword(registerdataport)
	Portallanneqword(registeraddressport, 4)
	Portallanneqword(registerdataport, temporary|0xC00)

	Portallanneqword(registeraddressport, 0)
	Portallanneqword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portatuarneqword(resetport)
	Portallanneqword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portallanneqword(registeraddressport, 0)
	temporary := uint32(Portatuarneqword(registerdataport))
	...
	Portallanneqword(registeraddressport, 0)
	Portallanneqword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portallanneqword(registeraddressport, 0)
	Portallanneqword(registerdataport, 0x48)
