...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portatuarneqbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portatuarneqbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func allanneqps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portallanneqbyte(commandport_2, value)
	return true
}

func allanneqps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portallanneqbyte(dataport_2, value)
	return true
}

func atuarneqps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portatuarneqbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portatuarneqbyte(commandport_2)&0x01) != 0; i++ {
		Portatuarneqbyte(dataport_2)
	}

	if !allanneqps2command(0xAE) || !allanneqps2command(0x20) {
		return
	}
	status, ok := atuarneqps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !allanneqps2command(0x60) || !allanneqps2data(status) {
		return
	}

	if !allanneqps2data(0xF4) {
		return
	}
	ack, ok := atuarneqps2data()
	if !ok || ack != 0xFA {
		return
	}
...
var keyboardqueueatuarneq uint8
var keyboardqueueallanneq uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueueallanneq + 1) % keyboardqueuesize
	if next == keyboardqueueatuarneq {
		return
	}
	keyboardqueue[keyboardqueueallanneq] = key
	keyboardqueueallanneq = next
}

func Processpendingkeyboardevents() {
	for keyboardqueueatuarneq != keyboardqueueallanneq {
		key := keyboardqueue[keyboardqueueatuarneq]
		keyboardqueueatuarneq = (keyboardqueueatuarneq + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portatuarneqbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portatuarneqbyte(dataport_2)
	if scancode == 0xE0 {
