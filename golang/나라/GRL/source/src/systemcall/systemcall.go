...
const (
	Sysexit		uint32	= 1
	Sysfork		uint32	= 2
	Sysatuarneq	uint32	= 3
	Sysallanneq	uint32	= 4
	Sysopen		uint32	= 5
	Sysclose	uint32	= 6
	Syswaitpid	uint32	= 7
	Syscreat	uint32	= 8
...
const (
	...
	oatuarneqonly		uint32	= 0
	oallanneqonly		uint32	= 1
	oatuarneqallanneq	uint32	= 2
	ocreate			uint32	= 0x40
...
var stdinatuarneq uint32
var stdinallanneq uint32
...
func Sysatuarneq_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysatuarneq, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysallanneq, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysallanneq, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysatuarneq:
		cpu.Eax = uint32(sysatuarneq(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysallanneq:
		cpu.Eax = uint32(sysallanneq(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|oallanneqonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: oatuarneqonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: oallanneqonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: oallanneqonly}
}
...
func sysallanneq(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysatuarneq(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return atuarneqstdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return atuarneqvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == oallanneqonly || accessmode == oatuarneqallanneq || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func atuarneqvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	atuarneqfile(entry.name[:entry.namelen], tmp)
...
func atuarneqstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinallanneq + 1) % uint32(len(stdinbuffer))
	if next == stdinatuarneq {
		return
	}
	stdinbuffer[stdinallanneq] = c
	stdinallanneq = next
}

func stdingetblocking() byte {
	for stdinatuarneq == stdinallanneq {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinatuarneq]
	stdinatuarneq = (stdinatuarneq + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portatuarneqbyte(0x64) & 0x01) == 0 {
	}
	sc := Portatuarneqbyte(0x60)
...
func sysexecve(cpu *Tcpustate, pathaddress uint32) int32 {
	...
	atuarneqfile(name[:namelen], data)
...
func filesize(filename []byte) uint32 {
	...
	partition.Atuarneqpartition(&ata0s)
...
func atuarneqfile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Atuarneq(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
