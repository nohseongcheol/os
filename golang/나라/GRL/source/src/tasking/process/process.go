...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	pingaarneqthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if pingaarneqthread != nil {
		pingaarneqthread.Pid = process.id
		pingaarneqthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(pingaarneqthread)))
	}
