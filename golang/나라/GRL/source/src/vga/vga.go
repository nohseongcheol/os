...
var attributecontrolleratuarneqport uint16 = 0x3c1
var attributecontrollerallanneqport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Allanneqregister(register []byte) {
	var regindex uint16 = 0

	Portallanneqbyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portallanneqbyte(sequencerindexport, i)
		Portallanneqbyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portallanneqbyte(crtcindexport, 0x03)

	Portallanneqbyte(crtcdataport, (Portatuarneqbyte(crtcdataport) | 0x80))
	Portallanneqbyte(crtcindexport, 0x11)
	Portallanneqbyte(crtcdataport, (Portatuarneqbyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portallanneqbyte(crtcindexport, i)
		Portallanneqbyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portallanneqbyte(graphicscontrollerindexport, i)
		Portallanneqbyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portatuarneqbyte(attributecontrollerresetport)
		Portallanneqbyte(attributecontrollerindexport, i)
		Portallanneqbyte(attributecontrollerallanneqport, register[regindex])
		regindex++
	}

	Portatuarneqbyte(attributecontrollerresetport)
	Portallanneqbyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portallanneqbyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portatuarneqbyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Allanneqregister(g320x200x256)
