/*
	Copyright 2020. (노성철, nsch78@nate.com, nsch@naver.com) All right reserved
*/

#include <fcntl.h>
#include <system/syscall.h>

enum { SYS_open = 5, SYS_creat = 8, SYS_fcntl = 55 };

int open(const char *path, int oflag, ...)
{
    file_mode_type mode = 0;
    if ((oflag & create_if_missing) != 0) {
        __builtin_va_list args;
        __builtin_va_start(args, oflag);
        mode = __builtin_va_arg(args, file_mode_type);
        __builtin_va_end(args);
    }
    return (int)__syscall_result(
        __syscall6(SYS_open, (long)path, oflag, mode, 0, 0, 0));
}

int creat(const char *path, file_mode_type mode)
{
    return (int)__syscall_result(
        __syscall6(SYS_creat, (long)path, mode, 0, 0, 0, 0));
}

int fcntl(int file_descriptor, int cmd, ...)
{
    long argument = 0;
    if (cmd == duplicate_descriptor || cmd == set_descriptor_flags || cmd == set_file_status_flags) {
        __builtin_va_list args;
        __builtin_va_start(args, cmd);
        argument = __builtin_va_arg(args, long);
        __builtin_va_end(args);
    }
    return (int)__syscall_result(
        __syscall6(SYS_fcntl, file_descriptor, cmd, argument, 0, 0, 0));
}
