...
freestanding library in `./espacio_de_usuario/posix`.
...
```sh
cd ./espacio_de_usuario/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./espacio_de_usuario/posix/declaraciones` and link the archive into an EngOS i386 user
...
`./espacio_de_usuario/posix/TEST_RESULTS.md`. Build all runtime test executables
