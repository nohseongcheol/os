...
ssize_t Čitaj(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Čitaj, fd, buf, count));
}

ssize_t Zapiši(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Zapiši, fd, buf, count));
}

int Zatvori(int fd)
{
    return (int)__syscall_result(SC1(SYS_Zatvori, fd));
}
