...
GLOBL ·identifikacijatDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·identifikacijat<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·instalirajidt(SB),NOSPLIT,$0
	LEAQ ·identifikacijatDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·identifikacijat<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·RučkaPrekid(SB),NOSPLIT,$0-10
	...
	LEAQ ·prekidgateentry(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
//--------------------------- -----------------------------------------
TEXT ·dispatchPrekid(SB),NOSPLIT,$0
...
TEXT ·prekidgateentry(SB),NOSPLIT,$0
