...
ssize_t li(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_li, fd, buf, count));
}

ssize_t ekri(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ekri, fd, buf, count));
}
