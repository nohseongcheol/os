...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Ekri(0xA0)
	} else {
		self.deviceport.Ekri(0xB0)
	}
	self.controlport.Ekri(0)
	self.deviceport.Ekri(0xA0)

	var status uint8 = self.commandport.Li()
	if status == 0xFF {
	...
	self.sectorcountport.Ekri(0)
	self.lbalowport.Ekri(0)
	self.lbamidport.Ekri(0)
	self.lbahiport.Ekri(0)
	self.commandport.Ekri(0xEC)

	status = self.commandport.Li()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Li28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Ekri(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Ekri(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Ekri(0)
	self.sectorcountport.Ekri(1)

	self.lbalowport.Ekri(uint8(sector & 0x000000FF))
	self.lbamidport.Ekri(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Ekri(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Ekri(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Li()
...
func (self *TAdvancedtechnologyattachment) Ekri28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Ekri(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Ekri(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Ekri(0)
	self.sectorcountport.Ekri(1)
	self.lbalowport.Ekri(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Ekri(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Ekri(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Ekri(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Ekri(0xE0)
	} else {
		self.deviceport.Ekri(0xF0)
	}
	self.commandport.Ekri(0xE7)
