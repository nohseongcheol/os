...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portlibyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portlibyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func ekrips2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portekribyte(commandport_2, value)
	return true
}

func ekrips2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portekribyte(dataport_2, value)
	return true
}

func lips2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portlibyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portlibyte(commandport_2)&0x01) != 0; i++ {
		Portlibyte(dataport_2)
	}

	if !ekrips2command(0xAE) || !ekrips2command(0x20) {
		return
	}
	status, ok := lips2data()
	if !ok {
	...
	status |= 0x01
	status &^= 0x10
	if !ekrips2command(0x60) || !ekrips2data(status) {
		return
	}
...
var keyboardqueueli uint8
var keyboardqueueekri uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueueekri + 1) % keyboardqueuesize
	if next == keyboardqueueli {
		return
	}
	keyboardqueue[keyboardqueueekri] = key
	keyboardqueueekri = next
}

func Processpendingkeyboardevents() {
	for keyboardqueueli != keyboardqueueekri {
		key := keyboardqueue[keyboardqueueli]
		keyboardqueueli = (keyboardqueueli + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portlibyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portlibyte(dataport_2)
	if scancode == 0xE0 {
