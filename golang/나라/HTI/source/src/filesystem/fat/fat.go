...
func (self *TBiosparameterblock32) Len(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte) uint32 {
	...
	hd.Li28(partitionoffset, &bpbbytes, 90)
	...
	hd.Li28(rootstart, &direntbytes, 512)
...
func (self *TBiosparameterblock32) Li(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte, data []byte) {
		...
		for Size > 0 {
			...
			for ; Size > 0; Size -= 512 {
				...
				if dirent[i].size > 512 {
					...
					hd.Li28(filesector+uint32(sectoroffset), &buffer3, 512)

				} else {
					buffer3 = buffer_2[:dirent[i].size]
					hd.Li28(filesector+uint32(sectoroffset), &buffer3, int(dirent[i].size))
				}
			...
			hd.Li28(fatstart+fatsectorforcurrentcluster, &fatbuf, 512)
...
func (self *TDirectoryentryfat32) Init(data [32]byte) {
	...
	self.cDat = uint16(data[16]) | uint16(data[17])<<8
	...
	self.wDat = uint16(data[24]) | uint16(data[25])<<8
