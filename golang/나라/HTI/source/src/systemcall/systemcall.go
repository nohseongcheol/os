...
func Sysli_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysli, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysekri, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysekri, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysli:
		cpu.Eax = uint32(sysli(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysekri:
		cpu.Eax = uint32(sysekri(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|oekrionly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: olionly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: oekrionly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: oekrionly}
}
...
func sysekri(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysli(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return livfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == oekrionly || accessmode == oliekri || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func livfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	lifile(entry.name[:entry.namelen], tmp)
...
func listdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinekri + 1) % uint32(len(stdinbuffer))
	if next == stdinli {
		return
	}
	stdinbuffer[stdinekri] = c
	stdinekri = next
}

func stdingetblocking() byte {
	for stdinli == stdinekri {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinli]
	stdinli = (stdinli + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portlibyte(0x64) & 0x01) == 0 {
	}
	sc := Portlibyte(0x60)
...
func lifile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Li(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
