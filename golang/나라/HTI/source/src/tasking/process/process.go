...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	prensipalthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if prensipalthread != nil {
		prensipalthread.Pid = process.id
		prensipalthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(prensipalthread)))
	}
