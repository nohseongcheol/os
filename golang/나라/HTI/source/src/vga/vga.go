...
var attributecontrollerliport uint16 = 0x3c1
var attributecontrollerekriport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Ekriregister(register []byte) {
	var regindex uint16 = 0

	Portekribyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portekribyte(sequencerindexport, i)
		Portekribyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portekribyte(crtcindexport, 0x03)

	Portekribyte(crtcdataport, (Portlibyte(crtcdataport) | 0x80))
	Portekribyte(crtcindexport, 0x11)
	Portekribyte(crtcdataport, (Portlibyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portekribyte(crtcindexport, i)
		Portekribyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portekribyte(graphicscontrollerindexport, i)
		Portekribyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portlibyte(attributecontrollerresetport)
		Portekribyte(attributecontrollerindexport, i)
		Portekribyte(attributecontrollerekriport, register[regindex])
		regindex++
	}

	Portlibyte(attributecontrollerresetport)
	Portekribyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portekribyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portlibyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Ekriregister(g320x200x256)
