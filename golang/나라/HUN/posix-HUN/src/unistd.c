...
ssize_t Olvasás(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Olvasás, fd, buf, count));
}

ssize_t Írás(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Írás, fd, buf, count));
}

int Bezárás(int fd)
{
    return (int)__syscall_result(SC1(SYS_Bezárás, fd));
}
