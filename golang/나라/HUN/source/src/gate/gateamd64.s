...
GLOBL ·azonosítótDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·azonosítót<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·telepítésidt(SB),NOSPLIT,$0
	LEAQ ·azonosítótDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·azonosítót<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·FogantyúMegszakítás(SB),NOSPLIT,$0-10
	...
	LEAQ ·megszakításgatebejegyzés(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	...
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchMegszakítás(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchMegszakítás(SB),NOSPLIT,$0
...
TEXT ·megszakításgatebejegyzés(SB),NOSPLIT,$0
