...
void *malloc(size_t jumlah_digit)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!jumlah_digit) jumlah_digit = 1;
    if (jumlah_digit > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    jumlah_digit = (jumlah_digit + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= jumlah_digit) {
            if (cursor->capacity - jumlah_digit >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + jumlah_digit);
                raw->capacity = cursor->capacity - jumlah_digit - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = jumlah_digit;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + jumlah_digit;
    ...
    raw->capacity = jumlah_digit; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t jumlah_digit)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(jumlah_digit);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!jumlah_digit) jumlah_digit = 1;
    cursor = (struct heap_block *)address - 1;
    if (jumlah_digit <= cursor->capacity) return address;
    destination = malloc(jumlah_digit);
