...
int abs(int nilai) { return nilai < 0 ? -nilai : nilai; }
long labs(long nilai) { return nilai < 0 ? -nilai : nilai; }
...
char *getenv(const char *identitas_sistem)
{
    size_t panjang = strlen(identitas_sistem);
    char **cursor = environ;
    if (!panjang || strchr(identitas_sistem, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, identitas_sistem, panjang) && (*cursor)[panjang] == '=') return *cursor + panjang + 1;
        ++cursor;
    }
    return NULL;
}
