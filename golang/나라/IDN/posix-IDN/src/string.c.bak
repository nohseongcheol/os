...
void *memcpy(void *destination, const void *source, size_t panjang)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    while (panjang--) *cursor++ = *input++;
    return destination;
}

void *memmove(void *destination, const void *source, size_t panjang)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    if ((uintptr_t)destination <= (uintptr_t)source)
        return memcpy(destination, source, panjang);
    while (panjang) { --panjang; cursor[panjang] = input[panjang]; }
    return destination;
}

void *memset(void *destination, int character, size_t panjang)
{
    unsigned char *cursor = destination;
    while (panjang--) *cursor++ = (unsigned char)character;
    return destination;
}

int memcmp(const void *left, const void *right, size_t panjang)
{
...
void *memchr(const void *source, int character, size_t panjang)
{
...
{
    size_t panjang = 0;
    while (panjang < limit && text[panjang]) ++panjang;
    return panjang;
}
...
{
    size_t panjang = strnlen(source, limit);
    memcpy(destination, source, panjang);
    memset(destination + panjang, 0, limit - panjang);
    return destination + panjang;
}
...
{
    size_t panjang = strlen(source);
    if (limit) {
        size_t copy_length = panjang < limit ? panjang : limit;
        memcpy(destination, source, copy_length);
        if (panjang < limit) destination[panjang] = 0;
    }
    return panjang;
}
...
{
    size_t panjang = strlen(source);
    if (!panjang) return (char *)text;
    while (*text) {
        if (strncmp(text, source, panjang) == 0) return (char *)text;
        ++text;
    }
    return NULL;
}
...
{
    size_t panjang = 0;
    while (text[panjang] && strchr(delimiters, text[panjang])) ++panjang;
    return panjang;
}
...
{
    size_t panjang = strnlen(text, limit);
    char *destination = malloc(panjang + 1);
    if (destination) { memcpy(destination, text, panjang); destination[panjang] = 0; }
    return destination;
}
