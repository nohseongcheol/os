...
#define CHECK(nilai) do { if (!(nilai)) return __LINE__; } while (0)

static int compare_int(const void *left, const void *right)
{
    int result = *(const int *)left, nilai = *(const int *)right;
    return (result > nilai) - (result < nilai);
}

int __posix_library_test(void)
{
    char penyangga_transfer_2[64], source[64], *end_pointer, *saved_state, *address, *destination;
    const char *text = "abca", *input = " 	-0x80rest";
    int elements[257], index, nilai;
    size_t panjang;
    div_t result;
    ldiv_t decoded;
    ...
    CHECK(memset(penyangga_transfer_2, 0xab, sizeof(penyangga_transfer_2)) == penyangga_transfer_2);
    CHECK((unsigned char)penyangga_transfer_2[63] == 0xab);
    CHECK(memcpy(penyangga_transfer_2, "abcdef", 7) == penyangga_transfer_2);
    CHECK(memmove(penyangga_transfer_2 + 2, penyangga_transfer_2, 5) == penyangga_transfer_2 + 2);
    CHECK(memcmp(penyangga_transfer_2, "ababcde", 7) == 0);
    CHECK(memmove(penyangga_transfer_2, penyangga_transfer_2 + 2, 5) == penyangga_transfer_2);
    CHECK(memcmp(penyangga_transfer_2, "abcde", 5) == 0);
    CHECK(memmove(penyangga_transfer_2, penyangga_transfer_2, 0) == penyangga_transfer_2);
    CHECK(memcmp("\x80", "\x7f", 1) > 0);
    CHECK(memcmp(penyangga_transfer_2, source, 0) == 0);
    ...
    CHECK(strcpy(penyangga_transfer_2, text) == penyangga_transfer_2 && strcmp(penyangga_transfer_2, text) == 0);
    CHECK(stpcpy(penyangga_transfer_2, "xy") == penyangga_transfer_2 + 2 && penyangga_transfer_2[2] == 0);
    memset(penyangga_transfer_2, 7, sizeof(penyangga_transfer_2));
    CHECK(strncpy(penyangga_transfer_2, "xy", 5) == penyangga_transfer_2);
    CHECK(!memcmp(penyangga_transfer_2, "xy\0\0\0", 5) && penyangga_transfer_2[5] == 7);
    CHECK(stpncpy(penyangga_transfer_2, "abcdef", 3) == penyangga_transfer_2 + 3 && penyangga_transfer_2[3] == 0);
    CHECK(stpncpy(penyangga_transfer_2, "a", 4) == penyangga_transfer_2 + 1 && penyangga_transfer_2[3] == 0);
    CHECK(strncpy(penyangga_transfer_2, "z", 0) == penyangga_transfer_2 && penyangga_transfer_2[0] == 'a');
    strcpy(penyangga_transfer_2, "a");
    CHECK(strcat(penyangga_transfer_2, "bc") == penyangga_transfer_2 && !strcmp(penyangga_transfer_2, "abc"));
    CHECK(strncat(penyangga_transfer_2, "def", 2) == penyangga_transfer_2 && !strcmp(penyangga_transfer_2, "abcde"));
    CHECK(strncat(penyangga_transfer_2, "xyz", 0) == penyangga_transfer_2 && !strcmp(penyangga_transfer_2, "abcde"));
    ...
    CHECK(strxfrm(penyangga_transfer_2, "abc", 4) == 3 && !strcmp(penyangga_transfer_2, "abc"));
    ...
    strcpy(penyangga_transfer_2, ",,one;two,,");
    CHECK(!strcmp(strtok_r(penyangga_transfer_2, ",;", &saved_state), "one"));
    strcpy(source, "x:y");
    ...
    for (nilai = -1; nilai <= 255; ++nilai) {
        int low = nilai >= 'a' && nilai <= 'z', high = nilai >= 'A' && nilai <= 'Z';
        int digit = nilai >= '0' && nilai <= '9';
        CHECK(!!isalpha(nilai) == (low || high));
        CHECK(!!isalnum(nilai) == (low || high || digit));
        CHECK(!!isdigit(nilai) == digit);
        CHECK(!!islower(nilai) == low && !!isupper(nilai) == high);
        CHECK(!!isblank(nilai) == (nilai == ' ' || nilai == '\t'));
        CHECK(!!isspace(nilai) == (nilai == ' ' || (nilai >= 9 && nilai <= 13)));
        CHECK(!!iscntrl(nilai) == ((nilai >= 0 && nilai < 32) || nilai == 127));
        CHECK(!!isprint(nilai) == (nilai >= 32 && nilai <= 126));
        CHECK(!!isgraph(nilai) == (nilai >= 33 && nilai <= 126));
        CHECK(!!ispunct(nilai) == (nilai >= 33 && nilai <= 126 && !low && !high && !digit));
        CHECK(!!isxdigit(nilai) == (digit || (nilai >= 'a' && nilai <= 'f') || (nilai >= 'A' && nilai <= 'F')));
        CHECK(tolower(nilai) == (high ? nilai + 32 : nilai));
        CHECK(toupper(nilai) == (low ? nilai - 32 : nilai));
    }
    errno = EIO;
    ...
    for (panjang = 0; panjang <= 257; ++panjang) {
        for (index = 0; index < (int)panjang; ++index) elements[index] = ((index * 37 + (int)panjang) % 19) - 9;
        qsort(elements, panjang, sizeof(int), compare_int);
        for (index = 1; index < (int)panjang; ++index) CHECK(elements[index - 1] <= elements[index]);
        for (index = 0; index < (int)panjang; ++index) {
            int *found = bsearch(&elements[index], elements, panjang, sizeof(int), compare_int);
            CHECK(found && *found == elements[index]);
        }
        nilai = 100;
        CHECK(bsearch(&nilai, elements, panjang, sizeof(int), compare_int) == NULL);
    }
    environ = envp;
