...
static void say(const char *s) { (void)Tulis(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *identitas_sistem, int pass)
{
...
int main(void)
{
    ...
    report("recvfrom", recvfrom(server, penyangga_transfer_2, sizeof(penyangga_transfer_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       penyangga_transfer_2[0] == 'p' && penyangga_transfer_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, penyangga_transfer_2, sizeof(penyangga_transfer_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Tutup(client) == 0);
    report("close-server", Tutup(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
