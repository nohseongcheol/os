...
type Tamdam79c973 struct {
	TInterrupthandler
	उपकरणdescriptor	TPeripheralcomponentinterconnectउपकरणdescriptor
...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, उपकरणdescriptor TPeripheralcomponentinterconnectउपकरणdescriptor, handler IRawdatahandler) {

	self.उपकरणdescriptor = उपकरणdescriptor

	funcvalue = (*Tamdam79c973).Handleinterrupt
	var address uintptr
	address = uintptr(Pointer(&funcvalue))

	self.Init(uint8(0x20+उपकरणdescriptor.Interrupt), uintptr(Pointer(interrupt)), address)

	Macaddress0port = uint16(उपकरणdescriptor.Portbase)
	Macaddress2port = uint16(उपकरणdescriptor.Portbase) + 0x02
	Macaddress4port = uint16(उपकरणdescriptor.Portbase) + 0x04
	registerdataport = uint16(उपकरणdescriptor.Portbase) + 0x10
	registeraddressport = uint16(उपकरणdescriptor.Portbase) + 0x12
	resetport = uint16(उपकरणdescriptor.Portbase) + 0x14
	buscontrolregisterdataport = uint16(उपकरणdescriptor.Portbase) + 0x16
	...
	var Mac0 uint64 = uint64(Portpadhnaword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portpadhnaword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portpadhnaword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portpadhnaword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portpadhnaword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portpadhnaword(Macaddress4port) / 256)
	...
	console_2.MHexadecimalprint(uint8(उपकरणdescriptor.Interrupt))
	...
	Portlikhnaword(registeraddressport, 20)
	Portlikhnaword(buscontrolregisterdataport, 0x102)

	Portlikhnaword(registeraddressport, 0)
	Portlikhnaword(registerdataport, 0x04)
	...
	Portlikhnaword(registeraddressport, 1)
	Portlikhnaword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portlikhnaword(registeraddressport, 2)
	Portlikhnaword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portlikhnaword(registeraddressport, 0)
	Portlikhnaword(registerdataport, 0x41)

	Portlikhnaword(registeraddressport, 4)
	temporary := Portpadhnaword(registerdataport)
	Portlikhnaword(registeraddressport, 4)
	Portlikhnaword(registerdataport, temporary|0xC00)

	Portlikhnaword(registeraddressport, 0)
	Portlikhnaword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portpadhnaword(resetport)
	Portlikhnaword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portlikhnaword(registeraddressport, 0)
	temporary := uint32(Portpadhnaword(registerdataport))
	...
	Portlikhnaword(registeraddressport, 0)
	Portlikhnaword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portlikhnaword(registeraddressport, 0)
	Portlikhnaword(registerdataport, 0x48)
