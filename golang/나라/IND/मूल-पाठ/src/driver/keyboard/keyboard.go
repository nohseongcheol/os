...
func (self *TDefaultkeyboardeventhandler) Onkeyऊपर(key byte) {
}
...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portpadhnabyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portpadhnabyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func likhnaps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portlikhnabyte(commandport_2, value)
	return true
}

func likhnaps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portlikhnabyte(dataport_2, value)
	return true
}

func padhnaps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portpadhnabyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portpadhnabyte(commandport_2)&0x01) != 0; i++ {
		Portpadhnabyte(dataport_2)
	}

	if !likhnaps2command(0xAE) || !likhnaps2command(0x20) {
		return
	}
	status, ok := padhnaps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !likhnaps2command(0x60) || !likhnaps2data(status) {
		return
	}

	if !likhnaps2data(0xF4) {
		return
	}
	ack, ok := padhnaps2data()
	if !ok || ack != 0xFA {
		return
	}
...
var keyboardqueuepadhna uint8
var keyboardqueuelikhna uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuelikhna + 1) % keyboardqueuesize
	if next == keyboardqueuepadhna {
		return
	}
	keyboardqueue[keyboardqueuelikhna] = key
	keyboardqueuelikhna = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuepadhna != keyboardqueuelikhna {
		key := keyboardqueue[keyboardqueuepadhna]
		keyboardqueuepadhna = (keyboardqueuepadhna + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portpadhnabyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portpadhnabyte(dataport_2)
	if scancode == 0xE0 {
