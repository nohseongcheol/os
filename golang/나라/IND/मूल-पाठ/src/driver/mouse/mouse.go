...
func (self TDefaultmouseeventhandler) Onmouseऊपर(button int8)	{}
...
func sendmousecommand(value uint8) bool {
	if !likhnaps2command(0xD4) || !likhnaps2data(value) {
		return false
	}
...
func (self *TMousedriver) Initdriver(manager *TInterruptmanager, mouseeventhandler IMouseeventhandler) {
	...
	if !likhnaps2command(0xA8) || !likhnaps2command(0x20) {
		return
	}
...
func (self *TMousedriver) Handleinterrupt(esp uint32) uint32 {
	...
	data := Portpadhnabyte(dataport_2)
	...
	for i = 0; i < 3; i++ {
		...
		if (newbutton & mask) != (oldbutton & mask) {
			if (newbutton & mask) != 0 {
				...
				imouseeventhandler.Onmouseऊपर(int8(i + 1))
			}
		}
	}
	button_2 = newbutton
