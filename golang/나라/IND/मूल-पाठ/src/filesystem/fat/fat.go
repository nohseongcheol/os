...
func (self *TBiosparameterblock32) Len(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte) uint32 {
	...
	hd.Padhna28(partitionoffset, &bpbbytes, 90)
	...
	hd.Padhna28(rootstart, &direntbytes, 512)
...
func (self *TBiosparameterblock32) Padhna(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte, data []byte) {
		...
		for Size > 0 {
			...
			for ; Size > 0; Size -= 512 {
				...
				if dirent[i].size > 512 {
					...
					hd.Padhna28(filesector+uint32(sectoroffset), &buffer3, 512)

				} else {
					buffer3 = buffer_2[:dirent[i].size]
					hd.Padhna28(filesector+uint32(sectoroffset), &buffer3, int(dirent[i].size))
				}
			...
			hd.Padhna28(fatstart+fatsectorforcurrentcluster, &fatbuf, 512)
...
func (self *TDirectoryentryfat32) Init(data [32]byte) {
	...
	self.cसमयtenth = data[13]
	self.cसमय = uint16(data[14]) | uint16(data[15])<<8
	self.cdate = uint16(data[16]) | uint16(data[17])<<8
	self.aसमय = uint16(data[18]) | uint16(data[19])<<8
	self.firstclusterhi = uint16(data[20]) | uint16(data[21])<<8
	self.wसमय = uint16(data[22]) | uint16(data[23])<<8
