...
const (
	...
	opadhnaonly	uint32	= 0
	olikhnaonly	uint32	= 1
	opadhnalikhna	uint32	= 2
	ocreate		uint32	= 0x40
...
var stdinpadhna uint32
var stdinlikhna uint32
...
func Syspadhna_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Syspadhna, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Syslikhna, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Syslikhna, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func Sysopen_2(पथ uintptr, flags uint32, mode uint32) int32 {
	return int32(Syscall(Sysopen, uint32(पथ), flags, mode))
}
...
func Syscall(परमस ...uint32) uint32 {

	l := len(परमस)
	switch l {
	case 1:
		return Interrupt(परमस[0], 0, 0, 0, 0, 0)
	case 2:
		return Interrupt(परमस[0], परमस[1], 0, 0, 0, 0)
	case 3:
		return Interrupt(परमस[0], परमस[1], परमस[2], 0, 0, 0)
	case 4:
		return Interrupt(परमस[0], परमस[1], परमस[2], परमस[3], 0, 0)
	case 5:
		return Interrupt(परमस[0], परमस[1], परमस[2], परमस[3], परमस[4], 0)
	case 6:
		return Interrupt(परमस[0], परमस[1], परमस[2], परमस[3], परमस[4], परमस[5])
	default:
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Syspadhna:
		cpu.Eax = uint32(syspadhna(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Syslikhna:
		cpu.Eax = uint32(syslikhna(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|olikhnaonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: opadhnaonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: olikhnaonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: olikhnaonly}
}
...
func syslikhna(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func syspadhna(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return padhnastdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return padhnavfsfile(entry, buffer, count)
}

func sysopen(पथaddress uint32, flags uint32, mode uint32) int32 {
	_ = mode
	if पथaddress == 0 {
		return Efault
	}
	accessmode := flags & 3
	if accessmode == olikhnaonly || accessmode == opadhnalikhna || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
	...
	entry := &openfiletable[description]
	entry.flags = flags
	if isrootपथ(पथaddress) {
		entry.kind = fdkindrootdirectory
		entry.size = 0
	} else {
		namelen, name := copyपथ(पथaddress)
		if namelen == 0 {
			*entry = openfiledescription{}
			return Enoent
		}
...
func padhnavfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	padhnafile(entry.name[:entry.namelen], tmp)
...
func isrootपथ(पथaddress uint32) bool {
	if पथaddress == 0 {
		return false
	}
	पथ := Getbytesfrompointer(uintptr(पथaddress), 4, 4)
	if पथ[0] == '/' && पथ[1] == 0 {
		return true
	}
	if पथ[0] == '.' && पथ[1] == 0 {
...
func sysaccess(पथaddress uint32, mode uint32) int32 {
	if पथaddress == 0 {
		return Efault
	}
	if (mode & ^uint32(7)) != 0 {
...
func syschdir(पथaddress uint32) int32 {
	if पथaddress == 0 {
		return Efault
	}
	if !isrootपथ(पथaddress) {
...
func sysstat(पथaddress uint32, stataddress uint32) int32 {
	if पथaddress == 0 {
		return Efault
	}
	if isrootपथ(पथaddress) {
...
func padhnastdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinlikhna + 1) % uint32(len(stdinbuffer))
	if next == stdinpadhna {
		return
	}
	stdinbuffer[stdinlikhna] = c
	stdinlikhna = next
}

func stdingetblocking() byte {
	for stdinpadhna == stdinlikhna {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinpadhna]
	stdinpadhna = (stdinpadhna + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portpadhnabyte(0x64) & 0x01) == 0 {
	}
	sc := Portpadhnabyte(0x60)
...
func setupexecstack(cpu *Tcpustate, arguments_2 *execvector, environment *execvector) int32 {
	...
	if !Makerangeprivatewritable(getcr3(), Userstackउपर-stackbytes, stackbytes) {
		return Enomem
	}
	stack := Userstackउपर
...
func sysexecve(cpu *Tcpustate, पथaddress uint32) int32 {
	if पथaddress == 0 {
		return Efault
	}
...
func syswaitpid(pid int32, statusaddress uint32, options uint32) int32 {
	...
	for i := 0; i < len(processtable); i++ {
		...
		if p.used && matches && p.जनक == जनकpid {
			foundchild = true
			if p.exited {
				if statusaddress != 0 {
...
func allocateprocess(जनक uint32) uint32 {
	जनकprocess := findprocess(जनक)
	pid := Allocatepid()
	for i := 0; i < len(processtable); i++ {
		if !processtable[i].used {
			processtable[i] = processentry{
			...
			if जनकprocess != nil {
				processtable[i].programbreak = जनकprocess.programbreak
				for fd := 0; fd < maxfd; fd++ {
					if जनकprocess.fds[fd].used {
						processtable[i].fds[fd] = जनकprocess.fds[fd]
						description := जनकprocess.fds[fd].description
						if description >= 0 && description < maxopenfiles {
...
func copyपथ(पथaddress uint32) (uint32, [12]byte) {
	var name [12]byte
	if पथaddress == 0 {
		return 0, name
	}
	raw := Getbytesfrompointer(uintptr(पथaddress), 64, 64)
	var n uint32
	for i := 0; i < 64 && n < 12; i++ {
		c := raw[i]
		if c == 0 {
...
func filesize(filename []byte) uint32 {
	...
	partition.Padhnapartition(&ata0s)
...
func padhnafile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Padhna(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
