...
var attributecontrollerpadhnaport uint16 = 0x3c1
var attributecontrollerlikhnaport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Likhnaregister(register []byte) {
	var regindex uint16 = 0

	Portlikhnabyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portlikhnabyte(sequencerindexport, i)
		Portlikhnabyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portlikhnabyte(crtcindexport, 0x03)

	Portlikhnabyte(crtcdataport, (Portpadhnabyte(crtcdataport) | 0x80))
	Portlikhnabyte(crtcindexport, 0x11)
	Portlikhnabyte(crtcdataport, (Portpadhnabyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portlikhnabyte(crtcindexport, i)
		Portlikhnabyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portlikhnabyte(graphicscontrollerindexport, i)
		Portlikhnabyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portpadhnabyte(attributecontrollerresetport)
		Portlikhnabyte(attributecontrollerindexport, i)
		Portlikhnabyte(attributecontrollerlikhnaport, register[regindex])
		regindex++
	}

	Portpadhnabyte(attributecontrollerresetport)
	Portlikhnabyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portlikhnabyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portpadhnabyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Fillआयत(x uint32, y uint32, w uint32, h uint32, r uint8, g uint8, b uint8) {
	for Y := y; Y < y+h; Y++ {
		for X := x; X < x+w; X++ {
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Likhnaregister(g320x200x256)
