...
ssize_t خواندن(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_خواندن, fd, buf, count));
}

ssize_t نوشتن(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_نوشتن, fd, buf, count));
}

int بستن(int fd)
{
    return (int)__syscall_result(SC1(SYS_بستن, fd));
}
