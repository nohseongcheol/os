...
ssize_t Lestur(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Lestur, fd, buf, count));
}

ssize_t Skrift(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Skrift, fd, buf, count));
}

int Loka(int fd)
{
    return (int)__syscall_result(SC1(SYS_Loka, fd));
}
