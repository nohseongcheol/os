...
GLOBL ·auðkennitDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·auðkennit<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·setjauppidt(SB),NOSPLIT,$0
	LEAQ ·auðkennitDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·auðkennit<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·Haldfanginterrupt(SB),NOSPLIT,$0-10
	...
	LEAQ ·interruptgateentry(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
TEXT ·interruptgateentry(SB),NOSPLIT,$0
