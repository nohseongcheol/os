...
ssize_t קריאה(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_קריאה, fd, buf, count));
}

ssize_t כתיבה(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_כתיבה, fd, buf, count));
}

int סגור(int fd)
{
    return (int)__syscall_result(SC1(SYS_סגור, fd));
}
