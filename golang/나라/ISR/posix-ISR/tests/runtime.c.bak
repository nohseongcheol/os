...
{
    (void)כתיבה(STDOUT_FILENO, text, text_length(text));
}
...
static void test_open_and_io(void)
{
    ...
    fd = פתח("/USER2", O_RDONLY);
    report("open-readonly", fd >= 3);
    if (fd >= 0) {
        report("read-bytes", קריאה(fd, bytes, 2) == 2 &&
        ...
        report("close-file", סגור(fd) == 0);
        errno = 0;
        report("closed-fd", failed_with(סגור(fd), EBADF));
    }

    root = פתח("/", O_RDONLY | O_DIRECTORY);
    report("open-directory", root >= 3);
    if (root >= 0) {
        report("fstat-directory", fstat(root, &st) == 0 && S_ISDIR(st.st_mode));
        errno = 0;
        report("read-directory", קריאה(root, bytes, 1) == -1 && errno == EISDIR);
        (void)סגור(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", כתיבה(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", קריאה(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", קריאה(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", כתיבה(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("open-enoent", failed_with(פתח("/NOFILE", O_RDONLY), ENOENT));
    errno = 0;
    report("open-efault", failed_with(פתח((const char *)0, O_RDONLY), EFAULT));
    errno = 0;
    report("open-write-erofs", failed_with(פתח("/USER2", O_WRONLY), EROFS));
    errno = 0;
    report("open-rdwr-erofs", failed_with(פתח("/USER2", O_RDWR), EROFS));
    errno = 0;
    report("open-create-erofs", failed_with(פתח("/NEWFILE", O_CREAT | O_WRONLY, 0600), EROFS));
    errno = 0;
    report("open-trunc-erofs", failed_with(פתח("/USER2", O_TRUNC | O_RDONLY), EROFS));
    errno = 0;
    report("open-append-erofs", failed_with(פתח("/USER2", O_APPEND | O_RDONLY), EROFS));
    errno = 0;
    report("open-enotdir", failed_with(פתח("/USER2", O_RDONLY | O_DIRECTORY), ENOTDIR));
    errno = 0;
    report("creat-erofs", failed_with(creat("/NEWFILE", 0600), EROFS));
    errno = 0;
    report("close-ebadf", failed_with(סגור(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", קריאה(fd, &first, 1) == 1 && קריאה(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", סגור(fd) == 0 && fstat(copy, &st) == 0);
        fd = copy;
    }
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &status) == child) {
        report("fork-fd-isolation", קריאה(fd, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
