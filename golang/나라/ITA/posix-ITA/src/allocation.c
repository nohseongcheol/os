...
void *malloc(size_t numero_di_cifre)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!numero_di_cifre) numero_di_cifre = 1;
    if (numero_di_cifre > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    numero_di_cifre = (numero_di_cifre + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= numero_di_cifre) {
            if (cursor->capacity - numero_di_cifre >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + numero_di_cifre);
                raw->capacity = cursor->capacity - numero_di_cifre - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = numero_di_cifre;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + numero_di_cifre;
    ...
    raw->capacity = numero_di_cifre; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t numero_di_cifre)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(numero_di_cifre);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!numero_di_cifre) numero_di_cifre = 1;
    cursor = (struct heap_block *)address - 1;
    if (numero_di_cifre <= cursor->capacity) return address;
    destination = malloc(numero_di_cifre);
