...
static void say(const char *s) { (void)Scrittura(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *identità_del_sistema, int pass)
{
...
int main(void)
{
    ...
    char memoria_intermedia_di_trasferimento_2[8] = {0};
    ...
    report("recvfrom", recvfrom(server, memoria_intermedia_di_trasferimento_2, sizeof(memoria_intermedia_di_trasferimento_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       memoria_intermedia_di_trasferimento_2[0] == 'p' && memoria_intermedia_di_trasferimento_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, memoria_intermedia_di_trasferimento_2, sizeof(memoria_intermedia_di_trasferimento_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Chiudi(client) == 0);
    report("close-server", Chiudi(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
