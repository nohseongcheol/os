...
int abs(int القيمة) { return القيمة < 0 ? -القيمة : القيمة; }
long labs(long القيمة) { return القيمة < 0 ? -القيمة : القيمة; }
...
char *getenv(const char *هوية_النظام)
{
    size_t الطول = strlen(هوية_النظام);
    char **cursor = environ;
    if (!الطول || strchr(هوية_النظام, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, هوية_النظام, الطول) && (*cursor)[الطول] == '=') return *cursor + الطول + 1;
        ++cursor;
    }
    return NULL;
}
