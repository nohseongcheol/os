...
freestanding library in `./пространство_пользователя/posix`.
...
```sh
cd ./пространство_пользователя/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./пространство_пользователя/posix/объявления` and link the archive into an EngOS i386 user
...
`./пространство_пользователя/posix/TEST_RESULTS.md`. Build all runtime test executables
