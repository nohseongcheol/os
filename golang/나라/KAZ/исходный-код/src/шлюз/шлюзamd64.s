...
GLOBL ·иДЕНТИФИКАТОРtDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·иДЕНТИФИКАТОРt<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
GLOBL ·шлюзHandlers<>(SB), NOPTR, $NUM_IDT_ENTRIES*8
...
TEXT ·установитьidt(SB),NOSPLIT,$0
	LEAQ ·иДЕНТИФИКАТОРtDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·иДЕНТИФИКАТОРt<>(SB), BX
	MOVQ BX, 2(AX)
...
TEXT ·Ручкапрерывание(SB),NOSPLIT,$0-10
	...
	LEAQ ·прерываниешлюззапись(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	SUBQ $16, SP;                          \
	MOVQ R15, 0(SP);                       \
	MOVQ ·шлюзHandlers<>+8*num(SB), R15;   \
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchпрерывание(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchпрерывание(SB),NOSPLIT,$0
...
TEXT ·прерываниешлюззапись(SB),NOSPLIT,$0
