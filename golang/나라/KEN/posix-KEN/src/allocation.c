...
void *malloc(size_t size)
{
    ...
    void *address;
    if (!size) size = 1;
    if (size > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    size = (size + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= size) {
            if (cursor->capacity - size >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + size);
                raw->capacity = cursor->capacity - size - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = size;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + size;
    ...
    raw->capacity = size; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t size)
{
    ...
    if (size <= cursor->capacity) return address;
