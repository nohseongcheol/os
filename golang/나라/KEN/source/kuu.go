...
func GetfunctionJina(i interface{}) {
	var address = reflect.ValueOf(i).Pointer()
	var funcJina = runtime.FuncForPC(address).Name()
	var funcbytes []byte = []byte(funcJina)
...
func KKernelEntry(Pagedirectoryentry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	Kusomafile(linkerfile, linkerdata)
	...
	Kusomafile(user1file, user1data)
	...
	*code1pointer = uintptr(linkerentry)
	...
	Kusomafile(user2file, user2data)
	...
	*code2pointer = uintptr(linkerentry)
	...
	Kusomafile(user3file, user3data)
