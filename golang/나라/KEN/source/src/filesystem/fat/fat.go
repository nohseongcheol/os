...
func (self *TBiosparameterblock32) Init(data []byte) {
	...
	copy(self.softJina[:8], data[3:11])
...
func (self *TBiosparameterblock32) Len(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte) uint32 {
	...
	hd.Kusoma28(partitionoffset, &bpbbytes, 90)
	...
	hd.Kusoma28(rootstart, &direntbytes, 512)
	...
	for i := 0; i < 16; i++ {
		...
		if !Equalbytes(filename, dirent[i].jina[:len(filename)]) {
			continue
		}
...
func (self *TBiosparameterblock32) Kusoma(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte, data []byte) {
		...
		for Size > 0 {
			...
			for ; Size > 0; Size -= 512 {
				...
				if dirent[i].size > 512 {
					...
					hd.Kusoma28(filesector+uint32(sectoroffset), &buffer3, 512)

				} else {
					buffer3 = buffer_2[:dirent[i].size]
					hd.Kusoma28(filesector+uint32(sectoroffset), &buffer3, int(dirent[i].size))
				}
			...
			hd.Kusoma28(fatstart+fatsectorforcurrentcluster, &fatbuf, 512)
