...
func (self *TInterruptmanager) Init(hardwareinterruptoffset uint16, globaldescriptortable *TShareddescriptortable, taskmanager *TTaskmanager) {
	...
	Portkuandikabyte(Primarypiccommandioport, 0x11)
	Portkuandikabyte(Secondarypiccommandioport, 0x11)

	Portkuandikabyte(Primarypicdataioport, 0x20)
	Portkuandikabyte(Secondarypicdataioport, 0x28)

	Portkuandikabyte(Primarypicdataioport, 0x04)
	Portkuandikabyte(Secondarypicdataioport, 0x02)

	Portkuandikabyte(Primarypicdataioport, 0x01)
	Portkuandikabyte(Secondarypicdataioport, 0x01)

	Portkuandikabyte(Primarypicdataioport, 0xF8)
	Portkuandikabyte(Secondarypicdataioport, 0xEF)
...
func (self *TInterruptmanager) Dohandleinterrupt(interrupt uint8, esp uint32) uint32 {
	...
	if 0x20 <= interrupt && interrupt < 0x30 {
		if 0x28 <= interrupt {
			Portkuandikabyte(Secondarypiccommandioport, 0x20)
		}
		Portkuandikabyte(Primarypiccommandioport, 0x20)
	}
	return esp
}
...
func exceptionJina(interrupt uint32) string {
	switch interrupt {
	case 0x00:
...
func Handleexception(esp uint32, interrupt uint32) uint32 {
	...
	MEmergencylogstring(" ")
	MEmergencylogstring(exceptionJina(interrupt))
