...
const (
	...
	okusomaonly	uint32	= 0
	okuandikaonly	uint32	= 1
	okusomakuandika	uint32	= 2
	ocreate		uint32	= 0x40
...
var stdinkusoma uint32
var stdinkuandika uint32
...
func Syskusoma_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Syskusoma, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Syskuandika, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Syskuandika, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Syskusoma:
		cpu.Eax = uint32(syskusoma(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Syskuandika:
		cpu.Eax = uint32(syskuandika(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|okuandikaonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: okusomaonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: okuandikaonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: okuandikaonly}
}
...
func syskuandika(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func syskusoma(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return kusomastdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return kusomavfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == okuandikaonly || accessmode == okusomakuandika || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
	...
	if isrootpath(pathaddress) {
		...
		jinalen, jina := copypath(pathaddress)
		if jinalen == 0 {
			*entry = openfiledescription{}
			return Enoent
		}
		size := filesize(jina[:jinalen])
		if size == 0 {
			*entry = openfiledescription{}
			return Enoent
		}
...
func kusomavfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	kusomafile(entry.jina[:entry.jinalen], tmp)
...
func sysaccess(pathaddress uint32, mode uint32) int32 {
	...
	if !exists {
		jinalen, jina := copypath(pathaddress)
		exists = jinalen != 0 && filesize(jina[:jinalen]) != 0
	}
	if !exists {
		return Enoent
	}
...
func sysstat(pathaddress uint32, stataddress uint32) int32 {
	...
	inode := uint32(2)
	for i := uint32(0); i < jinalen; i++ {
		inode = inode*33 + uint32(jina[i])
	}
...
func sysuname(address_2 uint32) int32 {
	...
	jina := (*posixutsname)(Pointer(uintptr(address_2)))
	*jina = posixutsname{}
	copyutsfield(&jina.Sysname, "EngOS")
	copyutsfield(&jina.Nodename, "engos")
	copyutsfield(&jina.Release, "0.1-posix")
	copyutsfield(&jina.Version, "POSIX.1-2017 phase 1")
	copyutsfield(&jina.Machine, "i386")
	return 0
}
...
func copysocketJina(fd int32, address_2 uint32, lengthaddress uint32, peer bool) int32 {
	socket, err := socketforfd(fd)
	if err != 0 {
		return err
	}
...
func syssocketcall(call uint32, arguments_2 uint32) int32 {
	...
	case 4, 5:
		return Eopnotsupp
	case 6:
		return copysocketJina(int32(socketcallargument(arguments_2, 0)), socketcallargument(arguments_2, 1), socketcallargument(arguments_2, 2), false)
	case 7:
		return copysocketJina(int32(socketcallargument(arguments_2, 0)), socketcallargument(arguments_2, 1), socketcallargument(arguments_2, 2), true)
	case 9:
...
func kusomastdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinkuandika + 1) % uint32(len(stdinbuffer))
	if next == stdinkusoma {
		return
	}
	stdinbuffer[stdinkuandika] = c
	stdinkuandika = next
}

func stdingetblocking() byte {
	for stdinkusoma == stdinkuandika {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinkusoma]
	stdinkusoma = (stdinkusoma + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portkusomabyte(0x64) & 0x01) == 0 {
	}
	sc := Portkusomabyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Kusomapartition(&ata0s)
...
func kusomafile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Kusoma(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
