...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	kuuthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if kuuthread != nil {
		kuuthread.Pid = process.id
		kuuthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(kuuthread)))
	}
