...
ssize_t Окуу(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Окуу, fd, buf, count));
}

ssize_t Жазуу(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Жазуу, fd, buf, count));
}

int Жабуу(int fd)
{
    return (int)__syscall_result(SC1(SYS_Жабуу, fd));
}
