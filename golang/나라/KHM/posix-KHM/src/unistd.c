...
ssize_t aan(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_aan, fd, buf, count));
}

ssize_t sase(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_sase, fd, buf, count));
}
