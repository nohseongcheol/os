...
func KKernelEntry(Pageថតentry uintptr, stacktop uintptr, stackbottom uintptr) {

	MSerialloginit()
	console.MPrint("\n=== KHM BOOT ===\n")

	console.MPrintunsignedinteger32(uint32(Pageថតentry), 0, 2)
	console.MPrintunsignedinteger32(uint32(Pageថតentry), 10, 2)
	...
	paging := &Paging{}
	paging.Init(Pageថតentry, 0x500000, memorymanager)
	paging.Sharedmemoryregion()

	Setcr3(uint32(Pageថតentry))
	Enablepaging()
	...
	processhelper.Init(memorymanager, Pageថតentry)
	...
	processhelper.Spawn(taska, threadhelper, sche, uint32(Pageថតentry), true)
	processhelper.Spawn(taskb, threadhelper, sche, uint32(Pageថតentry), true)
	processhelper.Spawn(taskc, threadhelper, sche, uint32(Pageថតentry), true)
	processhelper.Spawn(taskd1, threadhelper, sche, uint32(Pageថតentry), true)
	processhelper.Spawn(inputeventtask, threadhelper, sche, uint32(Pageថតentry), true)
	...
	Aanfile(linkerfile, linkerdata)

	elf0 := Elf{}
	linkerentry := elf0.Getentry(linkerdata)
	elf0.Parse(linkerdata[:], uint32(Pageថតentry))
	...
	Aanfile(lib1file, lib1data)

	lib1elf := Elf{}
	lib1elf.Parse(lib1data[:], uint32(Pageថតentry))
	...
	Aanfile(lib2file, lib2data)

	lib2elf := Elf{}
	lib2elf.Parse(lib2data[:], uint32(Pageថតentry))
	...
	user1address := memorymanager.Malloc(size)
	user1data := Getbytesfrompointer(uintptr(user1address), int(size), int(size))
	Aanfile(user1file, user1data)

	elf2 := Elf{}

	user1entry := elf2.Getentry(user1data)
	elf2.Parse(user1data[:], uint32(Pageថតentry+0x1000))
	សកលoffsettable := elf2.Got
	...
	*code1pointer = uintptr(linkerentry)
	...
	proc2 := processhelper.Spawn(func1val, threadhelper, sche, uint32(Pageថតentry+0x1000), false)
	thr2 := (*TThread)(proc2.Threads.Getat(0))
	thr2.Cpustate.Ecx = user1entry
	thr2.Cpustate.Edx = សកលoffsettable
	...
	user2address := memorymanager.Malloc(size)
	user2data := Getbytesfrompointer(uintptr(user2address), int(size), int(size))
	Aanfile(user2file, user2data)

	elf3 := Elf{}

	user2entry := elf3.Getentry(user2data)
	elf3.Parse(user2data[:], uint32(Pageថតentry+0x2000))
	សកលoffsettable = elf3.Got
	...
	*code2pointer = uintptr(linkerentry)
	...
	proc3 := processhelper.Spawn(func2val, threadhelper, sche, uint32(Pageថតentry+0x2000), false)
	thr3 := (*TThread)(proc3.Threads.Getat(0))
	thr3.Cpustate.Ecx = user2entry
	thr3.Cpustate.Edx = សកលoffsettable
	...
	user3address := memorymanager.Malloc(size)
	user3data := Getbytesfrompointer(uintptr(user3address), int(size), int(size))
	Aanfile(user3file, user3data)

	elf4 := Elf{}

	user3entry := elf4.Getentry(user3data)
	elf4.Parse(user3data[:], uint32(Pageថតentry+0x3000))
	សកលoffsettable = elf4.Got
	...
	*code3pointer = uintptr(linkerentry)
	...
	proc4 := processhelper.Spawn(func3val, threadhelper, sche, uint32(Pageថតentry+0x3000), false)
	thr4 := (*TThread)(proc4.Threads.Getat(0))
	thr4.Cpustate.Ecx = user3entry
	thr4.Cpustate.Edx = សកលoffsettable
	thr4.Cpustate.Esi = uint32(uintptr(Pointer(PValue3linkmap.First)))

	processhelper.Spawn(TFunction1, threadhelper, sche, uint32(Pageថតentry+0x4000), true)
