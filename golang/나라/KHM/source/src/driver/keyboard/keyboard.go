...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portaanbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portaanbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func saseps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portsasebyte(commandport_2, value)
	return true
}

func saseps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portsasebyte(dataport_2, value)
	return true
}

func aanps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portaanbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portaanbyte(commandport_2)&0x01) != 0; i++ {
		Portaanbyte(dataport_2)
	}

	if !saseps2command(0xAE) || !saseps2command(0x20) {
		return
	}
	status, ok := aanps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !saseps2command(0x60) || !saseps2data(status) {
		return
	}
...
var keyboardqueueaan uint8
var keyboardqueuesase uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuesase + 1) % keyboardqueuesize
	if next == keyboardqueueaan {
		return
	}
	keyboardqueue[keyboardqueuesase] = key
	keyboardqueuesase = next
}

func Processpendingkeyboardevents() {
	for keyboardqueueaan != keyboardqueuesase {
		key := keyboardqueue[keyboardqueueaan]
		keyboardqueueaan = (keyboardqueueaan + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portaanbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portaanbyte(dataport_2)
	if scancode == 0xE0 {
