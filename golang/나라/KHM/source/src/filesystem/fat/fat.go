...
func (self *TBiosparameterblock32) Init(data []byte) {
	...
	self.rootថតentry = (uint16(data[17]) | uint16(data[18])<<8)
...
func (self *TBiosparameterblock32) Len(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte) uint32 {
	...
	hd.Aan28(partitionoffset, &bpbbytes, 90)
	...
	hd.Aan28(rootstart, &direntbytes, 512)

	var dirent = [16]Tថតentryfat32{}
...
func (self *TBiosparameterblock32) Aan(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte, data []byte) {
		...
		for Size > 0 {
			...
			for ; Size > 0; Size -= 512 {
				...
				if dirent[i].size > 512 {
					...
					hd.Aan28(filesector+uint32(sectoroffset), &buffer3, 512)

				} else {
					buffer3 = buffer_2[:dirent[i].size]
					hd.Aan28(filesector+uint32(sectoroffset), &buffer3, int(dirent[i].size))
				}
			...
			hd.Aan28(fatstart+fatsectorforcurrentcluster, &fatbuf, 512)
...
func (self *Tថតentryfat32) Init(data [32]byte) {
