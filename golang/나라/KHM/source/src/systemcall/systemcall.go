...
func Sysaan_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysaan, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Syssase, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Syssase, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysaan:
		cpu.Eax = uint32(sysaan(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Syssase:
		cpu.Eax = uint32(syssase(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|osaseonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: oaanonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: osaseonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: osaseonly}
}

func រកprocess(pid uint32) *processentry {
	for i := 0; i < len(processtable); i++ {
...
func ensurecurrentprocess() *processentry {
	pid := Currentpid()
	if process := រកprocess(pid); process != nil {
		return process
	}
	for i := 0; i < len(processtable); i++ {
...
func syssase(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	...
	if entry.kind != fdkindconsole {
		...
		if entry.kind == fdkindfat || entry.kind == fdkindrootថត {
			return Erofs
		}
		return Ebadf
	}
...
func sysaan(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return aanstdin(address, count)
	}
	if entry.kind == fdkindrootថត {
		return Eisdir
	}
	if entry.kind == fdkindsocket {
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return aanvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == osaseonly || accessmode == oaansase || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func aanvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	aanfile(entry.name[:entry.namelen], tmp)
...
func aanstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinsase + 1) % uint32(len(stdinbuffer))
	if next == stdinaan {
		return
	}
	stdinbuffer[stdinsase] = c
	stdinsase = next
}

func stdingetblocking() byte {
	for stdinaan == stdinsase {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinaan]
	stdinaan = (stdinaan + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portaanbyte(0x64) & 0x01) == 0 {
	}
	sc := Portaanbyte(0x60)
...
func setupexecstack(cpu *Tcpustate, arguments_2 *execvector, environment *execvector) int32 {
	...
	if !Makerangeឯកជនwritable(getcr3(), Userstacktop-stackbytes, stackbytes) {
		return Enomem
	}
	stack := Userstacktop
...
func sysfork(cpu *Tcpustate) int32 {
	...
	childpageថត := Cloneaddressspacecow(getcr3())
	if threadpointer == nil || stackpointer == nil || childpageថត == 0 {
		discardprocess(pid)
		return Einval
	}
...
func allocateprocess(parent uint32) uint32 {
	parentprocess := រកprocess(parent)
	pid := Allocatepid()
	for i := 0; i < len(processtable); i++ {
		if !processtable[i].used {
			processtable[i] = processentry{
...
func aanfile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Aan(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
