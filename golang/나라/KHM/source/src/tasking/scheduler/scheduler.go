...
func initpit(frequency uint32) {
	...
	Portsasebyte(0x43, 0x36)
	Portsasebyte(0x40, uint8(divisor&0xFF))
	Portsasebyte(0x40, uint8((divisor>>8)&0xFF))
}
...
func Initialthreaduserjump(thread *TThread) {
	...
	សកលoffsettable_2 := thread.Cpustate.Edx
	dynamic := thread.Cpustate.Esi

	Portsasebyte(0x20, 0x20)
	jumpusermodeiret(eip, useresp, eflags_2, userprocentry, សកលoffsettable_2, dynamic)
