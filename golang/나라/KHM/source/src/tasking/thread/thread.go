...
func (self *TThreadhelper) Createfromfunction(entrypoint_2 func(), Pageថតentry uint32, iskernel bool) TThread {
	result := TThread{}
	...
	result.Pid = 0
	result.Parentpid = 0
	result.Pageថតentry = Pageថតentry
...
func (self *TThreadhelper) Createpointerfromfunction(entrypoint_2 func(), Pageថតentry uint32, iskernel bool) *TThread {
	result := (*TThread)(self.mem.Malloc(uint32(Sizeof(TThread{}))))
	if result == nil {
		return nil
	}
	*result = self.Createfromfunction(entrypoint_2, Pageថតentry, iskernel)
