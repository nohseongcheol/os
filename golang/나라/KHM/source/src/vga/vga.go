...
var attributecontrolleraanport uint16 = 0x3c1
var attributecontrollersaseport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Saseregister(register []byte) {
	var regindex uint16 = 0

	Portsasebyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portsasebyte(sequencerindexport, i)
		Portsasebyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portsasebyte(crtcindexport, 0x03)

	Portsasebyte(crtcdataport, (Portaanbyte(crtcdataport) | 0x80))
	Portsasebyte(crtcindexport, 0x11)
	Portsasebyte(crtcdataport, (Portaanbyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portsasebyte(crtcindexport, i)
		Portsasebyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portsasebyte(graphicscontrollerindexport, i)
		Portsasebyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portaanbyte(attributecontrollerresetport)
		Portsasebyte(attributecontrollerindexport, i)
		Portsasebyte(attributecontrollersaseport, register[regindex])
		regindex++
	}

	Portaanbyte(attributecontrollerresetport)
	Portsasebyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portsasebyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portaanbyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Saseregister(g320x200x256)
