...
GLOBL ·식별자tDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·식별자t<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
GLOBL ·관문Handlers<>(SB), NOPTR, $NUM_IDT_ENTRIES*8
...
TEXT ·설치개입중단서술자표(SB),NOSPLIT,$0
	LEAQ ·식별자tDescriptor<>(SB), AX
...
TEXT ·H처리개입중단(SB),NOSPLIT,$0-10
	...
	LEAQ ·개입중단관문항목(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
