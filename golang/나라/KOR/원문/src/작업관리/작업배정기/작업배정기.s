...
TEXT ·개입중단복귀로사용자영역진입(SB),NOSPLIT,$0
	...
	IRETL
TEXT ·jump사용자모드iret1(SB),NOSPLIT,$0
	RET

TEXT ·종료실행흐름대기(SB),NOSPLIT,$0
	STI
	BYTE $0xF4
	BYTE $0xEB; BYTE $0xFD
...
TEXT ·부동소수점저장기보존(SB),NOSPLIT,$0
    MOVL ·임시공간_2+0(FP), AX
    FXSAVE (AX)
    RET

TEXT ·부동소수점저장기복원(SB),NOSPLIT,$0
    MOVL ·임시공간_2+0(FP), AX
    FXRSTOR (AX)
    RET

TEXT ·확장연산상태저장(SB),NOSPLIT,$0
	MOVL p1+0(FP), AX
	FXSAVE (AX)
	RET
TEXT ·확장연산상태복원(SB),NOSPLIT,$0
	MOVL p1+0(FP), AX
	FXRSTOR (AX)
	RET
