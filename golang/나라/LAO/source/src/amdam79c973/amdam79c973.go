...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portanword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portanword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portanword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portanword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portanword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portanword(Macaddress4port) / 256)
	...
	Portkhianword(registeraddressport, 20)
	Portkhianword(buscontrolregisterdataport, 0x102)

	Portkhianword(registeraddressport, 0)
	Portkhianword(registerdataport, 0x04)
	...
	Portkhianword(registeraddressport, 1)
	Portkhianword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portkhianword(registeraddressport, 2)
	Portkhianword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portkhianword(registeraddressport, 0)
	Portkhianword(registerdataport, 0x41)

	Portkhianword(registeraddressport, 4)
	temporary := Portanword(registerdataport)
	Portkhianword(registeraddressport, 4)
	Portkhianword(registerdataport, temporary|0xC00)

	Portkhianword(registeraddressport, 0)
	Portkhianword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portanword(resetport)
	Portkhianword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portkhianword(registeraddressport, 0)
	temporary := uint32(Portanword(registerdataport))
	...
	Portkhianword(registeraddressport, 0)
	Portkhianword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portkhianword(registeraddressport, 0)
	Portkhianword(registerdataport, 0x48)
