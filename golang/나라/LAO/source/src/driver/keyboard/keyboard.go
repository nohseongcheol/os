...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	status |= 0x01
	status &^= 0x10
	if !khianps2command(0x60) || !khianps2data(status) {
		return
	}
...
func queuekeyboardbyte(key byte) {
	next := (keyboardqueuekhian + 1) % keyboardqueuesize
...
func Processpendingkeyboardevents() {
	for keyboardqueuean != keyboardqueuekhian {
		...
		keyboardqueuean = (keyboardqueuean + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portanbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portanbyte(dataport_2)
	if scancode == 0xE0 {
