...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: oanonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: okhianonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: okhianonly}
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == okhianonly || accessmode == oankhian || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func anvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
