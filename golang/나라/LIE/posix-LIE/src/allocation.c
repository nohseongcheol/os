...
void *malloc(size_t Ziffernanzahl)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!Ziffernanzahl) Ziffernanzahl = 1;
    if (Ziffernanzahl > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    Ziffernanzahl = (Ziffernanzahl + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= Ziffernanzahl) {
            if (cursor->capacity - Ziffernanzahl >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + Ziffernanzahl);
                raw->capacity = cursor->capacity - Ziffernanzahl - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = Ziffernanzahl;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    address = Speicherende_verschieben(0);
    if (address == (void *)-1) return NULL;
    padding = (0U - (uintptr_t)address) & 15U;
    total = padding + sizeof(struct heap_block) + Ziffernanzahl;
    if ((uintptr_t)address > (uintptr_t)INT_MAX - total) { errno = ENOMEM; return NULL; }
    address = Speicherende_verschieben((int)total);
    if (address == (void *)-1) return NULL;
    raw = (struct heap_block *)((unsigned char *)address + padding);
    raw->capacity = Ziffernanzahl; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t Ziffernanzahl)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(Ziffernanzahl);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!Ziffernanzahl) Ziffernanzahl = 1;
    cursor = (struct heap_block *)address - 1;
    if (Ziffernanzahl <= cursor->capacity) return address;
    destination = malloc(Ziffernanzahl);
