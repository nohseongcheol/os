...
ssize_t kiyavima(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_kiyavima, fd, buf, count));
}

ssize_t livima(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_livima, fd, buf, count));
}
