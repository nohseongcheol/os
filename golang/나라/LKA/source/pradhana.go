...
func (self *TMykeyboardeventhandler) Onkeyපහළ(key byte) {
	foo := [1]byte{' '}
	foo[0] = key
...
func (self *TMykeyboardeventhandler) Onkeyඉහළ(key byte)	{}
...
func (self *TMymouseeventhandler) Onmouseපහළ(button int8) {
	buffer := []byte("x")
	mouseconsole.MPrintxy(buffer, uint16(previousx), uint16(previousy))
}
func (self *TMymouseeventhandler) Onmouseඉහළ(button int8)	{}
...
var උපකරණයdescriptor TPeripheralcomponentinterconnectඋපකරණයdescriptor
...
func (self TMypcicontrollerhandler) Ongetdriver(උපකරණය TPeripheralcomponentinterconnectඋපකරණයdescriptor) {
	if උපකරණය.Vendorid == 0x1022 && උපකරණය.Dඋපකරණයid == 0x2000 {
		console.MPrintxy([]byte("["), 0, 12)
		console.MPrint(([]byte)("AMD am79c973"))
		console.MPrint([]byte(":"))
		console.MUnsignedinteger16print(උපකරණය.Vendorid)
		console.MPrint([]byte(":"))
		console.MUnsignedinteger16print(උපකරණය.Dඋපකරණයid)
		console.MPrint([]byte(":"))
		console.MUnsignedinteger16print(uint16(උපකරණය.Portbase))
		console.MPrint([]byte(":"))
		console.MUnsignedinteger32print(උපකරණය.Interrupt)

		console.MPrint([]byte("]\n"))
		උපකරණයdescriptor = උපකරණය
		drivercount++
	}
}
func (self TMypcicontrollerhandler) Getdriver() TPeripheralcomponentinterconnectඋපකරණයdescriptor {
	return උපකරණයdescriptor
}
...
func memorytest(y int) {
	...
	allocated := uint32(uintptr(මතකයmanager.Malloc(1024)))
	console.MUnsignedinteger32printxy(allocated, 10, uint16(y))
	if y == 11 {
		මතකයmanager.Free(Pointer(uintptr(allocated)))
	}
}
...
func Getfunctionනම(i interface{}) {
	var address = reflect.ValueOf(i).Pointer()
	var funcනම = runtime.FuncForPC(address).Name()
	var funcbytes []byte = []byte(funcනම)
...
func KKernelEntry(Pagedirectoryentry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	මතකයmanager := &Tමතකයmanager{}
	මතකයmanager.Init(0, Maxqueuesize)

	paging := &Paging{}
	paging.Init(Pagedirectoryentry, 0x500000, මතකයmanager)
	paging.Sharedමතකයregion()
	...
	threadhelper.Init(මතකයmanager)

	processhelper := Processhelper{}
	processhelper.Init(මතකයmanager, Pagedirectoryentry)

	sche := &Scheduler{}
	sche.Init(Interruptmanager, මතකයmanager, tss)
	...
	linkeraddress := මතකයmanager.Malloc(size)
	linkerdata := Getbytesfrompointer(uintptr(linkeraddress), int(size), int(size))
	Kiyavimafile(linkerfile, linkerdata)
	...
	user1address := මතකයmanager.Malloc(size)
	user1data := Getbytesfrompointer(uintptr(user1address), int(size), int(size))
	Kiyavimafile(user1file, user1data)
	...
	Pඅගය1linkmap := linkmap.Clone()
	Pඅගය1linkmap.Append_to_list(uintptr(elf2.Dynamic))

	මතකයmanager.Free(user1address)

	var code1pointer *uintptr
	var func1val func()

	code1pointer = (*uintptr)(මතකයmanager.Malloc(4))
	*code1pointer = uintptr(linkerentry)
	...
	thr2.Cpustate.Esi = uint32(uintptr(Pointer(Pඅගය1linkmap.First)))
	...
	user2address := මතකයmanager.Malloc(size)
	user2data := Getbytesfrompointer(uintptr(user2address), int(size), int(size))
	Kiyavimafile(user2file, user2data)
	...
	Pඅගය2linkmap := linkmap.Clone()
	Pඅගය2linkmap.Append_to_list(uintptr(elf3.Dynamic))

	මතකයmanager.Free(user2address)

	var code2pointer *uintptr
	var func2val func()

	code2pointer = (*uintptr)(මතකයmanager.Malloc(4))
	*code2pointer = uintptr(linkerentry)
	...
	thr3.Cpustate.Esi = uint32(uintptr(Pointer(Pඅගය2linkmap.First)))
	...
	user3address := මතකයmanager.Malloc(size)
	user3data := Getbytesfrompointer(uintptr(user3address), int(size), int(size))
	Kiyavimafile(user3file, user3data)
	...
	Pඅගය3linkmap := linkmap.Clone()
	Pඅගය3linkmap.Append_to_list(uintptr(elf4.Dynamic))

	මතකයmanager.Free(user3address)

	var code3pointer *uintptr
	var func3val func()

	code3pointer = (*uintptr)(මතකයmanager.Malloc(4))
	*code3pointer = uintptr(linkerentry)
	...
	thr4.Cpustate.Esi = uint32(uintptr(Pointer(Pඅගය3linkmap.First)))
	...
	උපකරණයdescriptor = mypcicontrollerhandler.Getdriver()
