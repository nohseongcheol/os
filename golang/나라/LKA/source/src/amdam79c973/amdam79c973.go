...
var sendbufferdescriptionමතකය [2048 + 15]byte
var sendbuffer [2*1024 + 15][8]uint8
var currentsendbuffer uint8

var recvbufferdescription [8]TBufferdescriptor
var recvbufferdescriptionමතකය [2048 + 15]uint8
var recvbuffer [2*1024 + 15][8]uint8
var currentrecvbuffer uint8
var funcඅගය func(*Tamdam79c973, uint32) uint32

type Tamdam79c973 struct {
	TInterrupthandler
	උපකරණයdescriptor	TPeripheralcomponentinterconnectඋපකරණයdescriptor
...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, උපකරණයdescriptor TPeripheralcomponentinterconnectඋපකරණයdescriptor, handler IRawdatahandler) {

	self.උපකරණයdescriptor = උපකරණයdescriptor

	funcඅගය = (*Tamdam79c973).Handleinterrupt
	var address uintptr
	address = uintptr(Pointer(&funcඅගය))

	self.Init(uint8(0x20+උපකරණයdescriptor.Interrupt), uintptr(Pointer(interrupt)), address)

	Macaddress0port = uint16(උපකරණයdescriptor.Portbase)
	Macaddress2port = uint16(උපකරණයdescriptor.Portbase) + 0x02
	Macaddress4port = uint16(උපකරණයdescriptor.Portbase) + 0x04
	registerdataport = uint16(උපකරණයdescriptor.Portbase) + 0x10
	registeraddressport = uint16(උපකරණයdescriptor.Portbase) + 0x12
	resetport = uint16(උපකරණයdescriptor.Portbase) + 0x14
	buscontrolregisterdataport = uint16(උපකරණයdescriptor.Portbase) + 0x16
	...
	var Mac0 uint64 = uint64(Portkiyavimaවචන(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portkiyavimaවචන(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portkiyavimaවචන(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portkiyavimaවචන(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portkiyavimaවචන(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portkiyavimaවචන(Macaddress4port) / 256)
	...
	console_2.MHexadecimalprint(uint8(උපකරණයdescriptor.Interrupt))
	...
	Portlivimaවචන(registeraddressport, 20)
	Portlivimaවචන(buscontrolregisterdataport, 0x102)

	Portlivimaවචන(registeraddressport, 0)
	Portlivimaවචන(registerdataport, 0x04)
	...
	sendbufferdescription = *(*([8]TBufferdescriptor))(Pointer((uintptr((Pointer)(&sendbufferdescriptionමතකය)) + 15) & ^(uintptr)(0xF)))
	initblock.sendbufferdescriptionaddress = uintptr(Pointer(&sendbufferdescription))
	recvbufferdescription = *(*([8]TBufferdescriptor))(Pointer((uintptr((Pointer)(&recvbufferdescriptionමතකය)) + 15) & ^(uintptr)(0xF)))
	...
	Portlivimaවචන(registeraddressport, 1)
	Portlivimaවචන(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portlivimaවචන(registeraddressport, 2)
	Portlivimaවචන(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portlivimaවචන(registeraddressport, 0)
	Portlivimaවචන(registerdataport, 0x41)

	Portlivimaවචන(registeraddressport, 4)
	temporary := Portkiyavimaවචන(registerdataport)
	Portlivimaවචන(registeraddressport, 4)
	Portlivimaවචන(registerdataport, temporary|0xC00)

	Portlivimaවචන(registeraddressport, 0)
	Portlivimaවචන(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portkiyavimaවචන(resetport)
	Portlivimaවචන(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portlivimaවචන(registeraddressport, 0)
	temporary := uint32(Portkiyavimaවචන(registerdataport))
	...
	Portlivimaවචන(registeraddressport, 0)
	Portlivimaවචන(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portlivimaවචන(registeraddressport, 0)
	Portlivimaවචන(registerdataport, 0x48)
