...
func (self *Elf) Parse(data []byte, Pagedirectoryentry uint32) {
	...
	if elfheader.eshnum != 0 {
		...
		for i := uint32(0); i < uint32(elfheader.eshnum); i++ {
			...
			var sectනම []byte
			start := uint32(strtab.shoffset + sectheader.shනම)
			end := start
			for ; ; end++ {
			...
			if sectheader.shtype != 8 {
				...
				sectඅගය = data[sectheader.shoffset:endoffset]
			}

			if Equalbytes(sectනම, ([]byte)(".got.plt")) {
				console_2.MPrint("[")
				console_2.MPrint(sectනම)
				console_2.MPrint(":")
				self.Got = sectheader.shaddress
				console_2.MUnsignedinteger32print(self.Got)
				console_2.MPrint("]")
			}
			if Equalbytes(sectනම, ([]byte)(".dynamic")) {
				console_2.MPrint("[")
				console_2.MPrint(sectනම)
				console_2.MPrint(":")
			...
			if sectheader.shaddress > 0x1000 {
				...
				if sectheader.shtype == 8 {
					...
					Setblockinpagedirectory(sectඅගය, destination_2, size, Pagedirectoryentry)
				}
			}

			continue

			if Equalbytes(sectනම, ([]byte)(".text")) {
				...
				console_2.MPrint("]")
				copy(self.text[:sectheader.shsize], sectඅගය[:sectheader.shsize])
				self.textlen = sectheader.shsize
			}
			if Equalbytes(sectනම, ([]byte)(".rel.text")) {
				...
				for rt := uint32(0); rt < sectheader.shsize/8; rt++ {
					offset := *(*uint32)(Pointer(&sectඅගය[rt*8]))
					self.reltext[rt].offset = offset
					self.reltext[rt].oaddress = *(*uint32)(Pointer(&self.text[offset]))
					self.reltext[rt].number = *(*uint32)(Pointer(&sectඅගය[rt*8+4]))
					self.reltextlen++
				}
			}
			if Equalbytes(sectනම, ([]byte)(".dynsym")) {
			...
			if Equalbytes(sectනම, ([]byte)(".dynstr")) {
			...
			if Equalbytes(sectනම, ([]byte)(".strtab")) {
				...
				for st := uint32(1); st < sectheader.shsize; st++ {
					if sectඅගය[st] == 0x0 || sectඅගය[st] == ' ' {
						funcනම := sectඅගය[start+1 : st]
						console_2.MPrint("+")
						console_2.MPrint(funcනම)
						self.strtab[rt] = Bytestostring(funcනම)
						start = st
						rt++
					}
				}
