...
func (self_2 *TSegmentdescriptor) Init(base_2 uint32, limit_2 uint32, typeඅගය uint8, flags_2 uint8) {
...
func (self_2 *TShareddescriptortable) Init() {
	...
	size_2 := (*uint16)(Pointer(&destination_3[0]))
	(*size_2) = (uint16)((Sizeof(data_2)))
...
func (self_2 *TShareddescriptortable) Setdescriptor(idx int, base_2 uint32, limit_2 uint32, typeඅගය uint8, flags_2 uint8) {
	data_2.segmentdescriptor[idx].Init(base_2, limit_2, typeඅගය, flags_2)
}
...
func Settlssegment(index uint32, descriptor *Userdescriptor, table []Gdtentry_2) bool {
	...
	access := uint8(Privuser | Segnormal | Present)
