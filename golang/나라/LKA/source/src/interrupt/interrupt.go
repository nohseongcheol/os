...
func Nනවinterrupthandler(Interruptmanager uintptr, Interruptnumber uint8) *TInterrupthandler {
...
func (self *TInterruptmanager) Init(hardwareinterruptoffset uint16, globaldescriptortable *TShareddescriptortable, taskmanager *TTaskmanager) {
	...
	Portlivimabyte(Primarypiccommandioport, 0x11)
	Portlivimabyte(Secondarypiccommandioport, 0x11)

	Portlivimabyte(Primarypicdataioport, 0x20)
	Portlivimabyte(Secondarypicdataioport, 0x28)

	Portlivimabyte(Primarypicdataioport, 0x04)
	Portlivimabyte(Secondarypicdataioport, 0x02)

	Portlivimabyte(Primarypicdataioport, 0x01)
	Portlivimabyte(Secondarypicdataioport, 0x01)

	Portlivimabyte(Primarypicdataioport, 0xF8)
	Portlivimabyte(Secondarypicdataioport, 0xEF)
...
func (self *TInterruptmanager) Dohandleinterrupt(interrupt uint8, esp uint32) uint32 {
	...
	if 0x20 <= interrupt && interrupt < 0x30 {
		if 0x28 <= interrupt {
			Portlivimabyte(Secondarypiccommandioport, 0x20)
		}
		Portlivimabyte(Primarypiccommandioport, 0x20)
	}
	return esp
}
...
func exceptionනම(interrupt uint32) string {
	switch interrupt {
	case 0x00:
...
func exceptionframeඅගය(frame uint32, offset uint32) uint32 {
...
func Handleexception(esp uint32, interrupt uint32) uint32 {
	...
	MEmergencylogstring(" ")
	MEmergencylogstring(exceptionනම(interrupt))
	...
	if exceptionhaserrorcode(interrupt) {
		err = exceptionframeඅගය(esp, 0)
		eipoffset = 4
	}
	eip := exceptionframeඅගය(esp, eipoffset)
	cs := exceptionframeඅගය(esp, eipoffset+4)
	eflags_2 := exceptionframeඅගය(esp, eipoffset+8)
	...
	if (cs & 0x03) != 0 {
		...
		MEmergencylogunsignedinteger32(exceptionframeඅගය(esp, eipoffset+12))
		MEmergencylogstring(" ss=")
		MEmergencylogunsignedinteger32(exceptionframeඅගය(esp, eipoffset+16))
	}
