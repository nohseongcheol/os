...
func (self *TInternetprotocolprovider) Send(destinationipaddressnetworkbyteorder uint32, protocol uint8, datapointer uintptr, size uint32) {
	...
	var ඊලඟhopipaddressnetworkbyteorder uint32 = destinationipaddressnetworkbyteorder
	if (destinationipaddressnetworkbyteorder & self.Subnetmask) != (message.sourceipaddress & self.Subnetmask) {
		ඊලඟhopipaddressnetworkbyteorder = self.Gatewayip
	}

	var senddatapointer = uintptr(Pointer(&buffer1_2))
	ipconsole.MUnsignedinteger32print(ඊලඟhopipaddressnetworkbyteorder)

	var ethernettypebe = Unsignedinteger16r(0x0800)
	efhandler.Framesend(self.arpprovider.Resolve(ඊලඟhopipaddressnetworkbyteorder), ethernettypebe, senddatapointer, uint32(ipsize)+uint32(size))
