...
func (self *Tcowframemanager) Init(mem *Tමතකයmanager, framecount uint32) bool {
	self.mem = mem
...
func (self *Paging) Init(pagedirectoryentry uintptr, pagetableentry uint32, මතකයmanager *Tමතකයmanager) {
	...
	cowframemanager.Init(මතකයmanager, (512*1024*1024)>>12)

	for i := uint32(0); i <= virtlen; i++ {

		for pde := uint32(0); pde < pdelen; pde++ {

			addresspointer, _ := මතකයmanager.Alignedmalloc(0x1000)
...
func (self *Paging) Sharedමතකයregion() {
	...
	for i := uint32(1); i <= virtlen; i++ {
		...
		for ; pde < 12; pde++ {
			v := Getඅගය(uint32(kpagedirectoryentry) + pde*4)
			v = (v & 0xFFFFF000)
...
func Cloneaddressspacecow(sourcepagedirectory uint32) uint32 {
	if Activeමතකයmanager == nil || sourcepagedirectory == 0 {
		return 0
	}
	destinationpointer, _ := Activeමතකයmanager.Alignedmalloc(0x1000)
	...
	for pde := uint32(0); pde < 1024; pde++ {
		...
		sourcepde := Getඅගය(sourcepdeaddress)
		...
		destinationptpointer, _ := Activeමතකයmanager.Alignedmalloc(0x1000)
...
func Resolvecopyonlivimafault() bool {
	...
	නවpointer, _ := Activeමතකයmanager.Alignedmalloc(0x1000)
	if නවpointer == nil {
		return false
	}
	නවframe := uint32(uintptr(නවpointer)) & Pageframe

	source_2 := Getbytesfrompointer(uintptr(faultaddress&Pageframe), 0x1000, 0x1000)
	destination_2 := Getbytesfrompointer(uintptr(නවframe), 0x1000, 0x1000)
	copy(destination_2, source_2)
	cowframemanager.Decrement(oldframe)
	Setunsignedinteger32ataddress((නවframe|(pte&0xFFF)|Pagewritable)&^Pagecow, pteaddress)
	reloadcr3()
	return true
}
...
func makepageprivatewritablecurrent(pagedirectory uint32, virtualaddress uint32) bool {
	pde := Getඅගය(pagedirectory + ((virtualaddress>>22)&0x3FF)*4)
	if (pde & Pagepresent) == 0 {
		return false
	}
...
func Getඅගය(address uint32) uint32 {
	var orgඅගය uint32 = *(*uint32)(unsafe.Pointer(uintptr(address)))
	return orgඅගය
}
func Getඅගයinpagedirectory(address uint32, pagedirectory uint32) uint32 {
	if pagedirectory == 0 {
		return 0
	}
	oldcr3 := getcr3()
