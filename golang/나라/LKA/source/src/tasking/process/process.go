...
func (self *Process) Init(mem *mem.Tමතකයmanager) {
...
func (self *Processhelper) Init(mem *mem.Tමතකයmanager, kernelpagedirectoryentry uintptr) {
	self.mem = mem
...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	pradhanathread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if pradhanathread != nil {
		pradhanathread.Pid = process.id
		pradhanathread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(pradhanathread)))
	}
