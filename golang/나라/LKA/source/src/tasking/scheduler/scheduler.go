...
func (self *Schedulerdata) Getඊලඟreadythread() *TThread {
	if list.Size_2 <= 0 {
		return nil
	}
...
func (self *Scheduler) Init(manager *TInterruptmanager, mem *mem.Tමතකයmanager, tss *Tssentry) {
	schedata.Init()
	schedata.tss = tss
...
func initpit(frequency uint32) {
	...
	Portlivimabyte(0x43, 0x36)
	Portlivimabyte(0x40, uint8(divisor&0xFF))
	Portlivimabyte(0x40, uint8((divisor>>8)&0xFF))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	if schedata.tickcount == schedata.frequency {
		...
		if list.Size_2 > 0 && schedata.Enabled == true {
			var ඊලඟthread = schedata.Getඊලඟreadythread()
			if ඊලඟthread == nil {
				return esp
			}
			if schedata.currentthread == nil {
				...
				MEmergencylogunsignedinteger32(uint32(uintptr(Pointer(ඊලඟthread))))
				MEmergencylogstring(" cpu=")
				MEmergencylogunsignedinteger32(uint32(uintptr(Pointer(ඊලඟthread.Cpustate))))
				MEmergencylogstring(" state=")
				MEmergencylogunsignedinteger32(uint32(ඊලඟthread.Threadstate))
				MEmergencylogstring(" eip=")
				MEmergencylogunsignedinteger32(ඊලඟthread.Cpustate.Eip)
				MEmergencylogstring(" cs=")
				MEmergencylogunsignedinteger32(ඊලඟthread.Cpustate.Cs)
				MEmergencylogstring("\n")
			}
			...
			address := uintptr(Pointer(&(ඊලඟthread.Fpubuffer)))
			offset := ඊලඟthread.Fpuoffset
			...
			schedata.currentthread = ඊලඟthread
			...
			esp = uint32(uintptr(Pointer(ඊලඟthread.Cpustate)))
			if ඊලඟthread.Stack != 0 {
				schedata.tss.Setstack(Segkerneldata, ඊලඟthread.Stack+Threadstacksize)
			}

			setcr3(ඊලඟthread.Pagedirectoryentry)
			setgs(ඊලඟthread.Cpustate.Gs)
...
func Stopcurrentthread(cpustate *Tcpustate) *Tcpustate {
	...
	ඊලඟthread := schedata.Getඊලඟreadythread()
	if ඊලඟthread == nil || ඊලඟthread == stoppedthread || ඊලඟthread.Cpustate == nil || ඊලඟthread.Cpustate == cpustate {
		setthreadexitloopstate(cpustate)
		return cpustate
	}

	schedata.currentthread = ඊලඟthread
	if ඊලඟthread.Stack != 0 && schedata.tss != nil {
		schedata.tss.Setstack(Segkerneldata, ඊලඟthread.Stack+Threadstacksize)
	}
	setcr3(ඊලඟthread.Pagedirectoryentry)
	setgs(ඊලඟthread.Cpustate.Gs)
	return ඊලඟthread.Cpustate
}
