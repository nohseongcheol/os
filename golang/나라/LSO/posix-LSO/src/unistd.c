...
ssize_t bala(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_bala, fd, buf, count));
}

ssize_t ngola(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ngola, fd, buf, count));
}
