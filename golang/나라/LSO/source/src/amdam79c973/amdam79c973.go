...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portbalaword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portbalaword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portbalaword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portbalaword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portbalaword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portbalaword(Macaddress4port) / 256)
	...
	Portngolaword(registeraddressport, 20)
	Portngolaword(buscontrolregisterdataport, 0x102)

	Portngolaword(registeraddressport, 0)
	Portngolaword(registerdataport, 0x04)
	...
	Portngolaword(registeraddressport, 1)
	Portngolaword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portngolaword(registeraddressport, 2)
	Portngolaword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portngolaword(registeraddressport, 0)
	Portngolaword(registerdataport, 0x41)

	Portngolaword(registeraddressport, 4)
	temporary := Portbalaword(registerdataport)
	Portngolaword(registeraddressport, 4)
	Portngolaword(registerdataport, temporary|0xC00)

	Portngolaword(registeraddressport, 0)
	Portngolaword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portbalaword(resetport)
	Portngolaword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portngolaword(registeraddressport, 0)
	temporary := uint32(Portbalaword(registerdataport))
	...
	Portngolaword(registeraddressport, 0)
	Portngolaword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portngolaword(registeraddressport, 0)
	Portngolaword(registerdataport, 0x48)
