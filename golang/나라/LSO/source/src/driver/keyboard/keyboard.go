...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portbalabyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portbalabyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func ngolaps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portngolabyte(commandport_2, value)
	return true
}

func ngolaps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portngolabyte(dataport_2, value)
	return true
}

func balaps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portbalabyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portbalabyte(commandport_2)&0x01) != 0; i++ {
		Portbalabyte(dataport_2)
	}

	if !ngolaps2command(0xAE) || !ngolaps2command(0x20) {
		return
	}
	status, ok := balaps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !ngolaps2command(0x60) || !ngolaps2data(status) {
		return
	}

	if !ngolaps2data(0xF4) {
		return
	}
	ack, ok := balaps2data()
	if !ok || ack != 0xFA {
...
var keyboardqueuebala uint8
var keyboardqueuengola uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuengola + 1) % keyboardqueuesize
	if next == keyboardqueuebala {
		return
	}
	keyboardqueue[keyboardqueuengola] = key
	keyboardqueuengola = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuebala != keyboardqueuengola {
		key := keyboardqueue[keyboardqueuebala]
		keyboardqueuebala = (keyboardqueuebala + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portbalabyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portbalabyte(dataport_2)
	if scancode == 0xE0 {
