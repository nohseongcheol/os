...
var stdinbala uint32
var stdinngola uint32
...
func Sysbala_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysbala, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysngola, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysngola, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysbala:
		cpu.Eax = uint32(sysbala(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysngola:
		cpu.Eax = uint32(sysngola(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|ongolaonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: obalaonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: ongolaonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: ongolaonly}
}
...
func sysngola(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysbala(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return balastdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return balavfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == ongolaonly || accessmode == obalangola || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func balavfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	balafile(entry.name[:entry.namelen], tmp)
...
func balastdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinngola + 1) % uint32(len(stdinbuffer))
	if next == stdinbala {
		return
	}
	stdinbuffer[stdinngola] = c
	stdinngola = next
}

func stdingetblocking() byte {
	for stdinbala == stdinngola {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinbala]
	stdinbala = (stdinbala + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portbalabyte(0x64) & 0x01) == 0 {
	}
	sc := Portbalabyte(0x60)
...
func balafile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Bala(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
