...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	sehloohothread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if sehloohothread != nil {
		sehloohothread.Pid = process.id
		sehloohothread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(sehloohothread)))
	}
