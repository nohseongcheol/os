...
var attributecontrollerbalaport uint16 = 0x3c1
var attributecontrollerngolaport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Ngolaregister(register []byte) {
	var regindex uint16 = 0

	Portngolabyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portngolabyte(sequencerindexport, i)
		Portngolabyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portngolabyte(crtcindexport, 0x03)

	Portngolabyte(crtcdataport, (Portbalabyte(crtcdataport) | 0x80))
	Portngolabyte(crtcindexport, 0x11)
	Portngolabyte(crtcdataport, (Portbalabyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portngolabyte(crtcindexport, i)
		Portngolabyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portngolabyte(graphicscontrollerindexport, i)
		Portngolabyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portbalabyte(attributecontrollerresetport)
		Portngolabyte(attributecontrollerindexport, i)
		Portngolabyte(attributecontrollerngolaport, register[regindex])
		regindex++
	}

	Portbalabyte(attributecontrollerresetport)
	Portngolabyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portngolabyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portbalabyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Ngolaregister(g320x200x256)
