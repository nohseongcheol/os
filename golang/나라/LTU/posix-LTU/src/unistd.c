...
ssize_t Skaitymas(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Skaitymas, fd, buf, count));
}

ssize_t Rašymas(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Rašymas, fd, buf, count));
}

int Užverti(int fd)
{
    return (int)__syscall_result(SC1(SYS_Užverti, fd));
}
