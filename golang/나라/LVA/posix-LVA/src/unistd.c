...
ssize_t Lasīt(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Lasīt, fd, buf, count));
}

ssize_t Rakstīt(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Rakstīt, fd, buf, count));
}

int Aizvērt(int fd)
{
    return (int)__syscall_result(SC1(SYS_Aizvērt, fd));
}
