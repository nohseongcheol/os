...
freestanding library in `./espace_utilisateur/posix`.
...
```sh
cd ./espace_utilisateur/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./espace_utilisateur/posix/déclarations` and link the archive into an EngOS i386 user
...
`./espace_utilisateur/posix/TEST_RESULTS.md`. Build all runtime test executables
