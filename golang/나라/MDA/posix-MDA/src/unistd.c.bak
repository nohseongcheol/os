...
ssize_t Citire(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Citire, fd, buf, count));
}

ssize_t Scriere(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Scriere, fd, buf, count));
}

int Închide(int fd)
{
    return (int)__syscall_result(SC1(SYS_Închide, fd));
}
