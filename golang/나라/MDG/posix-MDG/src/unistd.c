...
ssize_t Mamaky(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Mamaky, fd, buf, count));
}

ssize_t Manoratra(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Manoratra, fd, buf, count));
}

int hidio(int fd)
{
    return (int)__syscall_result(SC1(SYS_hidio, fd));
}
