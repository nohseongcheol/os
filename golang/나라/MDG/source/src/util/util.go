...
var utilKonsoly TKonsoly = TKonsoly{}
...
func AtontayOctet(datapointer uintptr, habe int) {
	var buffer_2 [4 * 1024]byte = *(*([4 * 1024]byte))(Pointer(datapointer))
	var i int
	utilKonsoly.MAtontay([]byte("util["))
	for i = 0; i < habe; i++ {
		utilKonsoly.MHexadecimalAtontay(buffer_2[i])
	}
	utilKonsoly.MAtontay([]byte("]"))
}

func Atontaytest(params ...interface{}) {
	for _, param := range params {
		utilKonsoly.MAtontay([]byte(TypeOf(param).Name()))

	}
}
func EqualOctet(a, b []byte) bool {
	if len(a) != len(b) {
		return false
	}
	for i, v := range a {
...
func Octettostring(b []byte) string {
...
func GetOctetfrompointer(pointer_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		address	uintptr
		len	int
		cap	int
