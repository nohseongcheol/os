...
ssize_t kiyun(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_kiyun, fd, buf, count));
}

ssize_t liyun(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_liyun, fd, buf, count));
}
