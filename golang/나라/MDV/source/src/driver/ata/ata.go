...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Liyun(0xA0)
	} else {
		self.deviceport.Liyun(0xB0)
	}
	self.controlport.Liyun(0)
	self.deviceport.Liyun(0xA0)

	var status uint8 = self.commandport.Kiyun()
	if status == 0xFF {
	...
	self.sectorcountport.Liyun(0)
	self.lbalowport.Liyun(0)
	self.lbamidport.Liyun(0)
	self.lbahiport.Liyun(0)
	self.commandport.Liyun(0xEC)

	status = self.commandport.Kiyun()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Kiyun28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Liyun(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Liyun(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Liyun(0)
	self.sectorcountport.Liyun(1)

	self.lbalowport.Liyun(uint8(sector & 0x000000FF))
	self.lbamidport.Liyun(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Liyun(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Liyun(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Kiyun()
...
func (self *TAdvancedtechnologyattachment) Liyun28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Liyun(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Liyun(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Liyun(0)
	self.sectorcountport.Liyun(1)
	self.lbalowport.Liyun(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Liyun(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Liyun(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Liyun(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Liyun(0xE0)
	} else {
		self.deviceport.Liyun(0xF0)
	}
	self.commandport.Liyun(0xE7)
