...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	if !liyunps2command(0xAE) || !liyunps2command(0x20) {
		return
	}
...
var keyboardqueuekiyun uint8
var keyboardqueueliyun uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueueliyun + 1) % keyboardqueuesize
	if next == keyboardqueuekiyun {
		return
	}
	keyboardqueue[keyboardqueueliyun] = key
	keyboardqueueliyun = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuekiyun != keyboardqueueliyun {
		key := keyboardqueue[keyboardqueuekiyun]
		keyboardqueuekiyun = (keyboardqueuekiyun + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	...
	scancode := Portkiyunbyte(dataport_2)
	if scancode == 0xE0 {
