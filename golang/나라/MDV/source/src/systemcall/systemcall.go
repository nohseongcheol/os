...
var stdinkiyun uint32
var stdinliyun uint32
...
func Syskiyun_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Syskiyun, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysliyun, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysliyun, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Syskiyun:
		cpu.Eax = uint32(syskiyun(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysliyun:
		cpu.Eax = uint32(sysliyun(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|oliyunonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: okiyunonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: oliyunonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: oliyunonly}
}
...
func sysliyun(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func syskiyun(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return kiyunstdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return kiyunvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == oliyunonly || accessmode == okiyunliyun || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func kiyunvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	kiyunfile(entry.name[:entry.namelen], tmp)
...
func kiyunstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinliyun + 1) % uint32(len(stdinbuffer))
	if next == stdinkiyun {
		return
	}
	stdinbuffer[stdinliyun] = c
	stdinliyun = next
}

func stdingetblocking() byte {
	for stdinkiyun == stdinliyun {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinkiyun]
	stdinkiyun = (stdinkiyun + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portkiyunbyte(0x64) & 0x01) == 0 {
	}
	sc := Portkiyunbyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Kiyunpartition(&ata0s)
...
func kiyunfile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Kiyun(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
