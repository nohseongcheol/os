...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	maiganduthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if maiganduthread != nil {
		maiganduthread.Pid = process.id
		maiganduthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(maiganduthread)))
	}
