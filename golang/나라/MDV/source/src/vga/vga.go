...
var attributecontrollerkiyunport uint16 = 0x3c1
var attributecontrollerliyunport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Liyunregister(register []byte) {
	var regindex uint16 = 0

	Portliyunbyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portliyunbyte(sequencerindexport, i)
		Portliyunbyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portliyunbyte(crtcindexport, 0x03)

	Portliyunbyte(crtcdataport, (Portkiyunbyte(crtcdataport) | 0x80))
	Portliyunbyte(crtcindexport, 0x11)
	Portliyunbyte(crtcdataport, (Portkiyunbyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portliyunbyte(crtcindexport, i)
		Portliyunbyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portliyunbyte(graphicscontrollerindexport, i)
		Portliyunbyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portkiyunbyte(attributecontrollerresetport)
		Portliyunbyte(attributecontrollerindexport, i)
		Portliyunbyte(attributecontrollerliyunport, register[regindex])
		regindex++
	}

	Portkiyunbyte(attributecontrollerresetport)
	Portliyunbyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portliyunbyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portkiyunbyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Liyunregister(g320x200x256)
