...
ssize_t Читај(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Читај, fd, buf, count));
}

ssize_t Запиши(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Запиши, fd, buf, count));
}

int Затвори(int fd)
{
    return (int)__syscall_result(SC1(SYS_Затвори, fd));
}
