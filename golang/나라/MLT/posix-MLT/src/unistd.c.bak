...
ssize_t aqra(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_aqra, fd, buf, count));
}

ssize_t ikteb(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ikteb, fd, buf, count));
}
