...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portaqraword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portaqraword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portaqraword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portaqraword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portaqraword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portaqraword(Macaddress4port) / 256)
	...
	Portiktebword(registeraddressport, 20)
	Portiktebword(buscontrolregisterdataport, 0x102)

	Portiktebword(registeraddressport, 0)
	Portiktebword(registerdataport, 0x04)
	...
	Portiktebword(registeraddressport, 1)
	Portiktebword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portiktebword(registeraddressport, 2)
	Portiktebword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portiktebword(registeraddressport, 0)
	Portiktebword(registerdataport, 0x41)

	Portiktebword(registeraddressport, 4)
	temporary := Portaqraword(registerdataport)
	Portiktebword(registeraddressport, 4)
	Portiktebword(registerdataport, temporary|0xC00)

	Portiktebword(registeraddressport, 0)
	Portiktebword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portaqraword(resetport)
	Portiktebword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portiktebword(registeraddressport, 0)
	temporary := uint32(Portaqraword(registerdataport))
	...
	Portiktebword(registeraddressport, 0)
	Portiktebword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portiktebword(registeraddressport, 0)
	Portiktebword(registerdataport, 0x48)
