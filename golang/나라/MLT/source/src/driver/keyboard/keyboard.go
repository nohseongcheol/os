...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portaqrabyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portaqrabyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func iktebps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portiktebbyte(commandport_2, value)
	return true
}

func iktebps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portiktebbyte(dataport_2, value)
	return true
}

func aqraps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portaqrabyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portaqrabyte(commandport_2)&0x01) != 0; i++ {
		Portaqrabyte(dataport_2)
	}

	if !iktebps2command(0xAE) || !iktebps2command(0x20) {
		return
	}
	status, ok := aqraps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !iktebps2command(0x60) || !iktebps2data(status) {
		return
	}

	if !iktebps2data(0xF4) {
		return
	}
	ack, ok := aqraps2data()
	if !ok || ack != 0xFA {
...
var keyboardqueueaqra uint8
var keyboardqueueikteb uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueueikteb + 1) % keyboardqueuesize
	if next == keyboardqueueaqra {
		return
	}
	keyboardqueue[keyboardqueueikteb] = key
	keyboardqueueikteb = next
}

func Processpendingkeyboardevents() {
	for keyboardqueueaqra != keyboardqueueikteb {
		key := keyboardqueue[keyboardqueueaqra]
		keyboardqueueaqra = (keyboardqueueaqra + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portaqrabyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portaqrabyte(dataport_2)
	if scancode == 0xE0 {
