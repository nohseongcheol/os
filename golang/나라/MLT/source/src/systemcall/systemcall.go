...
var stdinaqra uint32
var stdinikteb uint32
...
func Sysaqra_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysaqra, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysikteb, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysikteb, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysaqra:
		cpu.Eax = uint32(sysaqra(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysikteb:
		cpu.Eax = uint32(sysikteb(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|oiktebonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: oaqraonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: oiktebonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: oiktebonly}
}
...
func sysikteb(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysaqra(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return aqrastdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return aqravfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == oiktebonly || accessmode == oaqraikteb || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func aqravfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	aqrafile(entry.name[:entry.namelen], tmp)
...
func aqrastdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinikteb + 1) % uint32(len(stdinbuffer))
	if next == stdinaqra {
		return
	}
	stdinbuffer[stdinikteb] = c
	stdinikteb = next
}

func stdingetblocking() byte {
	for stdinaqra == stdinikteb {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinaqra]
	stdinaqra = (stdinaqra + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portaqrabyte(0x64) & 0x01) == 0 {
	}
	sc := Portaqrabyte(0x60)
...
func aqrafile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Aqra(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
