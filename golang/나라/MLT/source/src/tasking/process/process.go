...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	ewlienithread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if ewlienithread != nil {
		ewlienithread.Pid = process.id
		ewlienithread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(ewlienithread)))
	}
