...
var attributecontrolleraqraport uint16 = 0x3c1
var attributecontrolleriktebport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Iktebregister(register []byte) {
	var regindex uint16 = 0

	Portiktebbyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portiktebbyte(sequencerindexport, i)
		Portiktebbyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portiktebbyte(crtcindexport, 0x03)

	Portiktebbyte(crtcdataport, (Portaqrabyte(crtcdataport) | 0x80))
	Portiktebbyte(crtcindexport, 0x11)
	Portiktebbyte(crtcdataport, (Portaqrabyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portiktebbyte(crtcindexport, i)
		Portiktebbyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portiktebbyte(graphicscontrollerindexport, i)
		Portiktebbyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portaqrabyte(attributecontrollerresetport)
		Portiktebbyte(attributecontrollerindexport, i)
		Portiktebbyte(attributecontrolleriktebport, register[regindex])
		regindex++
	}

	Portaqrabyte(attributecontrollerresetport)
	Portiktebbyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portiktebbyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portaqrabyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Iktebregister(g320x200x256)
