...
ssize_t hpatt(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_hpatt, fd, buf, count));
}

ssize_t yay(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_yay, fd, buf, count));
}
