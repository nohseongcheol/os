...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Porthpattword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Porthpattword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Porthpattword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Porthpattword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Porthpattword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Porthpattword(Macaddress4port) / 256)
	...
	Portyayword(registeraddressport, 20)
	Portyayword(buscontrolregisterdataport, 0x102)

	Portyayword(registeraddressport, 0)
	Portyayword(registerdataport, 0x04)
	...
	Portyayword(registeraddressport, 1)
	Portyayword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portyayword(registeraddressport, 2)
	Portyayword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portyayword(registeraddressport, 0)
	Portyayword(registerdataport, 0x41)

	Portyayword(registeraddressport, 4)
	temporary := Porthpattword(registerdataport)
	Portyayword(registeraddressport, 4)
	Portyayword(registerdataport, temporary|0xC00)

	Portyayword(registeraddressport, 0)
	Portyayword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Porthpattword(resetport)
	Portyayword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portyayword(registeraddressport, 0)
	temporary := uint32(Porthpattword(registerdataport))
	...
	Portyayword(registeraddressport, 0)
	Portyayword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portyayword(registeraddressport, 0)
	Portyayword(registerdataport, 0x48)
