...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Yay(0xA0)
	} else {
		self.deviceport.Yay(0xB0)
	}
	self.controlport.Yay(0)
	self.deviceport.Yay(0xA0)

	var status uint8 = self.commandport.Hpatt()
	if status == 0xFF {
	...
	self.sectorcountport.Yay(0)
	self.lbalowport.Yay(0)
	self.lbamidport.Yay(0)
	self.lbahiport.Yay(0)
	self.commandport.Yay(0xEC)

	status = self.commandport.Hpatt()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Hpatt28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Yay(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Yay(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Yay(0)
	self.sectorcountport.Yay(1)

	self.lbalowport.Yay(uint8(sector & 0x000000FF))
	self.lbamidport.Yay(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Yay(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Yay(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Hpatt()
...
func (self *TAdvancedtechnologyattachment) Yay28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Yay(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Yay(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Yay(0)
	self.sectorcountport.Yay(1)
	self.lbalowport.Yay(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Yay(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Yay(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Yay(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Yay(0xE0)
	} else {
		self.deviceport.Yay(0xF0)
	}
	self.commandport.Yay(0xE7)
