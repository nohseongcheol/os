...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Porthpattbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Porthpattbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func yayps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portyaybyte(commandport_2, value)
	return true
}

func yayps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portyaybyte(dataport_2, value)
	return true
}

func hpattps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Porthpattbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Porthpattbyte(commandport_2)&0x01) != 0; i++ {
		Porthpattbyte(dataport_2)
	}

	if !yayps2command(0xAE) || !yayps2command(0x20) {
		return
	}
	status, ok := hpattps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !yayps2command(0x60) || !yayps2data(status) {
		return
	}
...
var keyboardqueuehpatt uint8
var keyboardqueueyay uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueueyay + 1) % keyboardqueuesize
	if next == keyboardqueuehpatt {
		return
	}
	keyboardqueue[keyboardqueueyay] = key
	keyboardqueueyay = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuehpatt != keyboardqueueyay {
		key := keyboardqueue[keyboardqueuehpatt]
		keyboardqueuehpatt = (keyboardqueuehpatt + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Porthpattbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Porthpattbyte(dataport_2)
	if scancode == 0xE0 {
