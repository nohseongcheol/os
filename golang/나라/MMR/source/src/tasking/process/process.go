...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	ahtikathread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if ahtikathread != nil {
		ahtikathread.Pid = process.id
		ahtikathread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(ahtikathread)))
	}
