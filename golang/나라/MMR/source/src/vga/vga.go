...
var attributecontrollerhpattport uint16 = 0x3c1
var attributecontrolleryayport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Yayregister(register []byte) {
	var regindex uint16 = 0

	Portyaybyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portyaybyte(sequencerindexport, i)
		Portyaybyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portyaybyte(crtcindexport, 0x03)

	Portyaybyte(crtcdataport, (Porthpattbyte(crtcdataport) | 0x80))
	Portyaybyte(crtcindexport, 0x11)
	Portyaybyte(crtcdataport, (Porthpattbyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portyaybyte(crtcindexport, i)
		Portyaybyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portyaybyte(graphicscontrollerindexport, i)
		Portyaybyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Porthpattbyte(attributecontrollerresetport)
		Portyaybyte(attributecontrollerindexport, i)
		Portyaybyte(attributecontrolleryayport, register[regindex])
		regindex++
	}

	Porthpattbyte(attributecontrollerresetport)
	Portyaybyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portyaybyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Porthpattbyte(graphicscontrollerdataport) >> 2) & 0x03)
