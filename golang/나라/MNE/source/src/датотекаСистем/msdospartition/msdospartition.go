...
type TPartitionTabelaунос struct {
	bootable	uint8
...
func (isti *TPartitionTabelaунос) Init(data [16]byte) {
	...
	isti.Pokrenilba = Unsignedinteger32r(Низtounsignedinteger32(buffer1))

	var buffer2 [4]byte
	copy(buffer2[:4], data[12:16])
	isti.dužina = Unsignedinteger32r(Низtounsignedinteger32(buffer2))
}

type TGlavnibootСнимање struct {
	bootloader	[440]byte
	signature	uint32
	неупотребљен	uint16

	Primarypartition	[4]TPartitionTabelaунос

	magicnumber	uint16
}
type TmsdospartitionTabela struct {
	Mbr TGlavnibootСнимање
}

func (isti *TmsdospartitionTabela) Читањеpartition(hd *TНапредноТехнологијаattachment) {

	конзола_2 := TКонзола{}
	конзола_2.MŠtampaj(([]byte)("Reading MBR"))

	var partitionBajtova [512]byte
	var buffer_2 = partitionBajtova[:]
	hd.Читање28(0, &buffer_2, 512)

	isti.Mbr = TGlavnibootСнимање{}
	var i int = 0
	for ; i < 440; i++ {
	...
	isti.Mbr.signature = Unsignedinteger32r(Низtounsignedinteger32(buffer1))
	i += 4

	var buffer2 [2]byte
	copy(buffer2[:2], partitionBajtova[i:i+2])
	isti.Mbr.неупотребљен = Unsignedinteger16r(Низtounsignedinteger16(buffer2))
	i += 2
	...
	isti.Mbr.magicnumber = Unsignedinteger16r(Низtounsignedinteger16(buffer6))

	if isti.Mbr.magicnumber != 0xAA55 {
		конзола_2.MŠtampaj(([]byte)("illegal MBR"))
		return
	}
