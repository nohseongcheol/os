...
ssize_t Унших(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Унших, fd, buf, count));
}

ssize_t Бичих(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Бичих, fd, buf, count));
}

int Хаах(int fd)
{
    return (int)__syscall_result(SC1(SYS_Хаах, fd));
}
