...
var utilКонсол TКонсол = TКонсол{}
...
func ХэвлэхБайт(datapointer uintptr, хэмжээ int) {
	var buffer_2 [4 * 1024]byte = *(*([4 * 1024]byte))(Pointer(datapointer))
	var i int
	utilКонсол.MХэвлэх([]byte("util["))
	for i = 0; i < хэмжээ; i++ {
		utilКонсол.MHexadecimalХэвлэх(buffer_2[i])
	}
	utilКонсол.MХэвлэх([]byte("]"))
}

func Хэвлэхtest(params ...interface{}) {
	for _, param := range params {
		utilКонсол.MХэвлэх([]byte(TypeOf(param).Name()))
...
func БайтtoБИЧВЭР(b []byte) string {
...
func GetБайтfrompointer(pointer_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		address	uintptr
		len	int
		cap	int
