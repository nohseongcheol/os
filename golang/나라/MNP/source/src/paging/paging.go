...
func setcr3(page_directory uint32)
func getcr3() uint32
...
func (self *Tcowframemanager) Init(mem *TMemorymanager, framecount uint32) bool {
	...
	referencepointer := mem.Allocate_memory(referencebytes)
...
func Resolvecopyonwritefault() bool {
	...
	faultaddress := getcr2()
	page_directory := getcr3()
	pdeaddress := page_directory + ((faultaddress>>22)&0x3FF)*4
...
func Setbyteinpagedirectory(x byte, address uint32, page_directory uint32) {
	oldcr3 := getcr3()
...
func Setblockinpagedirectory(source_2 []byte, destination_2 []byte, size uint32, page_directory uint32) {
	if size == 0 || page_directory == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(page_directory)
	makerangeprivatewritablecurrent(page_directory, uint32(uintptr(unsafe.Pointer(&destination_2[0]))), size)
...
func Zeroblockinpagedirectory(address uint32, size uint32, page_directory uint32) {
	if size == 0 || page_directory == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(page_directory)
	makerangeprivatewritablecurrent(page_directory, address, size)
...
func makepageprivatewritablecurrent(page_directory uint32, virtualaddress uint32) bool {
	pde := Getvalue(page_directory + ((virtualaddress>>22)&0x3FF)*4)
	if (pde & Pagepresent) == 0 {
		return false
	}
...
func makerangeprivatewritablecurrent(page_directory uint32, address uint32, size uint32) bool {
	if size == 0 {
		return true
	}
	last := address + size - 1
	...
	for page := address & Pageframe; ; page += 0x1000 {
		if !makepageprivatewritablecurrent(page_directory, page) {
			return false
		}
...
func Makerangeprivatewritable(page_directory uint32, address uint32, size uint32) bool {
	if page_directory == 0 {
		return false
	}
	oldcr3 := getcr3()
	setcr3(page_directory)
	ok := makerangeprivatewritablecurrent(page_directory, address, size)
	setcr3(oldcr3)
	return ok
}

func Setunsignedinteger32inpagedirectory(x uint32, address uint32, page_directory uint32) {
	if page_directory == 0 {
		return
	}
	oldcr3 := getcr3()
...
func Getvalueinpagedirectory(address uint32, page_directory uint32) uint32 {
	if page_directory == 0 {
		return 0
	}
	oldcr3 := getcr3()
