...
freestanding library in `./ruang_pengguna/posix`.
...
```sh
cd ./ruang_pengguna/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./ruang_pengguna/posix/pengisytiharan` and link the archive into an EngOS i386 user
...
`./ruang_pengguna/posix/TEST_RESULTS.md`. Build all runtime test executables
