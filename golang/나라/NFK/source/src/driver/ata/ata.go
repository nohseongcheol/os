...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Write(0xA0)
	} else {
		self.deviceport.Write(0xB0)
	}
	self.controlport.Write(0)
	self.deviceport.Write(0xA0)

	var status uint8 = self.commandport.Read()
	if status == 0xFF {
	...
	self.sectorcountport.Write(0)
	self.lbalowport.Write(0)
	self.lbamidport.Write(0)
	self.lbahiport.Write(0)
	self.commandport.Write(0xEC)

	status = self.commandport.Read()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.deviceport.Write(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Write(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Write(0)
	self.sectorcountport.Write(1)

	self.lbalowport.Write(uint8(sector & 0x000000FF))
	self.lbamidport.Write(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Write(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Write(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Read()
...
func (self *TAdvancedtechnologyattachment) Write28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Write(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Write(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Write(0)
	self.sectorcountport.Write(1)
	self.lbalowport.Write(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Write(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Write(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Write(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Write(0xE0)
	} else {
		self.deviceport.Write(0xF0)
	}
	self.commandport.Write(0xE7)
