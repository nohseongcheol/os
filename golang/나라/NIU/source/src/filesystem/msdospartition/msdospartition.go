...
type TMaster_boot_record struct {
	bootloader	[440]byte
	signature	uint32
	unused		uint16
...
type Tmsdospartitiontable struct {
	Mbr TMaster_boot_record
}

func (self *Tmsdospartitiontable) Readpartition(hd *TAdvancedtechnologyattachment) {
	...
	hd.Read28(0, &buffer_2, 512)

	self.Mbr = TMaster_boot_record{}
	var i int = 0
	for ; i < 440; i++ {
