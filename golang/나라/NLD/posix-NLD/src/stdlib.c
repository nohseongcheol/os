...
int abs(int waarde) { return waarde < 0 ? -waarde : waarde; }
long labs(long waarde) { return waarde < 0 ? -waarde : waarde; }
...
char *getenv(const char *systeemidentiteit)
{
    size_t lengte = strlen(systeemidentiteit);
    char **cursor = environ;
    if (!lengte || strchr(systeemidentiteit, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, systeemidentiteit, lengte) && (*cursor)[lengte] == '=') return *cursor + lengte + 1;
        ++cursor;
    }
    return NULL;
}
