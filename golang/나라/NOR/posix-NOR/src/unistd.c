...
ssize_t Les(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Les, fd, buf, count));
}

ssize_t Skriv(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Skriv, fd, buf, count));
}

int Lukk(int fd)
{
    return (int)__syscall_result(SC1(SYS_Lukk, fd));
}
