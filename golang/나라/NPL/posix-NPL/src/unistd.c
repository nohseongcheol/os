...
ssize_t padhnu(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_padhnu, fd, buf, count));
}

ssize_t lekhnu(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_lekhnu, fd, buf, count));
}
