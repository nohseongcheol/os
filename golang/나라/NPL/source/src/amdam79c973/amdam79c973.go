...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portpadhnuword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portpadhnuword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portpadhnuword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portpadhnuword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portpadhnuword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portpadhnuword(Macaddress4port) / 256)
	...
	Portlekhnuword(registeraddressport, 20)
	Portlekhnuword(buscontrolregisterdataport, 0x102)

	Portlekhnuword(registeraddressport, 0)
	Portlekhnuword(registerdataport, 0x04)
	...
	Portlekhnuword(registeraddressport, 1)
	Portlekhnuword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portlekhnuword(registeraddressport, 2)
	Portlekhnuword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portlekhnuword(registeraddressport, 0)
	Portlekhnuword(registerdataport, 0x41)

	Portlekhnuword(registeraddressport, 4)
	temporary := Portpadhnuword(registerdataport)
	Portlekhnuword(registeraddressport, 4)
	Portlekhnuword(registerdataport, temporary|0xC00)

	Portlekhnuword(registeraddressport, 0)
	Portlekhnuword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portpadhnuword(resetport)
	Portlekhnuword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portlekhnuword(registeraddressport, 0)
	temporary := uint32(Portpadhnuword(registerdataport))
	...
	Portlekhnuword(registeraddressport, 0)
	Portlekhnuword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portlekhnuword(registeraddressport, 0)
	Portlekhnuword(registerdataport, 0x48)
