...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Lekhnu(0xA0)
	} else {
		self.deviceport.Lekhnu(0xB0)
	}
	self.controlport.Lekhnu(0)
	self.deviceport.Lekhnu(0xA0)

	var status uint8 = self.commandport.Padhnu()
	if status == 0xFF {
	...
	self.sectorcountport.Lekhnu(0)
	self.lbalowport.Lekhnu(0)
	self.lbamidport.Lekhnu(0)
	self.lbahiport.Lekhnu(0)
	self.commandport.Lekhnu(0xEC)

	status = self.commandport.Padhnu()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Padhnu28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Lekhnu(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Lekhnu(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Lekhnu(0)
	self.sectorcountport.Lekhnu(1)

	self.lbalowport.Lekhnu(uint8(sector & 0x000000FF))
	self.lbamidport.Lekhnu(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Lekhnu(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Lekhnu(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Padhnu()
	...
	for i := (count + (count % 2)); i < Bytespersector; i += 2 {
		self.dataport.Padhnu()
	}
}
func (self *TAdvancedtechnologyattachment) Lekhnu28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Lekhnu(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Lekhnu(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Lekhnu(0)
	self.sectorcountport.Lekhnu(1)
	self.lbalowport.Lekhnu(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Lekhnu(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Lekhnu(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Lekhnu(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Lekhnu(0xE0)
	} else {
		self.deviceport.Lekhnu(0xF0)
	}
	self.commandport.Lekhnu(0xE7)
