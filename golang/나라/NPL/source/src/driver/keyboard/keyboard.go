...
func waitps2आगतempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portpadhnubyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portpadhnubyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func lekhnups2command(value uint8) bool {
	if !waitps2आगतempty() {
		return false
	}
	Portlekhnubyte(commandport_2, value)
	return true
}

func lekhnups2data(value uint8) bool {
	if !waitps2आगतempty() {
		return false
	}
	Portlekhnubyte(dataport_2, value)
	return true
}

func padhnups2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portpadhnubyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portpadhnubyte(commandport_2)&0x01) != 0; i++ {
		Portpadhnubyte(dataport_2)
	}

	if !lekhnups2command(0xAE) || !lekhnups2command(0x20) {
		return
	}
	status, ok := padhnups2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !lekhnups2command(0x60) || !lekhnups2data(status) {
		return
	}

	if !lekhnups2data(0xF4) {
		return
	}
	ack, ok := padhnups2data()
	if !ok || ack != 0xFA {
		return
	}
...
var keyboardqueuepadhnu uint8
var keyboardqueuelekhnu uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuelekhnu + 1) % keyboardqueuesize
	if next == keyboardqueuepadhnu {
		return
	}
	keyboardqueue[keyboardqueuelekhnu] = key
	keyboardqueuelekhnu = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuepadhnu != keyboardqueuelekhnu {
		key := keyboardqueue[keyboardqueuepadhnu]
		keyboardqueuepadhnu = (keyboardqueuepadhnu + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portpadhnubyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portpadhnubyte(dataport_2)
	if scancode == 0xE0 {
