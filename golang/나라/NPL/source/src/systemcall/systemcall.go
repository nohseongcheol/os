...
const (
	...
	opadhnuonly	uint32	= 0
	olekhnuonly	uint32	= 1
	opadhnulekhnu	uint32	= 2
	ocreate		uint32	= 0x40
...
var stdinpadhnu uint32
var stdinlekhnu uint32
...
func Syspadhnu_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Syspadhnu, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Syslekhnu, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Syslekhnu, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Syspadhnu:
		cpu.Eax = uint32(syspadhnu(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Syslekhnu:
		cpu.Eax = uint32(syslekhnu(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|olekhnuonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: opadhnuonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: olekhnuonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: olekhnuonly}
}
...
func syslekhnu(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func syspadhnu(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return padhnustdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return padhnuvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == olekhnuonly || accessmode == opadhnulekhnu || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func padhnuvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	padhnufile(entry.name[:entry.namelen], tmp)
...
func padhnustdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinlekhnu + 1) % uint32(len(stdinbuffer))
	if next == stdinpadhnu {
		return
	}
	stdinbuffer[stdinlekhnu] = c
	stdinlekhnu = next
}

func stdingetblocking() byte {
	for stdinpadhnu == stdinlekhnu {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinpadhnu]
	stdinpadhnu = (stdinpadhnu + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portpadhnubyte(0x64) & 0x01) == 0 {
	}
	sc := Portpadhnubyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Padhnupartition(&ata0s)
...
func padhnufile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Padhnu(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
