...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	mukhyathread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if mukhyathread != nil {
		mukhyathread.Pid = process.id
		mukhyathread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(mukhyathread)))
	}
