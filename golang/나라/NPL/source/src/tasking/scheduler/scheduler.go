...
func (self *Scheduler) Aथपthread(thread *TThread) {
	if thread == nil {
		return
	}
	list.Append_to_list(uintptr(Pointer(thread)))
}
func Aथपrunnablethread(thread *TThread) {
	if thread == nil {
		return
	}
	list.Append_to_list(uintptr(Pointer(thread)))
...
func initpit(frequency uint32) {
	...
	Portlekhnubyte(0x43, 0x36)
	Portlekhnubyte(0x40, uint8(divisor&0xFF))
	Portlekhnubyte(0x40, uint8((divisor>>8)&0xFF))
}
