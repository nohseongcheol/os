...
var attributecontrollerpadhnuport uint16 = 0x3c1
var attributecontrollerlekhnuport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Lekhnuregister(register []byte) {
	var regindex uint16 = 0

	Portlekhnubyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portlekhnubyte(sequencerindexport, i)
		Portlekhnubyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portlekhnubyte(crtcindexport, 0x03)

	Portlekhnubyte(crtcdataport, (Portpadhnubyte(crtcdataport) | 0x80))
	Portlekhnubyte(crtcindexport, 0x11)
	Portlekhnubyte(crtcdataport, (Portpadhnubyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portlekhnubyte(crtcindexport, i)
		Portlekhnubyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portlekhnubyte(graphicscontrollerindexport, i)
		Portlekhnubyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portpadhnubyte(attributecontrollerresetport)
		Portlekhnubyte(attributecontrollerindexport, i)
		Portlekhnubyte(attributecontrollerlekhnuport, register[regindex])
		regindex++
	}

	Portpadhnubyte(attributecontrollerresetport)
	Portlekhnubyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portlekhnubyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portpadhnubyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Lekhnuregister(g320x200x256)
