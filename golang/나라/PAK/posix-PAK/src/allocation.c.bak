...
void *malloc(size_t ہندسوں_کی_تعداد)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!ہندسوں_کی_تعداد) ہندسوں_کی_تعداد = 1;
    if (ہندسوں_کی_تعداد > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    ہندسوں_کی_تعداد = (ہندسوں_کی_تعداد + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= ہندسوں_کی_تعداد) {
            if (cursor->capacity - ہندسوں_کی_تعداد >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + ہندسوں_کی_تعداد);
                raw->capacity = cursor->capacity - ہندسوں_کی_تعداد - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = ہندسوں_کی_تعداد;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + ہندسوں_کی_تعداد;
    ...
    raw->capacity = ہندسوں_کی_تعداد; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t ہندسوں_کی_تعداد)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(ہندسوں_کی_تعداد);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!ہندسوں_کی_تعداد) ہندسوں_کی_تعداد = 1;
    cursor = (struct heap_block *)address - 1;
    if (ہندسوں_کی_تعداد <= cursor->capacity) return address;
    destination = malloc(ہندسوں_کی_تعداد);
