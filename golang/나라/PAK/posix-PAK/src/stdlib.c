...
int abs(int قدر) { return قدر < 0 ? -قدر : قدر; }
long labs(long قدر) { return قدر < 0 ? -قدر : قدر; }
...
char *getenv(const char *نظام_کی_شناخت)
{
    size_t لمبائی = strlen(نظام_کی_شناخت);
    char **cursor = environ;
    if (!لمبائی || strchr(نظام_کی_شناخت, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, نظام_کی_شناخت, لمبائی) && (*cursor)[لمبائی] == '=') return *cursor + لمبائی + 1;
        ++cursor;
    }
    return NULL;
}
