...
{
    (void)لکھیں(STDOUT_FILENO, text, text_length(text));
}

static void report(const char *نظام_کی_شناخت, int passed)
{
    checks++;
    if (passed) {
        say("PTEST:PASS:");
...
static pid_t reap_nohang(pid_t child, int *حالت)
{
    unsigned int spins;
    pid_t result;

    for (spins = 0; spins < 2000000U; spins++) {
        result = waitpid(child, حالت, WNOHANG);
        if (result != 0)
            return result;
    }
    return 0;
...
static void test_open_and_io(void)
{
    ...
    فائل_کا_وصف_کنندہ = کھولیں("/USER2", O_RDONLY);
    report("open-readonly", فائل_کا_وصف_کنندہ >= 3);
    if (فائل_کا_وصف_کنندہ >= 0) {
        report("read-bytes", پڑھیں(فائل_کا_وصف_کنندہ, bytes, 2) == 2 &&
               (unsigned char)bytes[0] == 0x7f && bytes[1] == 'E');
        report("lseek-set", lseek(فائل_کا_وصف_کنندہ, 0, SEEK_SET) == 0);
        report("lseek-cur", lseek(فائل_کا_وصف_کنندہ, 1, SEEK_CUR) == 1);
        report("lseek-end", lseek(فائل_کا_وصف_کنندہ, -1, SEEK_END) > 0);
        errno = 0;
        report("lseek-negative", lseek(فائل_کا_وصف_کنندہ, -1, SEEK_SET) == -1 && errno == EINVAL);
        errno = 0;
        report("lseek-whence", lseek(فائل_کا_وصف_کنندہ, 0, 99) == -1 && errno == EINVAL);
        report("fstat-file", fstat(فائل_کا_وصف_کنندہ, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 2);
        errno = 0;
        report("fstat-efault", failed_with(fstat(فائل_کا_وصف_کنندہ, (struct stat *)0), EFAULT));
        report("fsync-file", fsync(فائل_کا_وصف_کنندہ) == 0);
        report("close-file", بندکریں(فائل_کا_وصف_کنندہ) == 0);
        errno = 0;
        report("closed-fd", failed_with(بندکریں(فائل_کا_وصف_کنندہ), EBADF));
    }

    root = کھولیں("/", O_RDONLY | O_DIRECTORY);
    report("open-directory", root >= 3);
    if (root >= 0) {
        report("fstat-directory", fstat(root, &st) == 0 && S_ISDIR(st.st_mode));
        errno = 0;
        report("read-directory", پڑھیں(root, bytes, 1) == -1 && errno == EISDIR);
        (void)بندکریں(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", لکھیں(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", پڑھیں(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", پڑھیں(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", لکھیں(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("open-enoent", failed_with(کھولیں("/NOFILE", O_RDONLY), ENOENT));
    errno = 0;
    report("open-efault", failed_with(کھولیں((const char *)0, O_RDONLY), EFAULT));
    errno = 0;
    report("open-write-erofs", failed_with(کھولیں("/USER2", O_WRONLY), EROFS));
    errno = 0;
    report("open-rdwr-erofs", failed_with(کھولیں("/USER2", O_RDWR), EROFS));
    errno = 0;
    report("open-create-erofs", failed_with(کھولیں("/NEWFILE", O_CREAT | O_WRONLY, 0600), EROFS));
    errno = 0;
    report("open-trunc-erofs", failed_with(کھولیں("/USER2", O_TRUNC | O_RDONLY), EROFS));
    errno = 0;
    report("open-append-erofs", failed_with(کھولیں("/USER2", O_APPEND | O_RDONLY), EROFS));
    errno = 0;
    report("open-enotdir", failed_with(کھولیں("/USER2", O_RDONLY | O_DIRECTORY), ENOTDIR));
    errno = 0;
    report("creat-erofs", failed_with(creat("/NEWFILE", 0600), EROFS));
    errno = 0;
    report("close-ebadf", failed_with(بندکریں(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    copy = dup(فائل_کا_وصف_کنندہ);
    report("dup", copy >= 0 && copy != فائل_کا_وصف_کنندہ);
    if (copy >= 0) {
        report("dup-shared-offset", پڑھیں(فائل_کا_وصف_کنندہ, &first, 1) == 1 && پڑھیں(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", بندکریں(فائل_کا_وصف_کنندہ) == 0 && fstat(copy, &st) == 0);
        فائل_کا_وصف_کنندہ = copy;
    }

    target = dup2(فائل_کا_وصف_کنندہ, 20);
    report("dup2", target == 20 && fstat(20, &st) == 0);
    report("dup2-same", dup2(فائل_کا_وصف_کنندہ, فائل_کا_وصف_کنندہ) == فائل_کا_وصف_کنندہ);
    if (target == 20)
        (void)بندکریں(20);
    errno = 0;
    report("dup2-old-ebadf", failed_with(dup2(-1, 10), EBADF));
    errno = 0;
    report("dup2-new-ebadf", failed_with(dup2(فائل_کا_وصف_کنندہ, 99), EBADF));

    report("fcntl-getfd", fcntl(فائل_کا_وصف_کنندہ, F_GETFD) == 0);
    report("fcntl-setfd", fcntl(فائل_کا_وصف_کنندہ, F_SETFD, FD_CLOEXEC) == 0 &&
           fcntl(فائل_کا_وصف_کنندہ, F_GETFD) == FD_CLOEXEC);
    high = fcntl(فائل_کا_وصف_کنندہ, F_DUPFD, 10);
    report("fcntl-dupfd", high >= 10);
    if (high >= 0) {
        report("fcntl-fd-flags-independent", fcntl(high, F_GETFD) == 0);
        (void)بندکریں(high);
    }
    report("fcntl-getfl", (fcntl(فائل_کا_وصف_کنندہ, F_GETFL) & O_ACCMODE) == O_RDONLY);
    report("fcntl-setfl", fcntl(فائل_کا_وصف_کنندہ, F_SETFL, O_APPEND) == 0 &&
           (fcntl(فائل_کا_وصف_کنندہ, F_GETFL) & O_APPEND) != 0);
    errno = 0;
    report("fcntl-command-einval", failed_with(fcntl(فائل_کا_وصف_کنندہ, 999), EINVAL));
    errno = 0;
    report("fcntl-fd-ebadf", failed_with(fcntl(-1, F_GETFD), EBADF));
    errno = 0;
    report("dup-ebadf", failed_with(dup(-1), EBADF));
    (void)بندکریں(فائل_کا_وصف_کنندہ);
}
...
static void test_terminal_and_uname(void)
{
    ...
    if (فائل_کا_وصف_کنندہ >= 0) {
        errno = 0;
        report("isatty-enotty", isatty(فائل_کا_وصف_کنندہ) == 0 && errno == ENOTTY);
        (void)بندکریں(فائل_کا_وصف_کنندہ);
    } else {
    ...
    report("uname", uname(&نظام_کی_شناخت) == 0 && نظام_کی_شناخت.sysname[0] != 0 &&
           نظام_کی_شناخت.nodename[0] != 0 && text_equal(نظام_کی_شناخت.machine, "i386"));
    errno = 0;
...
void posix_test_process(void)
{
    ...
    report("fork", child > 0);
    if (child > 0) {
        waited = reap_nohang(child, &حالت);
        report("waitpid", waited == child && WIFEXITED(حالت) && WEXITSTATUS(حالت) == 23);
        errno = 0;
        report("waitpid-echild", waitpid(child, &حالت, WNOHANG) == -1 && errno == ECHILD);
    }
    ...
    if (child > 0 && reap_nohang(child, &حالت) == child) {
        report("fork-fd-isolation", پڑھیں(فائل_کا_وصف_کنندہ, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
    ...
    if (child > 0) {
        waited = reap_nohang(child, &حالت);
        report("execve", waited == child && WIFEXITED(حالت) && WEXITSTATUS(حالت) == 37);
    }
    ...
    if (child > 0) {
        ...
        for (spin = 0; spin < 2000000U; spin++)
            (void)getpid();
        waited = wait(&حالت);
        report("wait", waited == child && WIFEXITED(حالت) && WEXITSTATUS(حالت) == 29);
    }
}
