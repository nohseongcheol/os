...
static void say(const char *s) { (void)لکھیں(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *نظام_کی_شناخت, int pass)
{
...
int main(void)
{
    ...
    char منتقلی_کا_عارضی_ذخیرہ_2[8] = {0};
    ...
    report("recvfrom", recvfrom(server, منتقلی_کا_عارضی_ذخیرہ_2, sizeof(منتقلی_کا_عارضی_ذخیرہ_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       منتقلی_کا_عارضی_ذخیرہ_2[0] == 'p' && منتقلی_کا_عارضی_ذخیرہ_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, منتقلی_کا_عارضی_ذخیرہ_2, sizeof(منتقلی_کا_عارضی_ذخیرہ_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", بندکریں(client) == 0);
    report("close-server", بندکریں(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
