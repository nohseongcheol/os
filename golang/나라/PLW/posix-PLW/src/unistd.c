...
ssize_t parhna(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_parhna, fd, buf, count));
}

ssize_t likhna(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_likhna, fd, buf, count));
}
