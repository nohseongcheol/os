...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Likhna(0xA0)
	} else {
		self.deviceport.Likhna(0xB0)
	}
	self.controlport.Likhna(0)
	self.deviceport.Likhna(0xA0)

	var status uint8 = self.commandport.Parhna()
	if status == 0xFF {
	...
	status = self.commandport.Parhna()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Parhna28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Likhna(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Likhna(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Parhna()
...
func (self *TAdvancedtechnologyattachment) Likhna28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Likhna(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Likhna(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Likhna(0xE0)
	} else {
		self.deviceport.Likhna(0xF0)
	}
