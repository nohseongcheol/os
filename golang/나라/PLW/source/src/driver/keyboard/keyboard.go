...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portparhnabyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portparhnabyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func parhnaps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portparhnabyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portparhnabyte(commandport_2)&0x01) != 0; i++ {
		Portparhnabyte(dataport_2)
	}

	if !likhnaps2command(0xAE) || !likhnaps2command(0x20) {
		return
	}
	status, ok := parhnaps2data()
	if !ok {
		return
	}
	status |= 0x01
...
func Processpendingkeyboardevents() {
	for keyboardqueueparhna != keyboardqueuelikhna {
		key := keyboardqueue[keyboardqueueparhna]
		keyboardqueueparhna = (keyboardqueueparhna + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portparhnabyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portparhnabyte(dataport_2)
	if scancode == 0xE0 {
