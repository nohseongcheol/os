...
const (
	...
	oparhnaonly	uint32	= 0
	olikhnaonly	uint32	= 1
	oparhnalikhna	uint32	= 2
	ocreate		uint32	= 0x40
...
func Sysparhna_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysparhna, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysparhna:
		cpu.Eax = uint32(sysparhna(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Syslikhna:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: oparhnaonly}
...
func sysparhna(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return parhnastdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return parhnavfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == olikhnaonly || accessmode == oparhnalikhna || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func parhnavfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	parhnafile(entry.name[:entry.namelen], tmp)
...
func parhnastdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func stdingetblocking() byte {
	for stdinparhna == stdinlikhna {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinparhna]
	stdinparhna = (stdinparhna + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portparhnabyte(0x64) & 0x01) == 0 {
	}
	sc := Portparhnabyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Parhnapartition(&ata0s)
...
func parhnafile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Parhna(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
