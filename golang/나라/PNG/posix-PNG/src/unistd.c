...
ssize_t ritim(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ritim, fd, buf, count));
}

ssize_t raitim(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_raitim, fd, buf, count));
}
