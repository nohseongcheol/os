...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portritimword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portritimword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portritimword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portritimword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portritimword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portritimword(Macaddress4port) / 256)
	...
	Portraitimword(registeraddressport, 20)
	Portraitimword(buscontrolregisterdataport, 0x102)

	Portraitimword(registeraddressport, 0)
	Portraitimword(registerdataport, 0x04)
	...
	Portraitimword(registeraddressport, 1)
	Portraitimword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portraitimword(registeraddressport, 2)
	Portraitimword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portraitimword(registeraddressport, 0)
	Portraitimword(registerdataport, 0x41)

	Portraitimword(registeraddressport, 4)
	temporary := Portritimword(registerdataport)
	Portraitimword(registeraddressport, 4)
	Portraitimword(registerdataport, temporary|0xC00)

	Portraitimword(registeraddressport, 0)
	Portraitimword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portritimword(resetport)
	Portraitimword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portraitimword(registeraddressport, 0)
	temporary := uint32(Portritimword(registerdataport))
	...
	Portraitimword(registeraddressport, 0)
	Portraitimword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portraitimword(registeraddressport, 0)
	Portraitimword(registerdataport, 0x48)
