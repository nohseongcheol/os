...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Raitim(0xA0)
	} else {
		self.deviceport.Raitim(0xB0)
	}
	self.controlport.Raitim(0)
	self.deviceport.Raitim(0xA0)

	var status uint8 = self.commandport.Ritim()
	if status == 0xFF {
	...
	self.sectorcountport.Raitim(0)
	self.lbalowport.Raitim(0)
	self.lbamidport.Raitim(0)
	self.lbahiport.Raitim(0)
	self.commandport.Raitim(0xEC)

	status = self.commandport.Ritim()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Ritim28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Raitim(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Raitim(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Raitim(0)
	self.sectorcountport.Raitim(1)

	self.lbalowport.Raitim(uint8(sector & 0x000000FF))
	self.lbamidport.Raitim(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Raitim(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Raitim(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Ritim()
...
func (self *TAdvancedtechnologyattachment) Raitim28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Raitim(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Raitim(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Raitim(0)
	self.sectorcountport.Raitim(1)
	self.lbalowport.Raitim(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Raitim(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Raitim(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Raitim(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Raitim(0xE0)
	} else {
		self.deviceport.Raitim(0xF0)
	}
	self.commandport.Raitim(0xE7)
