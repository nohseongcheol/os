...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portritimbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portritimbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func raitimps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portraitimbyte(commandport_2, value)
	return true
}

func raitimps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portraitimbyte(dataport_2, value)
	return true
}

func ritimps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portritimbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portritimbyte(commandport_2)&0x01) != 0; i++ {
		Portritimbyte(dataport_2)
	}

	if !raitimps2command(0xAE) || !raitimps2command(0x20) {
		return
	}
	status, ok := ritimps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !raitimps2command(0x60) || !raitimps2data(status) {
		return
	}

	if !raitimps2data(0xF4) {
		return
	}
	ack, ok := ritimps2data()
	if !ok || ack != 0xFA {
...
var keyboardqueueritim uint8
var keyboardqueueraitim uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueueraitim + 1) % keyboardqueuesize
	if next == keyboardqueueritim {
		return
	}
	keyboardqueue[keyboardqueueraitim] = key
	keyboardqueueraitim = next
}

func Processpendingkeyboardevents() {
	for keyboardqueueritim != keyboardqueueraitim {
		key := keyboardqueue[keyboardqueueritim]
		keyboardqueueritim = (keyboardqueueritim + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portritimbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portritimbyte(dataport_2)
	if scancode == 0xE0 {
