...
func (self *TBiosparameterblock32) Len(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte) uint32 {
	...
	hd.Ritim28(partitionoffset, &bpbbytes, 90)
	...
	hd.Ritim28(rootstart, &direntbytes, 512)
...
func (self *TBiosparameterblock32) Ritim(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte, data []byte) {
		...
		for Size > 0 {
			...
			for ; Size > 0; Size -= 512 {
				...
				if dirent[i].size > 512 {
					...
					hd.Ritim28(filesector+uint32(sectoroffset), &buffer3, 512)

				} else {
					buffer3 = buffer_2[:dirent[i].size]
					hd.Ritim28(filesector+uint32(sectoroffset), &buffer3, int(dirent[i].size))
				}
			...
			hd.Ritim28(fatstart+fatsectorforcurrentcluster, &fatbuf, 512)
