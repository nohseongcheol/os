...
const (
	...
	oritimonly	uint32	= 0
	oraitimonly	uint32	= 1
	oritimraitim	uint32	= 2
	ocreate		uint32	= 0x40
...
var stdinritim uint32
var stdinraitim uint32
...
func Sysritim_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysritim, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysraitim, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysraitim, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysritim:
		cpu.Eax = uint32(sysritim(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysraitim:
		cpu.Eax = uint32(sysraitim(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|oraitimonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: oritimonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: oraitimonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: oraitimonly}
}
...
func sysraitim(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysritim(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return ritimstdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return ritimvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == oraitimonly || accessmode == oritimraitim || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func ritimvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	ritimfile(entry.name[:entry.namelen], tmp)
...
func ritimstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinraitim + 1) % uint32(len(stdinbuffer))
	if next == stdinritim {
		return
	}
	stdinbuffer[stdinraitim] = c
	stdinraitim = next
}

func stdingetblocking() byte {
	for stdinritim == stdinraitim {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinritim]
	stdinritim = (stdinritim + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portritimbyte(0x64) & 0x01) == 0 {
	}
	sc := Portritimbyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Ritimpartition(&ata0s)
...
func ritimfile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Ritim(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
