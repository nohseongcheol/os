...
var attributecontrollerritimport uint16 = 0x3c1
var attributecontrollerraitimport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Raitimregister(register []byte) {
	var regindex uint16 = 0

	Portraitimbyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portraitimbyte(sequencerindexport, i)
		Portraitimbyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portraitimbyte(crtcindexport, 0x03)

	Portraitimbyte(crtcdataport, (Portritimbyte(crtcdataport) | 0x80))
	Portraitimbyte(crtcindexport, 0x11)
	Portraitimbyte(crtcdataport, (Portritimbyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portraitimbyte(crtcindexport, i)
		Portraitimbyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portraitimbyte(graphicscontrollerindexport, i)
		Portraitimbyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portritimbyte(attributecontrollerresetport)
		Portraitimbyte(attributecontrollerindexport, i)
		Portraitimbyte(attributecontrollerraitimport, register[regindex])
		regindex++
	}

	Portritimbyte(attributecontrollerresetport)
	Portraitimbyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portraitimbyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portritimbyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Raitimregister(g320x200x256)
