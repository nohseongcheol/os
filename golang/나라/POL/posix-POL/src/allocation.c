...
void *malloc(size_t liczba_cyfr)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!liczba_cyfr) liczba_cyfr = 1;
    if (liczba_cyfr > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    liczba_cyfr = (liczba_cyfr + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= liczba_cyfr) {
            if (cursor->capacity - liczba_cyfr >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + liczba_cyfr);
                raw->capacity = cursor->capacity - liczba_cyfr - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = liczba_cyfr;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + liczba_cyfr;
    ...
    raw->capacity = liczba_cyfr; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t liczba_cyfr)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(liczba_cyfr);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!liczba_cyfr) liczba_cyfr = 1;
    cursor = (struct heap_block *)address - 1;
    if (liczba_cyfr <= cursor->capacity) return address;
    destination = malloc(liczba_cyfr);
