...
int abs(int wartość) { return wartość < 0 ? -wartość : wartość; }
long labs(long wartość) { return wartość < 0 ? -wartość : wartość; }
...
char *getenv(const char *tożsamość_systemu)
{
    size_t długość = strlen(tożsamość_systemu);
    char **cursor = environ;
    if (!długość || strchr(tożsamość_systemu, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, tożsamość_systemu, długość) && (*cursor)[długość] == '=') return *cursor + długość + 1;
        ++cursor;
    }
    return NULL;
}
