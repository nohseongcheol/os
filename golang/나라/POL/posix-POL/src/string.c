...
void *memcpy(void *destination, const void *source, size_t długość)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    while (długość--) *cursor++ = *input++;
    return destination;
}

void *memmove(void *destination, const void *source, size_t długość)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    if ((uintptr_t)destination <= (uintptr_t)source)
        return memcpy(destination, source, długość);
    while (długość) { --długość; cursor[długość] = input[długość]; }
    return destination;
}

void *memset(void *destination, int character, size_t długość)
{
    unsigned char *cursor = destination;
    while (długość--) *cursor++ = (unsigned char)character;
    return destination;
}

int memcmp(const void *left, const void *right, size_t długość)
{
...
void *memchr(const void *source, int character, size_t długość)
{
...
{
    size_t długość = 0;
    while (długość < limit && text[długość]) ++długość;
    return długość;
}
...
{
    size_t długość = strnlen(source, limit);
    memcpy(destination, source, długość);
    memset(destination + długość, 0, limit - długość);
    return destination + długość;
}
...
{
    size_t długość = strlen(source);
    if (limit) {
        size_t copy_length = długość < limit ? długość : limit;
        memcpy(destination, source, copy_length);
        if (długość < limit) destination[długość] = 0;
    }
    return długość;
}
...
{
    size_t długość = strlen(source);
    if (!długość) return (char *)text;
    while (*text) {
        if (strncmp(text, source, długość) == 0) return (char *)text;
        ++text;
    }
    return NULL;
}
...
{
    size_t długość = 0;
    while (text[długość] && strchr(delimiters, text[długość])) ++długość;
    return długość;
}
...
{
    size_t długość = strnlen(text, limit);
    char *destination = malloc(długość + 1);
    if (destination) { memcpy(destination, text, długość); destination[długość] = 0; }
    return destination;
}
