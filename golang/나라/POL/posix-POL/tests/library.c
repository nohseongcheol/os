...
#define CHECK(wartość) do { if (!(wartość)) return __LINE__; } while (0)

static int compare_int(const void *left, const void *right)
{
    int result = *(const int *)left, wartość = *(const int *)right;
    return (result > wartość) - (result < wartość);
}

int __posix_library_test(void)
{
    char bufor_przesyłania_2[64], source[64], *end_pointer, *saved_state, *address, *destination;
    const char *text = "abca", *input = " 	-0x80rest";
    int elements[257], index, wartość;
    size_t długość;
    div_t result;
    ldiv_t decoded;
    ...
    CHECK(memset(bufor_przesyłania_2, 0xab, sizeof(bufor_przesyłania_2)) == bufor_przesyłania_2);
    CHECK((unsigned char)bufor_przesyłania_2[63] == 0xab);
    CHECK(memcpy(bufor_przesyłania_2, "abcdef", 7) == bufor_przesyłania_2);
    CHECK(memmove(bufor_przesyłania_2 + 2, bufor_przesyłania_2, 5) == bufor_przesyłania_2 + 2);
    CHECK(memcmp(bufor_przesyłania_2, "ababcde", 7) == 0);
    CHECK(memmove(bufor_przesyłania_2, bufor_przesyłania_2 + 2, 5) == bufor_przesyłania_2);
    CHECK(memcmp(bufor_przesyłania_2, "abcde", 5) == 0);
    CHECK(memmove(bufor_przesyłania_2, bufor_przesyłania_2, 0) == bufor_przesyłania_2);
    CHECK(memcmp("\x80", "\x7f", 1) > 0);
    CHECK(memcmp(bufor_przesyłania_2, source, 0) == 0);
    ...
    CHECK(strcpy(bufor_przesyłania_2, text) == bufor_przesyłania_2 && strcmp(bufor_przesyłania_2, text) == 0);
    CHECK(stpcpy(bufor_przesyłania_2, "xy") == bufor_przesyłania_2 + 2 && bufor_przesyłania_2[2] == 0);
    memset(bufor_przesyłania_2, 7, sizeof(bufor_przesyłania_2));
    CHECK(strncpy(bufor_przesyłania_2, "xy", 5) == bufor_przesyłania_2);
    CHECK(!memcmp(bufor_przesyłania_2, "xy\0\0\0", 5) && bufor_przesyłania_2[5] == 7);
    CHECK(stpncpy(bufor_przesyłania_2, "abcdef", 3) == bufor_przesyłania_2 + 3 && bufor_przesyłania_2[3] == 0);
    CHECK(stpncpy(bufor_przesyłania_2, "a", 4) == bufor_przesyłania_2 + 1 && bufor_przesyłania_2[3] == 0);
    CHECK(strncpy(bufor_przesyłania_2, "z", 0) == bufor_przesyłania_2 && bufor_przesyłania_2[0] == 'a');
    strcpy(bufor_przesyłania_2, "a");
    CHECK(strcat(bufor_przesyłania_2, "bc") == bufor_przesyłania_2 && !strcmp(bufor_przesyłania_2, "abc"));
    CHECK(strncat(bufor_przesyłania_2, "def", 2) == bufor_przesyłania_2 && !strcmp(bufor_przesyłania_2, "abcde"));
    CHECK(strncat(bufor_przesyłania_2, "xyz", 0) == bufor_przesyłania_2 && !strcmp(bufor_przesyłania_2, "abcde"));
    ...
    CHECK(strxfrm(bufor_przesyłania_2, "abc", 4) == 3 && !strcmp(bufor_przesyłania_2, "abc"));
    ...
    strcpy(bufor_przesyłania_2, ",,one;two,,");
    CHECK(!strcmp(strtok_r(bufor_przesyłania_2, ",;", &saved_state), "one"));
    strcpy(source, "x:y");
    ...
    for (wartość = -1; wartość <= 255; ++wartość) {
        int low = wartość >= 'a' && wartość <= 'z', high = wartość >= 'A' && wartość <= 'Z';
        int digit = wartość >= '0' && wartość <= '9';
        CHECK(!!isalpha(wartość) == (low || high));
        CHECK(!!isalnum(wartość) == (low || high || digit));
        CHECK(!!isdigit(wartość) == digit);
        CHECK(!!islower(wartość) == low && !!isupper(wartość) == high);
        CHECK(!!isblank(wartość) == (wartość == ' ' || wartość == '\t'));
        CHECK(!!isspace(wartość) == (wartość == ' ' || (wartość >= 9 && wartość <= 13)));
        CHECK(!!iscntrl(wartość) == ((wartość >= 0 && wartość < 32) || wartość == 127));
        CHECK(!!isprint(wartość) == (wartość >= 32 && wartość <= 126));
        CHECK(!!isgraph(wartość) == (wartość >= 33 && wartość <= 126));
        CHECK(!!ispunct(wartość) == (wartość >= 33 && wartość <= 126 && !low && !high && !digit));
        CHECK(!!isxdigit(wartość) == (digit || (wartość >= 'a' && wartość <= 'f') || (wartość >= 'A' && wartość <= 'F')));
        CHECK(tolower(wartość) == (high ? wartość + 32 : wartość));
        CHECK(toupper(wartość) == (low ? wartość - 32 : wartość));
    }
    errno = EIO;
    ...
    for (długość = 0; długość <= 257; ++długość) {
        for (index = 0; index < (int)długość; ++index) elements[index] = ((index * 37 + (int)długość) % 19) - 9;
        qsort(elements, długość, sizeof(int), compare_int);
        for (index = 1; index < (int)długość; ++index) CHECK(elements[index - 1] <= elements[index]);
        for (index = 0; index < (int)długość; ++index) {
            int *found = bsearch(&elements[index], elements, długość, sizeof(int), compare_int);
            CHECK(found && *found == elements[index]);
        }
        wartość = 100;
        CHECK(bsearch(&wartość, elements, długość, sizeof(int), compare_int) == NULL);
    }
    environ = envp;
