...
{
    unsigned int długość = 0;
    while (text[długość] != 0)
        długość++;
    return długość;
}
...
{
    (void)Zapis(STDOUT_FILENO, text, text_length(text));
}

static void report(const char *tożsamość_systemu, int passed)
{
    checks++;
    if (passed) {
        say("PTEST:PASS:");
...
static pid_t reap_nohang(pid_t child, int *stan)
{
    unsigned int spins;
    pid_t result;

    for (spins = 0; spins < 2000000U; spins++) {
        result = waitpid(child, stan, WNOHANG);
        if (result != 0)
            return result;
    }
    return 0;
...
static void test_open_and_io(void)
{
    ...
    report("open-readonly", deskryptor_pliku >= 3);
    if (deskryptor_pliku >= 0) {
        report("read-bytes", Odczyt(deskryptor_pliku, bytes, 2) == 2 &&
               (unsigned char)bytes[0] == 0x7f && bytes[1] == 'E');
        report("lseek-set", lseek(deskryptor_pliku, 0, SEEK_SET) == 0);
        report("lseek-cur", lseek(deskryptor_pliku, 1, SEEK_CUR) == 1);
        report("lseek-end", lseek(deskryptor_pliku, -1, SEEK_END) > 0);
        errno = 0;
        report("lseek-negative", lseek(deskryptor_pliku, -1, SEEK_SET) == -1 && errno == EINVAL);
        errno = 0;
        report("lseek-whence", lseek(deskryptor_pliku, 0, 99) == -1 && errno == EINVAL);
        report("fstat-file", fstat(deskryptor_pliku, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 2);
        errno = 0;
        report("fstat-efault", failed_with(fstat(deskryptor_pliku, (struct stat *)0), EFAULT));
        report("fsync-file", fsync(deskryptor_pliku) == 0);
        report("close-file", Zamknij(deskryptor_pliku) == 0);
        errno = 0;
        report("closed-fd", failed_with(Zamknij(deskryptor_pliku), EBADF));
    }

    root = Otwórz("/", O_RDONLY | O_DIRECTORY);
    report("open-directory", root >= 3);
    if (root >= 0) {
        report("fstat-directory", fstat(root, &st) == 0 && S_ISDIR(st.st_mode));
        errno = 0;
        report("read-directory", Odczyt(root, bytes, 1) == -1 && errno == EISDIR);
        (void)Zamknij(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", Zapis(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", Odczyt(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", Odczyt(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", Zapis(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("open-enoent", failed_with(Otwórz("/NOFILE", O_RDONLY), ENOENT));
    errno = 0;
    report("open-efault", failed_with(Otwórz((const char *)0, O_RDONLY), EFAULT));
    errno = 0;
    report("open-write-erofs", failed_with(Otwórz("/USER2", O_WRONLY), EROFS));
    errno = 0;
    report("open-rdwr-erofs", failed_with(Otwórz("/USER2", O_RDWR), EROFS));
    errno = 0;
    report("open-create-erofs", failed_with(Otwórz("/NEWFILE", O_CREAT | O_WRONLY, 0600), EROFS));
    errno = 0;
    report("open-trunc-erofs", failed_with(Otwórz("/USER2", O_TRUNC | O_RDONLY), EROFS));
    errno = 0;
    report("open-append-erofs", failed_with(Otwórz("/USER2", O_APPEND | O_RDONLY), EROFS));
    errno = 0;
    report("open-enotdir", failed_with(Otwórz("/USER2", O_RDONLY | O_DIRECTORY), ENOTDIR));
    errno = 0;
    report("creat-erofs", failed_with(creat("/NEWFILE", 0600), EROFS));
    errno = 0;
    report("close-ebadf", failed_with(Zamknij(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    copy = dup(deskryptor_pliku);
    report("dup", copy >= 0 && copy != deskryptor_pliku);
    if (copy >= 0) {
        report("dup-shared-offset", Odczyt(deskryptor_pliku, &first, 1) == 1 && Odczyt(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Zamknij(deskryptor_pliku) == 0 && fstat(copy, &st) == 0);
        deskryptor_pliku = copy;
    }

    target = dup2(deskryptor_pliku, 20);
    report("dup2", target == 20 && fstat(20, &st) == 0);
    report("dup2-same", dup2(deskryptor_pliku, deskryptor_pliku) == deskryptor_pliku);
    if (target == 20)
        (void)Zamknij(20);
    errno = 0;
    report("dup2-old-ebadf", failed_with(dup2(-1, 10), EBADF));
    errno = 0;
    report("dup2-new-ebadf", failed_with(dup2(deskryptor_pliku, 99), EBADF));

    report("fcntl-getfd", fcntl(deskryptor_pliku, F_GETFD) == 0);
    report("fcntl-setfd", fcntl(deskryptor_pliku, F_SETFD, FD_CLOEXEC) == 0 &&
           fcntl(deskryptor_pliku, F_GETFD) == FD_CLOEXEC);
    high = fcntl(deskryptor_pliku, F_DUPFD, 10);
    report("fcntl-dupfd", high >= 10);
    if (high >= 0) {
        report("fcntl-fd-flags-independent", fcntl(high, F_GETFD) == 0);
        (void)Zamknij(high);
    }
    report("fcntl-getfl", (fcntl(deskryptor_pliku, F_GETFL) & O_ACCMODE) == O_RDONLY);
    report("fcntl-setfl", fcntl(deskryptor_pliku, F_SETFL, O_APPEND) == 0 &&
           (fcntl(deskryptor_pliku, F_GETFL) & O_APPEND) != 0);
    errno = 0;
    report("fcntl-command-einval", failed_with(fcntl(deskryptor_pliku, 999), EINVAL));
    errno = 0;
...
static void test_terminal_and_uname(void)
{
    ...
    if (deskryptor_pliku >= 0) {
        errno = 0;
        report("isatty-enotty", isatty(deskryptor_pliku) == 0 && errno == ENOTTY);
        (void)Zamknij(deskryptor_pliku);
    } else {
    ...
    report("uname", uname(&tożsamość_systemu) == 0 && tożsamość_systemu.sysname[0] != 0 &&
           tożsamość_systemu.nodename[0] != 0 && text_equal(tożsamość_systemu.machine, "i386"));
    errno = 0;
...
void posix_test_process(void)
{
    ...
    report("fork", child > 0);
    if (child > 0) {
        waited = reap_nohang(child, &stan);
        report("waitpid", waited == child && WIFEXITED(stan) && WEXITSTATUS(stan) == 23);
        errno = 0;
        report("waitpid-echild", waitpid(child, &stan, WNOHANG) == -1 && errno == ECHILD);
    }
    ...
    if (child > 0 && reap_nohang(child, &stan) == child) {
        report("fork-fd-isolation", Odczyt(deskryptor_pliku, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
    ...
    if (child > 0) {
        waited = reap_nohang(child, &stan);
        report("execve", waited == child && WIFEXITED(stan) && WEXITSTATUS(stan) == 37);
    }
    ...
    if (child > 0) {
        ...
        for (spin = 0; spin < 2000000U; spin++)
            (void)getpid();
        waited = wait(&stan);
        report("wait", waited == child && WIFEXITED(stan) && WEXITSTATUS(stan) == 29);
    }
}
