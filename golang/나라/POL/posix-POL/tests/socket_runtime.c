...
static void say(const char *s) { (void)Zapis(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *tożsamość_systemu, int pass)
{
...
int main(void)
{
    ...
    report("recvfrom", recvfrom(server, bufor_przesyłania_2, sizeof(bufor_przesyłania_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       bufor_przesyłania_2[0] == 'p' && bufor_przesyłania_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, bufor_przesyłania_2, sizeof(bufor_przesyłania_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Zamknij(client) == 0);
    report("close-server", Zamknij(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
