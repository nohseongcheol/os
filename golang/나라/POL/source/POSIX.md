...
freestanding library in `./przestrzeń_użytkownika/posix`.
...
```sh
cd ./przestrzeń_użytkownika/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./przestrzeń_użytkownika/posix/deklaracje` and link the archive into an EngOS i386 user
...
`./przestrzeń_użytkownika/posix/TEST_RESULTS.md`. Build all runtime test executables
