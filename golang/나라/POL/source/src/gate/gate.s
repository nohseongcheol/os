...
GLOBL ·identyfikatortDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·identyfikatort<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·instalacjaidt(SB),NOSPLIT,$0
	LEAQ ·identyfikatortDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·identyfikatort<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·UchwytPrzerwanie(SB),NOSPLIT,$0-10
	...
	LEAQ ·przerwaniegatewpis(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	...
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchPrzerwanie(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchPrzerwanie(SB),NOSPLIT,$0
...
TEXT ·przerwaniegatewpis(SB),NOSPLIT,$0
