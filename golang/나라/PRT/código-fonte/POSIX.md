...
freestanding library in `./espaço_do_utilizador/posix`.
...
```sh
cd ./espaço_do_utilizador/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./espaço_do_utilizador/posix/declarações` and link the archive into an EngOS i386 user
...
`./espaço_do_utilizador/posix/TEST_RESULTS.md`. Build all runtime test executables
