...
void *memcpy(void *destination, const void *source, size_t comprimento)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    while (comprimento--) *cursor++ = *input++;
    return destination;
}

void *memmove(void *destination, const void *source, size_t comprimento)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    if ((uintptr_t)destination <= (uintptr_t)source)
        return memcpy(destination, source, comprimento);
    while (comprimento) { --comprimento; cursor[comprimento] = input[comprimento]; }
    return destination;
}

void *memset(void *destination, int character, size_t comprimento)
{
    unsigned char *cursor = destination;
    while (comprimento--) *cursor++ = (unsigned char)character;
    return destination;
}

int memcmp(const void *left, const void *right, size_t comprimento)
{
...
void *memchr(const void *source, int character, size_t comprimento)
{
...
size_t strnlen(const char *text, size_t limit)
{
    ...
    while (comprimento < limit && text[comprimento]) ++comprimento;
    return comprimento;
}
...
{
    size_t comprimento = strnlen(source, limit);
    memcpy(destination, source, comprimento);
    memset(destination + comprimento, 0, limit - comprimento);
    return destination + comprimento;
}
...
char *strncat(char *destination, const char *source, size_t limit)
{
    ...
    size_t comprimento = strnlen(source, limit);
    memcpy(cursor, source, comprimento);
    cursor[comprimento] = 0;
    return destination;
}
...
{
    size_t comprimento = strlen(source);
    if (limit) {
        size_t copy_length = comprimento < limit ? comprimento : limit;
        memcpy(destination, source, copy_length);
        if (comprimento < limit) destination[comprimento] = 0;
    }
    return comprimento;
}
...
{
    size_t comprimento = strlen(source);
    if (!comprimento) return (char *)text;
    while (*text) {
        if (strncmp(text, source, comprimento) == 0) return (char *)text;
        ++text;
    }
    return NULL;
}
...
{
    size_t comprimento = 0;
    while (text[comprimento] && strchr(delimiters, text[comprimento])) ++comprimento;
    return comprimento;
}
...
{
    size_t comprimento = strnlen(text, limit);
    char *destination = malloc(comprimento + 1);
    if (destination) { memcpy(destination, text, comprimento); destination[comprimento] = 0; }
    return destination;
}
