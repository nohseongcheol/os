...
ssize_t moñeẽ(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_moñeẽ, fd, buf, count));
}

ssize_t hai(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_hai, fd, buf, count));
}
