...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Hai(0xA0)
	} else {
		self.deviceport.Hai(0xB0)
	}
	self.controlport.Hai(0)
	self.deviceport.Hai(0xA0)

	var status uint8 = self.commandport.Moñeẽ()
	if status == 0xFF {
	...
	self.sectorcountport.Hai(0)
	self.lbalowport.Hai(0)
	self.lbamidport.Hai(0)
	self.lbahiport.Hai(0)
	self.commandport.Hai(0xEC)

	status = self.commandport.Moñeẽ()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Moñeẽ28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Hai(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Hai(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Hai(0)
	self.sectorcountport.Hai(1)

	self.lbalowport.Hai(uint8(sector & 0x000000FF))
	self.lbamidport.Hai(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Hai(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Hai(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Moñeẽ()
...
func (self *TAdvancedtechnologyattachment) Hai28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Hai(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Hai(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Hai(0)
	self.sectorcountport.Hai(1)
	self.lbalowport.Hai(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Hai(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Hai(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Hai(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Hai(0xE0)
	} else {
		self.deviceport.Hai(0xF0)
	}
	self.commandport.Hai(0xE7)
