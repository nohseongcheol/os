...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portmoñeẽbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portmoñeẽbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func haips2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Porthaibyte(commandport_2, value)
	return true
}

func haips2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Porthaibyte(dataport_2, value)
	return true
}

func moñeẽps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portmoñeẽbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portmoñeẽbyte(commandport_2)&0x01) != 0; i++ {
		Portmoñeẽbyte(dataport_2)
	}

	if !haips2command(0xAE) || !haips2command(0x20) {
		return
	}
	status, ok := moñeẽps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !haips2command(0x60) || !haips2data(status) {
		return
	}
...
var keyboardqueuemoñeẽ uint8
var keyboardqueuehai uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuehai + 1) % keyboardqueuesize
	if next == keyboardqueuemoñeẽ {
		return
	}
	keyboardqueue[keyboardqueuehai] = key
	keyboardqueuehai = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuemoñeẽ != keyboardqueuehai {
		key := keyboardqueue[keyboardqueuemoñeẽ]
		keyboardqueuemoñeẽ = (keyboardqueuemoñeẽ + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portmoñeẽbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portmoñeẽbyte(dataport_2)
	if scancode == 0xE0 {
