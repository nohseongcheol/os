...
func Sysmoñeẽ_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysmoñeẽ, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Syshai, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Syshai, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysmoñeẽ:
		cpu.Eax = uint32(sysmoñeẽ(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Syshai:
		cpu.Eax = uint32(syshai(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|ohaionly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: omoñeẽonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: ohaionly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: ohaionly}
}
...
func syshai(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysmoñeẽ(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return moñeẽstdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return moñeẽvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == ohaionly || accessmode == omoñeẽhai || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func moñeẽvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	moñeẽfile(entry.name[:entry.namelen], tmp)
...
func moñeẽstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinhai + 1) % uint32(len(stdinbuffer))
	if next == stdinmoñeẽ {
		return
	}
	stdinbuffer[stdinhai] = c
	stdinhai = next
}

func stdingetblocking() byte {
	for stdinmoñeẽ == stdinhai {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinmoñeẽ]
	stdinmoñeẽ = (stdinmoñeẽ + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portmoñeẽbyte(0x64) & 0x01) == 0 {
	}
	sc := Portmoñeẽbyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Moñeẽpartition(&ata0s)
...
func moñeẽfile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Moñeẽ(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
