...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	tenondeguathread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if tenondeguathread != nil {
		tenondeguathread.Pid = process.id
		tenondeguathread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(tenondeguathread)))
	}
