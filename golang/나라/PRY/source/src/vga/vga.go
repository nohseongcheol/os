...
var attributecontrollermoñeẽport uint16 = 0x3c1
var attributecontrollerhaiport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Hairegister(register []byte) {
	var regindex uint16 = 0

	Porthaibyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Porthaibyte(sequencerindexport, i)
		Porthaibyte(sequencerdataport, register[regindex])
		regindex++
	}

	Porthaibyte(crtcindexport, 0x03)

	Porthaibyte(crtcdataport, (Portmoñeẽbyte(crtcdataport) | 0x80))
	Porthaibyte(crtcindexport, 0x11)
	Porthaibyte(crtcdataport, (Portmoñeẽbyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Porthaibyte(crtcindexport, i)
		Porthaibyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Porthaibyte(graphicscontrollerindexport, i)
		Porthaibyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portmoñeẽbyte(attributecontrollerresetport)
		Porthaibyte(attributecontrollerindexport, i)
		Porthaibyte(attributecontrollerhaiport, register[regindex])
		regindex++
	}

	Portmoñeẽbyte(attributecontrollerresetport)
	Porthaibyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Porthaibyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portmoñeẽbyte(graphicscontrollerdataport) >> 2) & 0x03)
