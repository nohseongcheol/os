...
freestanding library in `./فضاء_المستخدم/posix`.
...
```sh
cd ./فضاء_المستخدم/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./فضاء_المستخدم/posix/تصريحات` and link the archive into an EngOS i386 user
...
`./فضاء_المستخدم/posix/TEST_RESULTS.md`. Build all runtime test executables
