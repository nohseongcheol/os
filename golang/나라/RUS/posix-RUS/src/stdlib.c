...
int abs(int значение) { return значение < 0 ? -значение : значение; }
long labs(long значение) { return значение < 0 ? -значение : значение; }
...
char *getenv(const char *сведения_о_системе)
{
    size_t длина = strlen(сведения_о_системе);
    char **cursor = environ;
    if (!длина || strchr(сведения_о_системе, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, сведения_о_системе, длина) && (*cursor)[длина] == '=') return *cursor + длина + 1;
        ++cursor;
    }
    return NULL;
}
