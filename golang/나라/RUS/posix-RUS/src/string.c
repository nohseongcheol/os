...
void *memcpy(void *destination, const void *source, size_t длина)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    while (длина--) *cursor++ = *input++;
    return destination;
}

void *memmove(void *destination, const void *source, size_t длина)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    if ((uintptr_t)destination <= (uintptr_t)source)
        return memcpy(destination, source, длина);
    while (длина) { --длина; cursor[длина] = input[длина]; }
    return destination;
}

void *memset(void *destination, int character, size_t длина)
{
    unsigned char *cursor = destination;
    while (длина--) *cursor++ = (unsigned char)character;
    return destination;
}

int memcmp(const void *left, const void *right, size_t длина)
{
...
void *memchr(const void *source, int character, size_t длина)
{
...
{
    size_t длина = 0;
    while (длина < limit && text[длина]) ++длина;
    return длина;
}
...
{
    size_t длина = strnlen(source, limit);
    memcpy(destination, source, длина);
    memset(destination + длина, 0, limit - длина);
    return destination + длина;
}
...
{
    size_t длина = strlen(source);
    if (limit) {
        size_t copy_length = длина < limit ? длина : limit;
        memcpy(destination, source, copy_length);
        if (длина < limit) destination[длина] = 0;
    }
    return длина;
}
...
{
    size_t длина = strlen(source);
    if (!длина) return (char *)text;
    while (*text) {
        if (strncmp(text, source, длина) == 0) return (char *)text;
        ++text;
    }
    return NULL;
}
...
{
    size_t длина = 0;
    while (text[длина] && strchr(delimiters, text[длина])) ++длина;
    return длина;
}
...
{
    size_t длина = strnlen(text, limit);
    char *destination = malloc(длина + 1);
    if (destination) { memcpy(destination, text, длина); destination[длина] = 0; }
    return destination;
}
