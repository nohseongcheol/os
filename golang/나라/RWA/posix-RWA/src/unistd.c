...
ssize_t gusoma(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_gusoma, fd, buf, count));
}

ssize_t kwandika(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_kwandika, fd, buf, count));
}

int Gufunga(int fd)
{
    return (int)__syscall_result(SC1(SYS_Gufunga, fd));
}
