...
ssize_t jàng(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_jàng, fd, buf, count));
}

ssize_t bind_2(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_bind_2, fd, buf, count));
}
