...
var networkKaartconsole TConsole = TConsole{}
...
func (self *TRawdatahandler) Onrawdatareceive(datapointer uintptr, size int) bool {
	networkKaartconsole.MPrintxy(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawdatahandler) Send(datapointer uintptr, size uint32) {
	networkKaartconsole.MPrintxy(([]byte)("TRawDataSend"), 10, 10)
...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portjàngword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portjàngword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portjàngword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portjàngword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portjàngword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portjàngword(Macaddress4port) / 256)
	...
	Portbindword(registeraddressport, 20)
	Portbindword(buscontrolregisterdataport, 0x102)

	Portbindword(registeraddressport, 0)
	Portbindword(registerdataport, 0x04)
	...
	Portbindword(registeraddressport, 1)
	Portbindword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portbindword(registeraddressport, 2)
	Portbindword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portbindword(registeraddressport, 0)
	Portbindword(registerdataport, 0x41)

	Portbindword(registeraddressport, 4)
	temporary := Portjàngword(registerdataport)
	Portbindword(registeraddressport, 4)
	Portbindword(registerdataport, temporary|0xC00)

	Portbindword(registeraddressport, 0)
	Portbindword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portjàngword(resetport)
	Portbindword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portbindword(registeraddressport, 0)
	temporary := uint32(Portjàngword(registerdataport))
	...
	Portbindword(registeraddressport, 0)
	Portbindword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portbindword(registeraddressport, 0)
	Portbindword(registerdataport, 0x48)
