...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Bind_2(0xA0)
	} else {
		self.deviceport.Bind_2(0xB0)
	}
	self.controlport.Bind_2(0)
	self.deviceport.Bind_2(0xA0)

	var status uint8 = self.commandport.Jàng()
	if status == 0xFF {
	...
	self.sectorcountport.Bind_2(0)
	self.lbalowport.Bind_2(0)
	self.lbamidport.Bind_2(0)
	self.lbahiport.Bind_2(0)
	self.commandport.Bind_2(0xEC)

	status = self.commandport.Jàng()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Jàng28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Bind_2(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Bind_2(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Bind_2(0)
	self.sectorcountport.Bind_2(1)

	self.lbalowport.Bind_2(uint8(sector & 0x000000FF))
	self.lbamidport.Bind_2(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Bind_2(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Bind_2(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Jàng()
...
func (self *TAdvancedtechnologyattachment) Bind28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Bind_2(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Bind_2(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Bind_2(0)
	self.sectorcountport.Bind_2(1)
	self.lbalowport.Bind_2(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Bind_2(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Bind_2(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Bind_2(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Bind_2(0xE0)
	} else {
		self.deviceport.Bind_2(0xF0)
	}
	self.commandport.Bind_2(0xE7)
