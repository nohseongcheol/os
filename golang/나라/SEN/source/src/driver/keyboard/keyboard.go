...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portjàngbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portjàngbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func bindps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portbindbyte(commandport_2, value)
	return true
}

func bindps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portbindbyte(dataport_2, value)
	return true
}

func jàngps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portjàngbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portjàngbyte(commandport_2)&0x01) != 0; i++ {
		Portjàngbyte(dataport_2)
	}

	if !bindps2command(0xAE) || !bindps2command(0x20) {
		return
	}
	status, ok := jàngps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !bindps2command(0x60) || !bindps2data(status) {
		return
	}
...
var keyboardqueuejàng uint8
var keyboardqueuebind uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuebind + 1) % keyboardqueuesize
	if next == keyboardqueuejàng {
		return
	}
	keyboardqueue[keyboardqueuebind] = key
	keyboardqueuebind = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuejàng != keyboardqueuebind {
		key := keyboardqueue[keyboardqueuejàng]
		keyboardqueuejàng = (keyboardqueuejàng + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portjàngbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portjàngbyte(dataport_2)
	if scancode == 0xE0 {
