	...
	if elfheader.eshnum != 0 {
		...
		for i := uint32(0); i < uint32(elfheader.eshnum); i++ {
			...
			if Equalbytes(sectname, ([]byte)(".text")) {
				...
				console_2.MPrint("]")
				copy(self.tekst[:sectheader.shsize], sectvalue[:sectheader.shsize])
				self.tekstlen = sectheader.shsize
			}
			if Equalbytes(sectname, ([]byte)(".rel.text")) {
				...
				for rt := uint32(0); rt < sectheader.shsize/8; rt++ {
					...
					self.relTekst[rt].number = *(*uint32)(Pointer(&sectvalue[rt*8+4]))
					self.relTekstlen++
				}
			}
		...
		for rt := uint32(0); rt < self.relTekstlen; rt++ {
			...
			console_2.MPrint(":")
			console_2.MUnsignedinteger32print(self.relTekst[rt].number)
			console_2.MPrint(":")
		...
		if tekstpointer != nil {
			memorymanager.Free(tekstpointer)
		}
