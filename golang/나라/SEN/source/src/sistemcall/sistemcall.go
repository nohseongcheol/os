...
import . "multitasking"
import . "driver/ata"
import . "fileSistem/msdospartition"
import . "fileSistem/fat"
import . "fileSistem/elf"
...
var stdinjàng uint32
var stdinbind uint32
...
func Sysjàng_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysjàng, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysbind, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysbind, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysjàng:
		cpu.Eax = uint32(sysjàng(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysbind:
		cpu.Eax = uint32(sysbind(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|obindonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: ojàngonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: obindonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: obindonly}
}
...
func sysbind(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysjàng(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return jàngstdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return jàngvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == obindonly || accessmode == ojàngbind || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func jàngvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	jàngfile(entry.name[:entry.namelen], tmp)
...
func jàngstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinbind + 1) % uint32(len(stdinbuffer))
	if next == stdinjàng {
		return
	}
	stdinbuffer[stdinbind] = c
	stdinbind = next
}

func stdingetblocking() byte {
	for stdinjàng == stdinbind {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinjàng]
	stdinjàng = (stdinjàng + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portjàngbyte(0x64) & 0x01) == 0 {
	}
	sc := Portjàngbyte(0x60)
...
func jàngfile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Jàng(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
