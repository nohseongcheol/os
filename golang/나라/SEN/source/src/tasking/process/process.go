...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	bumagthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if bumagthread != nil {
		bumagthread.Pid = process.id
		bumagthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(bumagthread)))
	}
