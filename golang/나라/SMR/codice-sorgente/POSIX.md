...
freestanding library in `./spazio_utente/posix`.
...
```sh
cd ./spazio_utente/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./spazio_utente/posix/dichiarazioni` and link the archive into an EngOS i386 user
...
`./spazio_utente/posix/TEST_RESULTS.md`. Build all runtime test executables
