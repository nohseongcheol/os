...
int abs(int valore) { return valore < 0 ? -valore : valore; }
long labs(long valore) { return valore < 0 ? -valore : valore; }
...
char *getenv(const char *identità_del_sistema)
{
    size_t lunghezza = strlen(identità_del_sistema);
    char **cursor = environ;
    if (!lunghezza || strchr(identità_del_sistema, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, identità_del_sistema, lunghezza) && (*cursor)[lunghezza] == '=') return *cursor + lunghezza + 1;
        ++cursor;
    }
    return NULL;
}
