...
ssize_t akhri(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_akhri, fd, buf, count));
}

ssize_t qor(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_qor, fd, buf, count));
}
