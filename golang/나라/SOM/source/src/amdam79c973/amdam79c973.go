...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portakhriword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portakhriword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portakhriword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portakhriword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portakhriword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portakhriword(Macaddress4port) / 256)
	...
	Portqorword(registeraddressport, 20)
	Portqorword(buscontrolregisterdataport, 0x102)

	Portqorword(registeraddressport, 0)
	Portqorword(registerdataport, 0x04)
	...
	Portqorword(registeraddressport, 1)
	Portqorword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portqorword(registeraddressport, 2)
	Portqorword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portqorword(registeraddressport, 0)
	Portqorword(registerdataport, 0x41)

	Portqorword(registeraddressport, 4)
	temporary := Portakhriword(registerdataport)
	Portqorword(registeraddressport, 4)
	Portqorword(registerdataport, temporary|0xC00)

	Portqorword(registeraddressport, 0)
	Portqorword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portakhriword(resetport)
	Portqorword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portqorword(registeraddressport, 0)
	temporary := uint32(Portakhriword(registerdataport))
	...
	Portqorword(registeraddressport, 0)
	Portqorword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portqorword(registeraddressport, 0)
	Portqorword(registerdataport, 0x48)
