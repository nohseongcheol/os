...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Qor(0xA0)
	} else {
		self.deviceport.Qor(0xB0)
	}
	self.controlport.Qor(0)
	self.deviceport.Qor(0xA0)

	var status uint8 = self.commandport.Akhri()
	if status == 0xFF {
	...
	self.sectorcountport.Qor(0)
	self.lbalowport.Qor(0)
	self.lbamidport.Qor(0)
	self.lbahiport.Qor(0)
	self.commandport.Qor(0xEC)

	status = self.commandport.Akhri()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Akhri28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Qor(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Qor(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Qor(0)
	self.sectorcountport.Qor(1)

	self.lbalowport.Qor(uint8(sector & 0x000000FF))
	self.lbamidport.Qor(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Qor(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Qor(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Akhri()
...
func (self *TAdvancedtechnologyattachment) Qor28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Qor(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Qor(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Qor(0)
	self.sectorcountport.Qor(1)
	self.lbalowport.Qor(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Qor(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Qor(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Qor(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Qor(0xE0)
	} else {
		self.deviceport.Qor(0xF0)
	}
	self.commandport.Qor(0xE7)
