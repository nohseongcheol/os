...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portakhribyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portakhribyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func qorps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portqorbyte(commandport_2, value)
	return true
}

func qorps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portqorbyte(dataport_2, value)
	return true
}

func akhrips2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portakhribyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portakhribyte(commandport_2)&0x01) != 0; i++ {
		Portakhribyte(dataport_2)
	}

	if !qorps2command(0xAE) || !qorps2command(0x20) {
		return
	}
	status, ok := akhrips2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !qorps2command(0x60) || !qorps2data(status) {
		return
	}
...
var keyboardqueueakhri uint8
var keyboardqueueqor uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueueqor + 1) % keyboardqueuesize
	if next == keyboardqueueakhri {
		return
	}
	keyboardqueue[keyboardqueueqor] = key
	keyboardqueueqor = next
}

func Processpendingkeyboardevents() {
	for keyboardqueueakhri != keyboardqueueqor {
		key := keyboardqueue[keyboardqueueakhri]
		keyboardqueueakhri = (keyboardqueueakhri + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portakhribyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portakhribyte(dataport_2)
	if scancode == 0xE0 {
