...
func Sysakhri_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysakhri, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysqor, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysqor, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysakhri:
		cpu.Eax = uint32(sysakhri(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysqor:
		cpu.Eax = uint32(sysqor(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|oqoronly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: oakhrionly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: oqoronly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: oqoronly}
}
...
func sysqor(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysakhri(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return akhristdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return akhrivfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == oqoronly || accessmode == oakhriqor || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func akhrivfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	akhrifile(entry.name[:entry.namelen], tmp)
...
func akhristdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinqor + 1) % uint32(len(stdinbuffer))
	if next == stdinakhri {
		return
	}
	stdinbuffer[stdinqor] = c
	stdinqor = next
}

func stdingetblocking() byte {
	for stdinakhri == stdinqor {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinakhri]
	stdinakhri = (stdinakhri + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portakhribyte(0x64) & 0x01) == 0 {
	}
	sc := Portakhribyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Akhripartition(&ata0s)
...
func akhrifile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Akhri(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
