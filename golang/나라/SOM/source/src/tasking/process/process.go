...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	weynthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if weynthread != nil {
		weynthread.Pid = process.id
		weynthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(weynthread)))
	}
