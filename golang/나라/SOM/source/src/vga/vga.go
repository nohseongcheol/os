...
var attributecontrollerakhriport uint16 = 0x3c1
var attributecontrollerqorport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Qorregister(register []byte) {
	var regindex uint16 = 0

	Portqorbyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portqorbyte(sequencerindexport, i)
		Portqorbyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portqorbyte(crtcindexport, 0x03)

	Portqorbyte(crtcdataport, (Portakhribyte(crtcdataport) | 0x80))
	Portqorbyte(crtcindexport, 0x11)
	Portqorbyte(crtcdataport, (Portakhribyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portqorbyte(crtcindexport, i)
		Portqorbyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portqorbyte(graphicscontrollerindexport, i)
		Portqorbyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portakhribyte(attributecontrollerresetport)
		Portqorbyte(attributecontrollerindexport, i)
		Portqorbyte(attributecontrollerqorport, register[regindex])
		regindex++
	}

	Portakhribyte(attributecontrollerresetport)
	Portqorbyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portqorbyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portakhribyte(graphicscontrollerdataport) >> 2) & 0x03)
