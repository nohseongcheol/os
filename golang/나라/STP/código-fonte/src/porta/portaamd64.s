...
GLOBL ·portaHandlers<>(SB), NOPTR, $NUM_IDT_ENTRIES*8
...
TEXT ·Manípulointerrupção(SB),NOSPLIT,$0-10
	...
	LEAQ ·interrupçãoportapontodeentrada(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	SUBQ $16, SP;                          \
	MOVQ R15, 0(SP);                       \
	MOVQ ·portaHandlers<>+8*num(SB), R15;   \
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchinterrupção(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchinterrupção(SB),NOSPLIT,$0
...
TEXT ·interrupçãoportapontodeentrada(SB),NOSPLIT,$0
