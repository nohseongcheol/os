...
void *malloc(size_t número_de_algarismos)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!número_de_algarismos) número_de_algarismos = 1;
    if (número_de_algarismos > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    número_de_algarismos = (número_de_algarismos + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= número_de_algarismos) {
            if (cursor->capacity - número_de_algarismos >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + número_de_algarismos);
                raw->capacity = cursor->capacity - número_de_algarismos - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = número_de_algarismos;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    address = mover_fim_da_memória_dinâmica(0);
    if (address == (void *)-1) return NULL;
    padding = (0U - (uintptr_t)address) & 15U;
    total = padding + sizeof(struct heap_block) + número_de_algarismos;
    if ((uintptr_t)address > (uintptr_t)INT_MAX - total) { errno = ENOMEM; return NULL; }
    address = mover_fim_da_memória_dinâmica((int)total);
    if (address == (void *)-1) return NULL;
    raw = (struct heap_block *)((unsigned char *)address + padding);
    raw->capacity = número_de_algarismos; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t número_de_algarismos)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(número_de_algarismos);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!número_de_algarismos) número_de_algarismos = 1;
    cursor = (struct heap_block *)address - 1;
    if (número_de_algarismos <= cursor->capacity) return address;
    destination = malloc(número_de_algarismos);
