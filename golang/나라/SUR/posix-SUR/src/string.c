...
void *memcpy(void *destination, const void *source, size_t lengte)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    while (lengte--) *cursor++ = *input++;
    return destination;
}

void *memmove(void *destination, const void *source, size_t lengte)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    if ((uintptr_t)destination <= (uintptr_t)source)
        return memcpy(destination, source, lengte);
    while (lengte) { --lengte; cursor[lengte] = input[lengte]; }
    return destination;
}

void *memset(void *destination, int character, size_t lengte)
{
    unsigned char *cursor = destination;
    while (lengte--) *cursor++ = (unsigned char)character;
    return destination;
}

int memcmp(const void *left, const void *right, size_t lengte)
{
...
void *memchr(const void *source, int character, size_t lengte)
{
...
{
    size_t lengte = 0;
    while (lengte < limit && text[lengte]) ++lengte;
    return lengte;
}
...
{
    size_t lengte = strnlen(source, limit);
    memcpy(destination, source, lengte);
    memset(destination + lengte, 0, limit - lengte);
    return destination + lengte;
}
...
{
    size_t lengte = strlen(source);
    if (limit) {
        size_t copy_length = lengte < limit ? lengte : limit;
        memcpy(destination, source, copy_length);
        if (lengte < limit) destination[lengte] = 0;
    }
    return lengte;
}
...
{
    size_t lengte = strlen(source);
    if (!lengte) return (char *)text;
    while (*text) {
        if (strncmp(text, source, lengte) == 0) return (char *)text;
        ++text;
    }
    return NULL;
}
...
{
    size_t lengte = 0;
    while (text[lengte] && strchr(delimiters, text[lengte])) ++lengte;
    return lengte;
}
...
{
    size_t lengte = strnlen(text, limit);
    char *destination = malloc(lengte + 1);
    if (destination) { memcpy(destination, text, lengte); destination[lengte] = 0; }
    return destination;
}
