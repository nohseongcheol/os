...
{
    (void)Schrijven(STDOUT_FILENO, text, text_length(text));
}

static void report(const char *systeemidentiteit, int passed)
{
    checks++;
    if (passed) {
        say("PTEST:PASS:");
...
static pid_t reap_nohang(pid_t child, int *toestand)
{
    unsigned int spins;
    pid_t result;

    for (spins = 0; spins < 2000000U; spins++) {
        result = waitpid(child, toestand, WNOHANG);
        if (result != 0)
            return result;
    }
    return 0;
...
static void test_open_and_io(void)
{
    ...
    report("open-readonly", bestandsdescriptor >= 3);
    if (bestandsdescriptor >= 0) {
        report("read-bytes", Lezen(bestandsdescriptor, bytes, 2) == 2 &&
               (unsigned char)bytes[0] == 0x7f && bytes[1] == 'E');
        report("lseek-set", lseek(bestandsdescriptor, 0, SEEK_SET) == 0);
        report("lseek-cur", lseek(bestandsdescriptor, 1, SEEK_CUR) == 1);
        report("lseek-end", lseek(bestandsdescriptor, -1, SEEK_END) > 0);
        errno = 0;
        report("lseek-negative", lseek(bestandsdescriptor, -1, SEEK_SET) == -1 && errno == EINVAL);
        errno = 0;
        report("lseek-whence", lseek(bestandsdescriptor, 0, 99) == -1 && errno == EINVAL);
        report("fstat-file", fstat(bestandsdescriptor, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 2);
        errno = 0;
        report("fstat-efault", failed_with(fstat(bestandsdescriptor, (struct stat *)0), EFAULT));
        report("fsync-file", fsync(bestandsdescriptor) == 0);
        report("close-file", Sluiten(bestandsdescriptor) == 0);
        errno = 0;
        report("closed-fd", failed_with(Sluiten(bestandsdescriptor), EBADF));
    }

    root = Openen("/", O_RDONLY | O_DIRECTORY);
    report("open-directory", root >= 3);
    if (root >= 0) {
        report("fstat-directory", fstat(root, &st) == 0 && S_ISDIR(st.st_mode));
        errno = 0;
        report("read-directory", Lezen(root, bytes, 1) == -1 && errno == EISDIR);
        (void)Sluiten(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", Schrijven(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", Lezen(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", Lezen(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", Schrijven(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("open-enoent", failed_with(Openen("/NOFILE", O_RDONLY), ENOENT));
    errno = 0;
    report("open-efault", failed_with(Openen((const char *)0, O_RDONLY), EFAULT));
    errno = 0;
    report("open-write-erofs", failed_with(Openen("/USER2", O_WRONLY), EROFS));
    errno = 0;
    report("open-rdwr-erofs", failed_with(Openen("/USER2", O_RDWR), EROFS));
    errno = 0;
    report("open-create-erofs", failed_with(Openen("/NEWFILE", O_CREAT | O_WRONLY, 0600), EROFS));
    errno = 0;
    report("open-trunc-erofs", failed_with(Openen("/USER2", O_TRUNC | O_RDONLY), EROFS));
    errno = 0;
    report("open-append-erofs", failed_with(Openen("/USER2", O_APPEND | O_RDONLY), EROFS));
    errno = 0;
    report("open-enotdir", failed_with(Openen("/USER2", O_RDONLY | O_DIRECTORY), ENOTDIR));
    errno = 0;
    report("creat-erofs", failed_with(creat("/NEWFILE", 0600), EROFS));
    errno = 0;
    report("close-ebadf", failed_with(Sluiten(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    copy = dup(bestandsdescriptor);
    report("dup", copy >= 0 && copy != bestandsdescriptor);
    if (copy >= 0) {
        report("dup-shared-offset", Lezen(bestandsdescriptor, &first, 1) == 1 && Lezen(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Sluiten(bestandsdescriptor) == 0 && fstat(copy, &st) == 0);
        bestandsdescriptor = copy;
    }

    target = dup2(bestandsdescriptor, 20);
    report("dup2", target == 20 && fstat(20, &st) == 0);
    report("dup2-same", dup2(bestandsdescriptor, bestandsdescriptor) == bestandsdescriptor);
    if (target == 20)
        (void)Sluiten(20);
    errno = 0;
    report("dup2-old-ebadf", failed_with(dup2(-1, 10), EBADF));
    errno = 0;
    report("dup2-new-ebadf", failed_with(dup2(bestandsdescriptor, 99), EBADF));

    report("fcntl-getfd", fcntl(bestandsdescriptor, F_GETFD) == 0);
    report("fcntl-setfd", fcntl(bestandsdescriptor, F_SETFD, FD_CLOEXEC) == 0 &&
           fcntl(bestandsdescriptor, F_GETFD) == FD_CLOEXEC);
    high = fcntl(bestandsdescriptor, F_DUPFD, 10);
    report("fcntl-dupfd", high >= 10);
    if (high >= 0) {
        report("fcntl-fd-flags-independent", fcntl(high, F_GETFD) == 0);
        (void)Sluiten(high);
    }
    report("fcntl-getfl", (fcntl(bestandsdescriptor, F_GETFL) & O_ACCMODE) == O_RDONLY);
    report("fcntl-setfl", fcntl(bestandsdescriptor, F_SETFL, O_APPEND) == 0 &&
           (fcntl(bestandsdescriptor, F_GETFL) & O_APPEND) != 0);
    errno = 0;
    report("fcntl-command-einval", failed_with(fcntl(bestandsdescriptor, 999), EINVAL));
    errno = 0;
...
static void test_terminal_and_uname(void)
{
    ...
    if (bestandsdescriptor >= 0) {
        errno = 0;
        report("isatty-enotty", isatty(bestandsdescriptor) == 0 && errno == ENOTTY);
        (void)Sluiten(bestandsdescriptor);
    } else {
    ...
    report("uname", uname(&systeemidentiteit) == 0 && systeemidentiteit.sysname[0] != 0 &&
           systeemidentiteit.nodename[0] != 0 && text_equal(systeemidentiteit.machine, "i386"));
    errno = 0;
...
void posix_test_process(void)
{
    ...
    report("fork", child > 0);
    if (child > 0) {
        waited = reap_nohang(child, &toestand);
        report("waitpid", waited == child && WIFEXITED(toestand) && WEXITSTATUS(toestand) == 23);
        errno = 0;
        report("waitpid-echild", waitpid(child, &toestand, WNOHANG) == -1 && errno == ECHILD);
    }
    ...
    if (child > 0 && reap_nohang(child, &toestand) == child) {
        report("fork-fd-isolation", Lezen(bestandsdescriptor, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
    ...
    if (child > 0) {
        waited = reap_nohang(child, &toestand);
        report("execve", waited == child && WIFEXITED(toestand) && WEXITSTATUS(toestand) == 37);
    }
    ...
    if (child > 0) {
        ...
        report("wait", waited == child && WIFEXITED(toestand) && WEXITSTATUS(toestand) == 29);
    }
}
