...
freestanding library in `./gebruikersruimte/posix`.
...
```sh
cd ./gebruikersruimte/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./gebruikersruimte/posix/declaraties` and link the archive into an EngOS i386 user
...
`./gebruikersruimte/posix/TEST_RESULTS.md`. Build all runtime test executables
