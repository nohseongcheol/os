...
ssize_t Čítanie(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Čítanie, fd, buf, count));
}

ssize_t Zápis(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Zápis, fd, buf, count));
}

int Zavrieť(int fd)
{
    return (int)__syscall_result(SC1(SYS_Zavrieť, fd));
}
