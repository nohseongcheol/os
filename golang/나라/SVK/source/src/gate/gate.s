...
GLOBL ·identifikátortDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·identifikátort<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·nainštalovaťidt(SB),NOSPLIT,$0
	LEAQ ·identifikátortDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·identifikátort<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·UškoPrerušenie(SB),NOSPLIT,$0-10
	...
	LEAQ ·prerušeniegatepoložka(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	...
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchPrerušenie(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchPrerušenie(SB),NOSPLIT,$0
...
TEXT ·prerušeniegatepoložka(SB),NOSPLIT,$0
