...
ssize_t Branje(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Branje, fd, buf, count));
}

ssize_t Pisanje(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Pisanje, fd, buf, count));
}

int Zapri(int fd)
{
    return (int)__syscall_result(SC1(SYS_Zapri, fd));
}
