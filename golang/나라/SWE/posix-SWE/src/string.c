...
void *memcpy(void *destination, const void *source, size_t längd)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    while (längd--) *cursor++ = *input++;
    return destination;
}

void *memmove(void *destination, const void *source, size_t längd)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    if ((uintptr_t)destination <= (uintptr_t)source)
        return memcpy(destination, source, längd);
    while (längd) { --längd; cursor[längd] = input[längd]; }
    return destination;
}

void *memset(void *destination, int character, size_t längd)
{
    unsigned char *cursor = destination;
    while (längd--) *cursor++ = (unsigned char)character;
    return destination;
}

int memcmp(const void *left, const void *right, size_t längd)
{
...
void *memchr(const void *source, int character, size_t längd)
{
...
{
    size_t längd = 0;
    while (längd < limit && text[längd]) ++längd;
    return längd;
}
...
{
    size_t längd = strnlen(source, limit);
    memcpy(destination, source, längd);
    memset(destination + längd, 0, limit - längd);
    return destination + längd;
}
...
{
    size_t längd = strlen(source);
    if (limit) {
        size_t copy_length = längd < limit ? längd : limit;
        memcpy(destination, source, copy_length);
        if (längd < limit) destination[längd] = 0;
    }
    return längd;
}
...
{
    size_t längd = strlen(source);
    if (!längd) return (char *)text;
    while (*text) {
        if (strncmp(text, source, längd) == 0) return (char *)text;
        ++text;
    }
    return NULL;
}
...
{
    size_t längd = 0;
    while (text[längd] && strchr(delimiters, text[längd])) ++längd;
    return längd;
}
...
{
    size_t längd = strnlen(text, limit);
    char *destination = malloc(längd + 1);
    if (destination) { memcpy(destination, text, längd); destination[längd] = 0; }
    return destination;
}
