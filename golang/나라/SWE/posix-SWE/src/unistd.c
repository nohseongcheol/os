...
void _exit(int tillstånd)
{
    SC1(SYS__exit, tillstånd);
    for (;;) {
...
{
    return (ssize_t)__syscall_result(SC3(SYS_Läs, filbeskrivare, överföringsbuffert, count));
}

ssize_t Skriv(int filbeskrivare, const void *överföringsbuffert, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Skriv, filbeskrivare, överföringsbuffert, count));
}

int Stäng(int filbeskrivare)
{
    return (int)__syscall_result(SC1(SYS_Stäng, filbeskrivare));
}

off_t lseek(int filbeskrivare, off_t offset, int whence)
{
    return (off_t)__syscall_result(SC3(SYS_lseek, filbeskrivare, offset, whence));
}
...
{
    return (int)__syscall_result(SC3(SYS_execve, sökväg, argument_3, envp));
}
...
{
    return (int)__syscall_result(SC2(SYS_access, sökväg, mode));
}

int chdir(const char *sökväg)
{
    return (int)__syscall_result(SC1(SYS_chdir, sökväg));
}

char *getcwd(char *överföringsbuffert, size_t antal_siffror)
{
    long result = __syscall_result(SC2(SYS_getcwd, överföringsbuffert, antal_siffror));
    return result < 0 ? (char *)0 : överföringsbuffert;
}

int dup(int filbeskrivare)
{
    return (int)__syscall_result(SC1(SYS_dup, filbeskrivare));
}
...
{
    return (int)__syscall_result(SC1(SYS_fsync, filbeskrivare));
}
...
pid_t waitpid(pid_t pid, int *tillstånd, int options)
{
    long result;
    do {
        result = SC3(7, pid, tillstånd, options);
