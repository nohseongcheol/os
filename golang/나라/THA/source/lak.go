...
func (self *TMykeyboardeventhandler) Onkeyลง(key byte) {
	foo := [1]byte{' '}
	foo[0] = key
...
func (self *TMymouseeventhandler) Onmouseลง(button int8) {
	buffer := []byte("x")
...
func Getfileขนาด(filename []byte) uint32 {
	...
	var ขนาด uint32 = bios.Len(&ata0s, partition.Mbr.Primarypartition[0], filename)
	ata0s.Flush()
...
func inputeventtask() {
	for {
		Pโพรเซสpendingkeyboardevents()
		Pโพรเซสpendingmouseevents()
		halt()
	}
}
...
func KKernelEntry(Pagedirectoryentry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memorymanager := &TMemorymanager{}
	memorymanager.Init(0, Maxqueueขนาด)

	paging := &Paging{}
	paging.Init(Pagedirectoryentry, 0x500000, memorymanager)
	paging.Sharedmemoryregion()

	Sกำหนดcr3(uint32(Pagedirectoryentry))
	Enablepaging()

	shareddescriptorตาราง := &TShareddescriptorตาราง{}
	shareddescriptorตาราง.Init()
	...
	tss.Install(shareddescriptorตาราง, 7, Segkerneldata, esp)
	...
	Interruptmanager.Init(0x20, shareddescriptorตาราง, taskmanager_2)
	...
	โพรเซสhelper := Pโพรเซสhelper{}
	โพรเซสhelper.Init(memorymanager, Pagedirectoryentry)
	...
	โพรเซสhelper.Spawn(taska, threadhelper, sche, uint32(Pagedirectoryentry), true)
	โพรเซสhelper.Spawn(taskb, threadhelper, sche, uint32(Pagedirectoryentry), true)
	โพรเซสhelper.Spawn(taskc, threadhelper, sche, uint32(Pagedirectoryentry), true)
	โพรเซสhelper.Spawn(taskd1, threadhelper, sche, uint32(Pagedirectoryentry), true)
	โพรเซสhelper.Spawn(inputeventtask, threadhelper, sche, uint32(Pagedirectoryentry), true)

	var ขนาด uint32

	var linkerfile []byte = ([]byte)("LINKER")
	ขนาด = Getfileขนาด(linkerfile)
	linkeraddress := memorymanager.Malloc(ขนาด)
	linkerdata := Getbytesfrompointer(uintptr(linkeraddress), int(ขนาด), int(ขนาด))
	Anfile(linkerfile, linkerdata)
	...
	ขนาด = Getfileขนาด(lib1file)

	lib1address := memorymanager.Malloc(ขนาด)
	lib1data := Getbytesfrompointer(uintptr(lib1address), int(ขนาด), int(ขนาด))
	Anfile(lib1file, lib1data)
	...
	ขนาด = Getfileขนาด(lib2file)

	lib2address := memorymanager.Malloc(ขนาด)
	lib2data := Getbytesfrompointer(uintptr(lib2address), int(ขนาด), int(ขนาด))
	Anfile(lib2file, lib2data)
	...
	ขนาด = Getfileขนาด(user1file)
	user1address := memorymanager.Malloc(ขนาด)
	user1data := Getbytesfrompointer(uintptr(user1address), int(ขนาด), int(ขนาด))
	Anfile(user1file, user1data)

	elf2 := Elf{}

	user1entry := elf2.Getentry(user1data)
	elf2.Parse(user1data[:], uint32(Pagedirectoryentry+0x1000))
	globaloffsetตาราง := elf2.Got
	...
	*code1pointer = uintptr(linkerentry)
	...
	proc2 := โพรเซสhelper.Spawn(func1val, threadhelper, sche, uint32(Pagedirectoryentry+0x1000), false)
	thr2 := (*TThread)(proc2.Threads.Getat(0))
	thr2.Cpuสถานะ.Ecx = user1entry
	thr2.Cpuสถานะ.Edx = globaloffsetตาราง
	thr2.Cpuสถานะ.Esi = uint32(uintptr(Pointer(PValue1linkmap.First)))

	console.MPrintxy("user1: ", 1, 10)
	console.MUnsignedinteger32print(elf2.Got)

	var user2file []byte = ([]byte)("USER2")
	ขนาด = Getfileขนาด(user2file)
	user2address := memorymanager.Malloc(ขนาด)
	user2data := Getbytesfrompointer(uintptr(user2address), int(ขนาด), int(ขนาด))
	Anfile(user2file, user2data)

	elf3 := Elf{}

	user2entry := elf3.Getentry(user2data)
	elf3.Parse(user2data[:], uint32(Pagedirectoryentry+0x2000))
	globaloffsetตาราง = elf3.Got
	...
	*code2pointer = uintptr(linkerentry)
	...
	proc3 := โพรเซสhelper.Spawn(func2val, threadhelper, sche, uint32(Pagedirectoryentry+0x2000), false)
	thr3 := (*TThread)(proc3.Threads.Getat(0))
	thr3.Cpuสถานะ.Ecx = user2entry
	thr3.Cpuสถานะ.Edx = globaloffsetตาราง
	thr3.Cpuสถานะ.Esi = uint32(uintptr(Pointer(PValue2linkmap.First)))

	console.MPrintxy("user2: ", 1, 11)
	console.MUnsignedinteger32print(thr3.Cpuสถานะ.Esi)

	liblinkmap.Print(1, 11)

	var user3file []byte = ([]byte)("USER3")
	ขนาด = Getfileขนาด(user3file)
	user3address := memorymanager.Malloc(ขนาด)
	user3data := Getbytesfrompointer(uintptr(user3address), int(ขนาด), int(ขนาด))
	Anfile(user3file, user3data)

	elf4 := Elf{}

	user3entry := elf4.Getentry(user3data)
	elf4.Parse(user3data[:], uint32(Pagedirectoryentry+0x3000))
	globaloffsetตาราง = elf4.Got
	...
	*code3pointer = uintptr(linkerentry)
	...
	proc4 := โพรเซสhelper.Spawn(func3val, threadhelper, sche, uint32(Pagedirectoryentry+0x3000), false)
	thr4 := (*TThread)(proc4.Threads.Getat(0))
	thr4.Cpuสถานะ.Ecx = user3entry
	thr4.Cpuสถานะ.Edx = globaloffsetตาราง
	thr4.Cpuสถานะ.Esi = uint32(uintptr(Pointer(PValue3linkmap.First)))

	โพรเซสhelper.Spawn(TFunction1, threadhelper, sche, uint32(Pagedirectoryentry+0x4000), true)
