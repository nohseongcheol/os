...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Khian(0xA0)
	} else {
		self.deviceport.Khian(0xB0)
	}
	self.controlport.Khian(0)
	self.deviceport.Khian(0xA0)

	var สถานะ uint8 = self.commandport.An()
	if สถานะ == 0xFF {
	...
	self.sectorcountport.Khian(0)
	self.lbalowport.Khian(0)
	self.lbamidport.Khian(0)
	self.lbahiport.Khian(0)
	self.commandport.Khian(0xEC)

	สถานะ = self.commandport.An()
	if สถานะ == 0x00 {
		console_2.MPrint(([]byte)("HDD Does not exist, ignoreign "))
		return
	}
	for (สถานะ&0x80) == 0x80 && (สถานะ&0x01) != 0x01 {
		สถานะ = self.commandport.An()
	}
...
func (self *TAdvancedtechnologyattachment) An28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Khian(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Khian(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Khian(0)
	self.sectorcountport.Khian(1)

	self.lbalowport.Khian(uint8(sector & 0x000000FF))
	self.lbamidport.Khian(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Khian(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Khian(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.An()
...
func (self *TAdvancedtechnologyattachment) Khian28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Khian(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Khian(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Khian(0)
	self.sectorcountport.Khian(1)
	self.lbalowport.Khian(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Khian(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Khian(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Khian(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Khian(0xE0)
	} else {
		self.deviceport.Khian(0xF0)
	}
	self.commandport.Khian(0xE7)
