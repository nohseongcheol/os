...
func (self *Elf) Parse(data []byte, Pagedirectoryentry uint32) {
	...
	if elfheader.eshnum != 0 {
		...
		sectheaderขนาด := uint32(Sizeof(Elfsectionheader{}))

		for i := uint32(0); i < uint32(elfheader.eshnum); i++ {
			sectheader := (*Elfsectionheader)(Pointer(&data[elfheader.eshoff+sectheaderขนาด*i]))
			...
			var sectvalue []byte
			if sectheader.shประเภท != 8 {
				endoffset := sectheader.shoffset + sectheader.shขนาด
			...
			if sectheader.shaddress > 0x1000 {
				ขนาด := sectheader.shขนาด
				if sectheader.shประเภท == 8 {
					Zeroblockขยายpagedirectory(sectheader.shaddress, ขนาด, Pagedirectoryentry)
				} else {
					ปลายทาง_2 := Getbytesfrompointer(uintptr(sectheader.shaddress), int(ขนาด), int(ขนาด))
					Sกำหนดblockขยายpagedirectory(sectvalue, ปลายทาง_2, ขนาด, Pagedirectoryentry)
				}
			}
			...
			if Equalbytes(sectname, ([]byte)(".text")) {
				...
				console_2.MPrint(":")
				console_2.MUnsignedinteger32print(sectheader.shขนาด)
				console_2.MPrint("]")
				copy(self.text[:sectheader.shขนาด], sectvalue[:sectheader.shขนาด])
				self.textlen = sectheader.shขนาด
			}
			if Equalbytes(sectname, ([]byte)(".rel.text")) {
				...
				console_2.MUnsignedinteger32print(sectheader.shขนาด)
				console_2.MPrint("]")
				for rt := uint32(0); rt < sectheader.shขนาด/8; rt++ {
			...
			if Equalbytes(sectname, ([]byte)(".strtab")) {
				...
				for st := uint32(1); st < sectheader.shขนาด; st++ {
