...
type TPartitionตารางentry struct {
	bootable	uint8
...
func (self *TPartitionตารางentry) Init(data [16]byte) {
	...
	self.ความยาว = Unsignedinteger32r(Arraytounsignedinteger32(buffer2))
}
...
type TMasterbootrecord struct {
	...
	Primarypartition	[4]TPartitionตารางentry

	magicnumber	uint16
}
type Tmsdospartitionตาราง struct {
	Mbr TMasterbootrecord
}

func (self *Tmsdospartitionตาราง) Anpartition(hd *TAdvancedtechnologyattachment) {
