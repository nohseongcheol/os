...
func (self *TPeripheralcomponentinterconnectcontroller) An(bus uint16, device_2 uint16, function uint16, registeroffset uint32) uint32 {
	var id uint32 = 0
	id = 0x1<<31 | (uint32(bus&0xFF) << 16) | (uint32(device_2&0x1f) << 11) | (uint32(function&0x07) << 8) | uint32(registeroffset&0xFC)

	Portkhiandword(self.commandport, id)

	result1 := Portandword(self.dataport)
	result2 := (result1 >> (8 * (registeroffset % 4)))

	return result2
}

func (self *TPeripheralcomponentinterconnectcontroller) Khian(bus uint16, device_2 uint16, function uint16, registeroffset uint32, value uint32) {
	var id uint32
	id = 0x1<<31 | uint32((bus&0xFF)<<16) | uint32((device_2&0x1f)<<11) | uint32((function&0x07)<<8) | uint32(registeroffset&0xFC)
	Portkhiandword(self.commandport, id)
	Portkhiandword(self.dataport, value)
}
func (self *TPeripheralcomponentinterconnectcontroller) Devicehasfunctions(bus uint16, device_2 uint16) bool {
	result := self.An(bus, device_2, 0, 0x0E)
	if (result & (1 << 7)) != 0 {
		return true
	} else {
		return false
	...
	for bus := 0; bus < 8; bus++ {
		for device_2 := 0; device_2 < 32; device_2++ {
			...
			for function := 0; function < numberfunctions; function++ {
				...
				for แถบnumber := 0; แถบnumber < 6; แถบnumber++ {
					var แถบ TBaseaddressregister = self.Getbaseaddressregister(uint16(bus), uint16(device_2), uint16(function), uint16(แถบnumber))
					if แถบ.address_2 != 0 && (แถบ.regtype == 1) {
						device.Portbase = แถบ.address_2
					}
...
func (self *TPeripheralcomponentinterconnectcontroller) Getdevicedescriptor(bus uint16, device_2 uint16, function uint16) TPeripheralcomponentinterconnectdevicedescriptor {
	...
	result.Vendorid = uint16(self.An(bus, device_2, function, 0x00))
	result.Deviceid = uint16(self.An(bus, device_2, function, 0x02))

	result.คลาสid = uint8(self.An(bus, device_2, function, 0x0b))
	result.subclassid = uint8(self.An(bus, device_2, function, 0x0a))
	result.interfaceid = uint8(self.An(bus, device_2, function, 0x09))

	result.revision = uint8(self.An(bus, device_2, function, 0x08))
	result.Interrupt = uint32(self.An(bus, device_2, function, 0x3C))

	return result
}
func (self *TPeripheralcomponentinterconnectcontroller) Getbaseaddressregister(bus uint16, device_2 uint16, function uint16, แถบ uint16) TBaseaddressregister {
	var result TBaseaddressregister

	headertype := self.An(bus, device_2, function, 0x0E) & 0x7F
	var maxbars int = int(6 - (4 * headertype))
	if แถบ >= uint16(maxbars) {
		return result
	}

	แถบvalue := self.An(bus, device_2, function, uint32(0x10+4*แถบ))
	...
	if result.regtype == 0 {
		...
		result.address_2 = แถบvalue & ^uint32(0x3)
