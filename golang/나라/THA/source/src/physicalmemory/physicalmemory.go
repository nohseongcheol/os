...
func (self *Physicalmemorymanager) Sกำหนดbit(bit uint32) uint32 {
...
func (self *Physicalmemorymanager) Tทดสอบbit(bit uint32) uint32 {
	ret := self.memoryarray[bit/32] & (1 << (bit % 32))
	return ret
}
func (self *Physicalmemorymanager) Tรวมblock() uint32 {
	return self.maximumblock
}
func (self *Physicalmemorymanager) Usedblock() uint32 {
	return self.usedblock
}
func (self *Physicalmemorymanager) Amountจากmemory() uint32 {
	return self.memoryขนาด
}
func (self *Physicalmemorymanager) Getbitmaspขนาด() uint32 {
	return self.memoryขนาด / Blockขนาด / Blockperbyte
}

func (self *Physicalmemorymanager) Firstfree() uint32 {
	for i := uint32(0); i < self.Tรวมblock(); i++ {
...
func (self *Physicalmemorymanager) Firstfreeขนาด(ขนาด uint32) uint32 {
	if ขนาด == 0 {
		return 0xffffffff
	}
	if ขนาด == 1 {
		...
		if self.memoryarray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryarray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= ขนาด; count++ {
						if self.Tทดสอบbit(startingbit+count) == 0 {
							free++
						}
...
func (self *Physicalmemorymanager) Init(ขนาด uint32, bitmap []uint32) {
	self.memoryขนาด = ขนาด
	self.memoryarray = bitmap
	self.maximumblock = ขนาด / Blockขนาด
	self.usedblock = self.maximumblock
	memoryoper.Memกำหนด(uintptr(Pointer(&self.memoryarray)), 0xFF, self.usedblock/Blockperbyte)
}
...
func (self *Physicalmemorymanager) Pageroundลง(address_2 uint32) uint32 {
