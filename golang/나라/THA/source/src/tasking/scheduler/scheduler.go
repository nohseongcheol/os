package scheduler

import . "unsafe"
import . "reflect"

import . "console"
import . "gdt"
import . "port"
import . "util/list"

import . "interrupt"
import . "tasking/thread"
import . "tasking/tss"
import . "multitasking"
import mem "memorymanager"

const Schedulerfrequency = 1
const Kernelheapstart = 1024 * 1024
const schedulerdebug = false
const pitfrequency = 100

var list Linkedlist

type Schedulerdata struct {
	frequency	uint32
	tickcount	uint32

	switchforced	bool

	Enabled	bool

	currentthread	*TThread
	tss		*Tssentry
}

var schedata Schedulerdata = Schedulerdata{}

func (self *Schedulerdata) Init() {
	schedata.tickcount = 0
	schedata.frequency = Schedulerfrequency
	schedata.currentthread = nil
	schedata.Enabled = false
	schedata.switchforced = false

}

var console_2 = TConsole{}
var currentthreadindex int = 0
var nextโพรเซสid uint32 = 1

func Allocatepid() uint32 {
	pid := nextโพรเซสid
	nextโพรเซสid++
	return pid
}

func (self *Schedulerdata) Getnextreadythread() *TThread {
	if list.Sขนาด_2 <= 0 {
		return nil
	}

	if schedata.currentthread != nil {
		currentthreadindex = list.Indexจาก(uintptr(Pointer(schedata.currentthread)))
		if currentthreadindex < 0 {
			currentthreadindex = 0
		}
	} else {
		currentthreadindex = -1
	}

	for checked := 0; checked < list.Sขนาด_2; checked++ {
		currentthreadindex++
		if currentthreadindex >= list.Sขนาด_2 {
			currentthreadindex = 0
		}
		thread := (*TThread)(list.Getat(currentthreadindex))
		if thread != nil && thread.Threadสถานะ != Blocked && thread.Threadสถานะ != Stopped {
			if schedulerdebug {
				console_2.MPrint("ti:")
				console_2.MUnsignedinteger32print(uint32(currentthreadindex))
				console_2.MPrint(":")
				console_2.MUnsignedinteger32print(uint32(uintptr(Pointer(thread))))
			}
			return thread
		}
	}
	return schedata.currentthread

}
func (self *Scheduler) Addthread(thread *TThread) {
	if thread == nil {
		return
	}
	list.Append_to_list(uintptr(Pointer(thread)))
}
func Addrunnablethread(thread *TThread) {
	if thread == nil {
		return
	}
	list.Append_to_list(uintptr(Pointer(thread)))
}

func Currentpid() uint32 {
	if schedata.currentthread == nil || schedata.currentthread.Pid == 0 {
		return 1
	}
	return schedata.currentthread.Pid
}

func Currentparentpid() uint32 {
	if schedata.currentthread == nil {
		return 0
	}
	return schedata.currentthread.Parentpid
}
func (self *Scheduler) Rลบthread(thread *TThread) {
	list.Rลบ(uintptr(Pointer(thread)))
}

func (self *Scheduler) Rลบthreadat(index int) {
	list.Rลบat(index)
}

type Scheduler struct {
	TInterrupthandler
}

func (self *Scheduler) Init(manager *TInterruptmanager, mem *mem.TMemorymanager, tss *Tssentry) {
	schedata.Init()
	schedata.tss = tss
	initpit(pitfrequency)

	list = Linkedlist{}
	list.Init(mem)
	console_2.MPrint("list:")
	console_2.MUnsignedinteger32print(uint32(uintptr(Pointer(&list))))

	interrupthandler = handleinterrupt
	var address uintptr
	address = uintptr(Pointer(&interrupthandler))
	self.TInterrupthandler.Init(0x20, uintptr(Pointer(manager)), address)
}

func (self *Scheduler) Enabled(enabled bool) {
	schedata.Enabled = enabled
}

func initpit(frequency uint32) {
	if frequency == 0 {
		return
	}
	divisor := uint32(1193180) / frequency
	Portkhianbyte(0x43, 0x36)
	Portkhianbyte(0x40, uint8(divisor&0xFF))
	Portkhianbyte(0x40, uint8((divisor>>8)&0xFF))
}

func กำหนดds(dssegment uint32)
func กำหนดgs(gssegment uint32)

func fxsave(uint32)
func fxrstor(uint32)

func backupfpregs(buffer_2 uintptr)
func restorefpregs(buffer_2 uintptr)

var jmpuser uint32 = 0
var interrupthandler func(uint32) uint32

func schedulestack(fn func())
func กำหนดcr3(address uint32)
func getcr3() uint32

func handleinterrupt(esp uint32) uint32 {

	schedata.tickcount++

	if schedulerdebug {
		console_2.MPrintxy(([]byte)("sche1:"), 1, 17)

		console_2.MPrint(":")
		console_2.MUnsignedinteger32print(esp)
		console_2.MPrint(":")

		console_2.MUnsignedinteger32print(uint32(schedata.tickcount))
		console_2.MPrint(":")
		console_2.MUnsignedinteger32print(Kernelheapstart)
	}

	if schedata.tickcount == schedata.frequency {
		schedata.tickcount = 0

		if list.Sขนาด_2 > 0 && schedata.Enabled == true {
			var nextthread = schedata.Getnextreadythread()
			if nextthread == nil {
				return esp
			}
			if schedata.currentthread == nil {
				MEmergencylogstring("\nSCHED first esp=")
				MEmergencylogunsignedinteger32(esp)
				MEmergencylogstring(" thread=")
				MEmergencylogunsignedinteger32(uint32(uintptr(Pointer(nextthread))))
				MEmergencylogstring(" cpu=")
				MEmergencylogunsignedinteger32(uint32(uintptr(Pointer(nextthread.Cpuสถานะ))))
				MEmergencylogstring(" state=")
				MEmergencylogunsignedinteger32(uint32(nextthread.Threadสถานะ))
				MEmergencylogstring(" eip=")
				MEmergencylogunsignedinteger32(nextthread.Cpuสถานะ.Eip)
				MEmergencylogstring(" cs=")
				MEmergencylogunsignedinteger32(nextthread.Cpuสถานะ.Cs)
				MEmergencylogstring("\n")
			}

			if esp >= Kernelheapstart && schedata.currentthread != nil {
				schedata.currentthread.Cpuสถานะ = (*Tcpuสถานะ)(Pointer(uintptr(esp)))

				address := uintptr(Pointer(&(schedata.currentthread.Fpubuffer)))
				offset := (16 - (address % 16)) & 0xF
				schedata.currentthread.Fpuoffset = offset
				backupfpregs(address + offset)
				if schedulerdebug {
					console_2.MPrint(([]byte)("backup"))
					console_2.MUnsignedinteger32print(esp)
				}
			}

			address := uintptr(Pointer(&(nextthread.Fpubuffer)))
			offset := nextthread.Fpuoffset
			if offset != 0xffffffff {
				restorefpregs(address + offset)
				if schedulerdebug {
					console_2.MPrint(([]byte)("restore"))
				}
			}

			schedata.currentthread = nextthread

			if schedata.currentthread.Threadสถานะ == Started {
				schedata.currentthread.Threadสถานะ = Ready

				Initialthreaduserjump(schedata.currentthread)
				return esp
			}

			esp = uint32(uintptr(Pointer(nextthread.Cpuสถานะ)))
			if nextthread.Stack != 0 {
				schedata.tss.Sกำหนดstack(Segkerneldata, nextthread.Stack+Threadstackขนาด)
			}

			กำหนดcr3(nextthread.Pagedirectoryentry)
			กำหนดgs(nextthread.Cpuสถานะ.Gs)

		}

	}

	return esp
}

func jumpusermodeiret(uint32, uint32, uint32, uint32, uint32, uint32)
func Disableint()

func getesp() uint32
func threadexitloop()

func กำหนดthreadexitloopสถานะ(cpuสถานะ *Tcpuสถานะ) {
	cpuสถานะ.Eip = uint32(ValueOf(threadexitloop).Pointer())
	cpuสถานะ.Cs = Segkernelcode
	cpuสถานะ.Ds = Segkerneldata
	cpuสถานะ.Es = Segkerneldata
	cpuสถานะ.Fs = Segkerneldata
	cpuสถานะ.Gs = Segkernelgs
	cpuสถานะ.Ss = Segkerneldata
	cpuสถานะ.Eflags = 0x202
}

func Stopcurrentthread(cpuสถานะ *Tcpuสถานะ) *Tcpuสถานะ {
	if schedata.currentthread == nil {
		กำหนดthreadexitloopสถานะ(cpuสถานะ)
		return cpuสถานะ
	}

	stoppedthread := schedata.currentthread
	for i := 0; i < list.Sขนาด_2; i++ {
		thread := (*TThread)(list.Getat(i))
		if thread != nil && thread.Cpuสถานะ == cpuสถานะ {
			stoppedthread = thread
			break
		}
	}
	stoppedthread.Cpuสถานะ = cpuสถานะ
	stoppedthread.Threadสถานะ = Stopped
	schedata.currentthread = stoppedthread

	nextthread := schedata.Getnextreadythread()
	if nextthread == nil || nextthread == stoppedthread || nextthread.Cpuสถานะ == nil || nextthread.Cpuสถานะ == cpuสถานะ {
		กำหนดthreadexitloopสถานะ(cpuสถานะ)
		return cpuสถานะ
	}

	schedata.currentthread = nextthread
	if nextthread.Stack != 0 && schedata.tss != nil {
		schedata.tss.Sกำหนดstack(Segkerneldata, nextthread.Stack+Threadstackขนาด)
	}
	กำหนดcr3(nextthread.Pagedirectoryentry)
	กำหนดgs(nextthread.Cpuสถานะ.Gs)
	return nextthread.Cpuสถานะ
}

func Initialthreaduserjump(thread *TThread) {

	Disableint()

	schedata.tss.Sกำหนดstack(Segkerneldata, thread.Stack+Threadstackขนาด)

	กำหนดcr3(thread.Pagedirectoryentry)
	กำหนดgs(thread.Cpuสถานะ.Gs)

	schedata.currentthread = thread
	schedata.Enabled = true

	eip := thread.Cpuสถานะ.Eip
	useresp := thread.Userstack_2 + thread.Userstackขนาด_2
	eflags_2 := thread.Cpuสถานะ.Eflags
	cs := thread.Cpuสถานะ.Cs
	esp := schedata.tss.Getesp0()

	console_2.MPrint(([]byte)("jump["))
	console_2.MUnsignedinteger32print(eip)
	console_2.MPrint(([]byte)(":"))
	console_2.MUnsignedinteger32print(useresp)
	console_2.MPrint(([]byte)(":"))
	console_2.MUnsignedinteger32print(eflags_2)
	console_2.MPrint(([]byte)(":"))
	console_2.MUnsignedinteger32print(cs)
	console_2.MPrint(([]byte)(":"))

	console_2.MUnsignedinteger32print(esp)
	console_2.MPrint(([]byte)("]"))

	userprocentry := thread.Cpuสถานะ.Ecx
	globaloffsetตาราง_2 := thread.Cpuสถานะ.Edx
	dynamic := thread.Cpuสถานะ.Esi

	Portkhianbyte(0x20, 0x20)
	jumpusermodeiret(eip, useresp, eflags_2, userprocentry, globaloffsetตาราง_2, dynamic)
	console_2.MPrint(([]byte)("usermode end"))
}
func printesp(esp uint32) {
	console_2.MPrint(([]byte)("esp["))
	console_2.MUnsignedinteger32print(esp)
}
