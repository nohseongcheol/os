...
func (self *Tssentry) Install(gdt *TShareddescriptorตาราง, idx int, kernelss uint32, kernelesp uint32) {
	...
	gdt.Sกำหนดdescriptor(idx, base, uint32(Sizeof(Tssentry{})), 0xE9, 0)
	self.ss0 = kernelss
	self.esp0 = kernelesp
	self.iomap = uint16(Sizeof(Tssentry{}))

	flushtss(Segtaskสถานะ)

}
func (self *Tssentry) Sกำหนดstack(kernelss uint32, kernelesp uint32) {
	self.ss0 = kernelss
	self.esp0 = kernelesp
}
