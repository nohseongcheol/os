...
type IWidget interface {
	...
	Sกำหนดcolor(r uint32, g uint32, b uint32)
...
func (self *TWidget) Sกำหนดcolor(r uint32, g uint32, b uint32) {
	self.r = r
	self.g = g
	self.b = b
}
...
func (self *TWidgetmouseeventhandler) Onmouseลง(button int8) {

	mousewidget.Set_local_coordinates(uint32(previousx), uint32(previousy))
	mousewidget.Sกำหนดcolor(0xA8, 0x00, 0x00)
...
func (self *TWidgetmouseeventhandler) Onmousemove(x int8, y int8) {
	...
	mousewidget.Set_local_coordinates(uint32(previousx), uint32(previousy))
	mousewidget.Sกำหนดcolor(0x00, 0x00, 0x00)
	mousewidget.Draw(&mousevga)

	mousewidget.Set_local_coordinates(uint32(xposition), uint32(yposition))
	mousewidget.Sกำหนดcolor(0x00, 0x00, 0xA8)
	mousewidget.Draw(&mousevga)
