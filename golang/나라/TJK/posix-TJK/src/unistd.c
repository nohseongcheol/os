...
ssize_t Хондан(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Хондан, fd, buf, count));
}

ssize_t Навиштан(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Навиштан, fd, buf, count));
}

int Хуруҷ(int fd)
{
    return (int)__syscall_result(SC1(SYS_Хуруҷ, fd));
}
