...
{
    (void)Навиштан(STDOUT_FILENO, text, text_length(text));
}
...
static void test_open_and_io(void)
{
    ...
    fd = Кушодан("/USER2", O_RDONLY);
    report("open-readonly", fd >= 3);
    if (fd >= 0) {
        report("read-bytes", Хондан(fd, bytes, 2) == 2 &&
        ...
        report("close-file", Хуруҷ(fd) == 0);
        errno = 0;
        report("closed-fd", failed_with(Хуруҷ(fd), EBADF));
    }

    root = Кушодан("/", O_RDONLY | O_DIRECTORY);
    report("open-directory", root >= 3);
    if (root >= 0) {
        report("fstat-directory", fstat(root, &st) == 0 && S_ISDIR(st.st_mode));
        errno = 0;
        report("read-directory", Хондан(root, bytes, 1) == -1 && errno == EISDIR);
        (void)Хуруҷ(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", Навиштан(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", Хондан(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", Хондан(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", Навиштан(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("open-enoent", failed_with(Кушодан("/NOFILE", O_RDONLY), ENOENT));
    errno = 0;
    report("open-efault", failed_with(Кушодан((const char *)0, O_RDONLY), EFAULT));
    errno = 0;
    report("open-write-erofs", failed_with(Кушодан("/USER2", O_WRONLY), EROFS));
    errno = 0;
    report("open-rdwr-erofs", failed_with(Кушодан("/USER2", O_RDWR), EROFS));
    errno = 0;
    report("open-create-erofs", failed_with(Кушодан("/NEWFILE", O_CREAT | O_WRONLY, 0600), EROFS));
    errno = 0;
    report("open-trunc-erofs", failed_with(Кушодан("/USER2", O_TRUNC | O_RDONLY), EROFS));
    errno = 0;
    report("open-append-erofs", failed_with(Кушодан("/USER2", O_APPEND | O_RDONLY), EROFS));
    errno = 0;
    report("open-enotdir", failed_with(Кушодан("/USER2", O_RDONLY | O_DIRECTORY), ENOTDIR));
    errno = 0;
    report("creat-erofs", failed_with(creat("/NEWFILE", 0600), EROFS));
    errno = 0;
    report("close-ebadf", failed_with(Хуруҷ(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", Хондан(fd, &first, 1) == 1 && Хондан(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Хуруҷ(fd) == 0 && fstat(copy, &st) == 0);
        fd = copy;
    }
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &status) == child) {
        report("fork-fd-isolation", Хондан(fd, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
