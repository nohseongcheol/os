...
type IEthernetframehandler interface {
	...
	Ирсолкунед(destinationmacbe uint64, datapointer uintptr, size uint32)
	FrameИрсолкунед(destinationmacbe uint64, ethernettypebe uint16, datapointer uintptr, size uint32)
...
func (self *TEthernetframehandler) Ethernetframereceivewhen(datapointer uintptr, size int) bool {
	ethernetconsole.MЧопкардан(([]byte)("OnEtherFrameReceived"))
	return false
}
func (self *TEthernetframehandler) Ирсолкунед(destinationmacbe uint64, datapointer uintptr, size uint32) {
	Backend.FrameИрсолкунед(destinationmacbe, frame.ethernettypebe, datapointer, size)
}
func (self *TEthernetframehandler) FrameИрсолкунед(destinationmacbe uint64, ethernettypebe uint16, datapointer uintptr, size uint32) {
	Backend.FrameИрсолкунед(destinationmacbe, ethernettypebe, datapointer, size)
}
...
func (self *TEthernetframerawdatahandler) Ирсолкунед(datapointer uintptr, size uint32) {
	provider.Ирсолкунед(datapointer, size)
}
...
func (self *TEthernetframeprovider) Onrawdatareceive(datapointer uintptr, size int) bool {
	...
	if frame.destinationmacbe == 0x0000FFFFFFFFFFFF || Unsignedinteger48r(frame.destinationmacbe) == self.Getmacaddress() {
		if handler_2[frame.ethernettypebe] != nil {
			ethernetconsole.MЧопкардан(([]byte)("provider\n"))
	...
	ethernetconsole.MЧопкарданxy(([]byte)("spro["), 0, 1)
	ethernetconsole.MUnsignedinteger64Чопкардан(frame.sourcemacbe)
	ethernetconsole.MЧопкардан(([]byte)(":"))
	ethernetconsole.MUnsignedinteger64Чопкардан(frame.destinationmacbe)
	ethernetconsole.MЧопкардан(([]byte)(":]["))
	ethernetconsole.MUnsignedinteger64Чопкардан(self.Getmacaddress())
	ethernetconsole.MЧопкардан(([]byte)(":"))
	ethernetconsole.MUnsignedinteger16Чопкардан(frame.ethernettypebe)
	ethernetconsole.MЧопкардан(([]byte)("]"))

	return reply

}
func (self *TEthernetframeprovider) Ирсолкунед(datapointer uintptr, size uint32) {
	self.networkcard.Ирсолкунед(datapointer, size)
}
func (self *TEthernetframeprovider) FrameИрсолкунед(destinationmacbe uint64, ethernettypebe uint16, datapointer uintptr, size uint32) {
	...
	self.networkcard.Ирсолкунед(pointer, size+uint32(frameheadersize))
