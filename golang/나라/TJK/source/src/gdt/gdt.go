...
const (
	...
	SegИстифодакунандаcode	uint32	= 0x23
	SegИстифодакунандаdata	uint32	= 0x2B
	SegИстифодакунандаgs	uint32	= 0x33
...
func (self_2 *TShareddescriptortable) Init() {
	...
	terminal.MЧопкарданxy("gdt:", 1, 4)
	terminal.MUnsignedinteger32Чопкардан(uint32(uintptr(Pointer(&data_2.segmentdescriptor[3]))))
...
type Истифодакунандаdescriptor struct {
	Entrynumber	uint32
	Baseaddress	uint32
	Limit		uint32
	Flags		uint8
...
func Settlssegment(index uint32, descriptor *Истифодакунандаdescriptor, table []Gdtentry_2) bool {
	...
	access := uint8(PrivИстифодакунанда | SegОддӣ | Present)
