...
func (self *TInternetcontrolmessageprotocol) Internetprotocolreceivewhen(sourceipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, datapointer uintptr, size uint32) bool {
	...
	icmpconsole.MЧопкардан(([]byte)("icmp:OnInternet"))
	icmpconsole.MUnsignedinteger16Чопкардан(uint16(msg.TypeValue))
	icmpconsole.MЧопкардан(([]byte)(":"))

	switch msg.TypeValue {
	case 0:
		icmpconsole.MЧопкардан(([]byte)("ping response from "))
		break

	case 8:
		icmpconsole.MЧопкардан(([]byte)("ping send "))
		msg.TypeValue = 0
...
func (self *TInternetcontrolmessageprotocol) EchorequestИрсолкунед(ipnetworkbyteorder uint32) bool {
	...
	iphandler.Ирсолкунед(ipnetworkbyteorder, 0x01, datapointer, uint32(icmpsize))
