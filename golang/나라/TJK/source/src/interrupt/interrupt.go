...
func TestЧопкардан(position uint8, data uint8)
func setds(dssegment uint32)
func setgs(gssegment uint32)
func interruptХуруҷloop()
...
func Навinterrupthandler(Interruptmanager uintptr, Interruptnumber uint8) *TInterrupthandler {
...
func (self *TInterruptmanager) Init(hardwareinterruptoffset uint16, умумӣdescriptortable *TShareddescriptortable, taskmanager *TTaskmanager) {
	...
	PortНавиштанbyte(Primarypiccommandioport, 0x11)
	PortНавиштанbyte(Secondarypiccommandioport, 0x11)

	PortНавиштанbyte(Primarypicdataioport, 0x20)
	PortНавиштанbyte(Secondarypicdataioport, 0x28)

	PortНавиштанbyte(Primarypicdataioport, 0x04)
	PortНавиштанbyte(Secondarypicdataioport, 0x02)

	PortНавиштанbyte(Primarypicdataioport, 0x01)
	PortНавиштанbyte(Secondarypicdataioport, 0x01)

	PortНавиштанbyte(Primarypicdataioport, 0xF8)
	PortНавиштанbyte(Secondarypicdataioport, 0xEF)
...
func (self *TInterruptmanager) Dohandleinterrupt(interrupt uint8, esp uint32) uint32 {
	...
	if interruptdebug {
		console_2.MЧопкарданxy("[esp:", 1, 20)
		console_2.MUnsignedinteger32Чопкардан(uint32(interrupt))
	...
	if 0x20 <= interrupt && interrupt < 0x30 {
		if 0x28 <= interrupt {
			PortНавиштанbyte(Secondarypiccommandioport, 0x20)
		}
		PortНавиштанbyte(Primarypiccommandioport, 0x20)
	}
	return esp
}
...
func Handleinterrupt(esp uint32, interrupt uint32) uint32 {
	...
	if interruptdebug && interrupt != 0x80 && interrupt != 0x20 {
		console_2.MЧопкарданxy("[esp:", 1, 21)
		console_2.MUnsignedinteger32Чопкардан(uint32(interrupt))
		console_2.MЧопкардан(":")
		console_2.MUnsignedinteger32Чопкардан(esp)
	}
...
func handleunhandledsyscall(esp uint32) uint32 {
	...
	if cpu.Eax == 1 || cpu.Eax == 252 {
		cpu.Eip = uint32(ValueOf(interruptХуруҷloop).Pointer())
...
func exceptionhasХатоcode(interrupt uint32) bool {
	switch interrupt {
...
func exceptionНом(interrupt uint32) string {
	switch interrupt {
	case 0x00:
...
func чопкарданpagefaultinfo(err uint32) {
...
func чопкарданexceptionselectorinfo(err uint32) {
...
func Handleexception(esp uint32, interrupt uint32) uint32 {
	...
	if exceptionhasХатоcode(interrupt) {
		чопкарданexceptionselectorinfo(err)
	}
	MEmergencylogstring("\n=== ENGOS HALTED AFTER EXCEPTION ===\n")
	except++
...
func interrupthandler_2(interrupt uint8, esp uint32) uint32 {
	...
	console_2 := TConsole{}
	console_2.MЧопкардан(buffer)
	console_2.MHexadecimalЧопкардан(0x40)
	return esp
}
func чопкарданesp(esp uint32) {
	console_2 := TConsole{}
	console_2.MUnsignedinteger32Чопкарданxy(esp, 20, 21)
}
func gettls() uint32
func Чопкарданtls() {
