...
type IInternetprotocolhandler interface {
	...
	Ирсолкунед(destinationipaddressnetworkbyteorder uint32, pprotocol uint8, datapointer uintptr, size uint32)
...
func (self *TInternetprotocolhandler) Internetprotocolreceivewhen(sourceipaddressnetworkbyteorder uint32, destinationipaddressnetworkbyteorder uint32, datapointer uintptr, size uint32) bool {
	ipconsole.MЧопкардан(([]byte)("ipHandler:OnInternet"))
	return false
}
func (self *TInternetprotocolhandler) Ирсолкунед(destinationipaddressnetworkbyteorder uint32, pprotocol uint8, datapointer uintptr, size uint32) {

	ipprovider.Ирсолкунед(destinationipaddressnetworkbyteorder, pprotocol, datapointer, size)
}
...
func (self *Ipethernetframehandler) Ethernetframereceivewhen(datapointer uintptr, size int) bool {
	ipconsole.MЧопкардан(([]byte)("iphandler:onEtherfameRecv\n"))
	return ipprovider.Ethernetframereceivewhen(datapointer, uint32(size))

}

func (self *Ipethernetframehandler) Ирсолкунед(destinationipaddressnetworkbyteorder uint64, datapointer uintptr, size uint32) {
	ipconsole.MЧопкардан(([]byte)("ipefhandler:send\n"))
	var ethernettypebe = Unsignedinteger16r(0x0800)
	self.TEthernetframehandler.FrameИрсолкунед(destinationipaddressnetworkbyteorder, ethernettypebe, datapointer, size)
...
func (self *TInternetprotocolprovider) Ethernetframereceivewhen(ethernetframepayload uintptr, size uint32) bool {
	...
	ipconsole.MЧопкардан(([]byte)("ipmessage"))
	ipconsole.MUnsignedinteger32Чопкардан(internetprotocolmessage.sourceipaddress)
	ipconsole.MЧопкардан(([]byte)(":"))
	ipconsole.MUnsignedinteger32Чопкардан(internetprotocolmessage.destinationipaddress)
	ipconsole.MЧопкардан(([]byte)(":"))
	ipconsole.MUnsignedinteger16Чопкардан(uint16(internetprotocolmessage.headerlength))
	ipconsole.MЧопкардан(([]byte)(":"))
	ipconsole.MUnsignedinteger16Чопкардан(uint16(internetprotocolmessage.version))
	ipconsole.MЧопкардан(([]byte)(":"))
	ipconsole.MUnsignedinteger16Чопкардан(internetprotocolmessage.totallength)
	ipconsole.MЧопкардан(([]byte)(":"))
	ipconsole.MUnsignedinteger32Чопкардан(uint32(efhandler.Getipaddress()))
	ipconsole.MЧопкардан(([]byte)(":"))
	ipconsole.MЧопкардан(([]byte)("\n"))
...
func (self *TInternetprotocolprovider) Ирсолкунед(destinationipaddressnetworkbyteorder uint32, protocol uint8, datapointer uintptr, size uint32) {
	...
	ipconsole.MЧопкарданxy(([]byte)("ipprovider:send["), 1, 18)
	for i := 0; i < int(size)+int(ipsize); i++ {
		ipconsole.MHexadecimalЧопкардан(buffer1_2[i])
	}
	ipconsole.MЧопкардан(([]byte)(":"))
	ipconsole.MЧопкардан(([]byte)("]\n"))

	var навбатӣhopipaddressnetworkbyteorder uint32 = destinationipaddressnetworkbyteorder
	if (destinationipaddressnetworkbyteorder & self.Subnetmask) != (message.sourceipaddress & self.Subnetmask) {
		навбатӣhopipaddressnetworkbyteorder = self.Gatewayip
	}

	var ирсолкунедdatapointer = uintptr(Pointer(&buffer1_2))
	ipconsole.MUnsignedinteger32Чопкардан(навбатӣhopipaddressnetworkbyteorder)

	var ethernettypebe = Unsignedinteger16r(0x0800)
	efhandler.FrameИрсолкунед(self.arpprovider.Resolve(навбатӣhopipaddressnetworkbyteorder), ethernettypebe, ирсолкунедdatapointer, uint32(ipsize)+uint32(size))
