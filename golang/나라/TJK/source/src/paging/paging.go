...
type PageФеҳрастentry_2 uintptr

const (
	Pagepresent		uint32	= 0x001
	Pagewritable		uint32	= 0x002
	PageИстифодакунанда	uint32	= 0x004
...
func setcr3(pageФеҳраст uint32)
func getcr3() uint32
...
func (self *Paging) Init(pageФеҳрастentry uintptr, pagetableentry uint32, memorymanager *TMemorymanager) {

	PageФеҳрастentry = pageФеҳрастentry
	...
	for i := uint32(0); i <= virtlen; i++ {
		...
		for pde := uint32(0); pde < pdelen; pde++ {
			...
			Setunsignedinteger32ataddress(address|0x87, uint32(pageФеҳрастentry)+pde*4)

			for pte := uint32(0); pte < 1024; pte++ {
				Setunsignedinteger32ataddress((((pde+i*pdelen)*1024+pte)*0x1000)|0x117, address+pte*4)
			}
		}
		pageФеҳрастentry = pageФеҳрастentry + 0x1000
	}
...
func (self *Paging) Sharedmemoryregion() {
	...
	pageФеҳрастentry := PageФеҳрастentry
	kpageФеҳрастentry := PageФеҳрастentry
	...
	for i := uint32(1); i <= virtlen; i++ {
		...
		for ; pde < 12; pde++ {
			v := Getvalue(uint32(kpageФеҳрастentry) + pde*4)
			v = (v & 0xFFFFF000)
			Setunsignedinteger32ataddress(v|0x87, uint32(pageФеҳрастentry)+pde*4)
...
func handlepaginginterrupt(esp uint32) uint32 {
	if ResolveНусхабардоштанonНавиштанfault() {
		return esp
	}
	return Handlefatalinterruptframe(esp, 0x0E)
}

func Cloneaddressspacecow(sourcepageФеҳраст uint32) uint32 {
	if Activememorymanager == nil || sourcepageФеҳраст == 0 {
		return 0
	}
	...
	destinationpageФеҳраст := uint32(uintptr(destinationpointer))
	for i := uint32(0); i < 1024; i++ {
		Setunsignedinteger32ataddress(0, destinationpageФеҳраст+i*4)
	}

	for pde := uint32(0); pde < 1024; pde++ {
		sourcepdeaddress := sourcepageФеҳраст + pde*4
		sourcepde := Getvalue(sourcepdeaddress)
		if (sourcepde & Pagepresent) == 0 {
			continue
		}
		if issharedpde(pde) {
			Setunsignedinteger32ataddress(sourcepde, destinationpageФеҳраст+pde*4)
			continue
		}
		...
		Setunsignedinteger32ataddress((destinationpt | (sourcepde & 0xFFF)), destinationpageФеҳраст+pde*4)
	...
	reloadcr3()
	return destinationpageФеҳраст
}

func ResolveНусхабардоштанonНавиштанfault() bool {
	if Activememorymanager == nil {
		return false
	}
	faultaddress := getcr2()
	pageФеҳраст := getcr3()
	pdeaddress := pageФеҳраст + ((faultaddress>>22)&0x3FF)*4
	...
	навpointer, _ := Activememorymanager.Alignedmalloc(0x1000)
	if навpointer == nil {
		return false
	}
	навframe := uint32(uintptr(навpointer)) & Pageframe

	source_2 := Getbytesfrompointer(uintptr(faultaddress&Pageframe), 0x1000, 0x1000)
	destination_2 := Getbytesfrompointer(uintptr(навframe), 0x1000, 0x1000)
	copy(destination_2, source_2)
	cowframemanager.Decrement(oldframe)
	Setunsignedinteger32ataddress((навframe|(pte&0xFFF)|Pagewritable)&^Pagecow, pteaddress)
	reloadcr3()
	return true
}
...
func SetbyteinpageФеҳраст(x byte, address uint32, pageФеҳраст uint32) {
	oldcr3 := getcr3()
	setcr3(pageФеҳраст)
...
func SetblockinpageФеҳраст(source_2 []byte, destination_2 []byte, size uint32, pageФеҳраст uint32) {
	if size == 0 || pageФеҳраст == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(pageФеҳраст)
	makerangeprivatewritablecurrent(pageФеҳраст, uint32(uintptr(unsafe.Pointer(&destination_2[0]))), size)
...
func ZeroblockinpageФеҳраст(address uint32, size uint32, pageФеҳраст uint32) {
	if size == 0 || pageФеҳраст == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(pageФеҳраст)
	makerangeprivatewritablecurrent(pageФеҳраст, address, size)
...
func makepageprivatewritablecurrent(pageФеҳраст uint32, virtualaddress uint32) bool {
	pde := Getvalue(pageФеҳраст + ((virtualaddress>>22)&0x3FF)*4)
	if (pde & Pagepresent) == 0 {
		return false
	}
...
func makerangeprivatewritablecurrent(pageФеҳраст uint32, address uint32, size uint32) bool {
	if size == 0 {
		return true
	}
	last := address + size - 1
	...
	for page := address & Pageframe; ; page += 0x1000 {
		if !makepageprivatewritablecurrent(pageФеҳраст, page) {
			return false
		}
...
func Makerangeprivatewritable(pageФеҳраст uint32, address uint32, size uint32) bool {
	if pageФеҳраст == 0 {
		return false
	}
	oldcr3 := getcr3()
	setcr3(pageФеҳраст)
	ok := makerangeprivatewritablecurrent(pageФеҳраст, address, size)
	setcr3(oldcr3)
	return ok
}

func Setunsignedinteger32inpageФеҳраст(x uint32, address uint32, pageФеҳраст uint32) {
	if pageФеҳраст == 0 {
		return
	}
	oldcr3 := getcr3()
...
func GetvalueinpageФеҳраст(address uint32, pageФеҳраст uint32) uint32 {
	if pageФеҳраст == 0 {
		return 0
	}
	oldcr3 := getcr3()
...
func Нусхабардоштанpageframeblock(xpageФеҳраст uint32, ypageФеҳраст uint32, vaddress uint32) {
	if xpageФеҳраст == 0 || ypageФеҳраст == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(xpageФеҳраст)
	v = Getvalue(vaddress)
	Setunsignedinteger32inpageФеҳраст(v, vaddress, ypageФеҳраст)
