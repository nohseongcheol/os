...
type кушоданФайлТасвирот struct {
	used		bool
	refs		uint32
	kind		uint32
	flags		uint32
...
const (
	fdkindҒайри		uint32	= 0
	fdkindfat		uint32	= 1
	fdkindstdin		uint32	= 2
	fdkindconsole		uint32	= 3
	fdkindrootФеҳраст	uint32	= 4
	fdkindsocket		uint32	= 5

	oХонданonly	uint32	= 0
	oНавиштанonly	uint32	= 1
	oХонданНавиштан	uint32	= 2
	ocreate		uint32	= 0x40
	otruncate	uint32	= 0x200
	oappend		uint32	= 0x400
	oФеҳраст	uint32	= 0x10000

	seekset		uint32	= 0
	seekcurrent	uint32	= 1
	seekОхириҳуҷҷат	uint32	= 2
...
func syscallХато(err int32) uint32 {
	return *(*uint32)(Pointer(&err))
}

var кушоданФайлtable [maxКушоданfiles]кушоданФайлТасвирот
var processtable [32]processentry
var localsockets [maxsockets]localdatagramsocket
var навбатӣephemeralport uint16 = 49152

const (
	истифодакунандаheapbase		uint32	= 0x06000000
	истифодакунандаheaplimit	uint32	= 0x07000000
)

var stdinbuffer [128]byte
var stdinХондан uint32
var stdinНавиштан uint32

func Interrupt(eax, ebx, ecx, edx, esi, edi uint32) uint32

func SysХуруҷ_4(index uint32) {
	Syscall(SysХуруҷ_2, index)
}

func SysХондан_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(SysХондан, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func SysЧопкарданstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(SysНавиштан, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}

func SysЧопкарданunsignedinteger32(buffer uint32) {
	Syscall(9, buffer)
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(SysНавиштан, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func SysКушодан_2(path uintptr, flags uint32, mode uint32) int32 {
	return int32(Syscall(SysКушодан, uint32(path), flags, mode))
}

func SysХуруҷ_3(fd uint32) int32 {
	return int32(Syscall(SysХуруҷ, fd))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case SysХуруҷ_2:
		sysХуруҷ_2(cpu.Ebx)
		return uint32(uintptr(Pointer(Манъcurrentthread(cpu))))
	case SysrtХуруҷ:
		sysХуруҷ_2(cpu.Ebx)
		return uint32(uintptr(Pointer(Манъcurrentthread(cpu))))
	case Sysfork:
		...
		return esp
	case SysХондан:
		cpu.Eax = uint32(sysХондан(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case SysНавиштан:
		cpu.Eax = uint32(sysНавиштан(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case SysКушодан:
		cpu.Eax = uint32(sysКушодан(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysКушодан(cpu.Ebx, ocreate|oНавиштанonly|otruncate, cpu.Ecx))
		return esp
	case SysХуруҷ:
		cpu.Eax = uint32(sysХуруҷ(int32(cpu.Ebx)))
		return esp
	case Syswaitpid:
	...
	case Sysbrk:
		...
		return esp
	case 9:
		console_2.MUnsignedinteger32Чопкардан(cpu.Ebx)
		return esp

	default:
		console_2.MЧопкарданxy(([]byte)("sys["), 1, 23)
		console_2.MUnsignedinteger32Чопкардан(esp)
		console_2.MЧопкардан(([]byte)(":"))
		console_2.MUnsignedinteger32Чопкардан(cpu.Eax)
		console_2.MЧопкардан(([]byte)(":"))
		console_2.MUnsignedinteger32Чопкардан(cpu.Ebx)
		console_2.MЧопкардан(([]byte)(":"))
		console_2.MUnsignedinteger32Чопкардан(cpu.Ecx)
		console_2.MЧопкардан(([]byte)(":"))
		console_2.MUnsignedinteger32Чопкардан(cpu.Edx)
		console_2.MЧопкардан(([]byte)("]"))
		cpu.Eax = syscallХато(Enosys)
		return esp
	}
...
func initФайлdescriptor() {
	for i := 0; i < maxКушоданfiles; i++ {
		кушоданФайлtable[i] = кушоданФайлТасвирот{}
	}
	...
	навбатӣephemeralport = 49152
	кушоданФайлtable[0] = кушоданФайлТасвирот{used: true, kind: fdkindstdin, flags: oХонданonly}
	кушоданФайлtable[1] = кушоданФайлТасвирот{used: true, kind: fdkindconsole, flags: oНавиштанonly}
	кушоданФайлtable[2] = кушоданФайлТасвирот{used: true, kind: fdkindconsole, flags: oНавиштанonly}
}

func ёфтанprocess(pid uint32) *processentry {
	for i := 0; i < len(processtable); i++ {
...
func initializeprocessfds(process *processentry) {
	for fd := int32(0); fd <= stderrfd; fd++ {
		process.fds[fd] = fdentry{used: true, тасвирот: fd}
		кушоданФайлtable[fd].refs++
	}
}

func ensurecurrentprocess() *processentry {
	pid := Currentpid()
	if process := ёфтанprocess(pid); process != nil {
		return process
	}
	for i := 0; i < len(processtable); i++ {
		if !processtable[i].used {
			processtable[i] = processentry{
				...
				programbreak:	истифодакунандаheapbase,
			}
...
func getКушоданФайлfor(process *processentry, fd int32) *кушоданФайлТасвирот {
	if process == nil || fd < 0 || fd >= maxfd || !process.fds[fd].used {
		return nil
	}
	тасвирот := process.fds[fd].тасвирот
	if тасвирот < 0 || тасвирот >= maxКушоданfiles || !кушоданФайлtable[тасвирот].used {
		return nil
	}
	return &кушоданФайлtable[тасвирот]
}

func getКушоданФайл(fd int32) *кушоданФайлТасвирот {
	return getКушоданФайлfor(ensurecurrentprocess(), fd)
}

func allocateКушоданФайл() int32 {
	for i := int32(3); i < maxКушоданfiles; i++ {
		if !кушоданФайлtable[i].used {
			кушоданФайлtable[i] = кушоданФайлТасвирот{used: true, refs: 1}
			return i
		}
	}
	return Enfile
...
func allocatefd(process *processentry, тасвирот int32, minimum int32) int32 {
	if process == nil {
		return Enfile
	}
	...
	for fd := minimum; fd < maxfd; fd++ {
		if !process.fds[fd].used {
			process.fds[fd] = fdentry{used: true, тасвирот: тасвирот}
			return fd
		}
	}
	return Emfile
...
func releaseКушоданФайл(тасвирот int32) {
	if тасвирот < 0 || тасвирот >= maxКушоданfiles {
		return
	}
	entry := &кушоданФайлtable[тасвирот]
	if entry.refs > 0 {
		entry.refs--
	}

	if entry.refs == 0 && тасвирот > stderrfd {
		if entry.kind == fdkindsocket && entry.aux < maxsockets {
			localsockets[entry.aux] = localdatagramsocket{}
		}
		*entry = кушоданФайлТасвирот{}
	}
}

func хуруҷprocessfd(process *processentry, fd int32) int32 {
	if process == nil || getКушоданФайлfor(process, fd) == nil {
		return Ebadf
	}
	тасвирот := process.fds[fd].тасвирот
	process.fds[fd] = fdentry{}
	releaseКушоданФайл(тасвирот)
	return 0
}

func sysНавиштан(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	...
	entry := getКушоданФайл(fd)
	if entry == nil {
		return Ebadf
	}
	if entry.kind != fdkindconsole {
		if entry.kind == fdkindsocket {
			return socketИрсолкунедto(fd, address, count, 0, 0)
		}
		if entry.kind == fdkindfat || entry.kind == fdkindrootФеҳраст {
			return Erofs
		}
		return Ebadf
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	console_2.MЧопкардан(buffer)
	return int32(count)
}

func sysХондан(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return хонданstdin(address, count)
	}
	if entry.kind == fdkindrootФеҳраст {
		return Eisdir
	}
	if entry.kind == fdkindsocket {
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return хонданvfsФайл(entry, buffer, count)
}

func sysКушодан(pathaddress uint32, flags uint32, mode uint32) int32 {
	_ = mode
	if pathaddress == 0 {
		return Efault
	}
	accessmode := flags & 3
	if accessmode == oНавиштанonly || accessmode == oХонданНавиштан || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
	...
	if isrootpath(pathaddress) {
		entry.kind = fdkindrootФеҳраст
		entry.size = 0
	} else {
		номlen, ном := нусхабардоштанpath(pathaddress)
		if номlen == 0 {
			*entry = кушоданФайлТасвирот{}
			return Enoent
		}
		size := файлsize(ном[:номlen])
		if size == 0 {
			*entry = кушоданФайлТасвирот{}
			return Enoent
		}
	...
	fd := allocatefd(process, тасвирот, 3)
	if fd < 0 {
		*entry = кушоданФайлТасвирот{}
		return fd
	}
...
func sysХуруҷ(fd int32) int32 {
	return хуруҷprocessfd(ensurecurrentprocess(), fd)
}

func sysdup(fd int32, minimum int32) int32 {
	process := ensurecurrentprocess()
	entry := getКушоданФайлfor(process, fd)
	if entry == nil {
		return Ebadf
	}
	навfd := allocatefd(process, process.fds[fd].тасвирот, minimum)
	if навfd >= 0 {
		entry.refs++
	}
	return навfd
...
func sysdup2(oldfd int32, навfd int32) int32 {
	process := ensurecurrentprocess()
	entry := getКушоданФайлfor(process, oldfd)
	if entry == nil {
		return Ebadf
	}
	if навfd < 0 || навfd >= maxfd {
		return Ebadf
	}
	if oldfd == навfd {
		return навfd
	}
	if process.fds[навfd].used {
		хуруҷprocessfd(process, навfd)
	}
	process.fds[навfd] = fdentry{used: true, тасвирот: process.fds[oldfd].тасвирот}
	entry.refs++
	return навfd
}
...
func хонданvfsФайл(entry *кушоданФайлТасвирот, destination_2 []byte, count uint32) int32 {
	...
	хонданФайл(entry.ном[:entry.номlen], tmp)
...
func sysaccess(pathaddress uint32, mode uint32) int32 {
	...
	if !exists {
		номlen, ном := нусхабардоштанpath(pathaddress)
		exists = номlen != 0 && файлsize(ном[:номlen]) != 0
	}
	if !exists {
		return Enoent
	}
...
func sysstat(pathaddress uint32, stataddress uint32) int32 {
	...
	inode := uint32(2)
	for i := uint32(0); i < номlen; i++ {
		inode = inode*33 + uint32(ном[i])
	}
...
func sysbrk(address_2 uint32) uint32 {
	...
	if process.programbreak == 0 {
		process.programbreak = истифодакунандаheapbase
	}
	if address_2 == 0 {
		return process.programbreak
	}
	if address_2 < истифодакунандаheapbase || address_2 > истифодакунандаheaplimit {
		return process.programbreak
	}
...
func нусхабардоштанutsfield(destination *[65]byte, value string) {
	limit := len(value)
	if limit > 64 {
		limit = 64
	}
...
func sysuname(address_2 uint32) int32 {
	...
	ном := (*posixutsname)(Pointer(uintptr(address_2)))
	*ном = posixutsname{}
	нусхабардоштанutsfield(&ном.Sysname, "EngOS")
	нусхабардоштанutsfield(&ном.Nodename, "engos")
	нусхабардоштанutsfield(&ном.Release, "0.1-posix")
	нусхабардоштанutsfield(&ном.Version, "POSIX.1-2017 phase 1")
	нусхабардоштанutsfield(&ном.Machine, "i386")
	return 0
}
...
func allocatesocket(домен uint32, sockettype uint32, protocol uint32) int32 {
	if домен != afinet {
		return Eafnosupport
	}
...
func bindephemeral(socket *localdatagramsocket) int32 {
	for attempts := 0; attempts < 16384; attempts++ {
		port := swapunsignedinteger16(навбатӣephemeralport)
		навбатӣephemeralport++
		if навбатӣephemeralport < 49152 {
			навбатӣephemeralport = 49152
		}
...
func socketИрсолкунедto(fd int32, bufferaddress_2 uint32, length uint32, destinationaddress uint32, destinationlength uint32) int32 {
	socket, err := socketforfd(fd)
	if err != 0 {
		return err
	}
	...
	if destinationaddress != 0 {
		address_2, addressХато := socketaddress(destinationaddress, destinationlength)
		if addressХато != 0 {
			return addressХато
		}
	...
	if !socket.bound {
		if bindХато := bindephemeral(socket); bindХато != 0 {
			return bindХато
		}
	}
...
func socketreceivefrom(fd int32, bufferaddress_2 uint32, length uint32, sourceaddress uint32, sourcelengthaddress uint32) int32 {
	...
	нусхабардоштанlength := packet.size
	if нусхабардоштанlength > length {
		нусхабардоштанlength = length
	}
	if нусхабардоштанlength != 0 {
		destination := Getbytesfrompointer(uintptr(bufferaddress_2), int(нусхабардоштанlength), int(нусхабардоштанlength))
		copy(destination, packet.data[:нусхабардоштанlength])
	}
	if sourceaddress != 0 {
	...
	*packet = socketpacket{}
	...
	socket.count--
	return int32(нусхабардоштанlength)
}

func нусхабардоштанsocketНом(fd int32, address_2 uint32, lengthaddress uint32, peer bool) int32 {
	socket, err := socketforfd(fd)
	if err != 0 {
		return err
	}
...
func syssocketcall(call uint32, arguments_2 uint32) int32 {
	...
	case 4, 5:
		return Eopnotsupp
	case 6:
		return нусхабардоштанsocketНом(int32(socketcallargument(arguments_2, 0)), socketcallargument(arguments_2, 1), socketcallargument(arguments_2, 2), false)
	case 7:
		return нусхабардоштанsocketНом(int32(socketcallargument(arguments_2, 0)), socketcallargument(arguments_2, 1), socketcallargument(arguments_2, 2), true)
	case 9:
		return socketИрсолкунедto(int32(socketcallargument(arguments_2, 0)), socketcallargument(arguments_2, 1), socketcallargument(arguments_2, 2), 0, 0)
	case 10:
	...
	case 11:
		return socketИрсолкунедto(int32(socketcallargument(arguments_2, 0)), socketcallargument(arguments_2, 1), socketcallargument(arguments_2, 2), socketcallargument(arguments_2, 4), socketcallargument(arguments_2, 5))
	case 12:
...
func хонданstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	навбатӣ := (stdinНавиштан + 1) % uint32(len(stdinbuffer))
	if навбатӣ == stdinХондан {
		return
	}
	stdinbuffer[stdinНавиштан] = c
	stdinНавиштан = навбатӣ
}

func stdingetblocking() byte {
	for stdinХондан == stdinНавиштан {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinХондан]
	stdinХондан = (stdinХондан + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (PortХонданbyte(0x64) & 0x01) == 0 {
	}
	sc := PortХонданbyte(0x60)
...
func нусхабардоштанexecvector(address_2 uint32, result *execvector) int32 {
	*result = execvector{}
	if address_2 == 0 {
		return 0
	}
...
func setupexecstack(cpu *Tcpustate, arguments_2 *execvector, environment *execvector) int32 {
	...
	if !Makerangeprivatewritable(getcr3(), ИстифодакунандаstackБоло-stackbytes, stackbytes) {
		return Enomem
	}
	stack := ИстифодакунандаstackБоло
...
func sysexecve(cpu *Tcpustate, pathaddress uint32) int32 {
	...
	if result := нусхабардоштанexecvector(cpu.Ecx, &arguments_2); result < 0 {
		return result
	}
	if result := нусхабардоштанexecvector(cpu.Edx, &environment); result < 0 {
		return result
	}
	номlen, ном := нусхабардоштанpath(pathaddress)
	if номlen == 0 {
	...
	memorymanager := &mem.TMemorymanager{}
	файлpointer := memorymanager.Malloc(size)
	if файлpointer == nil {
		return Einval
	}
	data := Getbytesfrompointer(uintptr(файлpointer), int(size), int(size))
	хонданФайл(ном[:номlen], data)
	if size < 52 || data[0] != 0x7F || data[1] != 'E' || data[2] != 'L' || data[3] != 'F' {
		memorymanager.Free(файлpointer)
		return Enoexec
	}
	loader := Elf{}
	...
	хуруҷonexec(ensurecurrentprocess())
	cpu.Eip = entry
	cpu.Eax = 0
	return 0
}
...
func sysfork(cpu *Tcpustate) int32 {
	...
	childpageФеҳраст := Cloneaddressspacecow(getcr3())
	if threadpointer == nil || stackpointer == nil || childpageФеҳраст == 0 {
		discardprocess(pid)
		return Einval
	}
	...
	*child.Cpustate = *cpu
	...
	child.Истифодакунандаstack_2 = cpu.Esp
	child.Истифодакунандаstacksize_2 = 0
	child.Pid = pid
	child.Parentpid = parentpid
	child.PageФеҳрастentry = childpageФеҳраст
...
func sysХуруҷ_2(status uint32) {
	pid := Currentpid()
	for i := 0; i < len(processtable); i++ {
		if processtable[i].used && processtable[i].pid == pid {
			хуруҷҲамаprocessfds(&processtable[i])
...
func syswaitpid(pid int32, statusaddress uint32, параметрҳо uint32) int32 {
	if (параметрҳо & ^uint32(1)) != 0 {
		return Einval
	}
	parentpid := Currentpid()
	foundchild := false
...
func allocateprocess(parent uint32) uint32 {
	parentprocess := ёфтанprocess(parent)
	pid := Allocatepid()
	for i := 0; i < len(processtable); i++ {
		if !processtable[i].used {
			processtable[i] = processentry{
			...
			if parentprocess != nil {
				...
				for fd := 0; fd < maxfd; fd++ {
					if parentprocess.fds[fd].used {
						...
						тасвирот := parentprocess.fds[fd].тасвирот
						if тасвирот >= 0 && тасвирот < maxКушоданfiles {
							кушоданФайлtable[тасвирот].refs++
						}
					}
				}
			} else {
...
func хуруҷҲамаprocessfds(process *processentry) {
	if process == nil {
		return
	}
	for fd := int32(0); fd < maxfd; fd++ {
...
func discardprocess(pid uint32) {
	process := ёфтанprocess(pid)
	if process == nil {
		return
	}
	хуруҷҲамаprocessfds(process)
	*process = processentry{}
}

func нусхабардоштанpath(pathaddress uint32) (uint32, [12]byte) {
	var ном [12]byte
	if pathaddress == 0 {
		return 0, ном
	}
...
func файлsize(filename []byte) uint32 {
