...
func Чопкарданbytes(datapointer uintptr, size int) {
	var buffer_2 [4 * 1024]byte = *(*([4 * 1024]byte))(Pointer(datapointer))
	var i int
	utilconsole.MЧопкардан([]byte("util["))
	for i = 0; i < size; i++ {
		utilconsole.MHexadecimalЧопкардан(buffer_2[i])
	}
	utilconsole.MЧопкардан([]byte("]"))
}

func Чопкарданtest(params ...interface{}) {
	for _, param := range params {
		utilconsole.MЧопкардан([]byte(TypeOf(param).Name()))
...
func Getbytesfrompointer(pointer_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		address	uintptr
		len	int
		cap	int
