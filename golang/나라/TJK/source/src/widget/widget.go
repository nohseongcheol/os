...
type IWidget interface {
	...
	SetРанг(r uint32, g uint32, b uint32)
...
func (self *TWidget) SetРанг(r uint32, g uint32, b uint32) {
	self.r = r
	self.g = g
	self.b = b
}
...
func (self *TWidgetmouseeventhandler) Init(widget TWidget, vga TVideographicsarray) {
	mousewidget = widget
	mousevga = vga
}
func (self *TWidgetmouseeventhandler) OnmouseПоён(button int8) {

	mousewidget.Set_local_coordinates(uint32(previousx), uint32(previousy))
	mousewidget.SetРанг(0xA8, 0x00, 0x00)
...
func (self *TWidgetmouseeventhandler) OnmouseБоло(button int8) {
}

func (self *TWidgetmouseeventhandler) OnmouseТаҳвил(x int8, y int8) {
	xposition += int16(x)
	if xposition < 0 {
		xposition = 0
	}
	...
	mousewidget.Set_local_coordinates(uint32(previousx), uint32(previousy))
	mousewidget.SetРанг(0x00, 0x00, 0x00)
	mousewidget.Draw(&mousevga)

	mousewidget.Set_local_coordinates(uint32(xposition), uint32(yposition))
	mousewidget.SetРанг(0x00, 0x00, 0xA8)
	mousewidget.Draw(&mousevga)
