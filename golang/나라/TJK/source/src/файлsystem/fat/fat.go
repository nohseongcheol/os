...
type TBiosparameterblock32 struct {
	...
	fatНусхабардоштан	uint8
	rootФеҳрастentry	uint16
	totalsectors		uint16
	mediatype		uint8
...
func (self *TBiosparameterblock32) Init(data []byte) {
	...
	self.fatНусхабардоштан = data[16]
	self.rootФеҳрастentry = (uint16(data[17]) | uint16(data[18])<<8)
...
func (self *TBiosparameterblock32) Len(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte) uint32 {
	...
	hd.Хондан28(partitionoffset, &bpbbytes, 90)
	...
	var datastart = fatstart + fatsize*uint32(bpb.fatНусхабардоштан)
	...
	hd.Хондан28(rootstart, &direntbytes, 512)

	var dirent = [16]TФеҳрастentryfat32{}
	...
	for i := 0; i < 16; i++ {
		...
		if !Equalbytes(filename, dirent[i].ном[:len(filename)]) {
			continue
		}
...
func (self *TBiosparameterblock32) Хондан(hd *TAdvancedtechnologyattachment, partentry TPartitiontableentry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		var навбатӣФайлcluster = int32(firstФайлcluster)
		var buffer_2 [513]byte
		var fatbuffer [513]byte

		for Size > 0 {
			var файлsector = datastart + uint32(bpb.sectorspercluster)*uint32(навбатӣФайлcluster-2)
			...
			for ; Size > 0; Size -= 512 {
				...
				if dirent[i].size > 512 {
					...
					hd.Хондан28(файлsector+uint32(sectoroffset), &buffer3, 512)

				} else {
					buffer3 = buffer_2[:dirent[i].size]
					hd.Хондан28(файлsector+uint32(sectoroffset), &buffer3, int(dirent[i].size))
				}
			...
			var fatsectorforcurrentcluster = uint32(навбатӣФайлcluster / 128)
			var fatbuf = fatbuffer[:512]
			hd.Хондан28(fatstart+fatsectorforcurrentcluster, &fatbuf, 512)

			var fatoffsetinsectorforcurrentcluster = навбатӣФайлcluster % 128
			var startoffset = fatoffsetinsectorforcurrentcluster * 4
			var охириҳуҷҷатoffset = fatoffsetinsectorforcurrentcluster*4 + 4

			var buffer4 [4]byte
			copy(buffer4[:4], fatbuffer[startoffset:охириҳуҷҷатoffset])

			навбатӣФайлcluster = int32(Unsignedinteger32r(Arraytounsignedinteger32(buffer4)))
		}
	}
	memorymanager.Free(bpbpointer)
	memorymanager.Free(direntpointer)
...
type TФеҳрастentryfat32 struct {
	ном		[8]byte
	ext		[3]byte
	attributes	uint8
	reserved	uint8
...
func (self *TФеҳрастentryfat32) Init(data [32]byte) {
	copy(self.ном[:8], data[0:8])
