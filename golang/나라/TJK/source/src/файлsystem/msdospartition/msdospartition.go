...
type TPartitiontableentry struct {
	...
	охириҳуҷҷатhead		uint8
	охириҳуҷҷатsector	uint8
	охириҳуҷҷатcylinder	uint16
...
func (self *TPartitiontableentry) Init(data [16]byte) {
	...
	self.охириҳуҷҷатhead = data[5]
	self.охириҳуҷҷатsector = (data[6] >> 2)
	self.охириҳуҷҷатcylinder = Unsignedinteger16r(uint16(data[6]&0x03) | uint16(data[7]))
...
func (self *Tmsdospartitiontable) Хонданpartition(hd *TAdvancedtechnologyattachment) {

	console_2 := TConsole{}
	console_2.MЧопкардан(([]byte)("Reading MBR"))

	var partitionbytes [512]byte
	var buffer_2 = partitionbytes[:]
	hd.Хондан28(0, &buffer_2, 512)
	...
	if self.Mbr.magicnumber != 0xAA55 {
		console_2.MЧопкардан(([]byte)("illegal MBR"))
		return
	}
