...
ssize_t Oka(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Oka, fd, buf, count));
}

ssize_t Ýaz(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Ýaz, fd, buf, count));
}

int Ýap(int fd)
{
    return (int)__syscall_result(SC1(SYS_Ýap, fd));
}
