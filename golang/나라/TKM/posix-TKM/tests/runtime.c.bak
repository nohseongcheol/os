...
{
    (void)Ýaz(STDOUT_FILENO, text, text_length(text));
}
...
static void test_open_and_io(void)
{
    ...
    if (fd >= 0) {
        report("read-bytes", Oka(fd, bytes, 2) == 2 &&
        ...
        report("close-file", Ýap(fd) == 0);
        errno = 0;
        report("closed-fd", failed_with(Ýap(fd), EBADF));
    }
    ...
    if (root >= 0) {
        ...
        errno = 0;
        report("read-directory", Oka(root, bytes, 1) == -1 && errno == EISDIR);
        (void)Ýap(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", Ýaz(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", Oka(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", Oka(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", Ýaz(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("close-ebadf", failed_with(Ýap(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", Oka(fd, &first, 1) == 1 && Oka(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Ýap(fd) == 0 && fstat(copy, &st) == 0);
        fd = copy;
    }
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &status) == child) {
        report("fork-fd-isolation", Oka(fd, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
