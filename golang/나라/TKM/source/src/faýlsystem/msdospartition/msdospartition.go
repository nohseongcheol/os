...
func (self *Tmsdospartitiontable) Okapartition(hd *TAdvancedtechnologyattachment) {

	console_2 := TConsole{}
	console_2.MÇap(([]byte)("Reading MBR"))

	var partitionBaýtlar [512]byte
	var buffer_2 = partitionBaýtlar[:]
	hd.Oka28(0, &buffer_2, 512)

	self.Mbr = TMasterbootrecord{}
	var i int = 0
	for ; i < 440; i++ {
		self.Mbr.bootloader[i] = partitionBaýtlar[i]
	}

	var buffer1 [4]byte
	copy(buffer1[:4], partitionBaýtlar[i:i+4])
	self.Mbr.signature = Unsignedinteger32r(Arraytounsignedinteger32(buffer1))
	i += 4

	var buffer2 [2]byte
	copy(buffer2[:2], partitionBaýtlar[i:i+2])
	self.Mbr.unused = Unsignedinteger16r(Arraytounsignedinteger16(buffer2))
	i += 2

	var buffer3 [16]byte
	copy(buffer3[:16], partitionBaýtlar[i:i+16])
