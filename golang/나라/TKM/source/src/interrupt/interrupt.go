...
func TestÇap(position uint8, data uint8)
...
func Täzeinterrupthandler(Interruptmanager uintptr, Interruptnumber uint8) *TInterrupthandler {
...
func (self *TInterruptmanager) Init(hardwareinterruptoffset uint16, globaldescriptortable *TShareddescriptortable, taskmanager *TTaskmanager) {
	...
	PortÝazbyte(Primarypiccommandioport, 0x11)
	PortÝazbyte(Secondarypiccommandioport, 0x11)

	PortÝazbyte(Primarypicdataioport, 0x20)
	PortÝazbyte(Secondarypicdataioport, 0x28)

	PortÝazbyte(Primarypicdataioport, 0x04)
	PortÝazbyte(Secondarypicdataioport, 0x02)

	PortÝazbyte(Primarypicdataioport, 0x01)
	PortÝazbyte(Secondarypicdataioport, 0x01)

	PortÝazbyte(Primarypicdataioport, 0xF8)
	PortÝazbyte(Secondarypicdataioport, 0xEF)

	idtpointer := [6]uint8{0, 0, 0, 0, 0, 0}
	ululyk := (*uint16)(Pointer(&idtpointer[0]))
	(*ululyk) = (uint16)(Sizeof(idtdata) - 1)
...
func (self *TInterruptmanager) Interruptdescriptortableentryset(interrupt int,
	...
	(*access) = (Idtdescriptorpresent | DescriptorHil | ((Descriptorprivilegelevel & 3) << 5))
...
func (self *TInterruptmanager) Dohandleinterrupt(interrupt uint8, esp uint32) uint32 {
	...
	if interruptdebug {
		console_2.MÇapxy("[esp:", 1, 20)
		console_2.MUnsignedinteger32Çap(uint32(interrupt))
		console_2.MÇap(":")
	...
	if !handlerBitir && interrupt == uint8(self.hardwareinterruptoffset) && self.taskmanager != nil {
		esp = uint32(uintptr(Pointer(self.taskmanager.Schedule((*Tcpustate)(Pointer(uintptr(esp)))))))

	}
	if !handlerBitir && interrupt == 0x80 {
	...
	if 0x20 <= interrupt && interrupt < 0x30 {
		if 0x28 <= interrupt {
			PortÝazbyte(Secondarypiccommandioport, 0x20)
		}
		PortÝazbyte(Primarypiccommandioport, 0x20)
	}
	return esp
}
...
func exceptionhasHatacode(interrupt uint32) bool {
	switch interrupt {
...
func exceptionframeMykdar(frame uint32, offset uint32) uint32 {
...
func çappagefaultinfo(err uint32) {
...
func çapexceptionselectorinfo(err uint32) {
...
func Handleexception(esp uint32, interrupt uint32) uint32 {
	...
	if esp < 0x1000 {
		...
		if exceptionhasHatacode(interrupt) {
			MEmergencylogstring(" raw-error-or-bad-esp=")
			MEmergencylogunsignedinteger32(esp)
			çapexceptionselectorinfo(esp)
		}
	...
	if exceptionhasHatacode(interrupt) {
		err = exceptionframeMykdar(esp, 0)
		eipoffset = 4
	}
	eip := exceptionframeMykdar(esp, eipoffset)
	cs := exceptionframeMykdar(esp, eipoffset+4)
	eflags_2 := exceptionframeMykdar(esp, eipoffset+8)
	...
	if interrupt == 0x0E {
		...
		çappagefaultinfo(err)
	}

	if (cs & 0x03) != 0 {
		MEmergencylogstring(" useresp=")
		MEmergencylogunsignedinteger32(exceptionframeMykdar(esp, eipoffset+12))
		MEmergencylogstring(" ss=")
		MEmergencylogunsignedinteger32(exceptionframeMykdar(esp, eipoffset+16))
	}

	if exceptionhasHatacode(interrupt) {
		çapexceptionselectorinfo(err)
	}
	MEmergencylogstring("\n=== ENGOS HALTED AFTER EXCEPTION ===\n")
	except++
...
func interrupthandler_2(interrupt uint8, esp uint32) uint32 {
	...
	console_2 := TConsole{}
	console_2.MÇap(buffer)
	console_2.MHexadecimalÇap(0x40)
	return esp
}
func çapesp(esp uint32) {
	console_2 := TConsole{}
	console_2.MUnsignedinteger32Çapxy(esp, 20, 21)
}
func gettls() uint32
func Çaptls() {
