...
func (self *Tcowframemanager) Init(mem *TMemorymanager, framecount uint32) bool {
	...
	referenceBaýtlar := framecount * uint32(unsafe.Sizeof(uint16(0)))
	referencepointer := mem.Malloc(referenceBaýtlar)
...
func (self *Paging) Sharedmemoryregion() {
	...
	for i := uint32(1); i <= virtlen; i++ {
		...
		for ; pde < 12; pde++ {
			v := GetMykdar(uint32(kpagedirectoryentry) + pde*4)
			v = (v & 0xFFFFF000)
...
func handlepaginginterrupt(esp uint32) uint32 {
	if ResolveNusgalaonÝazfault() {
		return esp
	}
	return Handlefatalinterruptframe(esp, 0x0E)
}

func Cloneaddressspacecow(çeşmepagedirectory uint32) uint32 {
	if Activememorymanager == nil || çeşmepagedirectory == 0 {
		return 0
	}
	...
	for pde := uint32(0); pde < 1024; pde++ {
		çeşmepdeaddress := çeşmepagedirectory + pde*4
		çeşmepde := GetMykdar(çeşmepdeaddress)
		if (çeşmepde & Pagepresent) == 0 {
			continue
		}
		if issharedpde(pde) {
			Setunsignedinteger32ataddress(çeşmepde, destinationpagedirectory+pde*4)
			continue
		}
		...
		çeşmept := çeşmepde & Pageframe
		destinationpt := uint32(uintptr(destinationptpointer))
		Setunsignedinteger32ataddress((destinationpt | (çeşmepde & 0xFFF)), destinationpagedirectory+pde*4)

		for pte := uint32(0); pte < 1024; pte++ {
			pteaddress := çeşmept + pte*4
			entry := GetMykdar(pteaddress)
...
func ResolveNusgalaonÝazfault() bool {
	...
	täzepointer, _ := Activememorymanager.Alignedmalloc(0x1000)
	if täzepointer == nil {
		return false
	}
	täzeframe := uint32(uintptr(täzepointer)) & Pageframe

	çeşme_2 := GetBaýtlarfrompointer(uintptr(faultaddress&Pageframe), 0x1000, 0x1000)
	destination_2 := GetBaýtlarfrompointer(uintptr(täzeframe), 0x1000, 0x1000)
	copy(destination_2, çeşme_2)
	cowframemanager.Decrement(oldframe)
	Setunsignedinteger32ataddress((täzeframe|(pte&0xFFF)|Pagewritable)&^Pagecow, pteaddress)
	ýeneÝüklecr3()
	return true
}
...
func Setblockinpagedirectory(çeşme_2 []byte, destination_2 []byte, ululyk uint32, pagedirectory uint32) {
	if ululyk == 0 || pagedirectory == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(pagedirectory)
	makerangeprivatewritablecurrent(pagedirectory, uint32(uintptr(unsafe.Pointer(&destination_2[0]))), ululyk)

	for i := uint32(0); i < ululyk; i++ {
		destination_2[i] = çeşme_2[i]
	}
	setcr3(oldcr3)
}

func Zeroblockinpagedirectory(address uint32, ululyk uint32, pagedirectory uint32) {
	if ululyk == 0 || pagedirectory == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(pagedirectory)
	makerangeprivatewritablecurrent(pagedirectory, address, ululyk)
	destination_2 := GetBaýtlarfrompointer(uintptr(address), int(ululyk), int(ululyk))
	for i := uint32(0); i < ululyk; i++ {
		destination_2[i] = 0
	}
	setcr3(oldcr3)
...
func makepageprivatewritablecurrent(pagedirectory uint32, virtualaddress uint32) bool {
	pde := GetMykdar(pagedirectory + ((virtualaddress>>22)&0x3FF)*4)
	if (pde & Pagepresent) == 0 {
		return false
	}
	...
	täzeframe := uint32(uintptr(täzepointer)) & Pageframe
	çeşme_2 := GetBaýtlarfrompointer(uintptr(virtualaddress&Pageframe), 0x1000, 0x1000)
	destination_2 := GetBaýtlarfrompointer(uintptr(täzeframe), 0x1000, 0x1000)
	copy(destination_2, çeşme_2)
...
func makerangeprivatewritablecurrent(pagedirectory uint32, address uint32, ululyk uint32) bool {
	if ululyk == 0 {
		return true
	}
	last := address + ululyk - 1
	if last < address {
		return false
	}
...
func Makerangeprivatewritable(pagedirectory uint32, address uint32, ululyk uint32) bool {
	if pagedirectory == 0 {
		return false
	}
	oldcr3 := getcr3()
	setcr3(pagedirectory)
	ok := makerangeprivatewritablecurrent(pagedirectory, address, ululyk)
	setcr3(oldcr3)
	return ok
}
...
func GetMykdar(address uint32) uint32 {
	var orgMykdar uint32 = *(*uint32)(unsafe.Pointer(uintptr(address)))
	return orgMykdar
}
func GetMykdarinpagedirectory(address uint32, pagedirectory uint32) uint32 {
	if pagedirectory == 0 {
		return 0
	}
	oldcr3 := getcr3()
...
func Nusgalapageframeblock(xpagedirectory uint32, ypagedirectory uint32, vaddress uint32) {
	if xpagedirectory == 0 || ypagedirectory == 0 {
		return
	}
	oldcr3 := getcr3()
	setcr3(xpagedirectory)
	v = GetMykdar(vaddress)
