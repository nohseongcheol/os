...
const ProcUllançyheapUlulyk = 1 * 1024 * 1024
...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	esasythread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if esasythread != nil {
		esasythread.Pid = process.id
		esasythread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(esasythread)))
	}
...
func (self *Processhelper) Spawn(entrypoint func(), threadhelper *TThreadhelper, scheduler *Scheduler, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	if process.Threads != nil && process.Threads.Ululyk_2 > 0 {
...
func (self *Processhelper) nusgalapagedirectory(çeşmeentry uintptr, destinationentry uintptr) {
	çeşme_2 := Getunsignedinteger32arrayfrompointer(çeşmeentry, 1024, 1024)
