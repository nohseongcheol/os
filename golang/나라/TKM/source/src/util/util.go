...
func ÇapBaýtlar(datapointer uintptr, ululyk int) {
	var buffer_2 [4 * 1024]byte = *(*([4 * 1024]byte))(Pointer(datapointer))
	var i int
	utilconsole.MÇap([]byte("util["))
	for i = 0; i < ululyk; i++ {
		utilconsole.MHexadecimalÇap(buffer_2[i])
	}
	utilconsole.MÇap([]byte("]"))
}

func Çaptest(params ...interface{}) {
	for _, param := range params {
		utilconsole.MÇap([]byte(TypeOf(param).Name()))

	}
}
func EqualBaýtlar(a, b []byte) bool {
	if len(a) != len(b) {
		return false
	}
	for i, v := range a {
...
func Baýtlartostring(b []byte) string {
...
func GetBaýtlarfrompointer(pointer_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		address	uintptr
		len	int
		cap	int
