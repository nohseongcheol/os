...
ssize_t lau(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_lau, fd, buf, count));
}

ssize_t tohi(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_tohi, fd, buf, count));
}
