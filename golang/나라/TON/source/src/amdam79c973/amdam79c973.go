...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portlauword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portlauword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portlauword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portlauword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portlauword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portlauword(Macaddress4port) / 256)
	...
	Porttohiword(registeraddressport, 20)
	Porttohiword(buscontrolregisterdataport, 0x102)

	Porttohiword(registeraddressport, 0)
	Porttohiword(registerdataport, 0x04)
	...
	Porttohiword(registeraddressport, 1)
	Porttohiword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Porttohiword(registeraddressport, 2)
	Porttohiword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Porttohiword(registeraddressport, 0)
	Porttohiword(registerdataport, 0x41)

	Porttohiword(registeraddressport, 4)
	temporary := Portlauword(registerdataport)
	Porttohiword(registeraddressport, 4)
	Porttohiword(registerdataport, temporary|0xC00)

	Porttohiword(registeraddressport, 0)
	Porttohiword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portlauword(resetport)
	Porttohiword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Porttohiword(registeraddressport, 0)
	temporary := uint32(Portlauword(registerdataport))
	...
	Porttohiword(registeraddressport, 0)
	Porttohiword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Porttohiword(registeraddressport, 0)
	Porttohiword(registerdataport, 0x48)
