...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Tohi(0xA0)
	} else {
		self.deviceport.Tohi(0xB0)
	}
	self.controlport.Tohi(0)
	self.deviceport.Tohi(0xA0)

	var status uint8 = self.commandport.Lau()
	if status == 0xFF {
	...
	self.sectorcountport.Tohi(0)
	self.lbalowport.Tohi(0)
	self.lbamidport.Tohi(0)
	self.lbahiport.Tohi(0)
	self.commandport.Tohi(0xEC)

	status = self.commandport.Lau()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Lau28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Tohi(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Tohi(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Tohi(0)
	self.sectorcountport.Tohi(1)

	self.lbalowport.Tohi(uint8(sector & 0x000000FF))
	self.lbamidport.Tohi(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Tohi(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Tohi(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Lau()
...
func (self *TAdvancedtechnologyattachment) Tohi28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Tohi(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Tohi(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Tohi(0)
	self.sectorcountport.Tohi(1)
	self.lbalowport.Tohi(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Tohi(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Tohi(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Tohi(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Tohi(0xE0)
	} else {
		self.deviceport.Tohi(0xF0)
	}
	self.commandport.Tohi(0xE7)
