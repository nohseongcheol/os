...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portlaubyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portlaubyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func tohips2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Porttohibyte(commandport_2, value)
	return true
}

func tohips2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Porttohibyte(dataport_2, value)
	return true
}

func laups2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portlaubyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portlaubyte(commandport_2)&0x01) != 0; i++ {
		Portlaubyte(dataport_2)
	}

	if !tohips2command(0xAE) || !tohips2command(0x20) {
		return
	}
	status, ok := laups2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !tohips2command(0x60) || !tohips2data(status) {
		return
	}
...
var keyboardqueuelau uint8
var keyboardqueuetohi uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuetohi + 1) % keyboardqueuesize
	if next == keyboardqueuelau {
		return
	}
	keyboardqueue[keyboardqueuetohi] = key
	keyboardqueuetohi = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuelau != keyboardqueuetohi {
		key := keyboardqueue[keyboardqueuelau]
		keyboardqueuelau = (keyboardqueuelau + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portlaubyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portlaubyte(dataport_2)
	if scancode == 0xE0 {
