...
func Syslau_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Syslau, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Systohi, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Systohi, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Syslau:
		cpu.Eax = uint32(syslau(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Systohi:
		cpu.Eax = uint32(systohi(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|otohionly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: olauonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: otohionly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: otohionly}
}
...
func systohi(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func syslau(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return lauvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == otohionly || accessmode == olautohi || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func lauvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	laufile(entry.name[:entry.namelen], tmp)
...
func laustdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdintohi + 1) % uint32(len(stdinbuffer))
	if next == stdinlau {
		return
	}
	stdinbuffer[stdintohi] = c
	stdintohi = next
}

func stdingetblocking() byte {
	for stdinlau == stdintohi {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinlau]
	stdinlau = (stdinlau + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portlaubyte(0x64) & 0x01) == 0 {
	}
	sc := Portlaubyte(0x60)
...
func laufile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Lau(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
