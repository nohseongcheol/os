...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	tefitothread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if tefitothread != nil {
		tefitothread.Pid = process.id
		tefitothread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(tefitothread)))
	}
