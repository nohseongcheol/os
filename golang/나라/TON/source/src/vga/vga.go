...
var attributecontrollerlauport uint16 = 0x3c1
var attributecontrollertohiport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Tohiregister(register []byte) {
	var regindex uint16 = 0

	Porttohibyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Porttohibyte(sequencerindexport, i)
		Porttohibyte(sequencerdataport, register[regindex])
		regindex++
	}

	Porttohibyte(crtcindexport, 0x03)

	Porttohibyte(crtcdataport, (Portlaubyte(crtcdataport) | 0x80))
	Porttohibyte(crtcindexport, 0x11)
	Porttohibyte(crtcdataport, (Portlaubyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Porttohibyte(crtcindexport, i)
		Porttohibyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Porttohibyte(graphicscontrollerindexport, i)
		Porttohibyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portlaubyte(attributecontrollerresetport)
		Porttohibyte(attributecontrollerindexport, i)
		Porttohibyte(attributecontrollertohiport, register[regindex])
		regindex++
	}

	Portlaubyte(attributecontrollerresetport)
	Porttohibyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Porttohibyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portlaubyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Tohiregister(g320x200x256)
