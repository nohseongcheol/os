...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portreadbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portreadbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func writeps2command(value uint8) bool {
	...
	Portwritebyte(commandport_2, value)
	return true
}
...
func writeps2data(value uint8) bool {
	...
	Portwritebyte(dataport_2, value)
	return true
}
...
func readps2data() (uint8, bool) {
	...
	return Portreadbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portreadbyte(commandport_2)&0x01) != 0; i++ {
		Portreadbyte(dataport_2)
	}
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portreadbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portreadbyte(dataport_2)
	if scancode == 0xE0 {
