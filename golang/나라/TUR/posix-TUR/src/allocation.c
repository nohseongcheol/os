...
void *malloc(size_t rakam_sayısı)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!rakam_sayısı) rakam_sayısı = 1;
    if (rakam_sayısı > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    rakam_sayısı = (rakam_sayısı + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= rakam_sayısı) {
            if (cursor->capacity - rakam_sayısı >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + rakam_sayısı);
                raw->capacity = cursor->capacity - rakam_sayısı - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = rakam_sayısı;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + rakam_sayısı;
    ...
    raw->capacity = rakam_sayısı; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t rakam_sayısı)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(rakam_sayısı);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!rakam_sayısı) rakam_sayısı = 1;
    cursor = (struct heap_block *)address - 1;
    if (rakam_sayısı <= cursor->capacity) return address;
    destination = malloc(rakam_sayısı);
