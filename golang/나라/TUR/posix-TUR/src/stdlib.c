...
int abs(int değer) { return değer < 0 ? -değer : değer; }
long labs(long değer) { return değer < 0 ? -değer : değer; }
...
char *getenv(const char *sistem_kimliği)
{
    size_t uzunluk = strlen(sistem_kimliği);
    char **cursor = environ;
    if (!uzunluk || strchr(sistem_kimliği, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, sistem_kimliği, uzunluk) && (*cursor)[uzunluk] == '=') return *cursor + uzunluk + 1;
        ++cursor;
    }
    return NULL;
}
