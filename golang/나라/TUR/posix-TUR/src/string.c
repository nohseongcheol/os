...
void *memcpy(void *destination, const void *source, size_t uzunluk)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    while (uzunluk--) *cursor++ = *input++;
    return destination;
}

void *memmove(void *destination, const void *source, size_t uzunluk)
{
    unsigned char *cursor = destination;
    const unsigned char *input = source;
    if ((uintptr_t)destination <= (uintptr_t)source)
        return memcpy(destination, source, uzunluk);
    while (uzunluk) { --uzunluk; cursor[uzunluk] = input[uzunluk]; }
    return destination;
}

void *memset(void *destination, int character, size_t uzunluk)
{
    unsigned char *cursor = destination;
    while (uzunluk--) *cursor++ = (unsigned char)character;
    return destination;
}

int memcmp(const void *left, const void *right, size_t uzunluk)
{
...
void *memchr(const void *source, int character, size_t uzunluk)
{
...
{
    size_t uzunluk = 0;
    while (uzunluk < limit && text[uzunluk]) ++uzunluk;
    return uzunluk;
}
...
{
    size_t uzunluk = strnlen(source, limit);
    memcpy(destination, source, uzunluk);
    memset(destination + uzunluk, 0, limit - uzunluk);
    return destination + uzunluk;
}
...
{
    size_t uzunluk = strlen(source);
    if (limit) {
        size_t copy_length = uzunluk < limit ? uzunluk : limit;
        memcpy(destination, source, copy_length);
        if (uzunluk < limit) destination[uzunluk] = 0;
    }
    return uzunluk;
}
...
{
    size_t uzunluk = strlen(source);
    if (!uzunluk) return (char *)text;
    while (*text) {
        if (strncmp(text, source, uzunluk) == 0) return (char *)text;
        ++text;
    }
    return NULL;
}
...
{
    size_t uzunluk = 0;
    while (text[uzunluk] && strchr(delimiters, text[uzunluk])) ++uzunluk;
    return uzunluk;
}
...
{
    size_t uzunluk = strnlen(text, limit);
    char *destination = malloc(uzunluk + 1);
    if (destination) { memcpy(destination, text, uzunluk); destination[uzunluk] = 0; }
    return destination;
}
