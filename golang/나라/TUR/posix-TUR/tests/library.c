...
#define CHECK(değer) do { if (!(değer)) return __LINE__; } while (0)

static int compare_int(const void *left, const void *right)
{
    int result = *(const int *)left, değer = *(const int *)right;
    return (result > değer) - (result < değer);
}

int __posix_library_test(void)
{
    char aktarım_ara_belleği_2[64], source[64], *end_pointer, *saved_state, *address, *destination;
    const char *text = "abca", *input = " 	-0x80rest";
    int elements[257], index, değer;
    size_t uzunluk;
    div_t result;
    ldiv_t decoded;
    ...
    CHECK(memset(aktarım_ara_belleği_2, 0xab, sizeof(aktarım_ara_belleği_2)) == aktarım_ara_belleği_2);
    CHECK((unsigned char)aktarım_ara_belleği_2[63] == 0xab);
    CHECK(memcpy(aktarım_ara_belleği_2, "abcdef", 7) == aktarım_ara_belleği_2);
    CHECK(memmove(aktarım_ara_belleği_2 + 2, aktarım_ara_belleği_2, 5) == aktarım_ara_belleği_2 + 2);
    CHECK(memcmp(aktarım_ara_belleği_2, "ababcde", 7) == 0);
    CHECK(memmove(aktarım_ara_belleği_2, aktarım_ara_belleği_2 + 2, 5) == aktarım_ara_belleği_2);
    CHECK(memcmp(aktarım_ara_belleği_2, "abcde", 5) == 0);
    CHECK(memmove(aktarım_ara_belleği_2, aktarım_ara_belleği_2, 0) == aktarım_ara_belleği_2);
    CHECK(memcmp("\x80", "\x7f", 1) > 0);
    CHECK(memcmp(aktarım_ara_belleği_2, source, 0) == 0);
    ...
    CHECK(strcpy(aktarım_ara_belleği_2, text) == aktarım_ara_belleği_2 && strcmp(aktarım_ara_belleği_2, text) == 0);
    CHECK(stpcpy(aktarım_ara_belleği_2, "xy") == aktarım_ara_belleği_2 + 2 && aktarım_ara_belleği_2[2] == 0);
    memset(aktarım_ara_belleği_2, 7, sizeof(aktarım_ara_belleği_2));
    CHECK(strncpy(aktarım_ara_belleği_2, "xy", 5) == aktarım_ara_belleği_2);
    CHECK(!memcmp(aktarım_ara_belleği_2, "xy\0\0\0", 5) && aktarım_ara_belleği_2[5] == 7);
    CHECK(stpncpy(aktarım_ara_belleği_2, "abcdef", 3) == aktarım_ara_belleği_2 + 3 && aktarım_ara_belleği_2[3] == 0);
    CHECK(stpncpy(aktarım_ara_belleği_2, "a", 4) == aktarım_ara_belleği_2 + 1 && aktarım_ara_belleği_2[3] == 0);
    CHECK(strncpy(aktarım_ara_belleği_2, "z", 0) == aktarım_ara_belleği_2 && aktarım_ara_belleği_2[0] == 'a');
    strcpy(aktarım_ara_belleği_2, "a");
    CHECK(strcat(aktarım_ara_belleği_2, "bc") == aktarım_ara_belleği_2 && !strcmp(aktarım_ara_belleği_2, "abc"));
    CHECK(strncat(aktarım_ara_belleği_2, "def", 2) == aktarım_ara_belleği_2 && !strcmp(aktarım_ara_belleği_2, "abcde"));
    CHECK(strncat(aktarım_ara_belleği_2, "xyz", 0) == aktarım_ara_belleği_2 && !strcmp(aktarım_ara_belleği_2, "abcde"));
    ...
    CHECK(strxfrm(aktarım_ara_belleği_2, "abc", 4) == 3 && !strcmp(aktarım_ara_belleği_2, "abc"));
    ...
    strcpy(aktarım_ara_belleği_2, ",,one;two,,");
    CHECK(!strcmp(strtok_r(aktarım_ara_belleği_2, ",;", &saved_state), "one"));
    strcpy(source, "x:y");
    ...
    for (değer = -1; değer <= 255; ++değer) {
        int low = değer >= 'a' && değer <= 'z', high = değer >= 'A' && değer <= 'Z';
        int digit = değer >= '0' && değer <= '9';
        CHECK(!!isalpha(değer) == (low || high));
        CHECK(!!isalnum(değer) == (low || high || digit));
        CHECK(!!isdigit(değer) == digit);
        CHECK(!!islower(değer) == low && !!isupper(değer) == high);
        CHECK(!!isblank(değer) == (değer == ' ' || değer == '\t'));
        CHECK(!!isspace(değer) == (değer == ' ' || (değer >= 9 && değer <= 13)));
        CHECK(!!iscntrl(değer) == ((değer >= 0 && değer < 32) || değer == 127));
        CHECK(!!isprint(değer) == (değer >= 32 && değer <= 126));
        CHECK(!!isgraph(değer) == (değer >= 33 && değer <= 126));
        CHECK(!!ispunct(değer) == (değer >= 33 && değer <= 126 && !low && !high && !digit));
        CHECK(!!isxdigit(değer) == (digit || (değer >= 'a' && değer <= 'f') || (değer >= 'A' && değer <= 'F')));
        CHECK(tolower(değer) == (high ? değer + 32 : değer));
        CHECK(toupper(değer) == (low ? değer - 32 : değer));
    }
    errno = EIO;
    ...
    for (uzunluk = 0; uzunluk <= 257; ++uzunluk) {
        for (index = 0; index < (int)uzunluk; ++index) elements[index] = ((index * 37 + (int)uzunluk) % 19) - 9;
        qsort(elements, uzunluk, sizeof(int), compare_int);
        for (index = 1; index < (int)uzunluk; ++index) CHECK(elements[index - 1] <= elements[index]);
        for (index = 0; index < (int)uzunluk; ++index) {
            int *found = bsearch(&elements[index], elements, uzunluk, sizeof(int), compare_int);
            CHECK(found && *found == elements[index]);
        }
        değer = 100;
        CHECK(bsearch(&değer, elements, uzunluk, sizeof(int), compare_int) == NULL);
    }
    environ = envp;
