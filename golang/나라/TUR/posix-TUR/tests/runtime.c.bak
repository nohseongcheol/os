...
{
    (void)Yazma(STDOUT_FILENO, text, text_length(text));
}

static void report(const char *sistem_kimliği, int passed)
{
    checks++;
    if (passed) {
        say("PTEST:PASS:");
...
static pid_t reap_nohang(pid_t child, int *durum)
{
    unsigned int spins;
    pid_t result;

    for (spins = 0; spins < 2000000U; spins++) {
        result = waitpid(child, durum, WNOHANG);
        if (result != 0)
            return result;
    }
    return 0;
...
static void test_open_and_io(void)
{
    ...
    report("open-readonly", dosya_tanımlayıcısı >= 3);
    if (dosya_tanımlayıcısı >= 0) {
        report("read-bytes", Okuma(dosya_tanımlayıcısı, bytes, 2) == 2 &&
               (unsigned char)bytes[0] == 0x7f && bytes[1] == 'E');
        report("lseek-set", lseek(dosya_tanımlayıcısı, 0, SEEK_SET) == 0);
        report("lseek-cur", lseek(dosya_tanımlayıcısı, 1, SEEK_CUR) == 1);
        report("lseek-end", lseek(dosya_tanımlayıcısı, -1, SEEK_END) > 0);
        errno = 0;
        report("lseek-negative", lseek(dosya_tanımlayıcısı, -1, SEEK_SET) == -1 && errno == EINVAL);
        errno = 0;
        report("lseek-whence", lseek(dosya_tanımlayıcısı, 0, 99) == -1 && errno == EINVAL);
        report("fstat-file", fstat(dosya_tanımlayıcısı, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 2);
        errno = 0;
        report("fstat-efault", failed_with(fstat(dosya_tanımlayıcısı, (struct stat *)0), EFAULT));
        report("fsync-file", fsync(dosya_tanımlayıcısı) == 0);
        report("close-file", Kapat(dosya_tanımlayıcısı) == 0);
        errno = 0;
        report("closed-fd", failed_with(Kapat(dosya_tanımlayıcısı), EBADF));
    }

    root = Aç("/", O_RDONLY | O_DIRECTORY);
    report("open-directory", root >= 3);
    if (root >= 0) {
        report("fstat-directory", fstat(root, &st) == 0 && S_ISDIR(st.st_mode));
        errno = 0;
        report("read-directory", Okuma(root, bytes, 1) == -1 && errno == EISDIR);
        (void)Kapat(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", Yazma(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", Okuma(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", Okuma(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", Yazma(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("open-enoent", failed_with(Aç("/NOFILE", O_RDONLY), ENOENT));
    errno = 0;
    report("open-efault", failed_with(Aç((const char *)0, O_RDONLY), EFAULT));
    errno = 0;
    report("open-write-erofs", failed_with(Aç("/USER2", O_WRONLY), EROFS));
    errno = 0;
    report("open-rdwr-erofs", failed_with(Aç("/USER2", O_RDWR), EROFS));
    errno = 0;
    report("open-create-erofs", failed_with(Aç("/NEWFILE", O_CREAT | O_WRONLY, 0600), EROFS));
    errno = 0;
    report("open-trunc-erofs", failed_with(Aç("/USER2", O_TRUNC | O_RDONLY), EROFS));
    errno = 0;
    report("open-append-erofs", failed_with(Aç("/USER2", O_APPEND | O_RDONLY), EROFS));
    errno = 0;
    report("open-enotdir", failed_with(Aç("/USER2", O_RDONLY | O_DIRECTORY), ENOTDIR));
    errno = 0;
    report("creat-erofs", failed_with(creat("/NEWFILE", 0600), EROFS));
    errno = 0;
    report("close-ebadf", failed_with(Kapat(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    copy = dup(dosya_tanımlayıcısı);
    report("dup", copy >= 0 && copy != dosya_tanımlayıcısı);
    if (copy >= 0) {
        report("dup-shared-offset", Okuma(dosya_tanımlayıcısı, &first, 1) == 1 && Okuma(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Kapat(dosya_tanımlayıcısı) == 0 && fstat(copy, &st) == 0);
        dosya_tanımlayıcısı = copy;
    }

    target = dup2(dosya_tanımlayıcısı, 20);
    report("dup2", target == 20 && fstat(20, &st) == 0);
    report("dup2-same", dup2(dosya_tanımlayıcısı, dosya_tanımlayıcısı) == dosya_tanımlayıcısı);
    if (target == 20)
        (void)Kapat(20);
    errno = 0;
    report("dup2-old-ebadf", failed_with(dup2(-1, 10), EBADF));
    errno = 0;
    report("dup2-new-ebadf", failed_with(dup2(dosya_tanımlayıcısı, 99), EBADF));

    report("fcntl-getfd", fcntl(dosya_tanımlayıcısı, F_GETFD) == 0);
    report("fcntl-setfd", fcntl(dosya_tanımlayıcısı, F_SETFD, FD_CLOEXEC) == 0 &&
           fcntl(dosya_tanımlayıcısı, F_GETFD) == FD_CLOEXEC);
    high = fcntl(dosya_tanımlayıcısı, F_DUPFD, 10);
    report("fcntl-dupfd", high >= 10);
    if (high >= 0) {
        report("fcntl-fd-flags-independent", fcntl(high, F_GETFD) == 0);
        (void)Kapat(high);
    }
    report("fcntl-getfl", (fcntl(dosya_tanımlayıcısı, F_GETFL) & O_ACCMODE) == O_RDONLY);
    report("fcntl-setfl", fcntl(dosya_tanımlayıcısı, F_SETFL, O_APPEND) == 0 &&
           (fcntl(dosya_tanımlayıcısı, F_GETFL) & O_APPEND) != 0);
    errno = 0;
    report("fcntl-command-einval", failed_with(fcntl(dosya_tanımlayıcısı, 999), EINVAL));
    errno = 0;
...
static void test_terminal_and_uname(void)
{
    ...
    if (dosya_tanımlayıcısı >= 0) {
        errno = 0;
        report("isatty-enotty", isatty(dosya_tanımlayıcısı) == 0 && errno == ENOTTY);
        (void)Kapat(dosya_tanımlayıcısı);
    } else {
    ...
    report("uname", uname(&sistem_kimliği) == 0 && sistem_kimliği.sysname[0] != 0 &&
           sistem_kimliği.nodename[0] != 0 && text_equal(sistem_kimliği.machine, "i386"));
    errno = 0;
...
void posix_test_process(void)
{
    ...
    report("fork", child > 0);
    if (child > 0) {
        waited = reap_nohang(child, &durum);
        report("waitpid", waited == child && WIFEXITED(durum) && WEXITSTATUS(durum) == 23);
        errno = 0;
        report("waitpid-echild", waitpid(child, &durum, WNOHANG) == -1 && errno == ECHILD);
    }
    ...
    if (child > 0 && reap_nohang(child, &durum) == child) {
        report("fork-fd-isolation", Okuma(dosya_tanımlayıcısı, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
    ...
    if (child > 0) {
        waited = reap_nohang(child, &durum);
        report("execve", waited == child && WIFEXITED(durum) && WEXITSTATUS(durum) == 37);
    }
    ...
    if (child > 0) {
        ...
        for (spin = 0; spin < 2000000U; spin++)
            (void)getpid();
        waited = wait(&durum);
        report("wait", waited == child && WIFEXITED(durum) && WEXITSTATUS(durum) == 29);
    }
}
