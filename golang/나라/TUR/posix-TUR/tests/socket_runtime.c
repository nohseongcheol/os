...
static void say(const char *s) { (void)Yazma(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *sistem_kimliği, int pass)
{
...
int main(void)
{
    ...
    char aktarım_ara_belleği_2[8] = {0};
    ...
    report("recvfrom", recvfrom(server, aktarım_ara_belleği_2, sizeof(aktarım_ara_belleği_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       aktarım_ara_belleği_2[0] == 'p' && aktarım_ara_belleği_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, aktarım_ara_belleği_2, sizeof(aktarım_ara_belleği_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Kapat(client) == 0);
    report("close-server", Kapat(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
