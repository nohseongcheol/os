...
freestanding library in `./kullanıcı_alanı/posix`.
...
```sh
cd ./kullanıcı_alanı/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./kullanıcı_alanı/posix/bildirimler` and link the archive into an EngOS i386 user
...
`./kullanıcı_alanı/posix/TEST_RESULTS.md`. Build all runtime test executables
