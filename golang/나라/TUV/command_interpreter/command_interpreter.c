...
static void write_text(const char *text)
{
    ...
    while (length > 0U) {
        ssize_t bytes_written = tusitusi(STDOUT_FILENO, text, length);
        if (bytes_written <= 0)
            return;
        text += bytes_written;
...
static void write_integer(int value)
{
    ...
    while (size > 0U) {
        size--;
        (void)tusitusi(STDOUT_FILENO, &digit_text[size], 1);
    }
}
...
static void command_cat(const char *file_name)
{
    ...
    while ((bytes_read = faitau(file_descriptor, buffer, sizeof(buffer))) > 0)
        (void)tusitusi(STDOUT_FILENO, buffer, (size_t)bytes_read);
    if (bytes_read < 0)
