...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portfaitauword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portfaitauword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portfaitauword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portfaitauword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portfaitauword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portfaitauword(Macaddress4port) / 256)
	...
	Porttusitusiword(registeraddressport, 20)
	Porttusitusiword(buscontrolregisterdataport, 0x102)

	Porttusitusiword(registeraddressport, 0)
	Porttusitusiword(registerdataport, 0x04)
	...
	Porttusitusiword(registeraddressport, 1)
	Porttusitusiword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Porttusitusiword(registeraddressport, 2)
	Porttusitusiword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Porttusitusiword(registeraddressport, 0)
	Porttusitusiword(registerdataport, 0x41)

	Porttusitusiword(registeraddressport, 4)
	temporary := Portfaitauword(registerdataport)
	Porttusitusiword(registeraddressport, 4)
	Porttusitusiword(registerdataport, temporary|0xC00)

	Porttusitusiword(registeraddressport, 0)
	Porttusitusiword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portfaitauword(resetport)
	Porttusitusiword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Porttusitusiword(registeraddressport, 0)
	temporary := uint32(Portfaitauword(registerdataport))
	...
	Porttusitusiword(registeraddressport, 0)
	Porttusitusiword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Porttusitusiword(registeraddressport, 0)
	Porttusitusiword(registerdataport, 0x48)
