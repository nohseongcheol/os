...
const (
	...
	ofaitauonly	uint32	= 0
	otusitusionly	uint32	= 1
	ofaitautusitusi	uint32	= 2
	ocreate		uint32	= 0x40
...
var stdinfaitau uint32
var stdintusitusi uint32
...
func Sysfaitau_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysfaitau, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Systusitusi, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Systusitusi, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysfaitau:
		cpu.Eax = uint32(sysfaitau(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Systusitusi:
		cpu.Eax = uint32(systusitusi(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|otusitusionly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: ofaitauonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: otusitusionly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: otusitusionly}
}
...
func systusitusi(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysfaitau(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return faitaustdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return faitauvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == otusitusionly || accessmode == ofaitautusitusi || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func faitauvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	faitaufile(entry.name[:entry.namelen], tmp)
...
func faitaustdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdintusitusi + 1) % uint32(len(stdinbuffer))
	if next == stdinfaitau {
		return
	}
	stdinbuffer[stdintusitusi] = c
	stdintusitusi = next
}

func stdingetblocking() byte {
	for stdinfaitau == stdintusitusi {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinfaitau]
	stdinfaitau = (stdinfaitau + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portfaitaubyte(0x64) & 0x01) == 0 {
	}
	sc := Portfaitaubyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Faitaupartition(&ata0s)
...
func faitaufile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Faitau(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
