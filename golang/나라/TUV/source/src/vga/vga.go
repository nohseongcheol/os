...
var attributecontrollerfaitauport uint16 = 0x3c1
var attributecontrollertusitusiport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Tusitusiregister(register []byte) {
	var regindex uint16 = 0

	Porttusitusibyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Porttusitusibyte(sequencerindexport, i)
		Porttusitusibyte(sequencerdataport, register[regindex])
		regindex++
	}

	Porttusitusibyte(crtcindexport, 0x03)

	Porttusitusibyte(crtcdataport, (Portfaitaubyte(crtcdataport) | 0x80))
	Porttusitusibyte(crtcindexport, 0x11)
	Porttusitusibyte(crtcdataport, (Portfaitaubyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Porttusitusibyte(crtcindexport, i)
		Porttusitusibyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Porttusitusibyte(graphicscontrollerindexport, i)
		Porttusitusibyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portfaitaubyte(attributecontrollerresetport)
		Porttusitusibyte(attributecontrollerindexport, i)
		Porttusitusibyte(attributecontrollertusitusiport, register[regindex])
		regindex++
	}

	Portfaitaubyte(attributecontrollerresetport)
	Porttusitusibyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Porttusitusibyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portfaitaubyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Tusitusiregister(g320x200x256)
