...
GLOBL ·識別號tDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·識別號t<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
GLOBL ·入口Handlers<>(SB), NOPTR, $NUM_IDT_ENTRIES*8
...
TEXT ·安裝idt(SB),NOSPLIT,$0
	LEAQ ·識別號tDescriptor<>(SB), AX
...
TEXT ·H控制把中斷(SB),NOSPLIT,$0-10
	...
	LEAQ ·中斷入口項目(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	SUBQ $16, SP;                          \
	MOVQ R15, 0(SP);                       \
	MOVQ ·入口Handlers<>+8*num(SB), R15;   \
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatch中斷(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
