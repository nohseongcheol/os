...
ssize_t kusoma(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_kusoma, fd, buf, count));
}

ssize_t kuandika(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_kuandika, fd, buf, count));
}
