...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portkusomabyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portkusomabyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func kuandikaps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portkuandikabyte(commandport_2, value)
	return true
}

func kuandikaps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portkuandikabyte(dataport_2, value)
	return true
}

func kusomaps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portkusomabyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portkusomabyte(commandport_2)&0x01) != 0; i++ {
		Portkusomabyte(dataport_2)
	}

	if !kuandikaps2command(0xAE) || !kuandikaps2command(0x20) {
		return
	}
	status, ok := kusomaps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !kuandikaps2command(0x60) || !kuandikaps2data(status) {
		return
	}

	if !kuandikaps2data(0xF4) {
		return
	}
	ack, ok := kusomaps2data()
	if !ok || ack != 0xFA {
		return
	}
...
var keyboardqueuekusoma uint8
var keyboardqueuekuandika uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuekuandika + 1) % keyboardqueuesize
	if next == keyboardqueuekusoma {
		return
	}
	keyboardqueue[keyboardqueuekuandika] = key
	keyboardqueuekuandika = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuekusoma != keyboardqueuekuandika {
		key := keyboardqueue[keyboardqueuekusoma]
		keyboardqueuekusoma = (keyboardqueuekusoma + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portkusomabyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portkusomabyte(dataport_2)
	if scancode == 0xE0 {
