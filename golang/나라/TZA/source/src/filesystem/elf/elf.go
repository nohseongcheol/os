...
func (self *Elf) Parse(data []byte, Pagedirectoryentry uint32) {
	...
	if elfheader.eshnum != 0 {
		...
		for i := uint32(0); i < uint32(elfheader.eshnum); i++ {
			...
			var sectJina []byte
			start := uint32(strtab.shoffset + sectheader.shJina)
			end := start
			for ; ; end++ {
			...
			if Equalbytes(sectJina, ([]byte)(".got.plt")) {
				console_2.MPrint("[")
				console_2.MPrint(sectJina)
				console_2.MPrint(":")
				self.Got = sectheader.shaddress
				console_2.MUnsignedinteger32print(self.Got)
				console_2.MPrint("]")
			}
			if Equalbytes(sectJina, ([]byte)(".dynamic")) {
				console_2.MPrint("[")
				console_2.MPrint(sectJina)
				console_2.MPrint(":")
			...
			if Equalbytes(sectJina, ([]byte)(".text")) {
			...
			if Equalbytes(sectJina, ([]byte)(".rel.text")) {
			...
			if Equalbytes(sectJina, ([]byte)(".dynsym")) {
			...
			if Equalbytes(sectJina, ([]byte)(".dynstr")) {
			...
			if Equalbytes(sectJina, ([]byte)(".strtab")) {
				...
				for st := uint32(1); st < sectheader.shsize; st++ {
					if sectvalue[st] == 0x0 || sectvalue[st] == ' ' {
						funcJina := sectvalue[start+1 : st]
						console_2.MPrint("+")
						console_2.MPrint(funcJina)
						self.strtab[rt] = Bytestostring(funcJina)
						start = st
						rt++
					}
				}
