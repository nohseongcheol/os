...
var attributecontrollerkusomaport uint16 = 0x3c1
var attributecontrollerkuandikaport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Kuandikaregister(register []byte) {
	var regindex uint16 = 0

	Portkuandikabyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portkuandikabyte(sequencerindexport, i)
		Portkuandikabyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portkuandikabyte(crtcindexport, 0x03)

	Portkuandikabyte(crtcdataport, (Portkusomabyte(crtcdataport) | 0x80))
	Portkuandikabyte(crtcindexport, 0x11)
	Portkuandikabyte(crtcdataport, (Portkusomabyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portkuandikabyte(crtcindexport, i)
		Portkuandikabyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portkuandikabyte(graphicscontrollerindexport, i)
		Portkuandikabyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portkusomabyte(attributecontrollerresetport)
		Portkuandikabyte(attributecontrollerindexport, i)
		Portkuandikabyte(attributecontrollerkuandikaport, register[regindex])
		regindex++
	}

	Portkusomabyte(attributecontrollerresetport)
	Portkuandikabyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portkuandikabyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portkusomabyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Kuandikaregister(g320x200x256)
