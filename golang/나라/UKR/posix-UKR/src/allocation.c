...
void *malloc(size_t кількість_цифр)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!кількість_цифр) кількість_цифр = 1;
    if (кількість_цифр > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    кількість_цифр = (кількість_цифр + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= кількість_цифр) {
            if (cursor->capacity - кількість_цифр >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + кількість_цифр);
                raw->capacity = cursor->capacity - кількість_цифр - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = кількість_цифр;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + кількість_цифр;
    ...
    raw->capacity = кількість_цифр; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t кількість_цифр)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(кількість_цифр);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!кількість_цифр) кількість_цифр = 1;
    cursor = (struct heap_block *)address - 1;
    if (кількість_цифр <= cursor->capacity) return address;
    destination = malloc(кількість_цифр);
