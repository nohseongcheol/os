...
int abs(int значення) { return значення < 0 ? -значення : значення; }
long labs(long значення) { return значення < 0 ? -значення : значення; }
...
char *getenv(const char *відомості_про_систему)
{
    size_t довжина = strlen(відомості_про_систему);
    char **cursor = environ;
    if (!довжина || strchr(відомості_про_систему, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, відомості_про_систему, довжина) && (*cursor)[довжина] == '=') return *cursor + довжина + 1;
        ++cursor;
    }
    return NULL;
}
