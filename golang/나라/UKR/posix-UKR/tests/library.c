...
#define CHECK(значення) do { if (!(значення)) return __LINE__; } while (0)

static int compare_int(const void *left, const void *right)
{
    int result = *(const int *)left, значення = *(const int *)right;
    return (result > значення) - (result < значення);
}

int __posix_library_test(void)
{
    char буфер_передавання_2[64], source[64], *end_pointer, *saved_state, *address, *destination;
    const char *text = "abca", *input = " 	-0x80rest";
    int elements[257], index, значення;
    size_t довжина;
    div_t result;
    ldiv_t decoded;
    ...
    CHECK(memset(буфер_передавання_2, 0xab, sizeof(буфер_передавання_2)) == буфер_передавання_2);
    CHECK((unsigned char)буфер_передавання_2[63] == 0xab);
    CHECK(memcpy(буфер_передавання_2, "abcdef", 7) == буфер_передавання_2);
    CHECK(memmove(буфер_передавання_2 + 2, буфер_передавання_2, 5) == буфер_передавання_2 + 2);
    CHECK(memcmp(буфер_передавання_2, "ababcde", 7) == 0);
    CHECK(memmove(буфер_передавання_2, буфер_передавання_2 + 2, 5) == буфер_передавання_2);
    CHECK(memcmp(буфер_передавання_2, "abcde", 5) == 0);
    CHECK(memmove(буфер_передавання_2, буфер_передавання_2, 0) == буфер_передавання_2);
    CHECK(memcmp("\x80", "\x7f", 1) > 0);
    CHECK(memcmp(буфер_передавання_2, source, 0) == 0);
    ...
    CHECK(strcpy(буфер_передавання_2, text) == буфер_передавання_2 && strcmp(буфер_передавання_2, text) == 0);
    CHECK(stpcpy(буфер_передавання_2, "xy") == буфер_передавання_2 + 2 && буфер_передавання_2[2] == 0);
    memset(буфер_передавання_2, 7, sizeof(буфер_передавання_2));
    CHECK(strncpy(буфер_передавання_2, "xy", 5) == буфер_передавання_2);
    CHECK(!memcmp(буфер_передавання_2, "xy\0\0\0", 5) && буфер_передавання_2[5] == 7);
    CHECK(stpncpy(буфер_передавання_2, "abcdef", 3) == буфер_передавання_2 + 3 && буфер_передавання_2[3] == 0);
    CHECK(stpncpy(буфер_передавання_2, "a", 4) == буфер_передавання_2 + 1 && буфер_передавання_2[3] == 0);
    CHECK(strncpy(буфер_передавання_2, "z", 0) == буфер_передавання_2 && буфер_передавання_2[0] == 'a');
    strcpy(буфер_передавання_2, "a");
    CHECK(strcat(буфер_передавання_2, "bc") == буфер_передавання_2 && !strcmp(буфер_передавання_2, "abc"));
    CHECK(strncat(буфер_передавання_2, "def", 2) == буфер_передавання_2 && !strcmp(буфер_передавання_2, "abcde"));
    CHECK(strncat(буфер_передавання_2, "xyz", 0) == буфер_передавання_2 && !strcmp(буфер_передавання_2, "abcde"));
    ...
    CHECK(strxfrm(буфер_передавання_2, "abc", 4) == 3 && !strcmp(буфер_передавання_2, "abc"));
    ...
    strcpy(буфер_передавання_2, ",,one;two,,");
    CHECK(!strcmp(strtok_r(буфер_передавання_2, ",;", &saved_state), "one"));
    strcpy(source, "x:y");
    ...
    for (значення = -1; значення <= 255; ++значення) {
        int low = значення >= 'a' && значення <= 'z', high = значення >= 'A' && значення <= 'Z';
        int digit = значення >= '0' && значення <= '9';
        CHECK(!!isalpha(значення) == (low || high));
        CHECK(!!isalnum(значення) == (low || high || digit));
        CHECK(!!isdigit(значення) == digit);
        CHECK(!!islower(значення) == low && !!isupper(значення) == high);
        CHECK(!!isblank(значення) == (значення == ' ' || значення == '\t'));
        CHECK(!!isspace(значення) == (значення == ' ' || (значення >= 9 && значення <= 13)));
        CHECK(!!iscntrl(значення) == ((значення >= 0 && значення < 32) || значення == 127));
        CHECK(!!isprint(значення) == (значення >= 32 && значення <= 126));
        CHECK(!!isgraph(значення) == (значення >= 33 && значення <= 126));
        CHECK(!!ispunct(значення) == (значення >= 33 && значення <= 126 && !low && !high && !digit));
        CHECK(!!isxdigit(значення) == (digit || (значення >= 'a' && значення <= 'f') || (значення >= 'A' && значення <= 'F')));
        CHECK(tolower(значення) == (high ? значення + 32 : значення));
        CHECK(toupper(значення) == (low ? значення - 32 : значення));
    }
    errno = EIO;
    ...
    for (довжина = 0; довжина <= 257; ++довжина) {
        for (index = 0; index < (int)довжина; ++index) elements[index] = ((index * 37 + (int)довжина) % 19) - 9;
        qsort(elements, довжина, sizeof(int), compare_int);
        for (index = 1; index < (int)довжина; ++index) CHECK(elements[index - 1] <= elements[index]);
        for (index = 0; index < (int)довжина; ++index) {
            int *found = bsearch(&elements[index], elements, довжина, sizeof(int), compare_int);
            CHECK(found && *found == elements[index]);
        }
        значення = 100;
        CHECK(bsearch(&значення, elements, довжина, sizeof(int), compare_int) == NULL);
    }
    environ = envp;
