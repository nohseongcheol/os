...
{
    unsigned int довжина = 0;
    while (text[довжина] != 0)
        довжина++;
    return довжина;
}
...
{
    (void)Запис(STDOUT_FILENO, text, text_length(text));
}

static void report(const char *відомості_про_систему, int passed)
{
    checks++;
    if (passed) {
        say("PTEST:PASS:");
...
static pid_t reap_nohang(pid_t child, int *стан)
{
    unsigned int spins;
    pid_t result;

    for (spins = 0; spins < 2000000U; spins++) {
        result = waitpid(child, стан, WNOHANG);
        if (result != 0)
            return result;
    }
    return 0;
...
static void test_open_and_io(void)
{
    ...
    if (дескриптор_файла >= 0) {
        report("read-bytes", Читання(дескриптор_файла, bytes, 2) == 2 &&
               (unsigned char)bytes[0] == 0x7f && bytes[1] == 'E');
        report("lseek-set", lseek(дескриптор_файла, 0, SEEK_SET) == 0);
        report("lseek-cur", lseek(дескриптор_файла, 1, SEEK_CUR) == 1);
        report("lseek-end", lseek(дескриптор_файла, -1, SEEK_END) > 0);
        errno = 0;
        report("lseek-negative", lseek(дескриптор_файла, -1, SEEK_SET) == -1 && errno == EINVAL);
        errno = 0;
        report("lseek-whence", lseek(дескриптор_файла, 0, 99) == -1 && errno == EINVAL);
        report("fstat-file", fstat(дескриптор_файла, &st) == 0 && S_ISREG(st.st_mode) && st.st_size > 2);
        errno = 0;
        report("fstat-efault", failed_with(fstat(дескриптор_файла, (struct stat *)0), EFAULT));
        report("fsync-file", fsync(дескриптор_файла) == 0);
        report("close-file", Закрити(дескриптор_файла) == 0);
        errno = 0;
        report("closed-fd", failed_with(Закрити(дескриптор_файла), EBADF));
    }

    root = Відкрити("/", O_RDONLY | O_DIRECTORY);
    report("open-directory", root >= 3);
    if (root >= 0) {
        report("fstat-directory", fstat(root, &st) == 0 && S_ISDIR(st.st_mode));
        errno = 0;
        report("read-directory", Читання(root, bytes, 1) == -1 && errno == EISDIR);
        (void)Закрити(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", Запис(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", Читання(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", Читання(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", Запис(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    ...
    errno = 0;
    report("open-enoent", failed_with(Відкрити("/NOFILE", O_RDONLY), ENOENT));
    errno = 0;
    report("open-efault", failed_with(Відкрити((const char *)0, O_RDONLY), EFAULT));
    errno = 0;
    report("open-write-erofs", failed_with(Відкрити("/USER2", O_WRONLY), EROFS));
    errno = 0;
    report("open-rdwr-erofs", failed_with(Відкрити("/USER2", O_RDWR), EROFS));
    errno = 0;
    report("open-create-erofs", failed_with(Відкрити("/NEWFILE", O_CREAT | O_WRONLY, 0600), EROFS));
    errno = 0;
    report("open-trunc-erofs", failed_with(Відкрити("/USER2", O_TRUNC | O_RDONLY), EROFS));
    errno = 0;
    report("open-append-erofs", failed_with(Відкрити("/USER2", O_APPEND | O_RDONLY), EROFS));
    errno = 0;
    report("open-enotdir", failed_with(Відкрити("/USER2", O_RDONLY | O_DIRECTORY), ENOTDIR));
    errno = 0;
    report("creat-erofs", failed_with(creat("/NEWFILE", 0600), EROFS));
    errno = 0;
    report("close-ebadf", failed_with(Закрити(-1), EBADF));
...
static void test_dup_and_fcntl(void)
{
    ...
    copy = dup(дескриптор_файла);
    report("dup", copy >= 0 && copy != дескриптор_файла);
    if (copy >= 0) {
        report("dup-shared-offset", Читання(дескриптор_файла, &first, 1) == 1 && Читання(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", Закрити(дескриптор_файла) == 0 && fstat(copy, &st) == 0);
        дескриптор_файла = copy;
    }

    target = dup2(дескриптор_файла, 20);
    report("dup2", target == 20 && fstat(20, &st) == 0);
    report("dup2-same", dup2(дескриптор_файла, дескриптор_файла) == дескриптор_файла);
    if (target == 20)
        (void)Закрити(20);
    errno = 0;
    report("dup2-old-ebadf", failed_with(dup2(-1, 10), EBADF));
    errno = 0;
    report("dup2-new-ebadf", failed_with(dup2(дескриптор_файла, 99), EBADF));

    report("fcntl-getfd", fcntl(дескриптор_файла, F_GETFD) == 0);
    report("fcntl-setfd", fcntl(дескриптор_файла, F_SETFD, FD_CLOEXEC) == 0 &&
           fcntl(дескриптор_файла, F_GETFD) == FD_CLOEXEC);
    high = fcntl(дескриптор_файла, F_DUPFD, 10);
    report("fcntl-dupfd", high >= 10);
    if (high >= 0) {
        report("fcntl-fd-flags-independent", fcntl(high, F_GETFD) == 0);
        (void)Закрити(high);
    }
    report("fcntl-getfl", (fcntl(дескриптор_файла, F_GETFL) & O_ACCMODE) == O_RDONLY);
    report("fcntl-setfl", fcntl(дескриптор_файла, F_SETFL, O_APPEND) == 0 &&
           (fcntl(дескриптор_файла, F_GETFL) & O_APPEND) != 0);
    errno = 0;
    report("fcntl-command-einval", failed_with(fcntl(дескриптор_файла, 999), EINVAL));
    errno = 0;
...
static void test_terminal_and_uname(void)
{
    ...
    if (дескриптор_файла >= 0) {
        errno = 0;
        report("isatty-enotty", isatty(дескриптор_файла) == 0 && errno == ENOTTY);
        (void)Закрити(дескриптор_файла);
    } else {
    ...
    report("uname", uname(&відомості_про_систему) == 0 && відомості_про_систему.sysname[0] != 0 &&
           відомості_про_систему.nodename[0] != 0 && text_equal(відомості_про_систему.machine, "i386"));
    errno = 0;
...
void posix_test_process(void)
{
    ...
    report("fork", child > 0);
    if (child > 0) {
        waited = reap_nohang(child, &стан);
        report("waitpid", waited == child && WIFEXITED(стан) && WEXITSTATUS(стан) == 23);
        errno = 0;
        report("waitpid-echild", waitpid(child, &стан, WNOHANG) == -1 && errno == ECHILD);
    }
    ...
    if (child > 0 && reap_nohang(child, &стан) == child) {
        report("fork-fd-isolation", Читання(дескриптор_файла, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
    ...
    if (child > 0) {
        waited = reap_nohang(child, &стан);
        report("execve", waited == child && WIFEXITED(стан) && WEXITSTATUS(стан) == 37);
    }
    ...
    if (child > 0) {
        ...
        for (spin = 0; spin < 2000000U; spin++)
            (void)getpid();
        waited = wait(&стан);
        report("wait", waited == child && WIFEXITED(стан) && WEXITSTATUS(стан) == 29);
    }
}
