...
freestanding library in `./простір_користувача/posix`.
...
```sh
cd ./простір_користувача/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./простір_користувача/posix/оголошення` and link the archive into an EngOS i386 user
...
`./простір_користувача/posix/TEST_RESULTS.md`. Build all runtime test executables
