...
GLOBL ·іДЕНТИФІКАТОРtDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·іДЕНТИФІКАТОРt<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
TEXT ·встановитиidt(SB),NOSPLIT,$0
	LEAQ ·іДЕНТИФІКАТОРtDescriptor<>(SB), AX
	MOVW $(NUM_IDT_ENTRIES*IDT_ENTRY_SIZE)-1, 0(AX)
	LEAQ ·іДЕНТИФІКАТОРt<>(SB), BX
	MOVQ BX, 2(AX)
...
// used).
TEXT ·ЕлементкеруванняПереривання(SB),NOSPLIT,$0-10
	...
	LEAQ ·перериванняgateзапис(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	...
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchПереривання(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchПереривання(SB),NOSPLIT,$0
...
TEXT ·перериванняgateзапис(SB),NOSPLIT,$0
