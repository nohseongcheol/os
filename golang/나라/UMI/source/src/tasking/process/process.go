...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	process := (*Process)(self.mem.Allocate_memory(uint32(Sizeof(Process{}))))
	if process == nil {
		return Process{}
	}
	...
	mainthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if mainthread != nil {
		mainthread.Pid = process.id
		mainthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(mainthread)))
	}
