...
ssize_t Oʻqish(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Oʻqish, fd, buf, count));
}

ssize_t Yozish(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_Yozish, fd, buf, count));
}

int Yopish(int fd)
{
    return (int)__syscall_result(SC1(SYS_Yopish, fd));
}
