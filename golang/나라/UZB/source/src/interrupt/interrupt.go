...
func Yangiinterrupthandler(Interruptmanager uintptr, InterruptRAQAM uint8) *TInterrupthandler {
	interrupthandler_2 := new(TInterrupthandler)
	interrupthandler_2.InterruptRAQAM = InterruptRAQAM
...
func (self *TInterrupthandler) Init(InterruptRAQAM uint8, Interruptmanager uintptr, funcaddress uintptr) {

	handler_2[InterruptRAQAM] = funcaddress

	self.InterruptRAQAM = InterruptRAQAM
	self.Interruptmanager = Interruptmanager

}
func (self *TInterrupthandler) Sethandleinterruptfuction(InterruptRAQAM uint32, address uintptr) {
	handler_2[InterruptRAQAM] = address
}
func (self *TInterrupthandler) Destroy() {
	...
	if selfuintptr == Interruptmanager.Gethandler(self.InterruptRAQAM) {
		Interruptmanager.Sethandler(0, self.InterruptRAQAM)
	}

}
func (self *TInterrupthandler) Setinterruptmanager(Interruptmanager uintptr) {
}
func (self *TInterrupthandler) SetinterruptRAQAM(InterruptRAQAM uint8) {
	self.InterruptRAQAM = InterruptRAQAM
}
...
type TInterruptdescriptortableKorsatgich struct {
}

var idtdata [256 * 8]uint8
var Faolinterruptmanager uintptr = 0
...
type TInterruptmanager struct {
	...
	qurilmalarinterruptoffset	uint16

	taskmanager	*TTaskmanager
}

var PrimarypicBuyruqioport uint16 = 0x20
var Primarypicdataioport uint16 = 0x21
var SecondarypicBuyruqioport uint16 = 0xA0
var Secondarypicdataioport uint16 = 0xA1

func (self *TInterruptmanager) Init(qurilmalarinterruptoffset uint16, globaldescriptortable *TShareddescriptortable, taskmanager *TTaskmanager) {

	self.taskmanager = taskmanager

	self.qurilmalarinterruptoffset = qurilmalarinterruptoffset
	...
	PortYozishbyte(PrimarypicBuyruqioport, 0x11)
	PortYozishbyte(SecondarypicBuyruqioport, 0x11)

	PortYozishbyte(Primarypicdataioport, 0x20)
	PortYozishbyte(Secondarypicdataioport, 0x28)

	PortYozishbyte(Primarypicdataioport, 0x04)
	PortYozishbyte(Secondarypicdataioport, 0x02)

	PortYozishbyte(Primarypicdataioport, 0x01)
	PortYozishbyte(Secondarypicdataioport, 0x01)

	PortYozishbyte(Primarypicdataioport, 0xF8)
	PortYozishbyte(Secondarypicdataioport, 0xEF)

	idtKorsatgich := [6]uint8{0, 0, 0, 0, 0, 0}
	hajmi := (*uint16)(Pointer(&idtKorsatgich[0]))
	(*hajmi) = (uint16)(Sizeof(idtdata) - 1)

	base := (*uint32)(Pointer(&idtKorsatgich[2]))
	(*base) = uint32(uintptr(Pointer(&idtdata)))

	Lidt(uintptr(Pointer(&idtKorsatgich)))
}
...
func (self *TInterruptmanager) Interruptdescriptortableentryset(interrupt int,
	...
	DescriptorTuri uint8) {

	handleraddressPastbitlar := (*uint16)(Pointer(&idtdata[interrupt*8+0]))
	(*handleraddressPastbitlar) = uint16(handler & 0xFFFF)
	...
	ruxsat := (*uint8)(Pointer(&idtdata[interrupt*8+5]))
	(*ruxsat) = (Idtdescriptorpresent | DescriptorTuri | ((Descriptorprivilegelevel & 3) << 5))

	handleraddressYuqoribitlar := (*uint16)(Pointer(&idtdata[interrupt*8+6]))
	(*handleraddressYuqoribitlar) = uint16((handler >> 16) & 0xFFFF)

}

func (self *TInterruptmanager) Sethandler(handler uintptr, InterruptRAQAM uint8) {
	handler_2[InterruptRAQAM] = handler
}
func (self *TInterruptmanager) Gethandler(InterruptRAQAM uint8) uintptr {
	return handler_2[InterruptRAQAM]
}
func (self *TInterruptmanager) Dohandleinterrupt(interrupt uint8, esp uint32) uint32 {

	if interruptdebug {
		console_2.MChopetishxy("[esp:", 1, 20)
		console_2.MUnsignedinteger32Chopetish(uint32(interrupt))
		console_2.MChopetish(":")
		console_2.MUnsignedinteger32Chopetish(esp)
	}
	handlerBajarish := false
	if handler_2[interrupt] != 0 {

		myfunction := (*(*(func(uint32) uint32))(Pointer(handler_2[interrupt])))
		esp = myfunction(esp)
		handlerBajarish = true

	}

	if !handlerBajarish && interrupt == uint8(self.qurilmalarinterruptoffset) && self.taskmanager != nil {
		esp = uint32(uintptr(Pointer(self.taskmanager.Schedule((*Tcpustate)(Pointer(uintptr(esp)))))))

	}
	if !handlerBajarish && interrupt == 0x80 {
	...
	if 0x20 <= interrupt && interrupt < 0x30 {
		if 0x28 <= interrupt {
			PortYozishbyte(SecondarypicBuyruqioport, 0x20)
		}
		PortYozishbyte(PrimarypicBuyruqioport, 0x20)
	}
	return esp
}
...
func Handleinterrupt(esp uint32, interrupt uint32) uint32 {
	...
	if interruptdebug && interrupt != 0x80 && interrupt != 0x20 {
		console_2.MChopetishxy("[esp:", 1, 21)
		console_2.MUnsignedinteger32Chopetish(uint32(interrupt))
		console_2.MChopetish(":")
		console_2.MUnsignedinteger32Chopetish(esp)
	}

	if Faolinterruptmanager != 0 {
		p := (*TInterruptmanager)(Pointer(Faolinterruptmanager))
...
func exceptionhasXatocode(interrupt uint32) bool {
	switch interrupt {
...
func exceptionNomi(interrupt uint32) string {
	switch interrupt {
	case 0x00:
...
func exceptionRamkaQiymat(ramka uint32, offset uint32) uint32 {
	return *(*uint32)(Pointer(uintptr(ramka + offset)))
}
...
func chopetishSAHIFAfaultinfo(err uint32) {
...
func chopetishexceptionselectorinfo(err uint32) {
...
func Handleexception(esp uint32, interrupt uint32) uint32 {
	...
	if exceptionhasXatocode(interrupt) {
		err = exceptionRamkaQiymat(esp, 0)
		eipoffset = 4
	}
	eip := exceptionRamkaQiymat(esp, eipoffset)
	cs := exceptionRamkaQiymat(esp, eipoffset+4)
	eflags := exceptionRamkaQiymat(esp, eipoffset+8)
	...
	if interrupt == 0x0E {
		...
		chopetishSAHIFAfaultinfo(err)
	}

	if (cs & 0x03) != 0 {
		MEmergencylogstring(" useresp=")
		MEmergencylogunsignedinteger32(exceptionRamkaQiymat(esp, eipoffset+12))
		MEmergencylogstring(" ss=")
		MEmergencylogunsignedinteger32(exceptionRamkaQiymat(esp, eipoffset+16))
	}

	if exceptionhasXatocode(interrupt) {
		chopetishexceptionselectorinfo(err)
	}
	MEmergencylogstring("\n=== ENGOS HALTED AFTER EXCEPTION ===\n")
	except++
...
func HandlefatalinterruptRamka(savedesp uint32, interrupt uint32) uint32 {
...
func MySinash(interrupt uint8, esp uint32)
...
func interrupthandler_2(interrupt uint8, esp uint32) uint32 {
	...
	console_2 := TConsole{}
	console_2.MChopetish(buffer)
	console_2.MHexadecimalChopetish(0x40)
	return esp
}
func chopetishesp(esp uint32) {
	console_2 := TConsole{}
	console_2.MUnsignedinteger32Chopetishxy(esp, 20, 21)
}
func gettls() uint32
func Chopetishtls() {
