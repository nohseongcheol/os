...
func Getunsignedintegerpointerarrayfrompointer(address_reference_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		address	uintptr
		len	int
		cap	int
	}{address_reference_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func Getunsignedinteger32arrayfrompointer(address_reference_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		address	uintptr
...
func Getbytesfrompointer(address_reference_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		address	uintptr
		len	int
		cap	int
...
func Getfuncpointerfrompointer(address_reference_2 uintptr) func() {
	code1pointer_2 := uintptr(Pointer(&address_reference_2))
