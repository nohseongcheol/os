...
void *malloc(size_t số_chữ_số)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!số_chữ_số) số_chữ_số = 1;
    if (số_chữ_số > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    số_chữ_số = (số_chữ_số + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= số_chữ_số) {
            if (cursor->capacity - số_chữ_số >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + số_chữ_số);
                raw->capacity = cursor->capacity - số_chữ_số - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = số_chữ_số;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + số_chữ_số;
    ...
    raw->capacity = số_chữ_số; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t số_chữ_số)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(số_chữ_số);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!số_chữ_số) số_chữ_số = 1;
    cursor = (struct heap_block *)address - 1;
    if (số_chữ_số <= cursor->capacity) return address;
    destination = malloc(số_chữ_số);
