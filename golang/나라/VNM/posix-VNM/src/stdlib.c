...
int abs(int giá_trị) { return giá_trị < 0 ? -giá_trị : giá_trị; }
long labs(long giá_trị) { return giá_trị < 0 ? -giá_trị : giá_trị; }
...
char *getenv(const char *thông_tin_hệ_thống)
{
    size_t độ_dài = strlen(thông_tin_hệ_thống);
    char **cursor = environ;
    if (!độ_dài || strchr(thông_tin_hệ_thống, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, thông_tin_hệ_thống, độ_dài) && (*cursor)[độ_dài] == '=') return *cursor + độ_dài + 1;
        ++cursor;
    }
    return NULL;
}
