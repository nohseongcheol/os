...
static void say(const char *s) { (void)Ghi(STDOUT_FILENO, s, length_of(s)); }

static void report(const char *thông_tin_hệ_thống, int pass)
{
...
int main(void)
{
    ...
    report("recvfrom", recvfrom(server, bộ_đệm_truyền_2, sizeof(bộ_đệm_truyền_2), 0,
                                (struct sockaddr *)&source, &source_length) == 4 &&
                       bộ_đệm_truyền_2[0] == 'p' && bộ_đệm_truyền_2[3] == 'g' && source_length == 16);
    errno = 0;
    report("empty-eagain", recv(server, bộ_đệm_truyền_2, sizeof(bộ_đệm_truyền_2), 0) == -1 && errno == EAGAIN);
    ...
    report("close-client", Đóng(client) == 0);
    report("close-server", Đóng(server) == 0);
    say("NTEST:DONE\n");
    return 0;
}
