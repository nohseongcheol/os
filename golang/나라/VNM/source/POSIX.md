...
freestanding library in `./không_gian_người_dùng/posix`.
...
```sh
cd ./không_gian_người_dùng/posix
make clean all check
make runtime
```

The resulting library is `build/libengos_posix.a`. Include headers with
`-I./không_gian_người_dùng/posix/khai_báo` and link the archive into an EngOS i386 user
...
`./không_gian_người_dùng/posix/TEST_RESULTS.md`. Build all runtime test executables
