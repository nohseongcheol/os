# WorldOS VNM 완전 독립 소스

이 트리는 engos의 현행 기능을 대표 언어(vi)의 기능 의미에 따라 명명한 독립 빌드 소스입니다. 언어 이름 토큰과 해시는 식별자에 사용하지 않습니다. Go 언어·런타임 ABI와 POSIX 공개 ABI 이름은 호환성을 위해 유지합니다. `name-correspondence.tsv`, `file-correspondence.tsv`, `glossary.tsv`에서 변환 근거를 추적할 수 있습니다.
