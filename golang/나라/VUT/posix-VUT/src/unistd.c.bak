...
ssize_t ridim(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ridim, fd, buf, count));
}

ssize_t raetem(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_raetem, fd, buf, count));
}
