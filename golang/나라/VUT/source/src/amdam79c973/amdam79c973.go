...
func (self *Tamdam79c973) Initdriver(interrupt *TInterruptmanager, devicedescriptor TPeripheralcomponentinterconnectdevicedescriptor, handler IRawdatahandler) {
	...
	var Mac0 uint64 = uint64(Portridimword(Macaddress0port) % 256)
	var Mac1 uint64 = uint64(Portridimword(Macaddress0port) / 256)
	var Mac2 uint64 = uint64(Portridimword(Macaddress2port) % 256)
	var Mac3 uint64 = uint64(Portridimword(Macaddress2port) / 256)
	var Mac4 uint64 = uint64(Portridimword(Macaddress4port) % 256)
	var Mac5 uint64 = uint64(Portridimword(Macaddress4port) / 256)
	...
	Portraetemword(registeraddressport, 20)
	Portraetemword(buscontrolregisterdataport, 0x102)

	Portraetemword(registeraddressport, 0)
	Portraetemword(registerdataport, 0x04)
	...
	Portraetemword(registeraddressport, 1)
	Portraetemword(registerdataport, uint16(uintptr(Pointer(&initblock))&0xFFFF))

	Portraetemword(registeraddressport, 2)
	Portraetemword(registerdataport, uint16((uintptr(Pointer(&initblock))>>16)&0xFFFF))

}
func (self *Tamdam79c973) Activate() {
	Portraetemword(registeraddressport, 0)
	Portraetemword(registerdataport, 0x41)

	Portraetemword(registeraddressport, 4)
	temporary := Portridimword(registerdataport)
	Portraetemword(registeraddressport, 4)
	Portraetemword(registerdataport, temporary|0xC00)

	Portraetemword(registeraddressport, 0)
	Portraetemword(registerdataport, 0x42)

}
func (self *Tamdam79c973) Reset() int {
	Portridimword(resetport)
	Portraetemword(resetport, 0)
	return 10
}
...
func (self *Tamdam79c973) Handleinterrupt(esp uint32) uint32 {
	...
	Portraetemword(registeraddressport, 0)
	temporary := uint32(Portridimword(registerdataport))
	...
	Portraetemword(registeraddressport, 0)
	Portraetemword(registerdataport, uint16(temporary))
...
func (self *Tamdam79c973) Send(datapointer uintptr, size uint32) {
	...
	Portraetemword(registeraddressport, 0)
	Portraetemword(registerdataport, 0x48)
