...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Raetem(0xA0)
	} else {
		self.deviceport.Raetem(0xB0)
	}
	self.controlport.Raetem(0)
	self.deviceport.Raetem(0xA0)

	var status uint8 = self.commandport.Ridim()
	if status == 0xFF {
	...
	self.sectorcountport.Raetem(0)
	self.lbalowport.Raetem(0)
	self.lbamidport.Raetem(0)
	self.lbahiport.Raetem(0)
	self.commandport.Raetem(0xEC)

	status = self.commandport.Ridim()
	if status == 0x00 {
...
func (self *TAdvancedtechnologyattachment) Ridim28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Raetem(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Raetem(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Raetem(0)
	self.sectorcountport.Raetem(1)

	self.lbalowport.Raetem(uint8(sector & 0x000000FF))
	self.lbamidport.Raetem(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Raetem(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Raetem(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Ridim()
...
func (self *TAdvancedtechnologyattachment) Raetem28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Raetem(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Raetem(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Raetem(0)
	self.sectorcountport.Raetem(1)
	self.lbalowport.Raetem(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Raetem(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Raetem(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Raetem(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Raetem(0xE0)
	} else {
		self.deviceport.Raetem(0xF0)
	}
	self.commandport.Raetem(0xE7)
