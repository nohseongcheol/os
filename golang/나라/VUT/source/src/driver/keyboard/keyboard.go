...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portridimbyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portridimbyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func raetemps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portraetembyte(commandport_2, value)
	return true
}

func raetemps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portraetembyte(dataport_2, value)
	return true
}

func ridimps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portridimbyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portridimbyte(commandport_2)&0x01) != 0; i++ {
		Portridimbyte(dataport_2)
	}

	if !raetemps2command(0xAE) || !raetemps2command(0x20) {
		return
	}
	status, ok := ridimps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !raetemps2command(0x60) || !raetemps2data(status) {
		return
	}

	if !raetemps2data(0xF4) {
		return
	}
	ack, ok := ridimps2data()
	if !ok || ack != 0xFA {
...
var keyboardqueueridim uint8
var keyboardqueueraetem uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueueraetem + 1) % keyboardqueuesize
	if next == keyboardqueueridim {
		return
	}
	keyboardqueue[keyboardqueueraetem] = key
	keyboardqueueraetem = next
}

func Processpendingkeyboardevents() {
	for keyboardqueueridim != keyboardqueueraetem {
		key := keyboardqueue[keyboardqueueridim]
		keyboardqueueridim = (keyboardqueueridim + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portridimbyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portridimbyte(dataport_2)
	if scancode == 0xE0 {
