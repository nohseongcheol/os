...
const (
	...
	oridimonly	uint32	= 0
	oraetemonly	uint32	= 1
	oridimraetem	uint32	= 2
	ocreate		uint32	= 0x40
...
var stdinridim uint32
var stdinraetem uint32
...
func Sysridim_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysridim, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysraetem, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysraetem, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysridim:
		cpu.Eax = uint32(sysridim(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysraetem:
		cpu.Eax = uint32(sysraetem(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|oraetemonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: oridimonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: oraetemonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: oraetemonly}
}
...
func sysraetem(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysridim(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return ridimstdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return ridimvfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == oraetemonly || accessmode == oridimraetem || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func ridimvfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	ridimfile(entry.name[:entry.namelen], tmp)
...
func ridimstdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinraetem + 1) % uint32(len(stdinbuffer))
	if next == stdinridim {
		return
	}
	stdinbuffer[stdinraetem] = c
	stdinraetem = next
}

func stdingetblocking() byte {
	for stdinridim == stdinraetem {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinridim]
	stdinridim = (stdinridim + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portridimbyte(0x64) & 0x01) == 0 {
	}
	sc := Portridimbyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Ridimpartition(&ata0s)
...
func ridimfile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Ridim(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
