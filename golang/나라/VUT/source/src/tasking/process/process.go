...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	stambathread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if stambathread != nil {
		stambathread.Pid = process.id
		stambathread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(stambathread)))
	}
