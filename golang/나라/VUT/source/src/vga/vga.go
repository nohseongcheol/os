...
var attributecontrollerridimport uint16 = 0x3c1
var attributecontrollerraetemport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Raetemregister(register []byte) {
	var regindex uint16 = 0

	Portraetembyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portraetembyte(sequencerindexport, i)
		Portraetembyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portraetembyte(crtcindexport, 0x03)

	Portraetembyte(crtcdataport, (Portridimbyte(crtcdataport) | 0x80))
	Portraetembyte(crtcindexport, 0x11)
	Portraetembyte(crtcdataport, (Portridimbyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portraetembyte(crtcindexport, i)
		Portraetembyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portraetembyte(graphicscontrollerindexport, i)
		Portraetembyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portridimbyte(attributecontrollerresetport)
		Portraetembyte(attributecontrollerindexport, i)
		Portraetembyte(attributecontrollerraetemport, register[regindex])
		regindex++
	}

	Portridimbyte(attributecontrollerresetport)
	Portraetembyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portraetembyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portridimbyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Raetemregister(g320x200x256)
