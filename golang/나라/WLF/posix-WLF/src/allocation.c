...
void *malloc(size_t nombre_de_chiffres)
{
    ...
    void *address;
    if (!nombre_de_chiffres) nombre_de_chiffres = 1;
    if (nombre_de_chiffres > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    nombre_de_chiffres = (nombre_de_chiffres + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= nombre_de_chiffres) {
            if (cursor->capacity - nombre_de_chiffres >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + nombre_de_chiffres);
                raw->capacity = cursor->capacity - nombre_de_chiffres - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = nombre_de_chiffres;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    address = déplacer_la_fin_de_mémoire_dynamique(0);
    if (address == (void *)-1) return NULL;
    padding = (0U - (uintptr_t)address) & 15U;
    total = padding + sizeof(struct heap_block) + nombre_de_chiffres;
    if ((uintptr_t)address > (uintptr_t)INT_MAX - total) { errno = ENOMEM; return NULL; }
    address = déplacer_la_fin_de_mémoire_dynamique((int)total);
    if (address == (void *)-1) return NULL;
    raw = (struct heap_block *)((unsigned char *)address + padding);
    raw->capacity = nombre_de_chiffres; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t nombre_de_chiffres)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(nombre_de_chiffres);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!nombre_de_chiffres) nombre_de_chiffres = 1;
    cursor = (struct heap_block *)address - 1;
    if (nombre_de_chiffres <= cursor->capacity) return address;
    destination = malloc(nombre_de_chiffres);
