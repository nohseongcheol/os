...
func tusips2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Porttusibyte(commandport_2, value)
	return true
}

func tusips2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Porttusibyte(dataport_2, value)
	return true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	if !tusips2command(0xAE) || !tusips2command(0x20) {
		return
	}
	...
	status |= 0x01
	status &^= 0x10
	if !tusips2command(0x60) || !tusips2data(status) {
		return
	}
...
var keyboardqueuetusi uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuetusi + 1) % keyboardqueuesize
	if next == keyboardqueuefaitau {
		return
	}
	keyboardqueue[keyboardqueuetusi] = key
	keyboardqueuetusi = next
}

func Processpendingkeyboardevents() {
	for keyboardqueuefaitau != keyboardqueuetusi {
