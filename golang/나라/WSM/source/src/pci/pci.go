...
func (self *TPeripheralcomponentinterconnectcontroller) Faitau(bus uint16, device_2 uint16, function uint16, registeroffset uint32) uint32 {
	...
	Porttusidword(self.commandport, id)
...
func (self *TPeripheralcomponentinterconnectcontroller) Tusi(bus uint16, device_2 uint16, function uint16, registeroffset uint32, value uint32) {
	var id uint32
	id = 0x1<<31 | uint32((bus&0xFF)<<16) | uint32((device_2&0x1f)<<11) | uint32((function&0x07)<<8) | uint32(registeroffset&0xFC)
	Porttusidword(self.commandport, id)
	Porttusidword(self.dataport, value)
}
