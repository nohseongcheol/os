...
GLOBL ·الهويةtDescriptor<>(SB), NOPTR, $10
...
// [12 - 15] reserved
GLOBL ·الهويةt<>(SB), NOPTR, $NUM_IDT_ENTRIES*IDT_ENTRY_SIZE
...
GLOBL ·بوابةHandlers<>(SB), NOPTR, $NUM_IDT_ENTRIES*8
...
TEXT ·Hالتعاملمقاطعة(SB),NOSPLIT,$0-10
	...
	LEAQ ·مقاطعةبوابةentry(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	SUBQ $16, SP;                          \
	MOVQ R15, 0(SP);                       \
	MOVQ ·بوابةHandlers<>+8*num(SB), R15;   \
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchمقاطعة(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchمقاطعة(SB),NOSPLIT,$0
...
TEXT ·مقاطعةبوابةentry(SB),NOSPLIT,$0
