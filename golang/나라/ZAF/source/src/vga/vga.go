...
func (self *TVideographicsarray) Writeregister(register []byte) {
	var regindex uint16 = 0

	Portwritebyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portwritebyte(sequencerindexport, i)
		Portwritebyte(sequencerdataport, register[regindex])
		regindex++
	}
	...
	for i = 0; i < 25; i++ {
		...
		Portwritebyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portwritebyte(graphicscontrollerindexport, i)
		Portwritebyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}
	...
	for i = 0; i < 21; i++ {
		...
		Portwritebyte(attributecontrollerwriteport, register[regindex])
		regindex++
	}
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Writeregister(g320x200x256)
