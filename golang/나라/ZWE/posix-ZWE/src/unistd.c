...
ssize_t verenga(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_verenga, fd, buf, count));
}

ssize_t nyora(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_nyora, fd, buf, count));
}
