...
func (self *TAdvancedtechnologyattachment) Identify() {
	...
	if self.master {
		self.deviceport.Nyora(0xA0)
	} else {
		self.deviceport.Nyora(0xB0)
	}
	self.controlport.Nyora(0)
	self.deviceport.Nyora(0xA0)

	var status uint8 = self.commandport.Verenga()
	if status == 0xFF {
	...
	self.sectorcountport.Nyora(0)
	self.lbalowport.Nyora(0)
	self.lbamidport.Nyora(0)
	self.lbahiport.Nyora(0)
	self.commandport.Nyora(0xEC)

	status = self.commandport.Verenga()
	if status == 0x00 {
	...
	for i := 0; i < 256; i++ {
		var data = self.dataport.Verenga()
		text := []byte("  ")
...
func (self *TAdvancedtechnologyattachment) Verenga28(sector uint32, data *[]byte, count int) {
	var console_2 = TConsole{}
	...
	if self.master {
		self.deviceport.Nyora(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.deviceport.Nyora(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorport.Nyora(0)
	self.sectorcountport.Nyora(1)

	self.lbalowport.Nyora(uint8(sector & 0x000000FF))
	self.lbamidport.Nyora(uint8((sector & 0x0000FF00) >> 8))
	self.lbahiport.Nyora(uint8((sector & 0x00FF0000) >> 16))
	self.commandport.Nyora(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataport.Verenga()
	...
	for i := (count + (count % 2)); i < Bytespersector; i += 2 {
		self.dataport.Verenga()
	}
}
func (self *TAdvancedtechnologyattachment) Nyora28(sectornumber uint32, data []byte, count uint32) {
	...
	if self.master {
		self.deviceport.Nyora(uint8(0xE0 | uint8((sectornumber&0x0F000000)>>24)))
	} else {
		self.deviceport.Nyora(uint8(0xF0 | uint8((sectornumber&0x0F000000)>>24)))
	}

	self.errorport.Nyora(0)
	self.sectorcountport.Nyora(1)
	self.lbalowport.Nyora(uint8(sectornumber & 0x000000FF))
	self.lbamidport.Nyora(uint8((sectornumber & 0x0000FF00) >> 8))
	self.lbahiport.Nyora(uint8((sectornumber & 0x00FF0000) >> 16))
	self.commandport.Nyora(0x30)
...
func (self *TAdvancedtechnologyattachment) Flush() {
	if self.master {
		self.deviceport.Nyora(0xE0)
	} else {
		self.deviceport.Nyora(0xF0)
	}
	self.commandport.Nyora(0xE7)
