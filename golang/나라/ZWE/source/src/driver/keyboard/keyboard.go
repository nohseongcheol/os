...
func waitps2inputempty() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portverengabyte(commandport_2) & 0x02) == 0 {
			return true
		}
	}
	return false
...
func waitps2outputfull() bool {
	for i := 0; i < ps2waitlimit; i++ {
		if (Portverengabyte(commandport_2) & 0x01) != 0 {
			return true
		}
	}
	return false
...
func nyoraps2command(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portnyorabyte(commandport_2, value)
	return true
}

func nyoraps2data(value uint8) bool {
	if !waitps2inputempty() {
		return false
	}
	Portnyorabyte(dataport_2, value)
	return true
}

func verengaps2data() (uint8, bool) {
	if !waitps2outputfull() {
		return 0, false
	}
	return Portverengabyte(dataport_2), true
}
...
func (self *TKeyboarddriver) Initdriver(manager *TInterruptmanager, keyboardeventhandler IKeyboardeventhandler) {
	...
	for i := 0; i < 32 && (Portverengabyte(commandport_2)&0x01) != 0; i++ {
		Portverengabyte(dataport_2)
	}

	if !nyoraps2command(0xAE) || !nyoraps2command(0x20) {
		return
	}
	status, ok := verengaps2data()
	if !ok {
		return
	}
	status |= 0x01
	status &^= 0x10
	if !nyoraps2command(0x60) || !nyoraps2data(status) {
		return
	}

	if !nyoraps2data(0xF4) {
		return
	}
	ack, ok := verengaps2data()
	if !ok || ack != 0xFA {
		return
	}
...
var keyboardqueueverenga uint8
var keyboardqueuenyora uint8
var leftshift bool
var rightshift bool
var extendedscancode bool

func queuekeyboardbyte(key byte) {
	next := (keyboardqueuenyora + 1) % keyboardqueuesize
	if next == keyboardqueueverenga {
		return
	}
	keyboardqueue[keyboardqueuenyora] = key
	keyboardqueuenyora = next
}

func Processpendingkeyboardevents() {
	for keyboardqueueverenga != keyboardqueuenyora {
		key := keyboardqueue[keyboardqueueverenga]
		keyboardqueueverenga = (keyboardqueueverenga + 1) % keyboardqueuesize
		Stdinputbyte(key)
...
func (self *TKeyboarddriver) Handleinterrupt(esp uint32) uint32 {
	status := Portverengabyte(commandport_2)
	if (status&0x01) == 0 || (status&0x20) != 0 {
		return esp
	}

	scancode := Portverengabyte(dataport_2)
	if scancode == 0xE0 {
