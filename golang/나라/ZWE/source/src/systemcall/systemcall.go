...
const (
	...
	overengaonly	uint32	= 0
	onyoraonly	uint32	= 1
	overenganyora	uint32	= 2
	ocreate		uint32	= 0x40
...
var stdinverenga uint32
var stdinnyora uint32
...
func Sysverenga_2(fd uint32, buffer []byte) int32 {
	if len(buffer) == 0 {
		return 0
	}
	return int32(Syscall(Sysverenga, fd, uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer))))
}

func Sysprintstr(buffer string) {
	h := (*stringheader)(Pointer(&buffer))
	Syscall(Sysnyora, uint32(stdoutfd), uint32(h.Data), uint32(h.Len))
}
...
func Sysprintf(buffer []byte) {
	...
	Syscall(Sysnyora, uint32(stdoutfd), uint32(uintptr(Pointer(&buffer[0]))), uint32(len(buffer)))
}
...
func handleinterrupt(esp uint32) uint32 {
	...
	case Sysfork:
		...
		return esp
	case Sysverenga:
		cpu.Eax = uint32(sysverenga(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysnyora:
		cpu.Eax = uint32(sysnyora(int32(cpu.Ebx), cpu.Ecx, cpu.Edx))
		return esp
	case Sysopen:
		cpu.Eax = uint32(sysopen(cpu.Ebx, cpu.Ecx, cpu.Edx))
		return esp
	case Syscreat:
		cpu.Eax = uint32(sysopen(cpu.Ebx, ocreate|onyoraonly|otruncate, cpu.Ecx))
		return esp
	case Sysclose:
...
func initfiledescriptor() {
	...
	openfiletable[0] = openfiledescription{used: true, kind: fdkindstdin, flags: overengaonly}
	openfiletable[1] = openfiledescription{used: true, kind: fdkindconsole, flags: onyoraonly}
	openfiletable[2] = openfiledescription{used: true, kind: fdkindconsole, flags: onyoraonly}
}
...
func sysnyora(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
...
func sysverenga(fd int32, address uint32, count uint32) int32 {
	if count == 0 {
		return 0
	}
	if address == 0 || address+count < address {
	...
	if entry.kind == fdkindstdin {
		return verengastdin(address, count)
	}
	...
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
	return verengavfsfile(entry, buffer, count)
}
...
func sysopen(pathaddress uint32, flags uint32, mode uint32) int32 {
	...
	accessmode := flags & 3
	if accessmode == onyoraonly || accessmode == overenganyora || (flags&(ocreate|otruncate|oappend)) != 0 {
		return Erofs
	}
...
func verengavfsfile(entry *openfiledescription, destination_2 []byte, count uint32) int32 {
	...
	verengafile(entry.name[:entry.namelen], tmp)
...
func verengastdin(address uint32, count uint32) int32 {
	if address == 0 {
		return Einval
	}
	buffer := Getbytesfrompointer(uintptr(address), int(count), int(count))
...
func Stdinputbyte(c byte) {
	next := (stdinnyora + 1) % uint32(len(stdinbuffer))
	if next == stdinverenga {
		return
	}
	stdinbuffer[stdinnyora] = c
	stdinnyora = next
}

func stdingetblocking() byte {
	for stdinverenga == stdinnyora {
		sc := pollkeyboardscancode()
		if sc != 0 {
			Stdinputbyte(sc)
		}
	}
	c := stdinbuffer[stdinverenga]
	stdinverenga = (stdinverenga + 1) % uint32(len(stdinbuffer))
	return c
}

func pollkeyboardscancode() byte {
	for (Portverengabyte(0x64) & 0x01) == 0 {
	}
	sc := Portverengabyte(0x60)
...
func filesize(filename []byte) uint32 {
	...
	partition.Verengapartition(&ata0s)
...
func verengafile(filename []byte, data []byte) {
	var ata0s = TAdvancedtechnologyattachment{}
	ata0s.Init(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosparameterblock32{}
	bios.Verenga(&ata0s, partition.Mbr.Primarypartition[0], filename, data)
	ata0s.Flush()
}
