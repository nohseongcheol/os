...
func (self *Processhelper) Create(entrypoint func(), threadhelper *TThreadhelper, Pagedirectoryentry uint32, iskernel bool) Process {
	...
	chikuruthread := threadhelper.Createpointerfromfunction(entrypoint, Pagedirectoryentry, iskernel)
	if chikuruthread != nil {
		chikuruthread.Pid = process.id
		chikuruthread.Parentpid = 0
		process.Threads.Append_to_list(uintptr(Pointer(chikuruthread)))
	}
