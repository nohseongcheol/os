...
var attributecontrollerverengaport uint16 = 0x3c1
var attributecontrollernyoraport uint16 = 0x3c0
var attributecontrollerresetport uint16 = 0x3da

func (self *TVideographicsarray) Nyoraregister(register []byte) {
	var regindex uint16 = 0

	Portnyorabyte(micsport, register[regindex])
	regindex++

	var i uint8
	for i = 0; i < 5; i++ {
		Portnyorabyte(sequencerindexport, i)
		Portnyorabyte(sequencerdataport, register[regindex])
		regindex++
	}

	Portnyorabyte(crtcindexport, 0x03)

	Portnyorabyte(crtcdataport, (Portverengabyte(crtcdataport) | 0x80))
	Portnyorabyte(crtcindexport, 0x11)
	Portnyorabyte(crtcdataport, (Portverengabyte(crtcdataport) & ^uint8(0x80)))

	register[0x03] = register[0x03] | 0x80
	register[0x11] = register[0x11] & ^uint8(0x80)

	for i = 0; i < 25; i++ {
		Portnyorabyte(crtcindexport, i)
		Portnyorabyte(crtcdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 9; i++ {
		Portnyorabyte(graphicscontrollerindexport, i)
		Portnyorabyte(graphicscontrollerdataport, register[regindex])
		regindex++
	}

	for i = 0; i < 21; i++ {
		Portverengabyte(attributecontrollerresetport)
		Portnyorabyte(attributecontrollerindexport, i)
		Portnyorabyte(attributecontrollernyoraport, register[regindex])
		regindex++
	}

	Portverengabyte(attributecontrollerresetport)
	Portnyorabyte(attributecontrollerindexport, 0x20)

}

func (self *TVideographicsarray) Getframebuffersegment() uintptr {
	Portnyorabyte(graphicscontrollerindexport, 0x06)
	var segmentnumber uint8 = ((Portverengabyte(graphicscontrollerdataport) >> 2) & 0x03)
...
func (self *TVideographicsarray) Setmode(width uint32, height uint32, colordepth uint32) bool {
	...
	self.Nyoraregister(g320x200x256)
