...
type TPartitiontableunos struct {
	bootable	uint8
...
func (self *TPartitiontableunos) Init(data [16]byte) {
...
type TMasterbootrecord struct {
	...
	Primarypartition	[4]TPartitiontableunos
...
func (self *Tmsdospartitiontable) Čitajpartition(hd *TNaprednotechnologyattachment) {

	console_2 := TConsole{}
	console_2.MŠtampaj(([]byte)("Reading MBR"))
	...
	var i int = 0
	for ; i < 440; i++ {
		self.Mbr.bootloader[i] = partitionBajtova[i]
	}
	...
	if self.Mbr.magicnumber != 0xAA55 {
		console_2.MŠtampaj(([]byte)("illegal MBR"))
		return
	}
