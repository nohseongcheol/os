...
이 트리는 engos의 현행 기능을 대표 언어(zh_Hans)의 기능 의미에 따라 명명한 독립 빌드 소스입니다. 언어 이름 토큰과 해시는 식별자에 사용하지 않습니다. Go 언어·런타임 ABI와 POSIX 공개 ABI 이름은 호환성을 위해 유지합니다. `이름대응표.tsv`, `파일대응표.tsv`, `용어사전.tsv`에서 변환 근거를 추적할 수 있습니다.
