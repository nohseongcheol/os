...
void *malloc(size_t আকাৰ)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!আকাৰ) আকাৰ = 1;
    if (আকাৰ > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    আকাৰ = (আকাৰ + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= আকাৰ) {
            if (cursor->capacity - আকাৰ >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + আকাৰ);
                raw->capacity = cursor->capacity - আকাৰ - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = আকাৰ;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + আকাৰ;
    ...
    raw->capacity = আকাৰ; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t আকাৰ)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(আকাৰ);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!আকাৰ) আকাৰ = 1;
    cursor = (struct heap_block *)address - 1;
    if (আকাৰ <= cursor->capacity) return address;
    destination = malloc(আকাৰ);
