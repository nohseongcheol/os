...
ssize_t পঢ়া(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_পঢ়া, fd, buf, count));
}

ssize_t লিখা(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_লিখা, fd, buf, count));
}
...
char *getcwd(char *buf, size_t আকাৰ)
{
    long result = __syscall_result(SC2(SYS_getcwd, buf, আকাৰ));
