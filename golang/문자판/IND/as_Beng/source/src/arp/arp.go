...
func (self *TARPMessage) Vআৰম্ভ_কৰা(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, আকাৰ int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(আকাৰ))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, আকাৰ uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, আকাৰ)
}
...
func (self *TARPProvider) Vআৰম্ভ_কৰা(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vআৰম্ভ_কৰা(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, আকাৰ uint32) bool {
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var ঠিকনা_সূচক uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, ঠিকনা_সূচক, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, আকাৰ uint32) {
