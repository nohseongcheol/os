...
func (self *TBiosParameterBlock32) Vআৰম্ভ_কৰা(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vআৰম্ভ_কৰা(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].আকাৰ >= 0xFFFFFFFF {
			continue
		}
		...
		return dirent[i].আকাৰ
	}
	memoryManager.Free(bpbPtr)
	memoryManager.Free(direntPtr)
	return 0
}
func (self *TBiosParameterBlock32) Vপঢ়া(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].আকাৰ > 512 {
					buf3 = buffer[:512]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, 512)

				} else {
					buf3 = buffer[:dirent[i].আকাৰ]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].আকাৰ))
				}

				copy(data[int32(dirent[i].আকাৰ)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vআৰম্ভ_কৰা(data [32]byte) {
	...
	self.আকাৰ = Uint32_R(ArrayToUint32(buf))
}
