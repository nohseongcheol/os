...
func (self *TCOWFrameManager) Vআৰম্ভ_কৰা(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
...
func (self *Paging) Vআৰম্ভ_কৰা(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vআৰম্ভ_কৰা(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vআৰম্ভ_কৰা(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, আকাৰ uint32, pageDir uint32) {
	if আকাৰ == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), আকাৰ)

	for i := uint32(0); i < আকাৰ; i++ {
		dst[i] = src[i]
	}
	setCR3(oldCR3)
}

func ZeroBlockInPageDir(addr uint32, আকাৰ uint32, pageDir uint32) {
	if আকাৰ == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, আকাৰ)
	dst := GetBytesFromPtr(uintptr(addr), int(আকাৰ), int(আকাৰ))
	for i := uint32(0); i < আকাৰ; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, আকাৰ uint32) bool {
	if আকাৰ == 0 {
		return true
	}
	last := addr + আকাৰ - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, আকাৰ uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, আকাৰ)
	setCR3(oldCR3)
	return ok
}
