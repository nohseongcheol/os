...
func (self *TPeripheralComponentInterconnectDeviceDescriptor) Vআৰম্ভ_কৰা() {
}
...
func (self *TPeripheralComponentInterconnectController) Vআৰম্ভ_কৰা(iPCIControllerHandler IPCIControllerHandler) {
	self.dataPort = 0xCFC
...
func (self *TPeripheralComponentInterconnectController) Vপঢ়া(bus uint16, device uint16, function uint16, registeroffset uint32) uint32 {
	var id uint32 = 0
...
func (self *TPeripheralComponentInterconnectController) Vলিখা(bus uint16, device uint16, function uint16, registeroffset uint32, value uint32) {
	var id uint32
	id = 0x1<<31 | uint32((bus&0xFF)<<16) | uint32((device&0x1f)<<11) | uint32((function&0x07)<<8) | uint32(registeroffset&0xFC)
	PortWriteDword(self.commandPort, id)
...
func (self *TPeripheralComponentInterconnectController) DeviceHasFunctions(bus uint16, device uint16) bool {
	result := self.Vপঢ়া(bus, device, 0, 0x0E)
	if (result & (1 << 7)) != 0 {
		return true
	} else {
		return false
...
func (self *TPeripheralComponentInterconnectController) GetDeviceDescriptor(bus uint16, device uint16, function uint16) TPeripheralComponentInterconnectDeviceDescriptor {
	...
	result.Vendor_id = uint16(self.Vপঢ়া(bus, device, function, 0x00))
	result.Device_id = uint16(self.Vপঢ়া(bus, device, function, 0x02))

	result.class_id = uint8(self.Vপঢ়া(bus, device, function, 0x0b))
	result.subclass_id = uint8(self.Vপঢ়া(bus, device, function, 0x0a))
	result.interface_id = uint8(self.Vপঢ়া(bus, device, function, 0x09))

	result.revision = uint8(self.Vপঢ়া(bus, device, function, 0x08))
	result.Interrupt = uint32(self.Vপঢ়া(bus, device, function, 0x3C))
...
func (self *TPeripheralComponentInterconnectController) GetBaseAddressRegister(bus uint16, device uint16, function uint16, bar uint16) TBaseAddressRegister {
	...
	headertype := self.Vপঢ়া(bus, device, function, 0x0E) & 0x7F
	var maxBARs int = int(6 - (4 * headertype))
	if bar >= uint16(maxBARs) {
		return result
	}

	bar_value := self.Vপঢ়া(bus, device, function, uint32(0x10+4*bar))
