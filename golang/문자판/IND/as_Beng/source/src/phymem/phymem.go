...
func (self *PhysicalMemoryManager) FirstFreeSize(আকাৰ uint32) uint32 {
	if আকাৰ == 0 {
		return 0xffffffff
	}
	if আকাৰ == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= আকাৰ; count++ {
...
func (self *PhysicalMemoryManager) Vআৰম্ভ_কৰা(আকাৰ uint32, bitmap []uint32) {
	self.memorySize = আকাৰ
	self.memoryArray = bitmap
	self.maximumBlocks = আকাৰ / BLOCK_SIZE
