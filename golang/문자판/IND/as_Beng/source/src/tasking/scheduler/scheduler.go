...
type SchedulerData struct {
	...
	currentThread	*Tনিষ্পাদনধাৰা
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vআৰম্ভ_কৰা() {
...
func (self *SchedulerData) GetNextReadyThread() *Tনিষ্পাদনধাৰা {
	if list.Vআকাৰ <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vআকাৰ; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vআকাৰ {
			currentThreadIndex = 0
		}
		thread := (*Tনিষ্পাদনধাৰা)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tনিষ্পাদনধাৰা) {
	if thread == nil {
		return
	}
	list.PushBack(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tনিষ্পাদনধাৰা) {
	if thread == nil {
		return
	}
	list.PushBack(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tনিষ্পাদনধাৰা) {
...
func (self *Scheduler) Vআৰম্ভ_কৰা(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vআৰম্ভ_কৰা()
	sche_data.tss = tss
	...
	self.TInterruptHandler.Vআৰম্ভ_কৰা(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vআকাৰ > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vআকাৰ; i++ {
		thread := (*Tনিষ্পাদনধাৰা)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tনিষ্পাদনধাৰা) {
