...
func (self *Process) Vআৰম্ভ_কৰা(mem *mem.TMemoryManager) {
	self.ThreadList = LinkedList{}
	self.Threads = &self.ThreadList
	self.Threads.Vআৰম্ভ_কৰা(mem)
}
...
func (self *ProcessHelper) Vআৰম্ভ_কৰা(mem *mem.TMemoryManager, kernelPageDirEntry uintptr) {
	self.mem = mem
	self.processes = LinkedList{}
	self.processes.Vআৰম্ভ_কৰা(self.mem)
...
func (self *ProcessHelper) Spawn(entryPoint func(), threadHelper *TThreadHelper, scheduler *Scheduler, PageDirEntry uint32, isKernel bool) Process {
	...
	if process.Threads != nil && process.Threads.Vআকাৰ > 0 {
		thread := (*Tনিষ্পাদনধাৰা)(process.Threads.GetAt(0))
