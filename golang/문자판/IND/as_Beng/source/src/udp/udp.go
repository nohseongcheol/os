...
func (self *TUserDatagramProtocolHeader) Vআৰম্ভ_কৰা(buffer *TUserDatagramProtocolHeaderBuffer) {
...
type IUserDatagramProtocolHandler interface {
	HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, আকাৰ uint16)
}

type TUserDatagramProtocolHandler struct {
}

func (self *TUserDatagramProtocolHandler) Vআৰম্ভ_কৰা(backend TInternetProtocolProvider) {
}
func (self *TUserDatagramProtocolHandler) HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, আকাৰ uint16) {
}

type IUserDatagramProtocolSocket interface {
	HandleUserDatagramProtocolMessage(data uintptr, আকাৰ uint16)
}
...
func (self *TUserDatagramProtocolSocket) Vআৰম্ভ_কৰা(p_udpProvider TUserDatagramProtocolProvider, p_udpHandler IUserDatagramProtocolHandler) {
...
func (self *TUserDatagramProtocolSocket) HandleUserDatagramProtocolMessage(data uintptr, আকাৰ uint16) {
	if udpHandler != nil {
		udpHandler.HandleUserDatagramProtocolMessage(self, data, আকাৰ)
	}
}
func (self *TUserDatagramProtocolSocket) Send(p_data []byte, আকাৰ uint16) {
	var buffer [4096]byte
	for i := 0; i < int(আকাৰ); i++ {
		buffer[i] = p_data[i]
	}
	var data = uintptr(Pointer(&buffer))
	udpProvider.Send(self, data, আকাৰ)
}
...
func (self *TUserDatagramProtocolProvider) Vআৰম্ভ_কৰা(p_ipProvider TInternetProtocolProvider, p_ipHandler IInternetProtocolHandler) {
	ipHandler = p_ipHandler
	ipHandler.Vআৰম্ভ_কৰা(p_ipProvider, p_ipHandler, 0x11)
	numSockets = 0
	freePort = 1024
}
func (self *TUserDatagramProtocolProvider) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, internetprotocolPayload uintptr, আকাৰ uint32) bool {
	if আকাৰ < udpHeaderSize {
		return false
	}
	...
	if socket != nil {
		socket.HandleUserDatagramProtocolMessage(internetprotocolPayload+uintptr(udpHeaderSize), uint16(আকাৰ-udpHeaderSize))
	}
...
func (self *TUserDatagramProtocolProvider) Send(socket *TUserDatagramProtocolSocket, p_data uintptr, আকাৰ uint16) {
	var totalLength = uint32(আকাৰ) + udpHeaderSize
