...
func PrintBytes(dataPointer uintptr, আকাৰ int) {
...
func GetUintptrArrayFromPtr(ঠিকনা_সূচক_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{ঠিকনা_সূচক_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(ঠিকনা_সূচক_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(ঠিকনা_সূচক_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(ঠিকনা_সূচক_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&ঠিকনা_সূচক_2))
