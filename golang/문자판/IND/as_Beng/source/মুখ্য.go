...
import . "virtmem"
import . "paging"
import . "tasking/নিষ্পাদনধাৰা"
import . "tasking/scheduler"
import . "tasking/প্ৰক্ৰিয়া"
...
func GetFileSize(filename []byte) uint32 {
	...
	var আকাৰ uint32 = bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()
...
func ReadFile(filename []byte, data []byte) {
	...
	bios := TBiosParameterBlock32{}
	bios.Vপঢ়া(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vআৰম্ভ_কৰা(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vআৰম্ভ_কৰা(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vআৰম্ভ_কৰা(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vআৰম্ভ_কৰা()

	threadHelper := &TThreadHelper{}
	threadHelper.Vআৰম্ভ_কৰা(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vআৰম্ভ_কৰা(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vআৰম্ভ_কৰা(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vআৰম্ভ_কৰা(InterruptManager)
	...
	আকাৰ = GetFileSize(linkerFile)
	linkerAddr := memoryManager.Malloc(আকাৰ)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(আকাৰ), int(আকাৰ))
	...
	linkMap := LinkMap{}
	linkMap.Vআৰম্ভ_কৰা(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	আকাৰ = GetFileSize(lib1File)

	lib1Addr := memoryManager.Malloc(আকাৰ)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(আকাৰ), int(আকাৰ))
	...
	আকাৰ = GetFileSize(lib2File)

	lib2Addr := memoryManager.Malloc(আকাৰ)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(আকাৰ), int(আকাৰ))
	...
	আকাৰ = GetFileSize(user1file)
	user1addr := memoryManager.Malloc(আকাৰ)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(আকাৰ), int(আকাৰ))
	...
	*code1Ptr = uintptr(linkerEntry)
	...
	thr2 := (*Tনিষ্পাদনধাৰা)(proc2.Threads.GetAt(0))
	...
	আকাৰ = GetFileSize(user2file)
	user2addr := memoryManager.Malloc(আকাৰ)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(আকাৰ), int(আকাৰ))
	...
	*code2Ptr = uintptr(linkerEntry)
	...
	thr3 := (*Tনিষ্পাদনধাৰা)(proc3.Threads.GetAt(0))
	...
	আকাৰ = GetFileSize(user3file)
	user3addr := memoryManager.Malloc(আকাৰ)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(আকাৰ), int(আকাৰ))
	...
	*code3Ptr = uintptr(linkerEntry)
	...
	thr4 := (*Tনিষ্পাদনধাৰা)(proc4.Threads.GetAt(0))
	...
	pciController.Vআৰম্ভ_কৰা(myPCIControllerHandler)
