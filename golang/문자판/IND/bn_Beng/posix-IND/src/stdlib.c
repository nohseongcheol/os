...
int abs(int মান) { return মান < 0 ? -মান : মান; }
long labs(long মান) { return মান < 0 ? -মান : মান; }
...
char *getenv(const char *ব্যবস্থার_পরিচয়)
{
    size_t দৈর্ঘ্য = strlen(ব্যবস্থার_পরিচয়);
    char **cursor = environ;
    if (!দৈর্ঘ্য || strchr(ব্যবস্থার_পরিচয়, '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, ব্যবস্থার_পরিচয়, দৈর্ঘ্য) && (*cursor)[দৈর্ঘ্য] == '=') return *cursor + দৈর্ঘ্য + 1;
        ++cursor;
    }
    return NULL;
}
