...
int main(void)
{
    ...
    struct stat st;
    if (নথি_নির্দেশক < 0 || fstat(নথি_নির্দেশক, &st) < 0 || পড়া(নথি_নির্দেশক, স্থানান্তরের_অস্থায়ী_ভান্ডার_2, 4) != 4 ||
        ...
        goto failure;
    errno = 0;
    if (পড়া(-1, স্থানান্তরের_অস্থায়ী_ভান্ডার_2, 1) != -1 || errno != EBADF)
        goto failure;
    if (__posix_library_test() != 0)
        goto failure;
    if (লেখা(STDOUT_FILENO, "POSIX-NATIVE:PASS\n", 18) != 18)
        goto failure;
    return 0;
failure:
    লেখা(STDOUT_FILENO, "POSIX-NATIVE:FAIL\n", 18);
    return 1;
}
