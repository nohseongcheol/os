...
{
    (void)লেখা(STDOUT_FILENO, text, text_length(text));
}
...
static void test_open_and_io(void)
{
    ...
    if (নথি_নির্দেশক >= 0) {
        report("read-bytes", পড়া(নথি_নির্দেশক, bytes, 2) == 2 &&
    ...
    if (root >= 0) {
        ...
        errno = 0;
        report("read-directory", পড়া(root, bytes, 1) == -1 && errno == EISDIR);
        (void)close(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", লেখা(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", পড়া(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", পড়া(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", লেখা(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", পড়া(নথি_নির্দেশক, &first, 1) == 1 && পড়া(copy, &second, 1) == 1 &&
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &অবস্থা) == child) {
        report("fork-fd-isolation", পড়া(নথি_নির্দেশক, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
