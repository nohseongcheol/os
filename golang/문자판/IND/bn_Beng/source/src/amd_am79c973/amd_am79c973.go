...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, আকার int) bool
	Send(dataPointer uintptr, আকার uint32)
}
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, আকার int) bool {
	넷카드콘솔.M출력XY(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawDataHandler) Send(dataPointer uintptr, আকার uint32) {
	넷카드콘솔.M출력XY(([]byte)("TRawDataSend"), 10, 10)
	rawDataBackend.Send(dataPointer, আকার)
}
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Vআরম্ভ_করা(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, আকার uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + আকার - 1

	for i := 0; i < int(আকার); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(আকার)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-আকার)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var আকার uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if আকার > 64 {
				আকার -= 4
			}
			...
			var ঠিকানা_নির্দেশক uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(ঠিকানা_নির্দেশক, int(আকার)) {

					콘솔.M출력XY(([]byte)("self.Send"), 0, 22)

					self.Send(ঠিকানা_নির্দেশক, আকার)
				}
			}
