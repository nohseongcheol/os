...
func (self *TARPMessage) Vআরম্ভ_করা(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, আকার int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(আকার))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, আকার uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, আকার)
}
...
func (self *TARPProvider) Vআরম্ভ_করা(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vআরম্ভ_করা(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, আকার uint32) bool {
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var ঠিকানা_নির্দেশক uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, ঠিকানা_নির্দেশক, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, আকার uint32) {
