...
func (self *TEtherFrameHeader) Vআরম্ভ_করা(buffer TEtherFrameHeaderBuffer) {
...
type IEtherFrameHandler interface {
	Vআরম্ভ_করা(backend TEtherFrameProvider)
	SetHandler(handler IEtherFrameHandler, etherType uint16)
	OnEtherFrameReceived(dataPointer uintptr, আকার int) bool
	Send(dstMAC_BE uint64, dataPointer uintptr, আকার uint32)
	SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, আকার uint32)
...
func (self *TEtherFrameHandler) Vআরম্ভ_করা(backend TEtherFrameProvider) {
	Backend = backend
}
...
func (self *TEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, আকার int) bool {
	이더넷콘솔.M출력(([]byte)("OnEtherFrameReceived"))
	return false
}
func (self *TEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, আকার uint32) {
	Backend.SendFrame(dstMAC_BE, frame.etherType_BE, dataPointer, আকার)
}
func (self *TEtherFrameHandler) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, আকার uint32) {
	Backend.SendFrame(dstMAC_BE, etherType_BE, dataPointer, আকার)
}
...
func (self *TEtherFrameRawDataHandler) Vআরম্ভ_করা(p_provider TEtherFrameProvider, p_backend Tamd_am79c973) {
	provider = p_provider
	provider.Vআরম্ভ_করা(p_backend)
}
func (self *TEtherFrameRawDataHandler) OnRawDataReceived(dataPointer uintptr, আকার int) bool {
	return provider.OnRawDataReceived(dataPointer, আকার)
}
func (self *TEtherFrameRawDataHandler) Send(dataPointer uintptr, আকার uint32) {
	provider.Send(dataPointer, আকার)
}
...
func (self *TEtherFrameProvider) Vআরম্ভ_করা(backend Tamd_am79c973) {
...
func (self *TEtherFrameProvider) OnRawDataReceived(dataPointer uintptr, আকার int) bool {
	...
	if frame.dstMAC_BE == 0x0000FFFFFFFFFFFF || Uint48_R(frame.dstMAC_BE) == self.GetMACAddress() {
		if handlers[frame.etherType_BE] != nil {
			...
			var ঠিকানা_নির্দেশক uintptr = uintptr(Pointer(dataPointer)) + uintptr(frameHeaderSize)
			sendBack = handlers[frame.etherType_BE].OnEtherFrameReceived(ঠিকানা_নির্দেশক, আকার-frameHeaderSize)
...
func (self *TEtherFrameProvider) Send(dataPointer uintptr, আকার uint32) {
	self.netCard.Send(dataPointer, আকার)
}
func (self *TEtherFrameProvider) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, আকার uint32) {
	...
	var ঠিকানা_নির্দেশক uintptr = uintptr(Pointer(&buffer2))

	self.netCard.Send(ঠিকানা_নির্দেশক, আকার+uint32(frameHeaderSize))
