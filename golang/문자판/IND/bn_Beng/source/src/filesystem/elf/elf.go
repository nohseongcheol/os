	...
	if elfHeader.e_shnum != 0 {
		...
		for i := uint32(0); i < uint32(elfHeader.e_shnum); i++ {
			...
			if sectHeader.sh_addr > 0x1000 {
				আকার := sectHeader.sh_size
				if sectHeader.sh_type == 8 {
					ZeroBlockInPageDir(sectHeader.sh_addr, আকার, PageDirEntry)
				} else {
					dst := GetBytesFromPtr(uintptr(sectHeader.sh_addr), int(আকার), int(আকার))
					SetBlockInPageDir(sectValue, dst, আকার, PageDirEntry)
				}
			}
		...
		if textPtr != nil {
			memoryManager.Vস্মৃতি_মুক্ত_করা(textPtr)
		}
