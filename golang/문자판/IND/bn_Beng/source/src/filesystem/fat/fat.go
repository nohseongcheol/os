...
func (self *TBiosParameterBlock32) Vআরম্ভ_করা(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	bpbPtr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(90)
	...
	direntPtr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(512)
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vআরম্ভ_করা(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].আকার >= 0xFFFFFFFF {
			continue
		}
		...
		memoryManager.Vস্মৃতি_মুক্ত_করা(bpbPtr)
		memoryManager.Vস্মৃতি_মুক্ত_করা(direntPtr)
		return dirent[i].আকার
	}
	memoryManager.Vস্মৃতি_মুক্ত_করা(bpbPtr)
	memoryManager.Vস্মৃতি_মুক্ত_করা(direntPtr)
	return 0
}
func (self *TBiosParameterBlock32) Vপড়া(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].আকার > 512 {
					buf3 = buffer[:512]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, 512)

				} else {
					buf3 = buffer[:dirent[i].আকার]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].আকার))
				}

				copy(data[int32(dirent[i].আকার)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vআরম্ভ_করা(data [32]byte) {
	...
	self.আকার = Uint32_R(ArrayToUint32(buf))
}
