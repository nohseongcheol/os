...
func (self *TCOWFrameManager) Vআরম্ভ_করা(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
	self.frameCount = frameCount
	refBytes := frameCount * uint32(unsafe.Sizeof(uint16(0)))
	refPtr := mem.Vস্মৃতি_বরাদ্দ_করা(refBytes)
	if refPtr == nil {
		self.refs = nil
		self.frameCount = 0
		return false
...
func (self *Paging) Vআরম্ভ_করা(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vআরম্ভ_করা(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vআরম্ভ_করা(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, আকার uint32, pageDir uint32) {
	if আকার == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), আকার)

	for i := uint32(0); i < আকার; i++ {
		dst[i] = src[i]
	}
	setCR3(oldCR3)
}

func ZeroBlockInPageDir(addr uint32, আকার uint32, pageDir uint32) {
	if আকার == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, আকার)
	dst := GetBytesFromPtr(uintptr(addr), int(আকার), int(আকার))
	for i := uint32(0); i < আকার; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, আকার uint32) bool {
	if আকার == 0 {
		return true
	}
	last := addr + আকার - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, আকার uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, আকার)
	setCR3(oldCR3)
	return ok
}
