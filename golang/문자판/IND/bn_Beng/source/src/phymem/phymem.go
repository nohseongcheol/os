...
func (self *PhysicalMemoryManager) Vবরাদ্দ_বিট_উল্টানো(bit uint32) uint32 {
	self.memoryArray[bit/32] = self.memoryArray[bit/32] ^ (1 << (bit % 32))
	return self.memoryArray[bit/32]
}
...
func (self *PhysicalMemoryManager) FirstFreeSize(আকার uint32) uint32 {
	if আকার == 0 {
		return 0xffffffff
	}
	if আকার == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= আকার; count++ {
...
func (self *PhysicalMemoryManager) Vআরম্ভ_করা(আকার uint32, bitmap []uint32) {
	self.memorySize = আকার
	self.memoryArray = bitmap
	self.maximumBlocks = আকার / BLOCK_SIZE
