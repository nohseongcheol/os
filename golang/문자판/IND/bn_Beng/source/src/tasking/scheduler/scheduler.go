...
type SchedulerData struct {
	...
	currentThread	*Tনির্বাহধারা
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vআরম্ভ_করা() {
...
func (self *SchedulerData) GetNextReadyThread() *Tনির্বাহধারা {
	if list.Vআকার <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vআকার; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vআকার {
			currentThreadIndex = 0
		}
		thread := (*Tনির্বাহধারা)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tনির্বাহধারা) {
	if thread == nil {
		return
	}
	list.Vতালিকার_শেষে_যোগ_করা(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tনির্বাহধারা) {
	if thread == nil {
		return
	}
	list.Vতালিকার_শেষে_যোগ_করা(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tনির্বাহধারা) {
...
func (self *Scheduler) Vআরম্ভ_করা(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vআরম্ভ_করা()
	sche_data.tss = tss
	...
	self.TInterruptHandler.Vআরম্ভ_করা(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vআকার > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vআকার; i++ {
		thread := (*Tনির্বাহধারা)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tনির্বাহধারা) {
