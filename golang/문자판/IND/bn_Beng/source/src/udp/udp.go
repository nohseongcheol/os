...
func (self *TUserDatagramProtocolHeader) Vআরম্ভ_করা(buffer *TUserDatagramProtocolHeaderBuffer) {
...
type IUserDatagramProtocolHandler interface {
	HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, আকার uint16)
}

type TUserDatagramProtocolHandler struct {
}

func (self *TUserDatagramProtocolHandler) Vআরম্ভ_করা(backend TInternetProtocolProvider) {
}
func (self *TUserDatagramProtocolHandler) HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, আকার uint16) {
}

type IUserDatagramProtocolSocket interface {
	HandleUserDatagramProtocolMessage(data uintptr, আকার uint16)
}
...
func (self *TUserDatagramProtocolSocket) Vআরম্ভ_করা(p_udpProvider TUserDatagramProtocolProvider, p_udpHandler IUserDatagramProtocolHandler) {
...
func (self *TUserDatagramProtocolSocket) HandleUserDatagramProtocolMessage(data uintptr, আকার uint16) {
	if udpHandler != nil {
		udpHandler.HandleUserDatagramProtocolMessage(self, data, আকার)
	}
}
func (self *TUserDatagramProtocolSocket) Send(p_data []byte, আকার uint16) {
	var buffer [4096]byte
	for i := 0; i < int(আকার); i++ {
		buffer[i] = p_data[i]
	}
	var data = uintptr(Pointer(&buffer))
	udpProvider.Send(self, data, আকার)
}
...
func (self *TUserDatagramProtocolProvider) Vআরম্ভ_করা(p_ipProvider TInternetProtocolProvider, p_ipHandler IInternetProtocolHandler) {
	ipHandler = p_ipHandler
	ipHandler.Vআরম্ভ_করা(p_ipProvider, p_ipHandler, 0x11)
	numSockets = 0
	freePort = 1024
}
func (self *TUserDatagramProtocolProvider) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, internetprotocolPayload uintptr, আকার uint32) bool {
	if আকার < udpHeaderSize {
		return false
	}
	...
	if socket != nil {
		socket.HandleUserDatagramProtocolMessage(internetprotocolPayload+uintptr(udpHeaderSize), uint16(আকার-udpHeaderSize))
	}
...
func (self *TUserDatagramProtocolProvider) Connect(ip uint32, port uint16) *TUserDatagramProtocolSocket {
	...
	var socket = (*TUserDatagramProtocolSocket)(memoryManager.Vস্মৃতি_বরাদ্দ_করা(50))

	if socket != nil {

		socket.Vআরম্ভ_করা(*self, nil)
...
func (self *TUserDatagramProtocolProvider) Send(socket *TUserDatagramProtocolSocket, p_data uintptr, আকার uint16) {
	var totalLength = uint32(আকার) + udpHeaderSize
