...
func PrintBytes(dataPointer uintptr, আকার int) {
...
func GetUintptrArrayFromPtr(ঠিকানা_নির্দেশক_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{ঠিকানা_নির্দেশক_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(ঠিকানা_নির্দেশক_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(ঠিকানা_নির্দেশক_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(ঠিকানা_নির্দেশক_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&ঠিকানা_নির্দেশক_2))
