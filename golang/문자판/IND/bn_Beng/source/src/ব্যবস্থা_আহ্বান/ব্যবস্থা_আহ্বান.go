...
func (self *TSyscall) Vআরম্ভ_করা(manager *TInterruptManager) {
	initFileDescriptors()
	...
	self.TInterruptHandler.Vআরম্ভ_করা(0x80, uintptr(Pointer(manager)), addr)
}
...
func sysRead(fd int32, addr uint32, count uint32) int32 {
	...
	if entry.pos >= entry.আকার {
		return 0
	}
	remaining := entry.আকার - entry.pos
	if count > remaining {
		count = remaining
	}
	buf := GetBytesFromPtr(uintptr(addr), int(count), int(count))
...
func readVFSFile(entry *openFileDescription, dst []byte, count uint32) int32 {
	...
	tmpPtr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(entry.আকার)
	if tmpPtr == nil {
		return EINVAL
	}
	tmp := GetBytesFromPtr(uintptr(tmpPtr), int(entry.আকার), int(entry.আকার))
	readFile(entry.name[:entry.nameLen], tmp)
	copy(dst[:count], tmp[entry.pos:entry.pos+count])
	entry.pos += count
	memoryManager.Vস্মৃতি_মুক্ত_করা(tmpPtr)
	return int32(count)
}
...
func sysGetcwd(bufferAddr uint32, আকার uint32) int32 {
	if bufferAddr == 0 {
		return EFAULT
	}
	if আকার < 2 {
	...
	buffer := GetBytesFromPtr(uintptr(bufferAddr), int(আকার), int(আকার))
	buffer[0] = '/'
	buffer[1] = 0
	return 2
}

func fillPosixStat(statAddr uint32, mode uint32, আকার uint32, inode uint32) int32 {
	if statAddr == 0 {
		return EFAULT
	}
	...
	*stat = posixStat{}
	...
	stat.Mode = mode
	stat.Nlink = 1
	stat.Vআকার = int32(আকার)
	stat.Blksize = 512
	stat.Blocks = int32((আকার + 511) / 512)
	return 0
}
...
func sysStat(pathAddr uint32, statAddr uint32) int32 {
	...
	return fillPosixStat(statAddr, sIfreg|0444, আকার, inode)
}
...
func sysFstat(fd int32, statAddr uint32) int32 {
	...
	case fdKindRootDir:
		return fillPosixStat(statAddr, sIfdir|0555, 0, 1)
	case fdKindFat:
		return fillPosixStat(statAddr, sIfreg|0444, entry.আকার, uint32(fd+2))
	case fdKindSocket:
...
func socketSendTo(fd int32, bufferAddress uint32, length uint32, destinationAddress uint32, destinationLength uint32) int32 {
	...
	*packet = socketPacket{used: true, আকার: length, source: socket.local}
	if length != 0 {
...
func sysExecve(cpu *TCPUState, pathAddr uint32) int32 {
	...
	memoryManager := &mem.TMemoryManager{}
	filePtr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(আকার)
	if filePtr == nil {
		return EINVAL
	}
	data := GetBytesFromPtr(uintptr(filePtr), int(আকার), int(আকার))
	readFile(name[:nameLen], data)
	if আকার < 52 || data[0] != 0x7F || data[1] != 'E' || data[2] != 'L' || data[3] != 'F' {
		memoryManager.Vস্মৃতি_মুক্ত_করা(filePtr)
		return ENOEXEC
	}
	loader := Elf{}
...
func sysFork(cpu *TCPUState) int32 {
	...
	memoryManager := &mem.TMemoryManager{}
	threadPtr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(uint32(Sizeof(Tনির্বাহধারা{})))
	stackPtr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(THREAD_STACK_SIZE)
	childPageDir := CloneAddressSpaceCOW(getCR3())
	if threadPtr == nil || stackPtr == nil || childPageDir == 0 {
		discardProcess(pid)
		return EINVAL
	}
	child := (*Tনির্বাহধারা)(threadPtr)
...
func fileSize(filename []byte) uint32 {
	...
	আকার := bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()
	return আকার
}
