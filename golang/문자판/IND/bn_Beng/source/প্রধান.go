...
func GetFileSize(filename []byte) uint32 {
	...
	var আকার uint32 = bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()

	return আকার
}

func Vনথি_পড়া(filename []byte, data []byte) {
	var ata0s = TAdvancedTechnologyAttachment{}
	ata0s.Vআরম্ভ_করা(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosParameterBlock32{}
	bios.Vপড়া(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func memorytest(y int) {
	...
	allocated := uint32(uintptr(memoryManager.Vস্মৃতি_বরাদ্দ_করা(1024)))
	console.MUint32출력XY(allocated, 10, uint16(y))
	if y == 11 {
		memoryManager.Vস্মৃতি_মুক্ত_করা(Pointer(uintptr(allocated)))
	}
}
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vআরম্ভ_করা(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vআরম্ভ_করা(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vআরম্ভ_করা(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vআরম্ভ_করা()

	threadHelper := &TThreadHelper{}
	threadHelper.Vআরম্ভ_করা(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vআরম্ভ_করা(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vআরম্ভ_করা(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vআরম্ভ_করা(InterruptManager)
	...
	আকার = GetFileSize(linkerFile)
	linkerAddr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(আকার)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(আকার), int(আকার))
	Vনথি_পড়া(linkerFile, linkerData)
	...
	linkMap := LinkMap{}
	linkMap.Vআরম্ভ_করা(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	আকার = GetFileSize(lib1File)

	lib1Addr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(আকার)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(আকার), int(আকার))
	Vনথি_পড়া(lib1File, lib1Data)

	lib1elf := Elf{}
	lib1elf.Parse(lib1Data[:], uint32(PageDirEntry))
	memoryManager.Vস্মৃতি_মুক্ত_করা(lib1Addr)

	linkMap.Vতালিকার_শেষে_যোগ_করা(uintptr(lib1elf.Dynamic))

	var lib2File []byte = ([]byte)("LIB2")
	আকার = GetFileSize(lib2File)

	lib2Addr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(আকার)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(আকার), int(আকার))
	Vনথি_পড়া(lib2File, lib2Data)

	lib2elf := Elf{}
	lib2elf.Parse(lib2Data[:], uint32(PageDirEntry))
	memoryManager.Vস্মৃতি_মুক্ত_করা(lib2Addr)

	linkMap.Vতালিকার_শেষে_যোগ_করা(uintptr(lib2elf.Dynamic))
	...
	আকার = GetFileSize(user1file)
	user1addr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(আকার)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(আকার), int(আকার))
	Vনথি_পড়া(user1file, user1data)
	...
	P1LinkMap.Vতালিকার_শেষে_যোগ_করা(uintptr(elf2.Dynamic))

	memoryManager.Vস্মৃতি_মুক্ত_করা(user1addr)

	var code1Ptr *uintptr
	var func1Val func()

	code1Ptr = (*uintptr)(memoryManager.Vস্মৃতি_বরাদ্দ_করা(4))
	*code1Ptr = uintptr(linkerEntry)
	...
	thr2 := (*Tনির্বাহধারা)(proc2.Threads.GetAt(0))
	...
	আকার = GetFileSize(user2file)
	user2addr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(আকার)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(আকার), int(আকার))
	Vনথি_পড়া(user2file, user2data)
	...
	P2LinkMap.Vতালিকার_শেষে_যোগ_করা(uintptr(elf3.Dynamic))

	memoryManager.Vস্মৃতি_মুক্ত_করা(user2addr)

	var code2Ptr *uintptr
	var func2Val func()

	code2Ptr = (*uintptr)(memoryManager.Vস্মৃতি_বরাদ্দ_করা(4))
	*code2Ptr = uintptr(linkerEntry)
	...
	thr3 := (*Tনির্বাহধারা)(proc3.Threads.GetAt(0))
	...
	আকার = GetFileSize(user3file)
	user3addr := memoryManager.Vস্মৃতি_বরাদ্দ_করা(আকার)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(আকার), int(আকার))
	Vনথি_পড়া(user3file, user3data)
	...
	P3LinkMap.Vতালিকার_শেষে_যোগ_করা(uintptr(elf4.Dynamic))

	memoryManager.Vস্মৃতি_মুক্ত_করা(user3addr)

	var code3Ptr *uintptr
	var func3Val func()

	code3Ptr = (*uintptr)(memoryManager.Vস্মৃতি_বরাদ্দ_করা(4))
	*code3Ptr = uintptr(linkerEntry)
	...
	thr4 := (*Tনির্বাহধারা)(proc4.Threads.GetAt(0))
	...
	pciController.Vআরম্ভ_করা(myPCIControllerHandler)
