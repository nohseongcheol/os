...
ssize_t फरायनाय(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_फरायनाय, fd, buf, count));
}

ssize_t लिरनाय(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_लिरनाय, fd, buf, count));
}
