...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vलिरनाय(0xA0)
	} else {
		self.devicePort.Vलिरनाय(0xB0)
	}
	self.controlPort.Vलिरनाय(0)
	self.devicePort.Vलिरनाय(0xA0)

	var status uint8 = self.commandPort.Vफरायनाय()
	if status == 0xFF {
	...
	self.sectorCountPort.Vलिरनाय(0)
	self.lbaLowPort.Vलिरनाय(0)
	self.lbaMidPort.Vलिरनाय(0)
	self.lbaHiPort.Vलिरनाय(0)
	self.commandPort.Vलिरनाय(0xEC)

	status = self.commandPort.Vफरायनाय()
	if status == 0x00 {
	...
	for i := 0; i < 256; i++ {
		var data = self.dataPort.Vफरायनाय()
		text := []byte("  ")
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vलिरनाय(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vलिरनाय(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vलिरनाय(0)
	self.sectorCountPort.Vलिरनाय(1)

	self.lbaLowPort.Vलिरनाय(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vलिरनाय(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vलिरनाय(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vलिरनाय(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vफरायनाय()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vलिरनाय(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vलिरनाय(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vलिरनाय(0)
	self.sectorCountPort.Vलिरनाय(1)
	self.lbaLowPort.Vलिरनाय(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vलिरनाय(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vलिरनाय(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vलिरनाय(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vलिरनाय(0xE0)
	} else {
		self.devicePort.Vलिरनाय(0xF0)
	}
	self.commandPort.Vलिरनाय(0xE7)
