...
void *malloc(size_t કદ)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!કદ) કદ = 1;
    if (કદ > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    કદ = (કદ + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= કદ) {
            if (cursor->capacity - કદ >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + કદ);
                raw->capacity = cursor->capacity - કદ - sizeof(*raw);
    ...
    total = padding + sizeof(struct heap_block) + કદ;
    ...
    raw->capacity = કદ; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t કદ)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(કદ);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!કદ) કદ = 1;
    cursor = (struct heap_block *)address - 1;
    if (કદ <= cursor->capacity) return address;
    destination = malloc(કદ);
