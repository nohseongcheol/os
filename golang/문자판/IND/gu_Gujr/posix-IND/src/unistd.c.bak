...
ssize_t વાંચવું(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_વાંચવું, fd, buf, count));
}

ssize_t લખવું(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_લખવું, fd, buf, count));
}
...
char *getcwd(char *buf, size_t કદ)
{
    long result = __syscall_result(SC2(SYS_getcwd, buf, કદ));
