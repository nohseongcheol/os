...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, કદ int) bool
	Send(dataPointer uintptr, કદ uint32)
}
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, કદ int) bool {
	넷카드콘솔.M출력XY(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawDataHandler) Send(dataPointer uintptr, કદ uint32) {
	넷카드콘솔.M출력XY(([]byte)("TRawDataSend"), 10, 10)
	rawDataBackend.Send(dataPointer, કદ)
}
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Vઆરંભ_કરવો(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, કદ uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + કદ - 1

	for i := 0; i < int(કદ); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(કદ)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-કદ)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var કદ uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if કદ > 64 {
				કદ -= 4
			}
			...
			var સરનામા_નિર્દેશક uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(સરનામા_નિર્દેશક, int(કદ)) {

					콘솔.M출력XY(([]byte)("self.Send"), 0, 22)

					self.Send(સરનામા_નિર્દેશક, કદ)
				}
			}
