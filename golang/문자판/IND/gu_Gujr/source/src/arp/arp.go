...
func (self *TARPMessage) Vઆરંભ_કરવો(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, કદ int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(કદ))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, કદ uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, કદ)
}
...
func (self *TARPProvider) Vઆરંભ_કરવો(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vઆરંભ_કરવો(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, કદ uint32) bool {
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var સરનામા_નિર્દેશક uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, સરનામા_નિર્દેશક, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, કદ uint32) {
