...
func (self *TEtherFrameHeader) Vઆરંભ_કરવો(buffer TEtherFrameHeaderBuffer) {
...
type IEtherFrameHandler interface {
	Vઆરંભ_કરવો(backend TEtherFrameProvider)
	SetHandler(handler IEtherFrameHandler, etherType uint16)
	OnEtherFrameReceived(dataPointer uintptr, કદ int) bool
	Send(dstMAC_BE uint64, dataPointer uintptr, કદ uint32)
	SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, કદ uint32)
...
func (self *TEtherFrameHandler) Vઆરંભ_કરવો(backend TEtherFrameProvider) {
	Backend = backend
}
...
func (self *TEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, કદ int) bool {
	이더넷콘솔.M출력(([]byte)("OnEtherFrameReceived"))
	return false
}
func (self *TEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, કદ uint32) {
	Backend.SendFrame(dstMAC_BE, frame.etherType_BE, dataPointer, કદ)
}
func (self *TEtherFrameHandler) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, કદ uint32) {
	Backend.SendFrame(dstMAC_BE, etherType_BE, dataPointer, કદ)
}
...
func (self *TEtherFrameRawDataHandler) Vઆરંભ_કરવો(p_provider TEtherFrameProvider, p_backend Tamd_am79c973) {
	provider = p_provider
	provider.Vઆરંભ_કરવો(p_backend)
}
func (self *TEtherFrameRawDataHandler) OnRawDataReceived(dataPointer uintptr, કદ int) bool {
	return provider.OnRawDataReceived(dataPointer, કદ)
}
func (self *TEtherFrameRawDataHandler) Send(dataPointer uintptr, કદ uint32) {
	provider.Send(dataPointer, કદ)
}
...
func (self *TEtherFrameProvider) Vઆરંભ_કરવો(backend Tamd_am79c973) {
...
func (self *TEtherFrameProvider) OnRawDataReceived(dataPointer uintptr, કદ int) bool {
	...
	if frame.dstMAC_BE == 0x0000FFFFFFFFFFFF || Uint48_R(frame.dstMAC_BE) == self.GetMACAddress() {
		if handlers[frame.etherType_BE] != nil {
			...
			var સરનામા_નિર્દેશક uintptr = uintptr(Pointer(dataPointer)) + uintptr(frameHeaderSize)
			sendBack = handlers[frame.etherType_BE].OnEtherFrameReceived(સરનામા_નિર્દેશક, કદ-frameHeaderSize)
...
func (self *TEtherFrameProvider) Send(dataPointer uintptr, કદ uint32) {
	self.netCard.Send(dataPointer, કદ)
}
func (self *TEtherFrameProvider) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, કદ uint32) {
	...
	var સરનામા_નિર્દેશક uintptr = uintptr(Pointer(&buffer2))

	self.netCard.Send(સરનામા_નિર્દેશક, કદ+uint32(frameHeaderSize))
