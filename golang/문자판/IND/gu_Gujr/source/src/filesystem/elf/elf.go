	...
	if elfHeader.e_shnum != 0 {
		...
		for i := uint32(0); i < uint32(elfHeader.e_shnum); i++ {
			...
			if sectHeader.sh_addr > 0x1000 {
				કદ := sectHeader.sh_size
				if sectHeader.sh_type == 8 {
					ZeroBlockInPageDir(sectHeader.sh_addr, કદ, PageDirEntry)
				} else {
					dst := GetBytesFromPtr(uintptr(sectHeader.sh_addr), int(કદ), int(કદ))
					SetBlockInPageDir(sectValue, dst, કદ, PageDirEntry)
				}
			}
		...
		if textPtr != nil {
			memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(textPtr)
		}
