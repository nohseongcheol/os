...
func (self *TBiosParameterBlock32) Vઆરંભ_કરવો(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	bpbPtr := memoryManager.Vસ્મૃતિ_ફાળવવી(90)
	...
	direntPtr := memoryManager.Vસ્મૃતિ_ફાળવવી(512)
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vઆરંભ_કરવો(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].કદ >= 0xFFFFFFFF {
			continue
		}
		...
		memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(bpbPtr)
		memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(direntPtr)
		return dirent[i].કદ
	}
	memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(bpbPtr)
	memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(direntPtr)
...
func (self *TBiosParameterBlock32) Vવાંચવું(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].કદ > 512 {
					...
					buf3 = buffer[:dirent[i].કદ]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].કદ))
				}

				copy(data[int32(dirent[i].કદ)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vઆરંભ_કરવો(data [32]byte) {
	...
	self.કદ = Uint32_R(ArrayToUint32(buf))
}
