...
func (self *TCOWFrameManager) Vઆરંભ_કરવો(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
	self.frameCount = frameCount
	refBytes := frameCount * uint32(unsafe.Sizeof(uint16(0)))
	refPtr := mem.Vસ્મૃતિ_ફાળવવી(refBytes)
	if refPtr == nil {
		self.refs = nil
		self.frameCount = 0
		return false
...
func (self *Paging) Vઆરંભ_કરવો(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vઆરંભ_કરવો(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vઆરંભ_કરવો(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, કદ uint32, pageDir uint32) {
	if કદ == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), કદ)

	for i := uint32(0); i < કદ; i++ {
		dst[i] = src[i]
	}
	setCR3(oldCR3)
}

func ZeroBlockInPageDir(addr uint32, કદ uint32, pageDir uint32) {
	if કદ == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, કદ)
	dst := GetBytesFromPtr(uintptr(addr), int(કદ), int(કદ))
	for i := uint32(0); i < કદ; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, કદ uint32) bool {
	if કદ == 0 {
		return true
	}
	last := addr + કદ - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, કદ uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, કદ)
	setCR3(oldCR3)
	return ok
}
