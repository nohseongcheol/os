...
func (self *PhysicalMemoryManager) Vફાળવણી_બીટ_ઉલટાવવો(bit uint32) uint32 {
	self.memoryArray[bit/32] = self.memoryArray[bit/32] ^ (1 << (bit % 32))
	return self.memoryArray[bit/32]
}
...
func (self *PhysicalMemoryManager) FirstFreeSize(કદ uint32) uint32 {
	if કદ == 0 {
		return 0xffffffff
	}
	if કદ == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= કદ; count++ {
...
func (self *PhysicalMemoryManager) Vઆરંભ_કરવો(કદ uint32, bitmap []uint32) {
	self.memorySize = કદ
	self.memoryArray = bitmap
	self.maximumBlocks = કદ / BLOCK_SIZE
