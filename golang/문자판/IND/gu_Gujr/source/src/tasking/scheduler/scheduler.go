...
type SchedulerData struct {
	...
	currentThread	*Tઅમલપ્રવાહ
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vઆરંભ_કરવો() {
...
func (self *SchedulerData) GetNextReadyThread() *Tઅમલપ્રવાહ {
	if list.Vકદ <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vકદ; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vકદ {
			currentThreadIndex = 0
		}
		thread := (*Tઅમલપ્રવાહ)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tઅમલપ્રવાહ) {
	if thread == nil {
		return
	}
	list.Vસૂચિના_અંતે_ઉમેરવું(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tઅમલપ્રવાહ) {
	if thread == nil {
		return
	}
	list.Vસૂચિના_અંતે_ઉમેરવું(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tઅમલપ્રવાહ) {
...
func (self *Scheduler) Vઆરંભ_કરવો(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vઆરંભ_કરવો()
	sche_data.tss = tss
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vકદ > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vકદ; i++ {
		thread := (*Tઅમલપ્રવાહ)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tઅમલપ્રવાહ) {
