...
func (self *TThreadHelper) Vઆરંભ_કરવો(mem *mem.TMemoryManager) {
	self.mem = mem
	콘솔.M출력XY(([]byte)("thread:"), 1, 14)
}
func (self *TThreadHelper) CreateFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) Tઅમલપ્રવાહ {
	result := Tઅમલપ્રવાહ{}

	result.Stack = uint32(uintptr(self.mem.Vસ્મૃતિ_ફાળવવી(THREAD_STACK_SIZE)))
	if result.Stack == 0 {
		return result
	}
	콘솔.M출력(([]byte)("[mem:"))
...
func (self *TThreadHelper) CreatePtrFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) *Tઅમલપ્રવાહ {
	result := (*Tઅમલપ્રવાહ)(self.mem.Vસ્મૃતિ_ફાળવવી(uint32(Sizeof(Tઅમલપ્રવાહ{}))))
	if result == nil {
		return nil
	}
	*result = self.CreateFromFunction(entrypoint, PageDirEntry, isKernel)
	if result.CpuState == nil {
		self.mem.Vસ્મૃતિ_મુક્ત_કરવી(Pointer(result))
		return nil
	}
	return result
}
