...
func (self *TUserDatagramProtocolHeader) Vઆરંભ_કરવો(buffer *TUserDatagramProtocolHeaderBuffer) {
...
type IUserDatagramProtocolHandler interface {
	HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, કદ uint16)
}

type TUserDatagramProtocolHandler struct {
}

func (self *TUserDatagramProtocolHandler) Vઆરંભ_કરવો(backend TInternetProtocolProvider) {
}
func (self *TUserDatagramProtocolHandler) HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, કદ uint16) {
}

type IUserDatagramProtocolSocket interface {
	HandleUserDatagramProtocolMessage(data uintptr, કદ uint16)
}
...
func (self *TUserDatagramProtocolSocket) Vઆરંભ_કરવો(p_udpProvider TUserDatagramProtocolProvider, p_udpHandler IUserDatagramProtocolHandler) {
...
func (self *TUserDatagramProtocolSocket) HandleUserDatagramProtocolMessage(data uintptr, કદ uint16) {
	if udpHandler != nil {
		udpHandler.HandleUserDatagramProtocolMessage(self, data, કદ)
	}
}
func (self *TUserDatagramProtocolSocket) Send(p_data []byte, કદ uint16) {
	var buffer [4096]byte
	for i := 0; i < int(કદ); i++ {
		buffer[i] = p_data[i]
	}
	var data = uintptr(Pointer(&buffer))
	udpProvider.Send(self, data, કદ)
}
...
func (self *TUserDatagramProtocolProvider) Vઆરંભ_કરવો(p_ipProvider TInternetProtocolProvider, p_ipHandler IInternetProtocolHandler) {
	ipHandler = p_ipHandler
	ipHandler.Vઆરંભ_કરવો(p_ipProvider, p_ipHandler, 0x11)
	numSockets = 0
	freePort = 1024
}
func (self *TUserDatagramProtocolProvider) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, internetprotocolPayload uintptr, કદ uint32) bool {
	if કદ < udpHeaderSize {
		return false
	}
	...
	if socket != nil {
		socket.HandleUserDatagramProtocolMessage(internetprotocolPayload+uintptr(udpHeaderSize), uint16(કદ-udpHeaderSize))
	}
...
func (self *TUserDatagramProtocolProvider) Connect(ip uint32, port uint16) *TUserDatagramProtocolSocket {
	...
	var socket = (*TUserDatagramProtocolSocket)(memoryManager.Vસ્મૃતિ_ફાળવવી(50))

	if socket != nil {

		socket.Vઆરંભ_કરવો(*self, nil)
...
func (self *TUserDatagramProtocolProvider) Send(socket *TUserDatagramProtocolSocket, p_data uintptr, કદ uint16) {
	var totalLength = uint32(કદ) + udpHeaderSize
