...
func PrintBytes(dataPointer uintptr, કદ int) {
...
func GetUintptrArrayFromPtr(સરનામા_નિર્દેશક_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{સરનામા_નિર્દેશક_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(સરનામા_નિર્દેશક_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(સરનામા_નિર્દેશક_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(સરનામા_નિર્દેશક_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&સરનામા_નિર્દેશક_2))
