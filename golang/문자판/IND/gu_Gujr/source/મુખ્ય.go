...
func GetFileSize(filename []byte) uint32 {
	...
	var કદ uint32 = bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()

	return કદ
}

func Vદસ્તાવેજ_વાંચવો(filename []byte, data []byte) {
	var ata0s = TAdvancedTechnologyAttachment{}
	ata0s.Vઆરંભ_કરવો(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosParameterBlock32{}
	bios.Vવાંચવું(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func memorytest(y int) {
	...
	allocated := uint32(uintptr(memoryManager.Vસ્મૃતિ_ફાળવવી(1024)))
	console.MUint32출력XY(allocated, 10, uint16(y))
	if y == 11 {
		memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(Pointer(uintptr(allocated)))
	}
}
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vઆરંભ_કરવો(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vઆરંભ_કરવો(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vઆરંભ_કરવો(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vઆરંભ_કરવો()

	threadHelper := &TThreadHelper{}
	threadHelper.Vઆરંભ_કરવો(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vઆરંભ_કરવો(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vઆરંભ_કરવો(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vઆરંભ_કરવો(InterruptManager)
	...
	કદ = GetFileSize(linkerFile)
	linkerAddr := memoryManager.Vસ્મૃતિ_ફાળવવી(કદ)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(કદ), int(કદ))
	Vદસ્તાવેજ_વાંચવો(linkerFile, linkerData)
	...
	linkMap := LinkMap{}
	linkMap.Vઆરંભ_કરવો(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	કદ = GetFileSize(lib1File)

	lib1Addr := memoryManager.Vસ્મૃતિ_ફાળવવી(કદ)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(કદ), int(કદ))
	Vદસ્તાવેજ_વાંચવો(lib1File, lib1Data)

	lib1elf := Elf{}
	lib1elf.Parse(lib1Data[:], uint32(PageDirEntry))
	memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(lib1Addr)

	linkMap.Vસૂચિના_અંતે_ઉમેરવું(uintptr(lib1elf.Dynamic))

	var lib2File []byte = ([]byte)("LIB2")
	કદ = GetFileSize(lib2File)

	lib2Addr := memoryManager.Vસ્મૃતિ_ફાળવવી(કદ)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(કદ), int(કદ))
	Vદસ્તાવેજ_વાંચવો(lib2File, lib2Data)

	lib2elf := Elf{}
	lib2elf.Parse(lib2Data[:], uint32(PageDirEntry))
	memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(lib2Addr)

	linkMap.Vસૂચિના_અંતે_ઉમેરવું(uintptr(lib2elf.Dynamic))
	...
	કદ = GetFileSize(user1file)
	user1addr := memoryManager.Vસ્મૃતિ_ફાળવવી(કદ)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(કદ), int(કદ))
	Vદસ્તાવેજ_વાંચવો(user1file, user1data)
	...
	P1LinkMap.Vસૂચિના_અંતે_ઉમેરવું(uintptr(elf2.Dynamic))

	memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(user1addr)

	var code1Ptr *uintptr
	var func1Val func()

	code1Ptr = (*uintptr)(memoryManager.Vસ્મૃતિ_ફાળવવી(4))
	*code1Ptr = uintptr(linkerEntry)
	...
	thr2 := (*Tઅમલપ્રવાહ)(proc2.Threads.GetAt(0))
	...
	કદ = GetFileSize(user2file)
	user2addr := memoryManager.Vસ્મૃતિ_ફાળવવી(કદ)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(કદ), int(કદ))
	Vદસ્તાવેજ_વાંચવો(user2file, user2data)
	...
	P2LinkMap.Vસૂચિના_અંતે_ઉમેરવું(uintptr(elf3.Dynamic))

	memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(user2addr)

	var code2Ptr *uintptr
	var func2Val func()

	code2Ptr = (*uintptr)(memoryManager.Vસ્મૃતિ_ફાળવવી(4))
	*code2Ptr = uintptr(linkerEntry)
	...
	thr3 := (*Tઅમલપ્રવાહ)(proc3.Threads.GetAt(0))
	...
	કદ = GetFileSize(user3file)
	user3addr := memoryManager.Vસ્મૃતિ_ફાળવવી(કદ)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(કદ), int(કદ))
	Vદસ્તાવેજ_વાંચવો(user3file, user3data)
	...
	P3LinkMap.Vસૂચિના_અંતે_ઉમેરવું(uintptr(elf4.Dynamic))

	memoryManager.Vસ્મૃતિ_મુક્ત_કરવી(user3addr)

	var code3Ptr *uintptr
	var func3Val func()

	code3Ptr = (*uintptr)(memoryManager.Vસ્મૃતિ_ફાળવવી(4))
	*code3Ptr = uintptr(linkerEntry)
	...
	thr4 := (*Tઅમલપ્રવાહ)(proc4.Threads.GetAt(0))
	...
	pciController.Vઆરંભ_કરવો(myPCIControllerHandler)
