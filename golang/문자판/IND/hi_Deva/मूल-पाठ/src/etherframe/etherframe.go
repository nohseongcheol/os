...
func (self *TEtherFrameHeader) Vआरंभ_करना(buffer TEtherFrameHeaderBuffer) {
...
type IEtherFrameHandler interface {
	Vआरंभ_करना(backend TEtherFrameProvider)
...
func (self *TEtherFrameHandler) Vआरंभ_करना(backend TEtherFrameProvider) {
	Backend = backend
}
...
func (self *TEtherFrameRawDataHandler) Vआरंभ_करना(p_provider TEtherFrameProvider, p_backend Tamd_am79c973) {
	provider = p_provider
	provider.Vआरंभ_करना(p_backend)
}
...
func (self *TEtherFrameProvider) Vआरंभ_करना(backend Tamd_am79c973) {
...
func (self *TEtherFrameProvider) OnRawDataReceived(dataPointer uintptr, आकार int) bool {
	...
	if frame.dstMAC_BE == 0x0000FFFFFFFFFFFF || Uint48_R(frame.dstMAC_BE) == self.GetMACAddress() {
		if handlers[frame.etherType_BE] != nil {
			...
			var पता_संदर्भ uintptr = uintptr(Pointer(dataPointer)) + uintptr(frameHeaderSize)
			sendBack = handlers[frame.etherType_BE].OnEtherFrameReceived(पता_संदर्भ, आकार-frameHeaderSize)
...
func (self *TEtherFrameProvider) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, आकार uint32) {
	...
	var पता_संदर्भ uintptr = uintptr(Pointer(&buffer2))

	self.netCard.Send(पता_संदर्भ, आकार+uint32(frameHeaderSize))
