...
type SchedulerData struct {
	...
	currentThread	*Tनिष्पादन_धारा
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vआरंभ_करना() {
...
func (self *SchedulerData) GetNextReadyThread() *Tनिष्पादन_धारा {
	if list.Vआकार <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vआकार; checked++ {
		...
		thread := (*Tनिष्पादन_धारा)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tनिष्पादन_धारा) {
	if thread == nil {
		return
	}
	list.Vसूची_के_अंत_में_जोड़ना(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tनिष्पादन_धारा) {
	if thread == nil {
		return
	}
	list.Vसूची_के_अंत_में_जोड़ना(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tनिष्पादन_धारा) {
...
func (self *Scheduler) Vआरंभ_करना(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vआरंभ_करना()
	sche_data.tss = tss
	...
	self.TInterruptHandler.Vआरंभ_करना(0x20, uintptr(Pointer(manager)), addr)
}
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vआकार; i++ {
		thread := (*Tनिष्पादन_धारा)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tनिष्पादन_धारा) {
