...
void *malloc(size_t ಗಾತ್ರ)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!ಗಾತ್ರ) ಗಾತ್ರ = 1;
    if (ಗಾತ್ರ > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    ಗಾತ್ರ = (ಗಾತ್ರ + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= ಗಾತ್ರ) {
            if (cursor->capacity - ಗಾತ್ರ >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + ಗಾತ್ರ);
                raw->capacity = cursor->capacity - ಗಾತ್ರ - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = ಗಾತ್ರ;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + ಗಾತ್ರ;
    ...
    raw->capacity = ಗಾತ್ರ; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t ಗಾತ್ರ)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(ಗಾತ್ರ);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!ಗಾತ್ರ) ಗಾತ್ರ = 1;
    cursor = (struct heap_block *)address - 1;
    if (ಗಾತ್ರ <= cursor->capacity) return address;
    destination = malloc(ಗಾತ್ರ);
