...
ssize_t ಓದು(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ಓದು, fd, buf, count));
}

ssize_t ಬರೆ(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ಬರೆ, fd, buf, count));
}
...
char *getcwd(char *buf, size_t ಗಾತ್ರ)
{
    long result = __syscall_result(SC2(SYS_getcwd, buf, ಗಾತ್ರ));
