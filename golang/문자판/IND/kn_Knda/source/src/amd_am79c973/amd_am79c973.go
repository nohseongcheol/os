...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, ಗಾತ್ರ int) bool
	Send(dataPointer uintptr, ಗಾತ್ರ uint32)
}
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, ಗಾತ್ರ int) bool {
	넷카드콘솔.M출력XY(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawDataHandler) Send(dataPointer uintptr, ಗಾತ್ರ uint32) {
	넷카드콘솔.M출력XY(([]byte)("TRawDataSend"), 10, 10)
	rawDataBackend.Send(dataPointer, ಗಾತ್ರ)
}
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Vಆರಂಭಿಸು(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, ಗಾತ್ರ uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + ಗಾತ್ರ - 1

	for i := 0; i < int(ಗಾತ್ರ); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(ಗಾತ್ರ)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-ಗಾತ್ರ)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var ಗಾತ್ರ uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if ಗಾತ್ರ > 64 {
				ಗಾತ್ರ -= 4
			}
			...
			var ವಿಳಾಸ_ಸೂಚಕ uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(ವಿಳಾಸ_ಸೂಚಕ, int(ಗಾತ್ರ)) {

					콘솔.M출력XY(([]byte)("self.Send"), 0, 22)

					self.Send(ವಿಳಾಸ_ಸೂಚಕ, ಗಾತ್ರ)
				}
			}
