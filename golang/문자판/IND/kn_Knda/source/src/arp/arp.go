...
func (self *TARPMessage) Vಆರಂಭಿಸು(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, ಗಾತ್ರ int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(ಗಾತ್ರ))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, ಗಾತ್ರ uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, ಗಾತ್ರ)
}
...
func (self *TARPProvider) Vಆರಂಭಿಸು(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vಆರಂಭಿಸು(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, ಗಾತ್ರ uint32) bool {

	if ಗಾತ್ರ < arpMesgSize {
		return false
	}
	var arpBuffer *TARPMessageBuffer = (*TARPMessageBuffer)(Pointer(dataPointer))
	var arp TARPMessage = TARPMessage{}
	arp.Vಆರಂಭಿಸು(arpBuffer)
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var ವಿಳಾಸ_ಸೂಚಕ uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, ವಿಳಾಸ_ಸೂಚಕ, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, ಗಾತ್ರ uint32) {
