...
func (self *TEtherFrameHeader) Vಆರಂಭಿಸು(buffer TEtherFrameHeaderBuffer) {
...
type IEtherFrameHandler interface {
	Vಆರಂಭಿಸು(backend TEtherFrameProvider)
	SetHandler(handler IEtherFrameHandler, etherType uint16)
	OnEtherFrameReceived(dataPointer uintptr, ಗಾತ್ರ int) bool
	Send(dstMAC_BE uint64, dataPointer uintptr, ಗಾತ್ರ uint32)
	SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, ಗಾತ್ರ uint32)
...
func (self *TEtherFrameHandler) Vಆರಂಭಿಸು(backend TEtherFrameProvider) {
	Backend = backend
}
...
func (self *TEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, ಗಾತ್ರ int) bool {
	이더넷콘솔.M출력(([]byte)("OnEtherFrameReceived"))
	return false
}
func (self *TEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, ಗಾತ್ರ uint32) {
	Backend.SendFrame(dstMAC_BE, frame.etherType_BE, dataPointer, ಗಾತ್ರ)
}
func (self *TEtherFrameHandler) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, ಗಾತ್ರ uint32) {
	Backend.SendFrame(dstMAC_BE, etherType_BE, dataPointer, ಗಾತ್ರ)
}
...
func (self *TEtherFrameRawDataHandler) Vಆರಂಭಿಸು(p_provider TEtherFrameProvider, p_backend Tamd_am79c973) {
	provider = p_provider
	provider.Vಆರಂಭಿಸು(p_backend)
}
func (self *TEtherFrameRawDataHandler) OnRawDataReceived(dataPointer uintptr, ಗಾತ್ರ int) bool {
	return provider.OnRawDataReceived(dataPointer, ಗಾತ್ರ)
}
func (self *TEtherFrameRawDataHandler) Send(dataPointer uintptr, ಗಾತ್ರ uint32) {
	provider.Send(dataPointer, ಗಾತ್ರ)
}
...
func (self *TEtherFrameProvider) Vಆರಂಭಿಸು(backend Tamd_am79c973) {
...
func (self *TEtherFrameProvider) OnRawDataReceived(dataPointer uintptr, ಗಾತ್ರ int) bool {
	...
	if frame.dstMAC_BE == 0x0000FFFFFFFFFFFF || Uint48_R(frame.dstMAC_BE) == self.GetMACAddress() {
		if handlers[frame.etherType_BE] != nil {
			...
			var ವಿಳಾಸ_ಸೂಚಕ uintptr = uintptr(Pointer(dataPointer)) + uintptr(frameHeaderSize)
			sendBack = handlers[frame.etherType_BE].OnEtherFrameReceived(ವಿಳಾಸ_ಸೂಚಕ, ಗಾತ್ರ-frameHeaderSize)
...
func (self *TEtherFrameProvider) Send(dataPointer uintptr, ಗಾತ್ರ uint32) {
	self.netCard.Send(dataPointer, ಗಾತ್ರ)
}
func (self *TEtherFrameProvider) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, ಗಾತ್ರ uint32) {
	...
	var ವಿಳಾಸ_ಸೂಚಕ uintptr = uintptr(Pointer(&buffer2))

	self.netCard.Send(ವಿಳಾಸ_ಸೂಚಕ, ಗಾತ್ರ+uint32(frameHeaderSize))
