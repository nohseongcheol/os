	...
	if elfHeader.e_shnum != 0 {
		...
		for i := uint32(0); i < uint32(elfHeader.e_shnum); i++ {
			...
			if sectHeader.sh_addr > 0x1000 {
				ಗಾತ್ರ := sectHeader.sh_size
				if sectHeader.sh_type == 8 {
					ZeroBlockInPageDir(sectHeader.sh_addr, ಗಾತ್ರ, PageDirEntry)
				} else {
					dst := GetBytesFromPtr(uintptr(sectHeader.sh_addr), int(ಗಾತ್ರ), int(ಗಾತ್ರ))
					SetBlockInPageDir(sectValue, dst, ಗಾತ್ರ, PageDirEntry)
				}
			}
		...
		if textPtr != nil {
			memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(textPtr)
		}
