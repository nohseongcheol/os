...
func (self *TBiosParameterBlock32) Vಆರಂಭಿಸು(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	bpbPtr := memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(90)
	...
	direntPtr := memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(512)
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vಆರಂಭಿಸು(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].ಗಾತ್ರ >= 0xFFFFFFFF {
			continue
		}
		...
		memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(bpbPtr)
		memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(direntPtr)
		return dirent[i].ಗಾತ್ರ
	}
	memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(bpbPtr)
	memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(direntPtr)
	return 0
}
func (self *TBiosParameterBlock32) Vಓದು(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].ಗಾತ್ರ > 512 {
					buf3 = buffer[:512]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, 512)

				} else {
					buf3 = buffer[:dirent[i].ಗಾತ್ರ]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].ಗಾತ್ರ))
				}

				copy(data[int32(dirent[i].ಗಾತ್ರ)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vಆರಂಭಿಸು(data [32]byte) {
	...
	self.ಗಾತ್ರ = Uint32_R(ArrayToUint32(buf))
}
