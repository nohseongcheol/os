...
func (self *TInternetControlMessageProtocolMessage) Vಆರಂಭಿಸು(buffer TInternetControlMessageProtocolMessageBuffer) {
...
func (self *TICMPHandler) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, dataPointer uintptr, ಗಾತ್ರ uint32) bool {
	return icmp.OnInternetProtocolReceived(srcIP_BE, dstIP_BE, dataPointer, ಗಾತ್ರ)
}
...
func (self *TInternetControlMessageProtocol) Vಆರಂಭಿಸು(backend TInternetProtocolProvider, handler IInternetProtocolHandler) {
	ipHandler = handler
	ipHandler.Vಆರಂಭಿಸು(backend, handler, 0x01)
	icmp = self
}
func (self *TInternetControlMessageProtocol) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, dataPointer uintptr, ಗಾತ್ರ uint32) bool {
	if ಗಾತ್ರ < uint32(icmpSize) {
		return false
	}
...
func (self *TInternetControlMessageProtocol) SendEchoRequest(ip_be uint32) bool {
	...
	var buffer = (*TInternetControlMessageProtocolMessageBuffer)(memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(1024))
