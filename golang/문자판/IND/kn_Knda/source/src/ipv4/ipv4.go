...
func (self *TInternetProtocolV4Message) Vಆರಂಭಿಸು(buffer TInternetProtocolV4MessageBuffer) {
...
type IInternetProtocolHandler interface {
	Vಆರಂಭಿಸು(backend TInternetProtocolProvider, p_iHandler IInternetProtocolHandler, p_protocol uint8)
	OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, dataPointer uintptr, ಗಾತ್ರ uint32) bool
	Send(dstIP_BE uint32, p_protocol uint8, dataPointer uintptr, ಗಾತ್ರ uint32)
...
func (self *TInternetProtocolHandler) Vಆರಂಭಿಸು(backend TInternetProtocolProvider, p_iHandler IInternetProtocolHandler, p_protocol uint8) {
	protocol = p_protocol
	handlers[protocol] = p_iHandler
}
func (self *TInternetProtocolHandler) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, dataPointer uintptr, ಗಾತ್ರ uint32) bool {
	ip콘솔.M출력(([]byte)("ipHandler:OnInternet"))
	return false
}
func (self *TInternetProtocolHandler) Send(dstIP_BE uint32, p_protocol uint8, dataPointer uintptr, ಗಾತ್ರ uint32) {

	ipProvider.Send(dstIP_BE, p_protocol, dataPointer, ಗಾತ್ರ)
}
...
func (self *TIPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, ಗಾತ್ರ int) bool {
	ip콘솔.M출력(([]byte)("iphandler:onEtherfameRecv\n"))
	return ipProvider.OnEtherFrameReceived(dataPointer, uint32(ಗಾತ್ರ))

}

func (self *TIPEtherFrameHandler) Send(dstIP_BE uint64, dataPointer uintptr, ಗಾತ್ರ uint32) {
	ip콘솔.M출력(([]byte)("ipefhandler:send\n"))
	var etherType_BE = Uint16_R(0x0800)
	self.TEtherFrameHandler.SendFrame(dstIP_BE, etherType_BE, dataPointer, ಗಾತ್ರ)
...
func (self *TInternetProtocolProvider) Vಆರಂಭಿಸು(p_efProvider TEtherFrameProvider, p_efHandler IEtherFrameHandler, arp TARPProvider, gatewayIP uint32, subnetMask uint32) {
...
func (self *TInternetProtocolProvider) OnEtherFrameReceived(etherframePayload uintptr, ಗಾತ್ರ uint32) bool {
	if ಗಾತ್ರ < uint32(ipSize) {
		return false
	}
...
func (self *TInternetProtocolProvider) Send(dstIP_BE uint32, protocol uint8, dataPointer uintptr, ಗಾತ್ರ uint32) {
	...
	message.tos = 0
	message.totalLength = Uint16_R(uint16(ಗಾತ್ರ + uint32(ipSize)))
	...
	var etherType_BE = Uint16_R(0x0800)
	efHandler.SendFrame(self.arpProvider.Resolve(nextHopIP_BE), etherType_BE, sendDataPointer, uint32(ipSize)+uint32(ಗಾತ್ರ))
