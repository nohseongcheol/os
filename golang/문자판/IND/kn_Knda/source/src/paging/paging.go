...
func (self *TCOWFrameManager) Vಆರಂಭಿಸು(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
	self.frameCount = frameCount
	refBytes := frameCount * uint32(unsafe.Sizeof(uint16(0)))
	refPtr := mem.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(refBytes)
	if refPtr == nil {
		self.refs = nil
		self.frameCount = 0
		return false
...
func (self *Paging) Vಆರಂಭಿಸು(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vಆರಂಭಿಸು(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vಆರಂಭಿಸು(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, ಗಾತ್ರ uint32, pageDir uint32) {
	if ಗಾತ್ರ == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), ಗಾತ್ರ)

	for i := uint32(0); i < ಗಾತ್ರ; i++ {
		dst[i] = src[i]
	}
	setCR3(oldCR3)
}

func ZeroBlockInPageDir(addr uint32, ಗಾತ್ರ uint32, pageDir uint32) {
	if ಗಾತ್ರ == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, ಗಾತ್ರ)
	dst := GetBytesFromPtr(uintptr(addr), int(ಗಾತ್ರ), int(ಗಾತ್ರ))
	for i := uint32(0); i < ಗಾತ್ರ; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, ಗಾತ್ರ uint32) bool {
	if ಗಾತ್ರ == 0 {
		return true
	}
	last := addr + ಗಾತ್ರ - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, ಗಾತ್ರ uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, ಗಾತ್ರ)
	setCR3(oldCR3)
	return ok
}
