...
func (self *PhysicalMemoryManager) Vಹಂಚಿಕೆಯ_ಬಿಟ್_ಅನ್ನು_ತಿರುಗಿಸು(bit uint32) uint32 {
	self.memoryArray[bit/32] = self.memoryArray[bit/32] ^ (1 << (bit % 32))
	return self.memoryArray[bit/32]
}
...
func (self *PhysicalMemoryManager) FirstFreeSize(ಗಾತ್ರ uint32) uint32 {
	if ಗಾತ್ರ == 0 {
		return 0xffffffff
	}
	if ಗಾತ್ರ == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= ಗಾತ್ರ; count++ {
...
func (self *PhysicalMemoryManager) Vಆರಂಭಿಸು(ಗಾತ್ರ uint32, bitmap []uint32) {
	self.memorySize = ಗಾತ್ರ
	self.memoryArray = bitmap
	self.maximumBlocks = ಗಾತ್ರ / BLOCK_SIZE
