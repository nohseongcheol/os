...
type SchedulerData struct {
	...
	currentThread	*Tಕಾರ್ಯಧಾರೆ
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vಆರಂಭಿಸು() {
...
func (self *SchedulerData) GetNextReadyThread() *Tಕಾರ್ಯಧಾರೆ {
	if list.Vಗಾತ್ರ <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vಗಾತ್ರ; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vಗಾತ್ರ {
			currentThreadIndex = 0
		}
		thread := (*Tಕಾರ್ಯಧಾರೆ)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tಕಾರ್ಯಧಾರೆ) {
	if thread == nil {
		return
	}
	list.Vಪಟ್ಟಿಯ_ಕೊನೆಯಲ್ಲಿ_ಸೇರಿಸು(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tಕಾರ್ಯಧಾರೆ) {
	if thread == nil {
		return
	}
	list.Vಪಟ್ಟಿಯ_ಕೊನೆಯಲ್ಲಿ_ಸೇರಿಸು(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tಕಾರ್ಯಧಾರೆ) {
...
func (self *Scheduler) Vಆರಂಭಿಸು(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vಆರಂಭಿಸು()
	sche_data.tss = tss
	...
	self.TInterruptHandler.Vಆರಂಭಿಸು(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vಗಾತ್ರ > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vಗಾತ್ರ; i++ {
		thread := (*Tಕಾರ್ಯಧಾರೆ)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tಕಾರ್ಯಧಾರೆ) {
