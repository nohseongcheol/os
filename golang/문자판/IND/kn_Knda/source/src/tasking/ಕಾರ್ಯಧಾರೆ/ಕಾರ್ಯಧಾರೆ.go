...
func (self *TThreadHelper) Vಆರಂಭಿಸು(mem *mem.TMemoryManager) {
	self.mem = mem
	콘솔.M출력XY(([]byte)("thread:"), 1, 14)
}
func (self *TThreadHelper) CreateFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) Tಕಾರ್ಯಧಾರೆ {
	result := Tಕಾರ್ಯಧಾರೆ{}

	result.Stack = uint32(uintptr(self.mem.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(THREAD_STACK_SIZE)))
	if result.Stack == 0 {
		return result
	}
	콘솔.M출력(([]byte)("[mem:"))
...
func (self *TThreadHelper) CreatePtrFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) *Tಕಾರ್ಯಧಾರೆ {
	result := (*Tಕಾರ್ಯಧಾರೆ)(self.mem.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(uint32(Sizeof(Tಕಾರ್ಯಧಾರೆ{}))))
	if result == nil {
		return nil
	}
	*result = self.CreateFromFunction(entrypoint, PageDirEntry, isKernel)
	if result.CpuState == nil {
		self.mem.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(Pointer(result))
		return nil
	}
	return result
}
