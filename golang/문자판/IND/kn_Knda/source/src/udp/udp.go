...
func (self *TUserDatagramProtocolHeader) Vಆರಂಭಿಸು(buffer *TUserDatagramProtocolHeaderBuffer) {
...
type IUserDatagramProtocolHandler interface {
	HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, ಗಾತ್ರ uint16)
}

type TUserDatagramProtocolHandler struct {
}

func (self *TUserDatagramProtocolHandler) Vಆರಂಭಿಸು(backend TInternetProtocolProvider) {
}
func (self *TUserDatagramProtocolHandler) HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, ಗಾತ್ರ uint16) {
}

type IUserDatagramProtocolSocket interface {
	HandleUserDatagramProtocolMessage(data uintptr, ಗಾತ್ರ uint16)
}
...
func (self *TUserDatagramProtocolSocket) Vಆರಂಭಿಸು(p_udpProvider TUserDatagramProtocolProvider, p_udpHandler IUserDatagramProtocolHandler) {
...
func (self *TUserDatagramProtocolSocket) HandleUserDatagramProtocolMessage(data uintptr, ಗಾತ್ರ uint16) {
	if udpHandler != nil {
		udpHandler.HandleUserDatagramProtocolMessage(self, data, ಗಾತ್ರ)
	}
}
func (self *TUserDatagramProtocolSocket) Send(p_data []byte, ಗಾತ್ರ uint16) {
	var buffer [4096]byte
	for i := 0; i < int(ಗಾತ್ರ); i++ {
		buffer[i] = p_data[i]
	}
	var data = uintptr(Pointer(&buffer))
	udpProvider.Send(self, data, ಗಾತ್ರ)
}
...
func (self *TUserDatagramProtocolProvider) Vಆರಂಭಿಸು(p_ipProvider TInternetProtocolProvider, p_ipHandler IInternetProtocolHandler) {
	ipHandler = p_ipHandler
	ipHandler.Vಆರಂಭಿಸು(p_ipProvider, p_ipHandler, 0x11)
	numSockets = 0
	freePort = 1024
}
func (self *TUserDatagramProtocolProvider) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, internetprotocolPayload uintptr, ಗಾತ್ರ uint32) bool {
	if ಗಾತ್ರ < udpHeaderSize {
		return false
	}
	...
	if socket != nil {
		socket.HandleUserDatagramProtocolMessage(internetprotocolPayload+uintptr(udpHeaderSize), uint16(ಗಾತ್ರ-udpHeaderSize))
	}
...
func (self *TUserDatagramProtocolProvider) Connect(ip uint32, port uint16) *TUserDatagramProtocolSocket {
	...
	var socket = (*TUserDatagramProtocolSocket)(memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(50))

	if socket != nil {

		socket.Vಆರಂಭಿಸು(*self, nil)
...
func (self *TUserDatagramProtocolProvider) Send(socket *TUserDatagramProtocolSocket, p_data uintptr, ಗಾತ್ರ uint16) {
	var totalLength = uint32(ಗಾತ್ರ) + udpHeaderSize
