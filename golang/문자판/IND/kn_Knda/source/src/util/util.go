...
func PrintBytes(dataPointer uintptr, ಗಾತ್ರ int) {
...
func GetUintptrArrayFromPtr(ವಿಳಾಸ_ಸೂಚಕ_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{ವಿಳಾಸ_ಸೂಚಕ_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(ವಿಳಾಸ_ಸೂಚಕ_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(ವಿಳಾಸ_ಸೂಚಕ_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(ವಿಳಾಸ_ಸೂಚಕ_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&ವಿಳಾಸ_ಸೂಚಕ_2))
