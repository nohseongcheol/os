...
func GetFileSize(filename []byte) uint32 {
	...
	var ಗಾತ್ರ uint32 = bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()

	return ಗಾತ್ರ
}

func Vಕಡತವನ್ನು_ಓದು(filename []byte, data []byte) {
	var ata0s = TAdvancedTechnologyAttachment{}
	ata0s.Vಆರಂಭಿಸು(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosParameterBlock32{}
	bios.Vಓದು(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func LoadElf() {
	...
	elf.Parse(data[:ಗಾತ್ರ], 0x4f00000)
...
func memorytest(y int) {
	...
	allocated := uint32(uintptr(memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(1024)))
	console.MUint32출력XY(allocated, 10, uint16(y))
	if y == 11 {
		memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(Pointer(uintptr(allocated)))
	}
}
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vಆರಂಭಿಸು(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vಆರಂಭಿಸು(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vಆರಂಭಿಸು(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vಆರಂಭಿಸು()

	threadHelper := &TThreadHelper{}
	threadHelper.Vಆರಂಭಿಸು(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vಆರಂಭಿಸು(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vಆರಂಭಿಸು(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vಆರಂಭಿಸು(InterruptManager)
	...
	ಗಾತ್ರ = GetFileSize(linkerFile)
	linkerAddr := memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(ಗಾತ್ರ)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(ಗಾತ್ರ), int(ಗಾತ್ರ))
	Vಕಡತವನ್ನು_ಓದು(linkerFile, linkerData)
	...
	linkMap := LinkMap{}
	linkMap.Vಆರಂಭಿಸು(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	ಗಾತ್ರ = GetFileSize(lib1File)

	lib1Addr := memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(ಗಾತ್ರ)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(ಗಾತ್ರ), int(ಗಾತ್ರ))
	Vಕಡತವನ್ನು_ಓದು(lib1File, lib1Data)

	lib1elf := Elf{}
	lib1elf.Parse(lib1Data[:], uint32(PageDirEntry))
	memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(lib1Addr)

	linkMap.Vಪಟ್ಟಿಯ_ಕೊನೆಯಲ್ಲಿ_ಸೇರಿಸು(uintptr(lib1elf.Dynamic))

	var lib2File []byte = ([]byte)("LIB2")
	ಗಾತ್ರ = GetFileSize(lib2File)

	lib2Addr := memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(ಗಾತ್ರ)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(ಗಾತ್ರ), int(ಗಾತ್ರ))
	Vಕಡತವನ್ನು_ಓದು(lib2File, lib2Data)

	lib2elf := Elf{}
	lib2elf.Parse(lib2Data[:], uint32(PageDirEntry))
	memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(lib2Addr)

	linkMap.Vಪಟ್ಟಿಯ_ಕೊನೆಯಲ್ಲಿ_ಸೇರಿಸು(uintptr(lib2elf.Dynamic))
	...
	ಗಾತ್ರ = GetFileSize(user1file)
	user1addr := memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(ಗಾತ್ರ)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(ಗಾತ್ರ), int(ಗಾತ್ರ))
	Vಕಡತವನ್ನು_ಓದು(user1file, user1data)
	...
	P1LinkMap.Vಪಟ್ಟಿಯ_ಕೊನೆಯಲ್ಲಿ_ಸೇರಿಸು(uintptr(elf2.Dynamic))

	memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(user1addr)

	var code1Ptr *uintptr
	var func1Val func()

	code1Ptr = (*uintptr)(memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(4))
	*code1Ptr = uintptr(linkerEntry)
	...
	thr2 := (*Tಕಾರ್ಯಧಾರೆ)(proc2.Threads.GetAt(0))
	...
	ಗಾತ್ರ = GetFileSize(user2file)
	user2addr := memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(ಗಾತ್ರ)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(ಗಾತ್ರ), int(ಗಾತ್ರ))
	Vಕಡತವನ್ನು_ಓದು(user2file, user2data)
	...
	P2LinkMap.Vಪಟ್ಟಿಯ_ಕೊನೆಯಲ್ಲಿ_ಸೇರಿಸು(uintptr(elf3.Dynamic))

	memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(user2addr)

	var code2Ptr *uintptr
	var func2Val func()

	code2Ptr = (*uintptr)(memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(4))
	*code2Ptr = uintptr(linkerEntry)
	...
	thr3 := (*Tಕಾರ್ಯಧಾರೆ)(proc3.Threads.GetAt(0))
	...
	ಗಾತ್ರ = GetFileSize(user3file)
	user3addr := memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(ಗಾತ್ರ)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(ಗಾತ್ರ), int(ಗಾತ್ರ))
	Vಕಡತವನ್ನು_ಓದು(user3file, user3data)
	...
	P3LinkMap.Vಪಟ್ಟಿಯ_ಕೊನೆಯಲ್ಲಿ_ಸೇರಿಸು(uintptr(elf4.Dynamic))

	memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಬಿಡುಗಡೆಮಾಡು(user3addr)

	var code3Ptr *uintptr
	var func3Val func()

	code3Ptr = (*uintptr)(memoryManager.Vಸ್ಮೃತಿಯನ್ನು_ಹಂಚು(4))
	*code3Ptr = uintptr(linkerEntry)
	...
	thr4 := (*Tಕಾರ್ಯಧಾರೆ)(proc4.Threads.GetAt(0))
	...
	pciController.Vಆರಂಭಿಸು(myPCIControllerHandler)
