...
ssize_t वाचप(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_वाचप, fd, buf, count));
}

ssize_t बरोवप(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_बरोवप, fd, buf, count));
}
