...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vबरोवप(0xA0)
	} else {
		self.devicePort.Vबरोवप(0xB0)
	}
	self.controlPort.Vबरोवप(0)
	self.devicePort.Vबरोवप(0xA0)

	var status uint8 = self.commandPort.Vवाचप()
	if status == 0xFF {
	...
	self.sectorCountPort.Vबरोवप(0)
	self.lbaLowPort.Vबरोवप(0)
	self.lbaMidPort.Vबरोवप(0)
	self.lbaHiPort.Vबरोवप(0)
	self.commandPort.Vबरोवप(0xEC)

	status = self.commandPort.Vवाचप()
	if status == 0x00 {
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vबरोवप(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vबरोवप(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vबरोवप(0)
	self.sectorCountPort.Vबरोवप(1)

	self.lbaLowPort.Vबरोवप(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vबरोवप(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vबरोवप(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vबरोवप(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vवाचप()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vबरोवप(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vबरोवप(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vबरोवप(0)
	self.sectorCountPort.Vबरोवप(1)
	self.lbaLowPort.Vबरोवप(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vबरोवप(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vबरोवप(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vबरोवप(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vबरोवप(0xE0)
	} else {
		self.devicePort.Vबरोवप(0xF0)
	}
	self.commandPort.Vबरोवप(0xE7)
