...
ssize_t vachop(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_vachop, fd, buf, count));
}

ssize_t borovop(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_borovop, fd, buf, count));
}
