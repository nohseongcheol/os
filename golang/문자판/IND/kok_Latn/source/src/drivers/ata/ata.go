...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vborovop(0xA0)
	} else {
		self.devicePort.Vborovop(0xB0)
	}
	self.controlPort.Vborovop(0)
	self.devicePort.Vborovop(0xA0)

	var status uint8 = self.commandPort.Vvachop()
	if status == 0xFF {
	...
	self.sectorCountPort.Vborovop(0)
	self.lbaLowPort.Vborovop(0)
	self.lbaMidPort.Vborovop(0)
	self.lbaHiPort.Vborovop(0)
	self.commandPort.Vborovop(0xEC)

	status = self.commandPort.Vvachop()
	if status == 0x00 {
	...
	for i := 0; i < 256; i++ {
		var data = self.dataPort.Vvachop()
		text := []byte("  ")
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vborovop(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vborovop(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vborovop(0)
	self.sectorCountPort.Vborovop(1)

	self.lbaLowPort.Vborovop(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vborovop(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vborovop(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vborovop(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vvachop()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vborovop(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vborovop(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vborovop(0)
	self.sectorCountPort.Vborovop(1)
	self.lbaLowPort.Vborovop(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vborovop(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vborovop(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vborovop(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vborovop(0xE0)
	} else {
		self.devicePort.Vborovop(0xF0)
	}
	self.commandPort.Vborovop(0xE7)
