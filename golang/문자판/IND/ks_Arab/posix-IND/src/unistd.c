...
ssize_t پَرُن(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_پَرُن, fd, buf, count));
}

ssize_t لِکھُن(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_لِکھُن, fd, buf, count));
}
