...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vلِکھُن(0xA0)
	} else {
		self.devicePort.Vلِکھُن(0xB0)
	}
	self.controlPort.Vلِکھُن(0)
	self.devicePort.Vلِکھُن(0xA0)

	var status uint8 = self.commandPort.Vپَرُن()
	if status == 0xFF {
	...
	self.sectorCountPort.Vلِکھُن(0)
	self.lbaLowPort.Vلِکھُن(0)
	self.lbaMidPort.Vلِکھُن(0)
	self.lbaHiPort.Vلِکھُن(0)
	self.commandPort.Vلِکھُن(0xEC)

	status = self.commandPort.Vپَرُن()
	if status == 0x00 {
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vلِکھُن(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vلِکھُن(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vلِکھُن(0)
	self.sectorCountPort.Vلِکھُن(1)

	self.lbaLowPort.Vلِکھُن(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vلِکھُن(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vلِکھُن(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vلِکھُن(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vپَرُن()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vلِکھُن(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vلِکھُن(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vلِکھُن(0)
	self.sectorCountPort.Vلِکھُن(1)
	self.lbaLowPort.Vلِکھُن(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vلِکھُن(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vلِکھُن(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vلِکھُن(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vلِکھُن(0xE0)
	} else {
		self.devicePort.Vلِکھُن(0xF0)
	}
	self.commandPort.Vلِکھُن(0xE7)
