...
ssize_t परुन(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_परुन, fd, buf, count));
}

ssize_t लिखुन(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_लिखुन, fd, buf, count));
}
