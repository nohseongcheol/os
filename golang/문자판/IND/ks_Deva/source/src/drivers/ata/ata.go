...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vलिखुन(0xA0)
	} else {
		self.devicePort.Vलिखुन(0xB0)
	}
	self.controlPort.Vलिखुन(0)
	self.devicePort.Vलिखुन(0xA0)

	var status uint8 = self.commandPort.Vपरुन()
	if status == 0xFF {
	...
	self.sectorCountPort.Vलिखुन(0)
	self.lbaLowPort.Vलिखुन(0)
	self.lbaMidPort.Vलिखुन(0)
	self.lbaHiPort.Vलिखुन(0)
	self.commandPort.Vलिखुन(0xEC)

	status = self.commandPort.Vपरुन()
	if status == 0x00 {
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vलिखुन(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vलिखुन(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vलिखुन(0)
	self.sectorCountPort.Vलिखुन(1)

	self.lbaLowPort.Vलिखुन(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vलिखुन(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vलिखुन(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vलिखुन(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vपरुन()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vलिखुन(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vलिखुन(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vलिखुन(0)
	self.sectorCountPort.Vलिखुन(1)
	self.lbaLowPort.Vलिखुन(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vलिखुन(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vलिखुन(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vलिखुन(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vलिखुन(0xE0)
	} else {
		self.devicePort.Vलिखुन(0xF0)
	}
	self.commandPort.Vलिखुन(0xE7)
