...
func (self *TEtherFrameHeader) Init(buffer TEtherFrameHeaderBuffer) {
...
type IEtherFrameHandler interface {
	Init(backend TEtherFrameProvider)
	SetHandler(handler IEtherFrameHandler, etherType uint16)
	OnEtherFrameReceived(dataPointer uintptr, size int) bool
	Send(dstMAC_BE uint64, dataPointer uintptr, size uint32)
	SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, size uint32)
...
func (self *TEtherFrameHandler) Init(backend TEtherFrameProvider) {
	Backend = backend
}
...
func (self *TEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, size int) bool {
	이더넷콘솔.M출력(([]byte)("OnEtherFrameReceived"))
	return false
}
func (self *TEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, size uint32) {
	Backend.SendFrame(dstMAC_BE, frame.etherType_BE, dataPointer, size)
}
func (self *TEtherFrameHandler) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, size uint32) {
	Backend.SendFrame(dstMAC_BE, etherType_BE, dataPointer, size)
}
...
func (self *TEtherFrameRawDataHandler) Init(p_provider TEtherFrameProvider, p_backend Tamd_am79c973) {
	provider = p_provider
	provider.Init(p_backend)
}
func (self *TEtherFrameRawDataHandler) OnRawDataReceived(dataPointer uintptr, size int) bool {
	return provider.OnRawDataReceived(dataPointer, size)
}
func (self *TEtherFrameRawDataHandler) Send(dataPointer uintptr, size uint32) {
...
func (self *TEtherFrameProvider) Init(backend Tamd_am79c973) {
...
func (self *TEtherFrameProvider) OnRawDataReceived(dataPointer uintptr, size int) bool {
	...
	if frame.dstMAC_BE == 0x0000FFFFFFFFFFFF || Uint48_R(frame.dstMAC_BE) == self.GetMACAddress() {
		if handlers[frame.etherType_BE] != nil {
			...
			sendBack = handlers[frame.etherType_BE].OnEtherFrameReceived(pointer, size-frameHeaderSize)
...
func (self *TEtherFrameProvider) Send(dataPointer uintptr, size uint32) {
	self.netCard.Send(dataPointer, size)
}
func (self *TEtherFrameProvider) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, size uint32) {
	...
	var pointer uintptr = uintptr(Pointer(&buffer2))

	self.netCard.Send(pointer, size+uint32(frameHeaderSize))
