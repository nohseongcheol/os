...
static void write_text(const char *text)
{
    ...
    while (length > 0U) {
        ssize_t bytes_written = लिखब(STDOUT_FILENO, text, length);
        if (bytes_written <= 0)
            return;
        text += bytes_written;
...
static void write_integer(int value)
{
    ...
    while (आकार > 0U) {
        आकार--;
        (void)लिखब(STDOUT_FILENO, &digit_text[आकार], 1);
    }
}
...
static int read_line(struct input_stream *input, char *input_line, size_t capacity)
{
    ...
    for (;;) {
        if (input->position == input->length) {
            bytes_read = पढ़ब(input->input_descriptor, input->buffer, sizeof(input->buffer));
            if (bytes_read < 0) {
                if (errno == EINTR)
                    continue;
                return -1;
...
static void command_cat(const char *file_name)
{
    ...
    while ((bytes_read = पढ़ब(file_descriptor, buffer, sizeof(buffer))) > 0)
        (void)लिखब(STDOUT_FILENO, buffer, (size_t)bytes_read);
    if (bytes_read < 0)
