...
ssize_t पढ़ब(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_पढ़ब, fd, buf, count));
}

ssize_t लिखब(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_लिखब, fd, buf, count));
}
