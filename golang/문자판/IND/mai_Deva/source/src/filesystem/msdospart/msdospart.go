...
func (self *TPartitionTableEntry) Vआरंभ_करब(data [16]byte) {
...
func (self *TMSDOSPartitionTable) ReadPartitions(hd *TAdvancedTechnologyAttachment) {
	...
	self.MBR.PrimaryPartition[0].Vआरंभ_करब(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[1].Vआरंभ_करब(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[2].Vआरंभ_करब(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[3].Vआरंभ_करब(buf3)
	i += 16
