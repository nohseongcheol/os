...
func GetUintptrArrayFromPtr(पता_संदर्भ_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{पता_संदर्भ_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(पता_संदर्भ_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(पता_संदर्भ_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(पता_संदर्भ_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&पता_संदर्भ_2))
