...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vआरंभ_करब(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vआरंभ_करब(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vआरंभ_करब(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vआरंभ_करब()

	threadHelper := &TThreadHelper{}
	threadHelper.Vआरंभ_करब(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vआरंभ_करब(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vआरंभ_करब(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vआरंभ_करब(InterruptManager)
	...
	linkerAddr := memoryManager.Malloc(आकार)
	...
	linkMap := LinkMap{}
	linkMap.Vआरंभ_करब(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	आकार = GetFileSize(lib1File)

	lib1Addr := memoryManager.Malloc(आकार)
	...
	lib2Addr := memoryManager.Malloc(आकार)
	...
	user1addr := memoryManager.Malloc(आकार)
	...
	*code1Ptr = uintptr(linkerEntry)
	...
	user2addr := memoryManager.Malloc(आकार)
	...
	*code2Ptr = uintptr(linkerEntry)
	...
	user3addr := memoryManager.Malloc(आकार)
	...
	*code3Ptr = uintptr(linkerEntry)
	...
	pciController.Vआरंभ_करब(myPCIControllerHandler)
