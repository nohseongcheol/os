...
ssize_t വായിക്കുക(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_വായിക്കുക, fd, buf, count));
}

ssize_t എഴുതുക(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_എഴുതുക, fd, buf, count));
}
...
char *getcwd(char *buf, size_t വലുപ്പം)
{
    long result = __syscall_result(SC2(SYS_getcwd, buf, വലുപ്പം));
