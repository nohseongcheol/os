...
func (self *TARPMessage) Vആരംഭിക്കുക(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, വലുപ്പം int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(വലുപ്പം))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, വലുപ്പം uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, വലുപ്പം)
}
...
func (self *TARPProvider) Vആരംഭിക്കുക(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vആരംഭിക്കുക(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, വലുപ്പം uint32) bool {

	if വലുപ്പം < arpMesgSize {
		return false
	}
	var arpBuffer *TARPMessageBuffer = (*TARPMessageBuffer)(Pointer(dataPointer))
	var arp TARPMessage = TARPMessage{}
	arp.Vആരംഭിക്കുക(arpBuffer)
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var വിലാസ_സൂചിക uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, വിലാസ_സൂചിക, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, വലുപ്പം uint32) {
