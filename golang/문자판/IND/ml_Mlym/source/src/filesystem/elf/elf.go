	...
	if elfHeader.e_shnum != 0 {
		...
		for i := uint32(0); i < uint32(elfHeader.e_shnum); i++ {
			...
			if sectHeader.sh_addr > 0x1000 {
				വലുപ്പം := sectHeader.sh_size
				if sectHeader.sh_type == 8 {
					ZeroBlockInPageDir(sectHeader.sh_addr, വലുപ്പം, PageDirEntry)
				} else {
					dst := GetBytesFromPtr(uintptr(sectHeader.sh_addr), int(വലുപ്പം), int(വലുപ്പം))
					SetBlockInPageDir(sectValue, dst, വലുപ്പം, PageDirEntry)
				}
			}
		...
		if textPtr != nil {
			memoryManager.Vഓർമ്മസ്ഥലം_വിടുക(textPtr)
		}
