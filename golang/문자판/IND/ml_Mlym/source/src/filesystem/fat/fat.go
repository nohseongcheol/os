...
func (self *TBiosParameterBlock32) Vആരംഭിക്കുക(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	bpbPtr := memoryManager.Vഓർമ്മസ്ഥലം_അനുവദിക്കുക(90)
	...
	direntPtr := memoryManager.Vഓർമ്മസ്ഥലം_അനുവദിക്കുക(512)
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vആരംഭിക്കുക(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].വലുപ്പം >= 0xFFFFFFFF {
			continue
		}
		...
		memoryManager.Vഓർമ്മസ്ഥലം_വിടുക(bpbPtr)
		memoryManager.Vഓർമ്മസ്ഥലം_വിടുക(direntPtr)
		return dirent[i].വലുപ്പം
	}
	memoryManager.Vഓർമ്മസ്ഥലം_വിടുക(bpbPtr)
	memoryManager.Vഓർമ്മസ്ഥലം_വിടുക(direntPtr)
	return 0
}
func (self *TBiosParameterBlock32) Vവായിക്കുക(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		var SIZE = int32(dirent[i].വലുപ്പം)
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].വലുപ്പം > 512 {
					buf3 = buffer[:512]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, 512)

				} else {
					buf3 = buffer[:dirent[i].വലുപ്പം]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].വലുപ്പം))
				}

				copy(data[int32(dirent[i].വലുപ്പം)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vആരംഭിക്കുക(data [32]byte) {
	...
	self.വലുപ്പം = Uint32_R(ArrayToUint32(buf))
}
