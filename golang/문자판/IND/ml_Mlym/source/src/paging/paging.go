...
func (self *TCOWFrameManager) Vആരംഭിക്കുക(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
	self.frameCount = frameCount
	refBytes := frameCount * uint32(unsafe.Sizeof(uint16(0)))
	refPtr := mem.Vഓർമ്മസ്ഥലം_അനുവദിക്കുക(refBytes)
	if refPtr == nil {
		self.refs = nil
		self.frameCount = 0
		return false
...
func (self *Paging) Vആരംഭിക്കുക(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vആരംഭിക്കുക(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vആരംഭിക്കുക(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, വലുപ്പം uint32, pageDir uint32) {
	if വലുപ്പം == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), വലുപ്പം)

	for i := uint32(0); i < വലുപ്പം; i++ {
		dst[i] = src[i]
	}
	setCR3(oldCR3)
}

func ZeroBlockInPageDir(addr uint32, വലുപ്പം uint32, pageDir uint32) {
	if വലുപ്പം == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, വലുപ്പം)
	dst := GetBytesFromPtr(uintptr(addr), int(വലുപ്പം), int(വലുപ്പം))
	for i := uint32(0); i < വലുപ്പം; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, വലുപ്പം uint32) bool {
	if വലുപ്പം == 0 {
		return true
	}
	last := addr + വലുപ്പം - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, വലുപ്പം uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, വലുപ്പം)
	setCR3(oldCR3)
	return ok
}
