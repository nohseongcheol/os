...
func (self *PhysicalMemoryManager) Vഅനുവദന_ബിറ്റ്_മറിക്കുക(bit uint32) uint32 {
	self.memoryArray[bit/32] = self.memoryArray[bit/32] ^ (1 << (bit % 32))
	return self.memoryArray[bit/32]
}
...
func (self *PhysicalMemoryManager) FirstFreeSize(വലുപ്പം uint32) uint32 {
	if വലുപ്പം == 0 {
		return 0xffffffff
	}
	if വലുപ്പം == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= വലുപ്പം; count++ {
...
func (self *PhysicalMemoryManager) Vആരംഭിക്കുക(വലുപ്പം uint32, bitmap []uint32) {
	self.memorySize = വലുപ്പം
	self.memoryArray = bitmap
	self.maximumBlocks = വലുപ്പം / BLOCK_SIZE
