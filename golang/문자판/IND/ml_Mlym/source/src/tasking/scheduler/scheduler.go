...
type SchedulerData struct {
	...
	currentThread	*Tനിർവഹണധാര
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vആരംഭിക്കുക() {
...
func (self *SchedulerData) GetNextReadyThread() *Tനിർവഹണധാര {
	if list.Vവലുപ്പം <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vവലുപ്പം; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vവലുപ്പം {
			currentThreadIndex = 0
		}
		thread := (*Tനിർവഹണധാര)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tനിർവഹണധാര) {
	if thread == nil {
		return
	}
	list.Vപട്ടികയുടെ_അവസാനം_ചേർക്കുക(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tനിർവഹണധാര) {
	if thread == nil {
		return
	}
	list.Vപട്ടികയുടെ_അവസാനം_ചേർക്കുക(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tനിർവഹണധാര) {
...
func (self *Scheduler) Vആരംഭിക്കുക(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vആരംഭിക്കുക()
	sche_data.tss = tss
	initPIT(pitFrequency)

	list = LinkedList{}
	list.Vആരംഭിക്കുക(mem)
	콘솔.M출력("list:")
	...
	self.TInterruptHandler.Vആരംഭിക്കുക(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vവലുപ്പം > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vവലുപ്പം; i++ {
		thread := (*Tനിർവഹണധാര)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tനിർവഹണധാര) {
