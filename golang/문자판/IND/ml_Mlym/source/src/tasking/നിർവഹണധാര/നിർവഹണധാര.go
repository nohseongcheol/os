...
func (self *TThreadHelper) Vആരംഭിക്കുക(mem *mem.TMemoryManager) {
	self.mem = mem
	콘솔.M출력XY(([]byte)("thread:"), 1, 14)
}
func (self *TThreadHelper) CreateFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) Tനിർവഹണധാര {
	result := Tനിർവഹണധാര{}

	result.Stack = uint32(uintptr(self.mem.Vഓർമ്മസ്ഥലം_അനുവദിക്കുക(THREAD_STACK_SIZE)))
	if result.Stack == 0 {
		return result
	}
	콘솔.M출력(([]byte)("[mem:"))
...
func (self *TThreadHelper) CreatePtrFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) *Tനിർവഹണധാര {
	result := (*Tനിർവഹണധാര)(self.mem.Vഓർമ്മസ്ഥലം_അനുവദിക്കുക(uint32(Sizeof(Tനിർവഹണധാര{}))))
	if result == nil {
		return nil
	}
	*result = self.CreateFromFunction(entrypoint, PageDirEntry, isKernel)
	if result.CpuState == nil {
		self.mem.Vഓർമ്മസ്ഥലം_വിടുക(Pointer(result))
		return nil
	}
	return result
}
