...
func PrintBytes(dataPointer uintptr, വലുപ്പം int) {
...
func GetUintptrArrayFromPtr(വിലാസ_സൂചിക_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{വിലാസ_സൂചിക_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(വിലാസ_സൂചിക_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(വിലാസ_സൂചിക_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(വിലാസ_സൂചിക_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&വിലാസ_സൂചിക_2))
