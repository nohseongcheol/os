...
ssize_t পাবা(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_পাবা, fd, buf, count));
}

ssize_t ইবা(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ইবা, fd, buf, count));
}
