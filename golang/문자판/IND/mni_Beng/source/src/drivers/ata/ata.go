...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vইবা(0xA0)
	} else {
		self.devicePort.Vইবা(0xB0)
	}
	self.controlPort.Vইবা(0)
	self.devicePort.Vইবা(0xA0)

	var status uint8 = self.commandPort.Vপাবা()
	if status == 0xFF {
	...
	self.sectorCountPort.Vইবা(0)
	self.lbaLowPort.Vইবা(0)
	self.lbaMidPort.Vইবা(0)
	self.lbaHiPort.Vইবা(0)
	self.commandPort.Vইবা(0xEC)

	status = self.commandPort.Vপাবা()
	if status == 0x00 {
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vইবা(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vইবা(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vইবা(0)
	self.sectorCountPort.Vইবা(1)

	self.lbaLowPort.Vইবা(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vইবা(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vইবা(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vইবা(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vপাবা()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vইবা(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vইবা(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vইবা(0)
	self.sectorCountPort.Vইবা(1)
	self.lbaLowPort.Vইবা(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vইবা(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vইবা(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vইবা(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vইবা(0xE0)
	} else {
		self.devicePort.Vইবা(0xF0)
	}
	self.commandPort.Vইবা(0xE7)
