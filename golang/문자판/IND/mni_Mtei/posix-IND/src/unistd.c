...
ssize_t ꯄꯥꯕ(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ꯄꯥꯕ, fd, buf, count));
}

ssize_t ꯏꯕ(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ꯏꯕ, fd, buf, count));
}
