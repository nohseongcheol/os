...
{
    (void)ꯏꯕ(STDOUT_FILENO, text, text_length(text));
}
...
static void test_open_and_io(void)
{
    ...
    if (fd >= 0) {
        report("read-bytes", ꯄꯥꯕ(fd, bytes, 2) == 2 &&
    ...
    if (root >= 0) {
        ...
        errno = 0;
        report("read-directory", ꯄꯥꯕ(root, bytes, 1) == -1 && errno == EISDIR);
        (void)close(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", ꯏꯕ(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", ꯄꯥꯕ(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", ꯄꯥꯕ(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", ꯏꯕ(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", ꯄꯥꯕ(fd, &first, 1) == 1 && ꯄꯥꯕ(copy, &second, 1) == 1 &&
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &status) == child) {
        report("fork-fd-isolation", ꯄꯥꯕ(fd, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
