...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, size int) bool
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, size int) bool {
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Init(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, size uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + size - 1

	for i := 0; i < int(size); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(size)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-size)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var size uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if size > 64 {
				size -= 4
			}
			...
			var pointer uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(pointer, int(size)) {
