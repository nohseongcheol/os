...
func GetUintptrArrayFromPtr(ptr uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetUint32ArrayFromPtr(ptr uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetFuncPtrFromPtr(ptr uintptr) func() {
	code1ptr := uintptr(Pointer(&ptr))
