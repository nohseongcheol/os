...
ssize_t वाचणे(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_वाचणे, fd, buf, count));
}

ssize_t लिहिणे(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_लिहिणे, fd, buf, count));
}
