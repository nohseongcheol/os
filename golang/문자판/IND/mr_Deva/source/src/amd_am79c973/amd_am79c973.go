...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, आकार int) bool
	Send(dataPointer uintptr, आकार uint32)
}
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, आकार int) bool {
	넷카드콘솔.M출력XY(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawDataHandler) Send(dataPointer uintptr, आकार uint32) {
	넷카드콘솔.M출력XY(([]byte)("TRawDataSend"), 10, 10)
	rawDataBackend.Send(dataPointer, आकार)
}
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Vआरंभ_करणे(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, आकार uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + आकार - 1

	for i := 0; i < int(आकार); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(आकार)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-आकार)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var आकार uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if आकार > 64 {
				आकार -= 4
			}
			...
			var पत्ता_संदर्भ uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(पत्ता_संदर्भ, int(आकार)) {

					콘솔.M출력XY(([]byte)("self.Send"), 0, 22)

					self.Send(पत्ता_संदर्भ, आकार)
				}
			}
