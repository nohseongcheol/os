...
type SchedulerData struct {
	...
	currentThread	*Tकार्यप्रवाह
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vआरंभ_करणे() {
...
func (self *SchedulerData) GetNextReadyThread() *Tकार्यप्रवाह {
	if list.Vआकार <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vआकार; checked++ {
		...
		thread := (*Tकार्यप्रवाह)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tकार्यप्रवाह) {
	if thread == nil {
		return
	}
	list.Vसूचीच्या_शेवटी_जोडणे(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tकार्यप्रवाह) {
	if thread == nil {
		return
	}
	list.Vसूचीच्या_शेवटी_जोडणे(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tकार्यप्रवाह) {
...
func (self *Scheduler) Vआरंभ_करणे(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vआरंभ_करणे()
	sche_data.tss = tss
	...
	self.TInterruptHandler.Vआरंभ_करणे(0x20, uintptr(Pointer(manager)), addr)
}
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vआकार; i++ {
		thread := (*Tकार्यप्रवाह)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tकार्यप्रवाह) {
