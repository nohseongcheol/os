...
func (self *Process) Vआरंभ_करणे(mem *mem.TMemoryManager) {
	self.ThreadList = LinkedList{}
	self.Threads = &self.ThreadList
	self.Threads.Vआरंभ_करणे(mem)
}
...
func (self *ProcessHelper) Vआरंभ_करणे(mem *mem.TMemoryManager, kernelPageDirEntry uintptr) {
	self.mem = mem
	self.processes = LinkedList{}
	self.processes.Vआरंभ_करणे(self.mem)
...
func (self *ProcessHelper) Create(entryPoint func(), threadHelper *TThreadHelper, PageDirEntry uint32, isKernel bool) Process {
	process := (*Process)(self.mem.Vस्मृती_वाटप_करणे(uint32(Sizeof(Process{}))))
	if process == nil {
		return Process{}
	}
	process.Vआरंभ_करणे(self.mem)
	...
	if mainThread != nil {
		...
		process.Threads.Vसूचीच्या_शेवटी_जोडणे(uintptr(Pointer(mainThread)))
	}

	self.processes.Vसूचीच्या_शेवटी_जोडणे(uintptr(Pointer(process)))
...
func (self *ProcessHelper) Spawn(entryPoint func(), threadHelper *TThreadHelper, scheduler *Scheduler, PageDirEntry uint32, isKernel bool) Process {
	...
	if process.Threads != nil && process.Threads.Vआकार > 0 {
		thread := (*Tकार्यप्रवाह)(process.Threads.GetAt(0))
