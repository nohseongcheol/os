...
func Vधारिका_वाचणे(filename []byte, data []byte) {
	var ata0s = TAdvancedTechnologyAttachment{}
	ata0s.Vआरंभ_करणे(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosParameterBlock32{}
	bios.Vवाचणे(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func memorytest(y int) {
	...
	allocated := uint32(uintptr(memoryManager.Vस्मृती_वाटप_करणे(1024)))
	console.MUint32출력XY(allocated, 10, uint16(y))
	if y == 11 {
		memoryManager.Vस्मृती_मुक्त_करणे(Pointer(uintptr(allocated)))
	}
}
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vआरंभ_करणे(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vआरंभ_करणे(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vआरंभ_करणे(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vआरंभ_करणे()

	threadHelper := &TThreadHelper{}
	threadHelper.Vआरंभ_करणे(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vआरंभ_करणे(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vआरंभ_करणे(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vआरंभ_करणे(InterruptManager)
	...
	linkerAddr := memoryManager.Vस्मृती_वाटप_करणे(आकार)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(आकार), int(आकार))
	Vधारिका_वाचणे(linkerFile, linkerData)
	...
	linkMap := LinkMap{}
	linkMap.Vआरंभ_करणे(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	आकार = GetFileSize(lib1File)

	lib1Addr := memoryManager.Vस्मृती_वाटप_करणे(आकार)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(आकार), int(आकार))
	Vधारिका_वाचणे(lib1File, lib1Data)

	lib1elf := Elf{}
	lib1elf.Parse(lib1Data[:], uint32(PageDirEntry))
	memoryManager.Vस्मृती_मुक्त_करणे(lib1Addr)

	linkMap.Vसूचीच्या_शेवटी_जोडणे(uintptr(lib1elf.Dynamic))

	var lib2File []byte = ([]byte)("LIB2")
	आकार = GetFileSize(lib2File)

	lib2Addr := memoryManager.Vस्मृती_वाटप_करणे(आकार)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(आकार), int(आकार))
	Vधारिका_वाचणे(lib2File, lib2Data)

	lib2elf := Elf{}
	lib2elf.Parse(lib2Data[:], uint32(PageDirEntry))
	memoryManager.Vस्मृती_मुक्त_करणे(lib2Addr)

	linkMap.Vसूचीच्या_शेवटी_जोडणे(uintptr(lib2elf.Dynamic))
	...
	user1addr := memoryManager.Vस्मृती_वाटप_करणे(आकार)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(आकार), int(आकार))
	Vधारिका_वाचणे(user1file, user1data)
	...
	P1LinkMap.Vसूचीच्या_शेवटी_जोडणे(uintptr(elf2.Dynamic))

	memoryManager.Vस्मृती_मुक्त_करणे(user1addr)

	var code1Ptr *uintptr
	var func1Val func()

	code1Ptr = (*uintptr)(memoryManager.Vस्मृती_वाटप_करणे(4))
	*code1Ptr = uintptr(linkerEntry)
	...
	thr2 := (*Tकार्यप्रवाह)(proc2.Threads.GetAt(0))
	...
	user2addr := memoryManager.Vस्मृती_वाटप_करणे(आकार)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(आकार), int(आकार))
	Vधारिका_वाचणे(user2file, user2data)
	...
	P2LinkMap.Vसूचीच्या_शेवटी_जोडणे(uintptr(elf3.Dynamic))

	memoryManager.Vस्मृती_मुक्त_करणे(user2addr)

	var code2Ptr *uintptr
	var func2Val func()

	code2Ptr = (*uintptr)(memoryManager.Vस्मृती_वाटप_करणे(4))
	*code2Ptr = uintptr(linkerEntry)
	...
	thr3 := (*Tकार्यप्रवाह)(proc3.Threads.GetAt(0))
	...
	user3addr := memoryManager.Vस्मृती_वाटप_करणे(आकार)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(आकार), int(आकार))
	Vधारिका_वाचणे(user3file, user3data)
	...
	P3LinkMap.Vसूचीच्या_शेवटी_जोडणे(uintptr(elf4.Dynamic))

	memoryManager.Vस्मृती_मुक्त_करणे(user3addr)

	var code3Ptr *uintptr
	var func3Val func()

	code3Ptr = (*uintptr)(memoryManager.Vस्मृती_वाटप_करणे(4))
	*code3Ptr = uintptr(linkerEntry)
	...
	thr4 := (*Tकार्यप्रवाह)(proc4.Threads.GetAt(0))
	...
	pciController.Vआरंभ_करणे(myPCIControllerHandler)
