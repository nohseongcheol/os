	...
	if elfHeader.e_shnum != 0 {
		...
		for i := uint32(0); i < uint32(elfHeader.e_shnum); i++ {
			...
			if sectHeader.sh_addr > 0x1000 {
				आकार := sectHeader.sh_size
				if sectHeader.sh_type == 8 {
					ZeroBlockInPageDir(sectHeader.sh_addr, आकार, PageDirEntry)
				} else {
					dst := GetBytesFromPtr(uintptr(sectHeader.sh_addr), int(आकार), int(आकार))
					SetBlockInPageDir(sectValue, dst, आकार, PageDirEntry)
				}
			}
		...
		if textPtr != nil {
			memoryManager.Vस्मृति_मुक्त_गर्नु(textPtr)
		}
