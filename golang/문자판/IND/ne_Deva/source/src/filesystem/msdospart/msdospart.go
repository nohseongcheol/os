...
func (self *TPartitionTableEntry) Vआरम्भ_गर्नु(data [16]byte) {
...
func (self *TMSDOSPartitionTable) ReadPartitions(hd *TAdvancedTechnologyAttachment) {
	...
	self.MBR.PrimaryPartition[0].Vआरम्भ_गर्नु(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[1].Vआरम्भ_गर्नु(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[2].Vआरम्भ_गर्नु(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[3].Vआरम्भ_गर्नु(buf3)
	i += 16
