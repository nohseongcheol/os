...
type SchedulerData struct {
	...
	currentThread	*Tकार्यान्वयन_धारा
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vआरम्भ_गर्नु() {
...
func (self *SchedulerData) GetNextReadyThread() *Tकार्यान्वयन_धारा {
	if list.Vआकार <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vआकार; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vआकार {
			currentThreadIndex = 0
		}
		thread := (*Tकार्यान्वयन_धारा)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tकार्यान्वयन_धारा) {
	if thread == nil {
		return
	}
	list.Vसूचीको_अन्त्यमा_थप्नु(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tकार्यान्वयन_धारा) {
	if thread == nil {
		return
	}
	list.Vसूचीको_अन्त्यमा_थप्नु(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tकार्यान्वयन_धारा) {
...
func (self *Scheduler) Vआरम्भ_गर्नु(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vआरम्भ_गर्नु()
	sche_data.tss = tss
	initPIT(pitFrequency)

	list = LinkedList{}
	list.Vआरम्भ_गर्नु(mem)
	콘솔.M출력("list:")
	...
	self.TInterruptHandler.Vआरम्भ_गर्नु(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vआकार > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vआकार; i++ {
		thread := (*Tकार्यान्वयन_धारा)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tकार्यान्वयन_धारा) {
