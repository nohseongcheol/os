...
func (self *Tकार्यान्वयन_धारा) New() {
}
...
func (self *TThreadHelper) Vआरम्भ_गर्नु(mem *mem.TMemoryManager) {
	self.mem = mem
	콘솔.M출력XY(([]byte)("thread:"), 1, 14)
}
func (self *TThreadHelper) CreateFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) Tकार्यान्वयन_धारा {
	result := Tकार्यान्वयन_धारा{}

	result.Stack = uint32(uintptr(self.mem.Vस्मृति_छुट्याउनु(THREAD_STACK_SIZE)))
	if result.Stack == 0 {
		return result
	}
	콘솔.M출력(([]byte)("[mem:"))
...
func (self *TThreadHelper) CreatePtrFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) *Tकार्यान्वयन_धारा {
	result := (*Tकार्यान्वयन_धारा)(self.mem.Vस्मृति_छुट्याउनु(uint32(Sizeof(Tकार्यान्वयन_धारा{}))))
	if result == nil {
		return nil
	}
	*result = self.CreateFromFunction(entrypoint, PageDirEntry, isKernel)
	if result.CpuState == nil {
		self.mem.Vस्मृति_मुक्त_गर्नु(Pointer(result))
		return nil
	}
	return result
}
