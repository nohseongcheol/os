...
func GetFileSize(filename []byte) uint32 {
	...
	ata0s.Vआरम्भ_गर्नु(false, 0x1F0)
	ata0s.Identify()
	...
	var आकार uint32 = bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()

	return आकार
}

func Vसञ्चिका_पढ्नु(filename []byte, data []byte) {
	var ata0s = TAdvancedTechnologyAttachment{}
	ata0s.Vआरम्भ_गर्नु(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosParameterBlock32{}
	bios.Vपढ्नु(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func memorytest(y int) {
	...
	allocated := uint32(uintptr(memoryManager.Vस्मृति_छुट्याउनु(1024)))
	console.MUint32출력XY(allocated, 10, uint16(y))
	if y == 11 {
		memoryManager.Vस्मृति_मुक्त_गर्नु(Pointer(uintptr(allocated)))
	}
}
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vआरम्भ_गर्नु(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vआरम्भ_गर्नु(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vआरम्भ_गर्नु(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vआरम्भ_गर्नु()

	threadHelper := &TThreadHelper{}
	threadHelper.Vआरम्भ_गर्नु(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vआरम्भ_गर्नु(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vआरम्भ_गर्नु(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vआरम्भ_गर्नु(InterruptManager)
	...
	आकार = GetFileSize(linkerFile)
	linkerAddr := memoryManager.Vस्मृति_छुट्याउनु(आकार)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(आकार), int(आकार))
	Vसञ्चिका_पढ्नु(linkerFile, linkerData)
	...
	linkMap := LinkMap{}
	linkMap.Vआरम्भ_गर्नु(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	आकार = GetFileSize(lib1File)

	lib1Addr := memoryManager.Vस्मृति_छुट्याउनु(आकार)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(आकार), int(आकार))
	Vसञ्चिका_पढ्नु(lib1File, lib1Data)

	lib1elf := Elf{}
	lib1elf.Parse(lib1Data[:], uint32(PageDirEntry))
	memoryManager.Vस्मृति_मुक्त_गर्नु(lib1Addr)

	linkMap.Vसूचीको_अन्त्यमा_थप्नु(uintptr(lib1elf.Dynamic))

	var lib2File []byte = ([]byte)("LIB2")
	आकार = GetFileSize(lib2File)

	lib2Addr := memoryManager.Vस्मृति_छुट्याउनु(आकार)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(आकार), int(आकार))
	Vसञ्चिका_पढ्नु(lib2File, lib2Data)

	lib2elf := Elf{}
	lib2elf.Parse(lib2Data[:], uint32(PageDirEntry))
	memoryManager.Vस्मृति_मुक्त_गर्नु(lib2Addr)

	linkMap.Vसूचीको_अन्त्यमा_थप्नु(uintptr(lib2elf.Dynamic))
	...
	आकार = GetFileSize(user1file)
	user1addr := memoryManager.Vस्मृति_छुट्याउनु(आकार)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(आकार), int(आकार))
	Vसञ्चिका_पढ्नु(user1file, user1data)
	...
	P1LinkMap.Vसूचीको_अन्त्यमा_थप्नु(uintptr(elf2.Dynamic))

	memoryManager.Vस्मृति_मुक्त_गर्नु(user1addr)

	var code1Ptr *uintptr
	var func1Val func()

	code1Ptr = (*uintptr)(memoryManager.Vस्मृति_छुट्याउनु(4))
	*code1Ptr = uintptr(linkerEntry)
	...
	thr2 := (*Tकार्यान्वयन_धारा)(proc2.Threads.GetAt(0))
	...
	आकार = GetFileSize(user2file)
	user2addr := memoryManager.Vस्मृति_छुट्याउनु(आकार)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(आकार), int(आकार))
	Vसञ्चिका_पढ्नु(user2file, user2data)
	...
	P2LinkMap.Vसूचीको_अन्त्यमा_थप्नु(uintptr(elf3.Dynamic))

	memoryManager.Vस्मृति_मुक्त_गर्नु(user2addr)

	var code2Ptr *uintptr
	var func2Val func()

	code2Ptr = (*uintptr)(memoryManager.Vस्मृति_छुट्याउनु(4))
	*code2Ptr = uintptr(linkerEntry)
	...
	thr3 := (*Tकार्यान्वयन_धारा)(proc3.Threads.GetAt(0))
	...
	आकार = GetFileSize(user3file)
	user3addr := memoryManager.Vस्मृति_छुट्याउनु(आकार)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(आकार), int(आकार))
	Vसञ्चिका_पढ्नु(user3file, user3data)
	...
	P3LinkMap.Vसूचीको_अन्त्यमा_थप्नु(uintptr(elf4.Dynamic))

	memoryManager.Vस्मृति_मुक्त_गर्नु(user3addr)

	var code3Ptr *uintptr
	var func3Val func()

	code3Ptr = (*uintptr)(memoryManager.Vस्मृति_छुट्याउनु(4))
	*code3Ptr = uintptr(linkerEntry)
	...
	thr4 := (*Tकार्यान्वयन_धारा)(proc4.Threads.GetAt(0))
	...
	pciController.Vआरम्भ_गर्नु(myPCIControllerHandler)
