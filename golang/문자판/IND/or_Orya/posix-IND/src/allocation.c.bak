...
void *malloc(size_t ଆକାର)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!ଆକାର) ଆକାର = 1;
    if (ଆକାର > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    ଆକାର = (ଆକାର + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= ଆକାର) {
            if (cursor->capacity - ଆକାର >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + ଆକାର);
                raw->capacity = cursor->capacity - ଆକାର - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = ଆକାର;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + ଆକାର;
    ...
    raw->capacity = ଆକାର; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t ଆକାର)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(ଆକାର);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!ଆକାର) ଆକାର = 1;
    cursor = (struct heap_block *)address - 1;
    if (ଆକାର <= cursor->capacity) return address;
    destination = malloc(ଆକାର);
