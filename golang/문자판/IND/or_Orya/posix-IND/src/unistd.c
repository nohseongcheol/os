...
ssize_t ପଢ଼ିବା(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ପଢ଼ିବା, fd, buf, count));
}

ssize_t ଲେଖିବା(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ଲେଖିବା, fd, buf, count));
}
...
char *getcwd(char *buf, size_t ଆକାର)
{
    long result = __syscall_result(SC2(SYS_getcwd, buf, ଆକାର));
