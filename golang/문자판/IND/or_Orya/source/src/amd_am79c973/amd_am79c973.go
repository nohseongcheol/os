...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, ଆକାର int) bool
	Send(dataPointer uintptr, ଆକାର uint32)
}
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, ଆକାର int) bool {
	넷카드콘솔.M출력XY(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawDataHandler) Send(dataPointer uintptr, ଆକାର uint32) {
	넷카드콘솔.M출력XY(([]byte)("TRawDataSend"), 10, 10)
	rawDataBackend.Send(dataPointer, ଆକାର)
}
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Vଆରମ୍ଭ_କରିବା(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, ଆକାର uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + ଆକାର - 1

	for i := 0; i < int(ଆକାର); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(ଆକାର)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-ଆକାର)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var ଆକାର uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if ଆକାର > 64 {
				ଆକାର -= 4
			}
			...
			var ଠିକଣା_ସୂଚକ uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(ଠିକଣା_ସୂଚକ, int(ଆକାର)) {

					콘솔.M출력XY(([]byte)("self.Send"), 0, 22)

					self.Send(ଠିକଣା_ସୂଚକ, ଆକାର)
				}
			}
