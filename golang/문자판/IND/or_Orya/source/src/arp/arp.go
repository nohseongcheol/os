...
func (self *TARPMessage) Vଆରମ୍ଭ_କରିବା(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, ଆକାର int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(ଆକାର))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, ଆକାର uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, ଆକାର)
}
...
func (self *TARPProvider) Vଆରମ୍ଭ_କରିବା(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vଆରମ୍ଭ_କରିବା(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, ଆକାର uint32) bool {
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var ଠିକଣା_ସୂଚକ uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, ଠିକଣା_ସୂଚକ, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, ଆକାର uint32) {
