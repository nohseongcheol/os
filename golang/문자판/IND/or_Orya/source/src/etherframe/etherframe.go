...
func (self *TEtherFrameHeader) Vଆରମ୍ଭ_କରିବା(buffer TEtherFrameHeaderBuffer) {
...
type IEtherFrameHandler interface {
	Vଆରମ୍ଭ_କରିବା(backend TEtherFrameProvider)
	SetHandler(handler IEtherFrameHandler, etherType uint16)
	OnEtherFrameReceived(dataPointer uintptr, ଆକାର int) bool
	Send(dstMAC_BE uint64, dataPointer uintptr, ଆକାର uint32)
	SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, ଆକାର uint32)
...
func (self *TEtherFrameHandler) Vଆରମ୍ଭ_କରିବା(backend TEtherFrameProvider) {
	Backend = backend
}
...
func (self *TEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, ଆକାର int) bool {
	이더넷콘솔.M출력(([]byte)("OnEtherFrameReceived"))
	return false
}
func (self *TEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, ଆକାର uint32) {
	Backend.SendFrame(dstMAC_BE, frame.etherType_BE, dataPointer, ଆକାର)
}
func (self *TEtherFrameHandler) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, ଆକାର uint32) {
	Backend.SendFrame(dstMAC_BE, etherType_BE, dataPointer, ଆକାର)
}
...
func (self *TEtherFrameRawDataHandler) Vଆରମ୍ଭ_କରିବା(p_provider TEtherFrameProvider, p_backend Tamd_am79c973) {
	provider = p_provider
	provider.Vଆରମ୍ଭ_କରିବା(p_backend)
}
func (self *TEtherFrameRawDataHandler) OnRawDataReceived(dataPointer uintptr, ଆକାର int) bool {
	return provider.OnRawDataReceived(dataPointer, ଆକାର)
}
func (self *TEtherFrameRawDataHandler) Send(dataPointer uintptr, ଆକାର uint32) {
	provider.Send(dataPointer, ଆକାର)
}
...
func (self *TEtherFrameProvider) Vଆରମ୍ଭ_କରିବା(backend Tamd_am79c973) {
...
func (self *TEtherFrameProvider) OnRawDataReceived(dataPointer uintptr, ଆକାର int) bool {
	...
	if frame.dstMAC_BE == 0x0000FFFFFFFFFFFF || Uint48_R(frame.dstMAC_BE) == self.GetMACAddress() {
		if handlers[frame.etherType_BE] != nil {
			...
			var ଠିକଣା_ସୂଚକ uintptr = uintptr(Pointer(dataPointer)) + uintptr(frameHeaderSize)
			sendBack = handlers[frame.etherType_BE].OnEtherFrameReceived(ଠିକଣା_ସୂଚକ, ଆକାର-frameHeaderSize)
...
func (self *TEtherFrameProvider) Send(dataPointer uintptr, ଆକାର uint32) {
	self.netCard.Send(dataPointer, ଆକାର)
}
func (self *TEtherFrameProvider) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, ଆକାର uint32) {
	...
	var ଠିକଣା_ସୂଚକ uintptr = uintptr(Pointer(&buffer2))

	self.netCard.Send(ଠିକଣା_ସୂଚକ, ଆକାର+uint32(frameHeaderSize))
