...
func (self *TBiosParameterBlock32) Vଆରମ୍ଭ_କରିବା(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	bpbPtr := memoryManager.Malloc(90)
	...
	direntPtr := memoryManager.Malloc(512)
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vଆରମ୍ଭ_କରିବା(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].ଆକାର >= 0xFFFFFFFF {
			continue
		}
		...
		memoryManager.Free(bpbPtr)
		memoryManager.Free(direntPtr)
		return dirent[i].ଆକାର
	}
	memoryManager.Free(bpbPtr)
	memoryManager.Free(direntPtr)
	return 0
}
func (self *TBiosParameterBlock32) Vପଢ଼ିବା(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].ଆକାର > 512 {
					buf3 = buffer[:512]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, 512)

				} else {
					buf3 = buffer[:dirent[i].ଆକାର]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].ଆକାର))
				}

				copy(data[int32(dirent[i].ଆକାର)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vଆରମ୍ଭ_କରିବା(data [32]byte) {
	...
	self.ଆକାର = Uint32_R(ArrayToUint32(buf))
}
