...
func (self *TPartitionTableEntry) Vଆରମ୍ଭ_କରିବା(data [16]byte) {
...
func (self *TMSDOSPartitionTable) ReadPartitions(hd *TAdvancedTechnologyAttachment) {
	...
	self.MBR.PrimaryPartition[0].Vଆରମ୍ଭ_କରିବା(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[1].Vଆରମ୍ଭ_କରିବା(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[2].Vଆରମ୍ଭ_କରିବା(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[3].Vଆରମ୍ଭ_କରିବା(buf3)
	i += 16
