...
func (self *TCOWFrameManager) Vଆରମ୍ଭ_କରିବା(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
...
func (self *Paging) Vଆରମ୍ଭ_କରିବା(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vଆରମ୍ଭ_କରିବା(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vଆରମ୍ଭ_କରିବା(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, ଆକାର uint32, pageDir uint32) {
	if ଆକାର == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), ଆକାର)

	for i := uint32(0); i < ଆକାର; i++ {
		dst[i] = src[i]
	}
	setCR3(oldCR3)
}

func ZeroBlockInPageDir(addr uint32, ଆକାର uint32, pageDir uint32) {
	if ଆକାର == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, ଆକାର)
	dst := GetBytesFromPtr(uintptr(addr), int(ଆକାର), int(ଆକାର))
	for i := uint32(0); i < ଆକାର; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, ଆକାର uint32) bool {
	if ଆକାର == 0 {
		return true
	}
	last := addr + ଆକାର - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, ଆକାର uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, ଆକାର)
	setCR3(oldCR3)
	return ok
}
