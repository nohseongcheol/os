...
func (self *PhysicalMemoryManager) FirstFreeSize(ଆକାର uint32) uint32 {
	if ଆକାର == 0 {
		return 0xffffffff
	}
	if ଆକାର == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= ଆକାର; count++ {
...
func (self *PhysicalMemoryManager) Vଆରମ୍ଭ_କରିବା(ଆକାର uint32, bitmap []uint32) {
	self.memorySize = ଆକାର
	self.memoryArray = bitmap
	self.maximumBlocks = ଆକାର / BLOCK_SIZE
