...
func (self *TThreadHelper) Vଆରମ୍ଭ_କରିବା(mem *mem.TMemoryManager) {
	self.mem = mem
	콘솔.M출력XY(([]byte)("thread:"), 1, 14)
}
func (self *TThreadHelper) CreateFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) Tନିଷ୍ପାଦନ_ଧାରା {
	result := Tନିଷ୍ପାଦନ_ଧାରା{}

	result.Stack = uint32(uintptr(self.mem.Malloc(THREAD_STACK_SIZE)))
	if result.Stack == 0 {
		return result
	}
	콘솔.M출력(([]byte)("[mem:"))
...
func (self *TThreadHelper) CreatePtrFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) *Tନିଷ୍ପାଦନ_ଧାରା {
	result := (*Tନିଷ୍ପାଦନ_ଧାରା)(self.mem.Malloc(uint32(Sizeof(Tନିଷ୍ପାଦନ_ଧାରା{}))))
	if result == nil {
		return nil
	}
