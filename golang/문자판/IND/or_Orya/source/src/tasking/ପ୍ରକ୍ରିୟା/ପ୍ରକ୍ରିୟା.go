...
func (self *Process) Vଆରମ୍ଭ_କରିବା(mem *mem.TMemoryManager) {
	self.ThreadList = LinkedList{}
	self.Threads = &self.ThreadList
	self.Threads.Vଆରମ୍ଭ_କରିବା(mem)
}
...
func (self *ProcessHelper) Vଆରମ୍ଭ_କରିବା(mem *mem.TMemoryManager, kernelPageDirEntry uintptr) {
	self.mem = mem
	self.processes = LinkedList{}
	self.processes.Vଆରମ୍ଭ_କରିବା(self.mem)
...
func (self *ProcessHelper) Spawn(entryPoint func(), threadHelper *TThreadHelper, scheduler *Scheduler, PageDirEntry uint32, isKernel bool) Process {
	...
	if process.Threads != nil && process.Threads.Vଆକାର > 0 {
		thread := (*Tନିଷ୍ପାଦନ_ଧାରା)(process.Threads.GetAt(0))
