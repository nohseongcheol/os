...
func (self *TUserDatagramProtocolHeader) Vଆରମ୍ଭ_କରିବା(buffer *TUserDatagramProtocolHeaderBuffer) {
...
type IUserDatagramProtocolHandler interface {
	HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, ଆକାର uint16)
}

type TUserDatagramProtocolHandler struct {
}

func (self *TUserDatagramProtocolHandler) Vଆରମ୍ଭ_କରିବା(backend TInternetProtocolProvider) {
}
func (self *TUserDatagramProtocolHandler) HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, ଆକାର uint16) {
}

type IUserDatagramProtocolSocket interface {
	HandleUserDatagramProtocolMessage(data uintptr, ଆକାର uint16)
}
...
func (self *TUserDatagramProtocolSocket) Vଆରମ୍ଭ_କରିବା(p_udpProvider TUserDatagramProtocolProvider, p_udpHandler IUserDatagramProtocolHandler) {
...
func (self *TUserDatagramProtocolSocket) HandleUserDatagramProtocolMessage(data uintptr, ଆକାର uint16) {
	if udpHandler != nil {
		udpHandler.HandleUserDatagramProtocolMessage(self, data, ଆକାର)
	}
}
func (self *TUserDatagramProtocolSocket) Send(p_data []byte, ଆକାର uint16) {
	var buffer [4096]byte
	for i := 0; i < int(ଆକାର); i++ {
		buffer[i] = p_data[i]
	}
	var data = uintptr(Pointer(&buffer))
	udpProvider.Send(self, data, ଆକାର)
}
...
func (self *TUserDatagramProtocolProvider) Vଆରମ୍ଭ_କରିବା(p_ipProvider TInternetProtocolProvider, p_ipHandler IInternetProtocolHandler) {
	ipHandler = p_ipHandler
	ipHandler.Vଆରମ୍ଭ_କରିବା(p_ipProvider, p_ipHandler, 0x11)
	numSockets = 0
	freePort = 1024
}
func (self *TUserDatagramProtocolProvider) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, internetprotocolPayload uintptr, ଆକାର uint32) bool {
	if ଆକାର < udpHeaderSize {
		return false
	}
	...
	if socket != nil {
		socket.HandleUserDatagramProtocolMessage(internetprotocolPayload+uintptr(udpHeaderSize), uint16(ଆକାର-udpHeaderSize))
	}
...
func (self *TUserDatagramProtocolProvider) Send(socket *TUserDatagramProtocolSocket, p_data uintptr, ଆକାର uint16) {
	var totalLength = uint32(ଆକାର) + udpHeaderSize
