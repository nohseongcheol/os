...
func PrintBytes(dataPointer uintptr, ଆକାର int) {
...
func GetUintptrArrayFromPtr(ଠିକଣା_ସୂଚକ_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{ଠିକଣା_ସୂଚକ_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(ଠିକଣା_ସୂଚକ_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(ଠିକଣା_ସୂଚକ_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(ଠିକଣା_ସୂଚକ_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&ଠିକଣା_ସୂଚକ_2))
