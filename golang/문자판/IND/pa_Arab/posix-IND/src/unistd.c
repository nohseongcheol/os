...
ssize_t پڑھنا(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_پڑھنا, fd, buf, count));
}

ssize_t لکھنا(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_لکھنا, fd, buf, count));
}
