...
static void test_open_and_io(void)
{
    ...
    if (fd >= 0) {
        report("read-bytes", پڑھنا(fd, bytes, 2) == 2 &&
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", پڑھنا(fd, &first, 1) == 1 && پڑھنا(copy, &second, 1) == 1 &&
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &status) == child) {
        report("fork-fd-isolation", پڑھنا(fd, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
