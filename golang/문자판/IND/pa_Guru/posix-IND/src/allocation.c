...
void *malloc(size_t ਆਕਾਰ)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!ਆਕਾਰ) ਆਕਾਰ = 1;
    if (ਆਕਾਰ > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    ਆਕਾਰ = (ਆਕਾਰ + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= ਆਕਾਰ) {
            if (cursor->capacity - ਆਕਾਰ >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + ਆਕਾਰ);
                raw->capacity = cursor->capacity - ਆਕਾਰ - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = ਆਕਾਰ;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + ਆਕਾਰ;
    ...
    raw->capacity = ਆਕਾਰ; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t ਆਕਾਰ)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(ਆਕਾਰ);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!ਆਕਾਰ) ਆਕਾਰ = 1;
    cursor = (struct heap_block *)address - 1;
    if (ਆਕਾਰ <= cursor->capacity) return address;
    destination = malloc(ਆਕਾਰ);
