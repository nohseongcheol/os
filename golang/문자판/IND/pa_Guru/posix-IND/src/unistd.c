...
ssize_t ਪੜ੍ਹਨਾ(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ਪੜ੍ਹਨਾ, fd, buf, count));
}

ssize_t ਲਿਖਣਾ(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ਲਿਖਣਾ, fd, buf, count));
}
...
char *getcwd(char *buf, size_t ਆਕਾਰ)
{
    long result = __syscall_result(SC2(SYS_getcwd, buf, ਆਕਾਰ));
