...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, ਆਕਾਰ int) bool
	Send(dataPointer uintptr, ਆਕਾਰ uint32)
}
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, ਆਕਾਰ int) bool {
	넷카드콘솔.M출력XY(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawDataHandler) Send(dataPointer uintptr, ਆਕਾਰ uint32) {
	넷카드콘솔.M출력XY(([]byte)("TRawDataSend"), 10, 10)
	rawDataBackend.Send(dataPointer, ਆਕਾਰ)
}
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Vਆਰੰਭ_ਕਰਨਾ(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, ਆਕਾਰ uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + ਆਕਾਰ - 1

	for i := 0; i < int(ਆਕਾਰ); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(ਆਕਾਰ)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-ਆਕਾਰ)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var ਆਕਾਰ uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if ਆਕਾਰ > 64 {
				ਆਕਾਰ -= 4
			}
			...
			var ਪਤੇ_ਦਾ_ਹਵਾਲਾ uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(ਪਤੇ_ਦਾ_ਹਵਾਲਾ, int(ਆਕਾਰ)) {

					콘솔.M출력XY(([]byte)("self.Send"), 0, 22)

					self.Send(ਪਤੇ_ਦਾ_ਹਵਾਲਾ, ਆਕਾਰ)
				}
			}
