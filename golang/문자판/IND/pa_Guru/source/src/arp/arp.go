...
func (self *TARPMessage) Vਆਰੰਭ_ਕਰਨਾ(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, ਆਕਾਰ int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(ਆਕਾਰ))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, ਆਕਾਰ uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, ਆਕਾਰ)
}
...
func (self *TARPProvider) Vਆਰੰਭ_ਕਰਨਾ(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vਆਰੰਭ_ਕਰਨਾ(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, ਆਕਾਰ uint32) bool {
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var ਪਤੇ_ਦਾ_ਹਵਾਲਾ uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, ਪਤੇ_ਦਾ_ਹਵਾਲਾ, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, ਆਕਾਰ uint32) {
