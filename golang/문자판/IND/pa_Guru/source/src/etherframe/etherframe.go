...
func (self *TEtherFrameHeader) Vਆਰੰਭ_ਕਰਨਾ(buffer TEtherFrameHeaderBuffer) {
...
type IEtherFrameHandler interface {
	Vਆਰੰਭ_ਕਰਨਾ(backend TEtherFrameProvider)
	SetHandler(handler IEtherFrameHandler, etherType uint16)
	OnEtherFrameReceived(dataPointer uintptr, ਆਕਾਰ int) bool
	Send(dstMAC_BE uint64, dataPointer uintptr, ਆਕਾਰ uint32)
	SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, ਆਕਾਰ uint32)
...
func (self *TEtherFrameHandler) Vਆਰੰਭ_ਕਰਨਾ(backend TEtherFrameProvider) {
	Backend = backend
}
...
func (self *TEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, ਆਕਾਰ int) bool {
	이더넷콘솔.M출력(([]byte)("OnEtherFrameReceived"))
	return false
}
func (self *TEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, ਆਕਾਰ uint32) {
	Backend.SendFrame(dstMAC_BE, frame.etherType_BE, dataPointer, ਆਕਾਰ)
}
func (self *TEtherFrameHandler) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, ਆਕਾਰ uint32) {
	Backend.SendFrame(dstMAC_BE, etherType_BE, dataPointer, ਆਕਾਰ)
}
...
func (self *TEtherFrameRawDataHandler) Vਆਰੰਭ_ਕਰਨਾ(p_provider TEtherFrameProvider, p_backend Tamd_am79c973) {
	provider = p_provider
	provider.Vਆਰੰਭ_ਕਰਨਾ(p_backend)
}
func (self *TEtherFrameRawDataHandler) OnRawDataReceived(dataPointer uintptr, ਆਕਾਰ int) bool {
	return provider.OnRawDataReceived(dataPointer, ਆਕਾਰ)
}
func (self *TEtherFrameRawDataHandler) Send(dataPointer uintptr, ਆਕਾਰ uint32) {
	provider.Send(dataPointer, ਆਕਾਰ)
}
...
func (self *TEtherFrameProvider) Vਆਰੰਭ_ਕਰਨਾ(backend Tamd_am79c973) {
...
func (self *TEtherFrameProvider) OnRawDataReceived(dataPointer uintptr, ਆਕਾਰ int) bool {
	...
	if frame.dstMAC_BE == 0x0000FFFFFFFFFFFF || Uint48_R(frame.dstMAC_BE) == self.GetMACAddress() {
		if handlers[frame.etherType_BE] != nil {
			...
			var ਪਤੇ_ਦਾ_ਹਵਾਲਾ uintptr = uintptr(Pointer(dataPointer)) + uintptr(frameHeaderSize)
			sendBack = handlers[frame.etherType_BE].OnEtherFrameReceived(ਪਤੇ_ਦਾ_ਹਵਾਲਾ, ਆਕਾਰ-frameHeaderSize)
...
func (self *TEtherFrameProvider) Send(dataPointer uintptr, ਆਕਾਰ uint32) {
	self.netCard.Send(dataPointer, ਆਕਾਰ)
}
func (self *TEtherFrameProvider) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, ਆਕਾਰ uint32) {
	...
	var ਪਤੇ_ਦਾ_ਹਵਾਲਾ uintptr = uintptr(Pointer(&buffer2))

	self.netCard.Send(ਪਤੇ_ਦਾ_ਹਵਾਲਾ, ਆਕਾਰ+uint32(frameHeaderSize))
