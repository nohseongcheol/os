...
func (self *TBiosParameterBlock32) Vਆਰੰਭ_ਕਰਨਾ(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vਆਰੰਭ_ਕਰਨਾ(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].ਆਕਾਰ >= 0xFFFFFFFF {
			continue
		}
		...
		return dirent[i].ਆਕਾਰ
	}
	memoryManager.Free(bpbPtr)
	memoryManager.Free(direntPtr)
	return 0
}
func (self *TBiosParameterBlock32) Vਪੜ੍ਹਨਾ(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].ਆਕਾਰ > 512 {
					buf3 = buffer[:512]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, 512)

				} else {
					buf3 = buffer[:dirent[i].ਆਕਾਰ]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].ਆਕਾਰ))
				}

				copy(data[int32(dirent[i].ਆਕਾਰ)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vਆਰੰਭ_ਕਰਨਾ(data [32]byte) {
	...
	self.ਆਕਾਰ = Uint32_R(ArrayToUint32(buf))
}
