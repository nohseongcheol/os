...
func (self *TPartitionTableEntry) Vਆਰੰਭ_ਕਰਨਾ(data [16]byte) {
...
func (self *TMSDOSPartitionTable) ReadPartitions(hd *TAdvancedTechnologyAttachment) {
	...
	self.MBR.PrimaryPartition[0].Vਆਰੰਭ_ਕਰਨਾ(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[1].Vਆਰੰਭ_ਕਰਨਾ(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[2].Vਆਰੰਭ_ਕਰਨਾ(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[3].Vਆਰੰਭ_ਕਰਨਾ(buf3)
	i += 16
