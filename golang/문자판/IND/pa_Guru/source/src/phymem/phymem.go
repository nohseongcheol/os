...
func (self *PhysicalMemoryManager) FirstFreeSize(ਆਕਾਰ uint32) uint32 {
	if ਆਕਾਰ == 0 {
		return 0xffffffff
	}
	if ਆਕਾਰ == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= ਆਕਾਰ; count++ {
...
func (self *PhysicalMemoryManager) Vਆਰੰਭ_ਕਰਨਾ(ਆਕਾਰ uint32, bitmap []uint32) {
	self.memorySize = ਆਕਾਰ
	self.memoryArray = bitmap
	self.maximumBlocks = ਆਕਾਰ / BLOCK_SIZE
