...
type SchedulerData struct {
	...
	currentThread	*Tਕਾਰਜਧਾਰਾ
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vਆਰੰਭ_ਕਰਨਾ() {
...
func (self *SchedulerData) GetNextReadyThread() *Tਕਾਰਜਧਾਰਾ {
	if list.Vਆਕਾਰ <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vਆਕਾਰ; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vਆਕਾਰ {
			currentThreadIndex = 0
		}
		thread := (*Tਕਾਰਜਧਾਰਾ)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tਕਾਰਜਧਾਰਾ) {
	if thread == nil {
		return
	}
	list.PushBack(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tਕਾਰਜਧਾਰਾ) {
	if thread == nil {
		return
	}
	list.PushBack(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tਕਾਰਜਧਾਰਾ) {
...
func (self *Scheduler) Vਆਰੰਭ_ਕਰਨਾ(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vਆਰੰਭ_ਕਰਨਾ()
	sche_data.tss = tss
	...
	self.TInterruptHandler.Vਆਰੰਭ_ਕਰਨਾ(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vਆਕਾਰ > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vਆਕਾਰ; i++ {
		thread := (*Tਕਾਰਜਧਾਰਾ)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tਕਾਰਜਧਾਰਾ) {
