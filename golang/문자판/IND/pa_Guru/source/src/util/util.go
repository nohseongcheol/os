...
func PrintBytes(dataPointer uintptr, ਆਕਾਰ int) {
...
func GetUintptrArrayFromPtr(ਪਤੇ_ਦਾ_ਹਵਾਲਾ_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{ਪਤੇ_ਦਾ_ਹਵਾਲਾ_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(ਪਤੇ_ਦਾ_ਹਵਾਲਾ_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(ਪਤੇ_ਦਾ_ਹਵਾਲਾ_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(ਪਤੇ_ਦਾ_ਹਵਾਲਾ_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&ਪਤੇ_ਦਾ_ਹਵਾਲਾ_2))
