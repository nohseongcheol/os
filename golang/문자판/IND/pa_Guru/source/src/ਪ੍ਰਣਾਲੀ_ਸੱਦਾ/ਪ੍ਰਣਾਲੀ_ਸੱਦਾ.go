...
func (self *TSyscall) Vਆਰੰਭ_ਕਰਨਾ(manager *TInterruptManager) {
	initFileDescriptors()
	...
	self.TInterruptHandler.Vਆਰੰਭ_ਕਰਨਾ(0x80, uintptr(Pointer(manager)), addr)
}
...
func sysRead(fd int32, addr uint32, count uint32) int32 {
	...
	if entry.pos >= entry.ਆਕਾਰ {
		return 0
	}
	remaining := entry.ਆਕਾਰ - entry.pos
	if count > remaining {
		count = remaining
	}
	buf := GetBytesFromPtr(uintptr(addr), int(count), int(count))
...
func readVFSFile(entry *openFileDescription, dst []byte, count uint32) int32 {
	...
	tmpPtr := memoryManager.Malloc(entry.ਆਕਾਰ)
	if tmpPtr == nil {
		return EINVAL
	}
	tmp := GetBytesFromPtr(uintptr(tmpPtr), int(entry.ਆਕਾਰ), int(entry.ਆਕਾਰ))
...
func sysGetcwd(bufferAddr uint32, ਆਕਾਰ uint32) int32 {
	if bufferAddr == 0 {
		return EFAULT
	}
	if ਆਕਾਰ < 2 {
	...
	buffer := GetBytesFromPtr(uintptr(bufferAddr), int(ਆਕਾਰ), int(ਆਕਾਰ))
	buffer[0] = '/'
	buffer[1] = 0
	return 2
}

func fillPosixStat(statAddr uint32, mode uint32, ਆਕਾਰ uint32, inode uint32) int32 {
	if statAddr == 0 {
		return EFAULT
	}
	...
	*stat = posixStat{}
	...
	stat.Mode = mode
	stat.Nlink = 1
	stat.Vਆਕਾਰ = int32(ਆਕਾਰ)
	stat.Blksize = 512
	stat.Blocks = int32((ਆਕਾਰ + 511) / 512)
	return 0
}
...
func sysStat(pathAddr uint32, statAddr uint32) int32 {
	...
	return fillPosixStat(statAddr, sIfreg|0444, ਆਕਾਰ, inode)
}
...
func sysFstat(fd int32, statAddr uint32) int32 {
	...
	case fdKindRootDir:
		return fillPosixStat(statAddr, sIfdir|0555, 0, 1)
	case fdKindFat:
		return fillPosixStat(statAddr, sIfreg|0444, entry.ਆਕਾਰ, uint32(fd+2))
	case fdKindSocket:
...
func socketSendTo(fd int32, bufferAddress uint32, length uint32, destinationAddress uint32, destinationLength uint32) int32 {
	...
	*packet = socketPacket{used: true, ਆਕਾਰ: length, source: socket.local}
	if length != 0 {
...
func sysExecve(cpu *TCPUState, pathAddr uint32) int32 {
	...
	memoryManager := &mem.TMemoryManager{}
	filePtr := memoryManager.Malloc(ਆਕਾਰ)
	if filePtr == nil {
		return EINVAL
	}
	data := GetBytesFromPtr(uintptr(filePtr), int(ਆਕਾਰ), int(ਆਕਾਰ))
	readFile(name[:nameLen], data)
	if ਆਕਾਰ < 52 || data[0] != 0x7F || data[1] != 'E' || data[2] != 'L' || data[3] != 'F' {
...
func sysFork(cpu *TCPUState) int32 {
	...
	memoryManager := &mem.TMemoryManager{}
	threadPtr := memoryManager.Malloc(uint32(Sizeof(Tਕਾਰਜਧਾਰਾ{})))
...
func fileSize(filename []byte) uint32 {
	...
	ਆਕਾਰ := bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()
	return ਆਕਾਰ
}
