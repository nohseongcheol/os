...
func GetFileSize(filename []byte) uint32 {
	...
	var ਆਕਾਰ uint32 = bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()
...
func ReadFile(filename []byte, data []byte) {
	...
	bios := TBiosParameterBlock32{}
	bios.Vਪੜ੍ਹਨਾ(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vਆਰੰਭ_ਕਰਨਾ(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vਆਰੰਭ_ਕਰਨਾ(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vਆਰੰਭ_ਕਰਨਾ(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vਆਰੰਭ_ਕਰਨਾ()

	threadHelper := &TThreadHelper{}
	threadHelper.Vਆਰੰਭ_ਕਰਨਾ(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vਆਰੰਭ_ਕਰਨਾ(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vਆਰੰਭ_ਕਰਨਾ(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vਆਰੰਭ_ਕਰਨਾ(InterruptManager)
	...
	ਆਕਾਰ = GetFileSize(linkerFile)
	linkerAddr := memoryManager.Malloc(ਆਕਾਰ)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(ਆਕਾਰ), int(ਆਕਾਰ))
	...
	linkMap := LinkMap{}
	linkMap.Vਆਰੰਭ_ਕਰਨਾ(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	ਆਕਾਰ = GetFileSize(lib1File)

	lib1Addr := memoryManager.Malloc(ਆਕਾਰ)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(ਆਕਾਰ), int(ਆਕਾਰ))
	...
	ਆਕਾਰ = GetFileSize(lib2File)

	lib2Addr := memoryManager.Malloc(ਆਕਾਰ)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(ਆਕਾਰ), int(ਆਕਾਰ))
	...
	ਆਕਾਰ = GetFileSize(user1file)
	user1addr := memoryManager.Malloc(ਆਕਾਰ)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(ਆਕਾਰ), int(ਆਕਾਰ))
	...
	*code1Ptr = uintptr(linkerEntry)
	...
	thr2 := (*Tਕਾਰਜਧਾਰਾ)(proc2.Threads.GetAt(0))
	...
	ਆਕਾਰ = GetFileSize(user2file)
	user2addr := memoryManager.Malloc(ਆਕਾਰ)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(ਆਕਾਰ), int(ਆਕਾਰ))
	...
	*code2Ptr = uintptr(linkerEntry)
	...
	thr3 := (*Tਕਾਰਜਧਾਰਾ)(proc3.Threads.GetAt(0))
	...
	ਆਕਾਰ = GetFileSize(user3file)
	user3addr := memoryManager.Malloc(ਆਕਾਰ)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(ਆਕਾਰ), int(ਆਕਾਰ))
	...
	*code3Ptr = uintptr(linkerEntry)
	...
	thr4 := (*Tਕਾਰਜਧਾਰਾ)(proc4.Threads.GetAt(0))
	...
	pciController.Vਆਰੰਭ_ਕਰਨਾ(myPCIControllerHandler)
