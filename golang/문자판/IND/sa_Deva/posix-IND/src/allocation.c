...
void *malloc(size_t आकारः)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!आकारः) आकारः = 1;
    if (आकारः > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    आकारः = (आकारः + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= आकारः) {
            if (cursor->capacity - आकारः >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + आकारः);
                raw->capacity = cursor->capacity - आकारः - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = आकारः;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + आकारः;
    ...
    raw->capacity = आकारः; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t आकारः)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(आकारः);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!आकारः) आकारः = 1;
    cursor = (struct heap_block *)address - 1;
    if (आकारः <= cursor->capacity) return address;
    destination = malloc(आकारः);
