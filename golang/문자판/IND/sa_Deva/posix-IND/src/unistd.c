...
ssize_t पठनम्(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_पठनम्, fd, buf, count));
}

ssize_t लेखनम्(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_लेखनम्, fd, buf, count));
}
...
char *getcwd(char *buf, size_t आकारः)
{
    long result = __syscall_result(SC2(SYS_getcwd, buf, आकारः));
