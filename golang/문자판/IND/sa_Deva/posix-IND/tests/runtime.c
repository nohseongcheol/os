...
{
    (void)लेखनम्(STDOUT_FILENO, text, text_length(text));
}
...
static void test_open_and_io(void)
{
    ...
    if (fd >= 0) {
        report("read-bytes", पठनम्(fd, bytes, 2) == 2 &&
    ...
    if (root >= 0) {
        ...
        errno = 0;
        report("read-directory", पठनम्(root, bytes, 1) == -1 && errno == EISDIR);
        (void)close(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", लेखनम्(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", पठनम्(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", पठनम्(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", लेखनम्(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", पठनम्(fd, &first, 1) == 1 && पठनम्(copy, &second, 1) == 1 &&
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &status) == child) {
        report("fork-fd-isolation", पठनम्(fd, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
