...
func (self *TARPMessage) Vप्रारम्भः(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, आकारः int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(आकारः))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, आकारः uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, आकारः)
}
...
func (self *TARPProvider) Vप्रारम्भः(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vप्रारम्भः(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, आकारः uint32) bool {

	if आकारः < arpMesgSize {
		return false
	}
	var arpBuffer *TARPMessageBuffer = (*TARPMessageBuffer)(Pointer(dataPointer))
	var arp TARPMessage = TARPMessage{}
	arp.Vप्रारम्भः(arpBuffer)
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var सङ्केतसन्दर्भः uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, सङ्केतसन्दर्भः, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, आकारः uint32) {
