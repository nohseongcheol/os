...
func (self *TBiosParameterBlock32) Vप्रारम्भः(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vप्रारम्भः(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].आकारः >= 0xFFFFFFFF {
			continue
		}
		...
		return dirent[i].आकारः
	}
	memoryManager.Free(bpbPtr)
	memoryManager.Free(direntPtr)
	return 0
}
func (self *TBiosParameterBlock32) Vपठनम्(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].आकारः > 512 {
					buf3 = buffer[:512]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, 512)

				} else {
					buf3 = buffer[:dirent[i].आकारः]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].आकारः))
				}

				copy(data[int32(dirent[i].आकारः)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vप्रारम्भः(data [32]byte) {
	...
	self.आकारः = Uint32_R(ArrayToUint32(buf))
}
