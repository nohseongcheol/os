...
func (self *TCOWFrameManager) Vप्रारम्भः(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
...
func (self *Paging) Vप्रारम्भः(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vप्रारम्भः(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vप्रारम्भः(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, आकारः uint32, pageDir uint32) {
	if आकारः == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), आकारः)

	for i := uint32(0); i < आकारः; i++ {
		dst[i] = src[i]
	}
	setCR3(oldCR3)
}

func ZeroBlockInPageDir(addr uint32, आकारः uint32, pageDir uint32) {
	if आकारः == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, आकारः)
	dst := GetBytesFromPtr(uintptr(addr), int(आकारः), int(आकारः))
	for i := uint32(0); i < आकारः; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, आकारः uint32) bool {
	if आकारः == 0 {
		return true
	}
	last := addr + आकारः - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, आकारः uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, आकारः)
	setCR3(oldCR3)
	return ok
}
