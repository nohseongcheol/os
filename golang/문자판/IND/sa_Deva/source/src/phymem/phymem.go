...
func (self *PhysicalMemoryManager) FirstFreeSize(आकारः uint32) uint32 {
	if आकारः == 0 {
		return 0xffffffff
	}
	if आकारः == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= आकारः; count++ {
...
func (self *PhysicalMemoryManager) Vप्रारम्भः(आकारः uint32, bitmap []uint32) {
	self.memorySize = आकारः
	self.memoryArray = bitmap
	self.maximumBlocks = आकारः / BLOCK_SIZE
