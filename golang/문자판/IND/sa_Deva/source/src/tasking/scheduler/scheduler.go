...
type SchedulerData struct {
	...
	currentThread	*Tनिष्पादनधारा
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vप्रारम्भः() {
...
func (self *SchedulerData) GetNextReadyThread() *Tनिष्पादनधारा {
	if list.Vआकारः <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vआकारः; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vआकारः {
			currentThreadIndex = 0
		}
		thread := (*Tनिष्पादनधारा)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tनिष्पादनधारा) {
	if thread == nil {
		return
	}
	list.PushBack(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tनिष्पादनधारा) {
	if thread == nil {
		return
	}
	list.PushBack(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tनिष्पादनधारा) {
...
func (self *Scheduler) Vप्रारम्भः(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vप्रारम्भः()
	sche_data.tss = tss
	...
	self.TInterruptHandler.Vप्रारम्भः(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vआकारः > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vआकारः; i++ {
		thread := (*Tनिष्पादनधारा)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tनिष्पादनधारा) {
