...
func (self *Process) Vप्रारम्भः(mem *mem.TMemoryManager) {
	self.ThreadList = LinkedList{}
	self.Threads = &self.ThreadList
	self.Threads.Vप्रारम्भः(mem)
}
...
func (self *ProcessHelper) Vप्रारम्भः(mem *mem.TMemoryManager, kernelPageDirEntry uintptr) {
	self.mem = mem
	self.processes = LinkedList{}
	self.processes.Vप्रारम्भः(self.mem)
...
func (self *ProcessHelper) Spawn(entryPoint func(), threadHelper *TThreadHelper, scheduler *Scheduler, PageDirEntry uint32, isKernel bool) Process {
	...
	if process.Threads != nil && process.Threads.Vआकारः > 0 {
		thread := (*Tनिष्पादनधारा)(process.Threads.GetAt(0))
