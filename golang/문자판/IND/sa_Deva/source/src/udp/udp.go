...
func (self *TUserDatagramProtocolHeader) Vप्रारम्भः(buffer *TUserDatagramProtocolHeaderBuffer) {
...
type IUserDatagramProtocolHandler interface {
	HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, आकारः uint16)
}

type TUserDatagramProtocolHandler struct {
}

func (self *TUserDatagramProtocolHandler) Vप्रारम्भः(backend TInternetProtocolProvider) {
}
func (self *TUserDatagramProtocolHandler) HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, आकारः uint16) {
}

type IUserDatagramProtocolSocket interface {
	HandleUserDatagramProtocolMessage(data uintptr, आकारः uint16)
}
...
func (self *TUserDatagramProtocolSocket) Vप्रारम्भः(p_udpProvider TUserDatagramProtocolProvider, p_udpHandler IUserDatagramProtocolHandler) {
...
func (self *TUserDatagramProtocolSocket) HandleUserDatagramProtocolMessage(data uintptr, आकारः uint16) {
	if udpHandler != nil {
		udpHandler.HandleUserDatagramProtocolMessage(self, data, आकारः)
	}
}
func (self *TUserDatagramProtocolSocket) Send(p_data []byte, आकारः uint16) {
	var buffer [4096]byte
	for i := 0; i < int(आकारः); i++ {
		buffer[i] = p_data[i]
	}
	var data = uintptr(Pointer(&buffer))
	udpProvider.Send(self, data, आकारः)
}
...
func (self *TUserDatagramProtocolProvider) Vप्रारम्भः(p_ipProvider TInternetProtocolProvider, p_ipHandler IInternetProtocolHandler) {
	ipHandler = p_ipHandler
	ipHandler.Vप्रारम्भः(p_ipProvider, p_ipHandler, 0x11)
	numSockets = 0
	freePort = 1024
}
func (self *TUserDatagramProtocolProvider) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, internetprotocolPayload uintptr, आकारः uint32) bool {
	if आकारः < udpHeaderSize {
		return false
	}
	...
	if socket != nil {
		socket.HandleUserDatagramProtocolMessage(internetprotocolPayload+uintptr(udpHeaderSize), uint16(आकारः-udpHeaderSize))
	}
...
func (self *TUserDatagramProtocolProvider) Send(socket *TUserDatagramProtocolSocket, p_data uintptr, आकारः uint16) {
	var totalLength = uint32(आकारः) + udpHeaderSize
