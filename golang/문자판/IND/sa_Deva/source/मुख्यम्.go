...
func GetFileSize(filename []byte) uint32 {
	...
	var आकारः uint32 = bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()
...
func ReadFile(filename []byte, data []byte) {
	...
	bios := TBiosParameterBlock32{}
	bios.Vपठनम्(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func LoadElf() {
	...
	elf.Parse(data[:आकारः], 0x4f00000)
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vप्रारम्भः(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vप्रारम्भः(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vप्रारम्भः(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vप्रारम्भः()

	threadHelper := &TThreadHelper{}
	threadHelper.Vप्रारम्भः(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vप्रारम्भः(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vप्रारम्भः(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vप्रारम्भः(InterruptManager)
	...
	आकारः = GetFileSize(linkerFile)
	linkerAddr := memoryManager.Malloc(आकारः)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(आकारः), int(आकारः))
	...
	linkMap := LinkMap{}
	linkMap.Vप्रारम्भः(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	आकारः = GetFileSize(lib1File)

	lib1Addr := memoryManager.Malloc(आकारः)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(आकारः), int(आकारः))
	...
	आकारः = GetFileSize(lib2File)

	lib2Addr := memoryManager.Malloc(आकारः)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(आकारः), int(आकारः))
	...
	आकारः = GetFileSize(user1file)
	user1addr := memoryManager.Malloc(आकारः)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(आकारः), int(आकारः))
	...
	*code1Ptr = uintptr(linkerEntry)
	...
	thr2 := (*Tनिष्पादनधारा)(proc2.Threads.GetAt(0))
	...
	आकारः = GetFileSize(user2file)
	user2addr := memoryManager.Malloc(आकारः)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(आकारः), int(आकारः))
	...
	*code2Ptr = uintptr(linkerEntry)
	...
	thr3 := (*Tनिष्पादनधारा)(proc3.Threads.GetAt(0))
	...
	आकारः = GetFileSize(user3file)
	user3addr := memoryManager.Malloc(आकारः)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(आकारः), int(आकारः))
	...
	*code3Ptr = uintptr(linkerEntry)
	...
	thr4 := (*Tनिष्पादनधारा)(proc4.Threads.GetAt(0))
	...
	pciController.Vप्रारम्भः(myPCIControllerHandler)
