...
func ReadFile(filename []byte, data []byte) {
	...
	bios := TBiosParameterBlock32{}
	bios.Vপাড়হাও(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Init(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Init(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Init(0x20, 공용서술자테이블, 작업관리자)
	...
	processHelper.Init(memoryManager, PageDirEntry)
	...
	linkerAddr := memoryManager.Malloc(size)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(size), int(size))
	...
	lib1Addr := memoryManager.Malloc(size)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(size), int(size))
	...
	lib2Addr := memoryManager.Malloc(size)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(size), int(size))
	...
	user1addr := memoryManager.Malloc(size)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(size), int(size))
	...
	*code1Ptr = uintptr(linkerEntry)
	...
	user2addr := memoryManager.Malloc(size)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(size), int(size))
	...
	*code2Ptr = uintptr(linkerEntry)
	...
	user3addr := memoryManager.Malloc(size)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(size), int(size))
