...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vঅল(0xA0)
	} else {
		self.devicePort.Vঅল(0xB0)
	}
	self.controlPort.Vঅল(0)
	self.devicePort.Vঅল(0xA0)

	var status uint8 = self.commandPort.Vপাড়হাও()
	if status == 0xFF {
	...
	self.sectorCountPort.Vঅল(0)
	self.lbaLowPort.Vঅল(0)
	self.lbaMidPort.Vঅল(0)
	self.lbaHiPort.Vঅল(0)
	self.commandPort.Vঅল(0xEC)

	status = self.commandPort.Vপাড়হাও()
	if status == 0x00 {
	...
	for i := 0; i < 256; i++ {
		var data = self.dataPort.Vপাড়হাও()
		text := []byte("  ")
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vঅল(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vঅল(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vঅল(0)
	self.sectorCountPort.Vঅল(1)

	self.lbaLowPort.Vঅল(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vঅল(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vঅল(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vঅল(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vপাড়হাও()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vঅল(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vঅল(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vঅল(0)
	self.sectorCountPort.Vঅল(1)
	self.lbaLowPort.Vঅল(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vঅল(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vঅল(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vঅল(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vঅল(0xE0)
	} else {
		self.devicePort.Vঅল(0xF0)
	}
	self.commandPort.Vঅল(0xE7)
