...
func (self *Paging) Init(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Init(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, size uint32, pageDir uint32) {
	if size == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), size)
...
func ZeroBlockInPageDir(addr uint32, size uint32, pageDir uint32) {
	if size == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, size)
	dst := GetBytesFromPtr(uintptr(addr), int(size), int(size))
	for i := uint32(0); i < size; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, size uint32) bool {
	if size == 0 {
		return true
	}
	last := addr + size - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, size uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, size)
	setCR3(oldCR3)
	return ok
}
