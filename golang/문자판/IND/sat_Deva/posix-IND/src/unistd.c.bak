...
ssize_t पाड़हाव(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_पाड़हाव, fd, buf, count));
}

ssize_t ओल(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ओल, fd, buf, count));
}
