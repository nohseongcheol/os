...
{
    (void)ओल(STDOUT_FILENO, text, text_length(text));
}
...
static void test_open_and_io(void)
{
    ...
    if (fd >= 0) {
        report("read-bytes", पाड़हाव(fd, bytes, 2) == 2 &&
    ...
    if (root >= 0) {
        ...
        errno = 0;
        report("read-directory", पाड़हाव(root, bytes, 1) == -1 && errno == EISDIR);
        (void)close(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", ओल(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", पाड़हाव(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", पाड़हाव(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", ओल(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
...
static void test_dup_and_fcntl(void)
{
    ...
    if (copy >= 0) {
        report("dup-shared-offset", पाड़हाव(fd, &first, 1) == 1 && पाड़हाव(copy, &second, 1) == 1 &&
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &status) == child) {
        report("fork-fd-isolation", पाड़हाव(fd, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
