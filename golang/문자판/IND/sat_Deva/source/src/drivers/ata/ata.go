...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vओल(0xA0)
	} else {
		self.devicePort.Vओल(0xB0)
	}
	self.controlPort.Vओल(0)
	self.devicePort.Vओल(0xA0)

	var status uint8 = self.commandPort.Vपाड़हाव()
	if status == 0xFF {
	...
	self.sectorCountPort.Vओल(0)
	self.lbaLowPort.Vओल(0)
	self.lbaMidPort.Vओल(0)
	self.lbaHiPort.Vओल(0)
	self.commandPort.Vओल(0xEC)

	status = self.commandPort.Vपाड़हाव()
	if status == 0x00 {
	...
	for i := 0; i < 256; i++ {
		var data = self.dataPort.Vपाड़हाव()
		text := []byte("  ")
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vओल(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vओल(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vओल(0)
	self.sectorCountPort.Vओल(1)

	self.lbaLowPort.Vओल(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vओल(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vओल(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vओल(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vपाड़हाव()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vओल(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vओल(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vओल(0)
	self.sectorCountPort.Vओल(1)
	self.lbaLowPort.Vओल(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vओल(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vओल(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vओल(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vओल(0xE0)
	} else {
		self.devicePort.Vओल(0xF0)
	}
	self.commandPort.Vओल(0xE7)
