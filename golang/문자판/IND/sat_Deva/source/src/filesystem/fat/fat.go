...
func (self *TBiosParameterBlock32) Vपाड़हाव(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].size > 512 {
					...
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].size))
				}
