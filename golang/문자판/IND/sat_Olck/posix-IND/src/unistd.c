...
ssize_t ᱯᱟᱲᱦᱟᱣ(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ᱯᱟᱲᱦᱟᱣ, fd, buf, count));
}

ssize_t ᱚᱞ(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_ᱚᱞ, fd, buf, count));
}
