...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vᱚᱞ(0xA0)
	} else {
		self.devicePort.Vᱚᱞ(0xB0)
	}
	self.controlPort.Vᱚᱞ(0)
	self.devicePort.Vᱚᱞ(0xA0)

	var status uint8 = self.commandPort.Vᱯᱟᱲᱦᱟᱣ()
	if status == 0xFF {
	...
	self.sectorCountPort.Vᱚᱞ(0)
	self.lbaLowPort.Vᱚᱞ(0)
	self.lbaMidPort.Vᱚᱞ(0)
	self.lbaHiPort.Vᱚᱞ(0)
	self.commandPort.Vᱚᱞ(0xEC)

	status = self.commandPort.Vᱯᱟᱲᱦᱟᱣ()
	if status == 0x00 {
	...
	for i := 0; i < 256; i++ {
		var data = self.dataPort.Vᱯᱟᱲᱦᱟᱣ()
		text := []byte("  ")
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vᱚᱞ(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vᱚᱞ(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vᱚᱞ(0)
	self.sectorCountPort.Vᱚᱞ(1)

	self.lbaLowPort.Vᱚᱞ(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vᱚᱞ(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vᱚᱞ(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vᱚᱞ(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vᱯᱟᱲᱦᱟᱣ()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vᱚᱞ(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vᱚᱞ(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vᱚᱞ(0)
	self.sectorCountPort.Vᱚᱞ(1)
	self.lbaLowPort.Vᱚᱞ(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vᱚᱞ(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vᱚᱞ(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vᱚᱞ(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vᱚᱞ(0xE0)
	} else {
		self.devicePort.Vᱚᱞ(0xF0)
	}
	self.commandPort.Vᱚᱞ(0xE7)
