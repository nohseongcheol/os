...
ssize_t پڙهڻ(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_پڙهڻ, fd, buf, count));
}

ssize_t لکڻ(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_لکڻ, fd, buf, count));
}
