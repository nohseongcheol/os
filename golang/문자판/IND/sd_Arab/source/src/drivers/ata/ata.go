...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vلکڻ(0xA0)
	} else {
		self.devicePort.Vلکڻ(0xB0)
	}
	self.controlPort.Vلکڻ(0)
	self.devicePort.Vلکڻ(0xA0)

	var status uint8 = self.commandPort.Vپڙهڻ()
	if status == 0xFF {
	...
	self.sectorCountPort.Vلکڻ(0)
	self.lbaLowPort.Vلکڻ(0)
	self.lbaMidPort.Vلکڻ(0)
	self.lbaHiPort.Vلکڻ(0)
	self.commandPort.Vلکڻ(0xEC)

	status = self.commandPort.Vپڙهڻ()
	if status == 0x00 {
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vلکڻ(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vلکڻ(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vلکڻ(0)
	self.sectorCountPort.Vلکڻ(1)

	self.lbaLowPort.Vلکڻ(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vلکڻ(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vلکڻ(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vلکڻ(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vپڙهڻ()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vلکڻ(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vلکڻ(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vلکڻ(0)
	self.sectorCountPort.Vلکڻ(1)
	self.lbaLowPort.Vلکڻ(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vلکڻ(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vلکڻ(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vلکڻ(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vلکڻ(0xE0)
	} else {
		self.devicePort.Vلکڻ(0xF0)
	}
	self.commandPort.Vلکڻ(0xE7)
