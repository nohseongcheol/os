...
ssize_t पढ़णु(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_पढ़णु, fd, buf, count));
}

ssize_t लिखणु(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_लिखणु, fd, buf, count));
}
