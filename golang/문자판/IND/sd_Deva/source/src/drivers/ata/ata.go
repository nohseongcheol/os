...
func (self *TAdvancedTechnologyAttachment) Identify() {
	...
	if self.master {
		self.devicePort.Vलिखणु(0xA0)
	} else {
		self.devicePort.Vलिखणु(0xB0)
	}
	self.controlPort.Vलिखणु(0)
	self.devicePort.Vलिखणु(0xA0)

	var status uint8 = self.commandPort.Vपढ़णु()
	if status == 0xFF {
	...
	self.sectorCountPort.Vलिखणु(0)
	self.lbaLowPort.Vलिखणु(0)
	self.lbaMidPort.Vलिखणु(0)
	self.lbaHiPort.Vलिखणु(0)
	self.commandPort.Vलिखणु(0xEC)

	status = self.commandPort.Vपढ़णु()
	if status == 0x00 {
...
func (self *TAdvancedTechnologyAttachment) Read28(sector uint32, data *[]byte, count int) {
	...
	if self.master {
		self.devicePort.Vलिखणु(uint8(0xE0 | uint8((sector&0x0F000000)>>24)))
	} else {
		self.devicePort.Vलिखणु(uint8(0xF0 | uint8((sector&0x0F000000)>>24)))
	}
	self.errorPort.Vलिखणु(0)
	self.sectorCountPort.Vलिखणु(1)

	self.lbaLowPort.Vलिखणु(uint8(sector & 0x000000FF))
	self.lbaMidPort.Vलिखणु(uint8((sector & 0x0000FF00) >> 8))
	self.lbaHiPort.Vलिखणु(uint8((sector & 0x00FF0000) >> 16))
	self.commandPort.Vलिखणु(0x20)
	...
	var i int = 0
	for ; i < count; i += 2 {
		var wdata uint16 = self.dataPort.Vपढ़णु()
...
func (self *TAdvancedTechnologyAttachment) Write28(sectorNum uint32, data []byte, count uint32) {
	...
	if self.master {
		self.devicePort.Vलिखणु(uint8(0xE0 | uint8((sectorNum&0x0F000000)>>24)))
	} else {
		self.devicePort.Vलिखणु(uint8(0xF0 | uint8((sectorNum&0x0F000000)>>24)))
	}

	self.errorPort.Vलिखणु(0)
	self.sectorCountPort.Vलिखणु(1)
	self.lbaLowPort.Vलिखणु(uint8(sectorNum & 0x000000FF))
	self.lbaMidPort.Vलिखणु(uint8((sectorNum & 0x0000FF00) >> 8))
	self.lbaHiPort.Vलिखणु(uint8((sectorNum & 0x00FF0000) >> 16))
	self.commandPort.Vलिखणु(0x30)
...
func (self *TAdvancedTechnologyAttachment) Flush() {
	if self.master {
		self.devicePort.Vलिखणु(0xE0)
	} else {
		self.devicePort.Vलिखणु(0xF0)
	}
	self.commandPort.Vलिखणु(0xE7)
