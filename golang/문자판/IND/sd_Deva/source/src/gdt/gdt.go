...
func (자신 *T조각설명자) Init(기준 uint32, 한도 uint32, 유형 uint8, 표시들 uint8) {
...
func (자신 *T공용서술자테이블) Init() {
	...
	자료.조각설명자[4].Init(0, 64*1024*1024, 0xFA, 0xCF)
	자료.조각설명자[5].Init(0, 64*1024*1024, 0xF2, 0xCF)
	자료.조각설명자[6].Init(0x61f004, 0xffffff, 0xF2, 0x4F)
...
func (자신 *T공용서술자테이블) SetDescriptor(idx int, 기준 uint32, 한도 uint32, 유형 uint8, 표시들 uint8) {
	자료.조각설명자[idx].Init(기준, 한도, 유형, 표시들)
}
