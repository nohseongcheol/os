...
int abs(int மதிப்பு) { return மதிப்பு < 0 ? -மதிப்பு : மதிப்பு; }
long labs(long மதிப்பு) { return மதிப்பு < 0 ? -மதிப்பு : மதிப்பு; }
...
char *getenv(const char *அமைப்பின்_அடையாளம்)
{
    size_t நீளம் = strlen(அமைப்பின்_அடையாளம்);
    char **cursor = environ;
    if (!நீளம் || strchr(அமைப்பின்_அடையாளம், '=') || !cursor) return NULL;
    while (*cursor) {
        if (!strncmp(*cursor, அமைப்பின்_அடையாளம், நீளம்) && (*cursor)[நீளம்] == '=') return *cursor + நீளம் + 1;
        ++cursor;
    }
    return NULL;
}
