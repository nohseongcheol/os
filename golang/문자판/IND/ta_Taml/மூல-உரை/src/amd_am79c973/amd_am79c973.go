...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, அளவு int) bool
	Send(dataPointer uintptr, அளவு uint32)
}
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, அளவு int) bool {
	넷카드콘솔.M출력XY(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawDataHandler) Send(dataPointer uintptr, அளவு uint32) {
	넷카드콘솔.M출력XY(([]byte)("TRawDataSend"), 10, 10)
	rawDataBackend.Send(dataPointer, அளவு)
}
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Vதொடங்கு(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, அளவு uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + அளவு - 1

	for i := 0; i < int(அளவு); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(அளவு)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-அளவு)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var அளவு uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if அளவு > 64 {
				அளவு -= 4
			}
			...
			var முகவரிச்_சுட்டி uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(முகவரிச்_சுட்டி, int(அளவு)) {

					콘솔.M출력XY(([]byte)("self.Send"), 0, 22)

					self.Send(முகவரிச்_சுட்டி, அளவு)
				}
			}
