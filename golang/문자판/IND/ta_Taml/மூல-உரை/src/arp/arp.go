...
func (self *TARPMessage) Vதொடங்கு(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, அளவு int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(அளவு))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, அளவு uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, அளவு)
}
...
func (self *TARPProvider) Vதொடங்கு(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vதொடங்கு(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, அளவு uint32) bool {
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var முகவரிச்_சுட்டி uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, முகவரிச்_சுட்டி, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, அளவு uint32) {
