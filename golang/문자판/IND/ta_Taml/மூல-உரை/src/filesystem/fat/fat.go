...
func (self *TBiosParameterBlock32) Vதொடங்கு(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	bpbPtr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(90)
	...
	direntPtr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(512)
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vதொடங்கு(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].அளவு >= 0xFFFFFFFF {
			continue
		}
		...
		memoryManager.Vநினைவகத்தை_விடுவி(bpbPtr)
		memoryManager.Vநினைவகத்தை_விடுவி(direntPtr)
		return dirent[i].அளவு
	}
	memoryManager.Vநினைவகத்தை_விடுவி(bpbPtr)
	memoryManager.Vநினைவகத்தை_விடுவி(direntPtr)
	return 0
}
func (self *TBiosParameterBlock32) Vபடி(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].அளவு > 512 {
					buf3 = buffer[:512]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, 512)

				} else {
					buf3 = buffer[:dirent[i].அளவு]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].அளவு))
				}

				copy(data[int32(dirent[i].அளவு)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vதொடங்கு(data [32]byte) {
	...
	self.அளவு = Uint32_R(ArrayToUint32(buf))
}
