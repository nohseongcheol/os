...
func (self *TCOWFrameManager) Vதொடங்கு(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
	self.frameCount = frameCount
	refBytes := frameCount * uint32(unsafe.Sizeof(uint16(0)))
	refPtr := mem.Vநினைவகத்தை_ஒதுக்கு(refBytes)
	if refPtr == nil {
		self.refs = nil
		self.frameCount = 0
		return false
...
func (self *Paging) Vதொடங்கு(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vதொடங்கு(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vதொடங்கு(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, அளவு uint32, pageDir uint32) {
	if அளவு == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), அளவு)

	for i := uint32(0); i < அளவு; i++ {
		dst[i] = src[i]
	}
	setCR3(oldCR3)
}

func ZeroBlockInPageDir(addr uint32, அளவு uint32, pageDir uint32) {
	if அளவு == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, அளவு)
	dst := GetBytesFromPtr(uintptr(addr), int(அளவு), int(அளவு))
	for i := uint32(0); i < அளவு; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, அளவு uint32) bool {
	if அளவு == 0 {
		return true
	}
	last := addr + அளவு - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, அளவு uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, அளவு)
	setCR3(oldCR3)
	return ok
}
