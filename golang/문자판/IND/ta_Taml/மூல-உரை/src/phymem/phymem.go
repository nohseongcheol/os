...
func (self *PhysicalMemoryManager) Vஒதுக்கீட்டு_பிட்டைப்_புரட்டு(bit uint32) uint32 {
	self.memoryArray[bit/32] = self.memoryArray[bit/32] ^ (1 << (bit % 32))
	return self.memoryArray[bit/32]
}
...
func (self *PhysicalMemoryManager) FirstFreeSize(அளவு uint32) uint32 {
	if அளவு == 0 {
		return 0xffffffff
	}
	if அளவு == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= அளவு; count++ {
...
func (self *PhysicalMemoryManager) Vதொடங்கு(அளவு uint32, bitmap []uint32) {
	self.memorySize = அளவு
	self.memoryArray = bitmap
	self.maximumBlocks = அளவு / BLOCK_SIZE
