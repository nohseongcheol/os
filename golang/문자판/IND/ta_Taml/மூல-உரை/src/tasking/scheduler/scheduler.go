...
type SchedulerData struct {
	...
	currentThread	*Tசெயலிழை
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vதொடங்கு() {
...
func (self *SchedulerData) GetNextReadyThread() *Tசெயலிழை {
	if list.Vஅளவு <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vஅளவு; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vஅளவு {
			currentThreadIndex = 0
		}
		thread := (*Tசெயலிழை)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tசெயலிழை) {
	if thread == nil {
		return
	}
	list.Vபட்டியலின்_முடிவில்_சேர்(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tசெயலிழை) {
	if thread == nil {
		return
	}
	list.Vபட்டியலின்_முடிவில்_சேர்(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tசெயலிழை) {
...
func (self *Scheduler) Vதொடங்கு(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vதொடங்கு()
	sche_data.tss = tss
	...
	self.TInterruptHandler.Vதொடங்கு(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vஅளவு > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vஅளவு; i++ {
		thread := (*Tசெயலிழை)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tசெயலிழை) {
