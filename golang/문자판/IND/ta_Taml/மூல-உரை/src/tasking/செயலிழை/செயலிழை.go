...
func (self *TThreadHelper) Vதொடங்கு(mem *mem.TMemoryManager) {
	self.mem = mem
	콘솔.M출력XY(([]byte)("thread:"), 1, 14)
}
func (self *TThreadHelper) CreateFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) Tசெயலிழை {
	result := Tசெயலிழை{}

	result.Stack = uint32(uintptr(self.mem.Vநினைவகத்தை_ஒதுக்கு(THREAD_STACK_SIZE)))
	if result.Stack == 0 {
		return result
	}
	콘솔.M출력(([]byte)("[mem:"))
...
func (self *TThreadHelper) CreatePtrFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) *Tசெயலிழை {
	result := (*Tசெயலிழை)(self.mem.Vநினைவகத்தை_ஒதுக்கு(uint32(Sizeof(Tசெயலிழை{}))))
	if result == nil {
		return nil
	}
	*result = self.CreateFromFunction(entrypoint, PageDirEntry, isKernel)
	if result.CpuState == nil {
		self.mem.Vநினைவகத்தை_விடுவி(Pointer(result))
		return nil
	}
	return result
}
