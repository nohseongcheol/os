...
func (self *TUserDatagramProtocolHeader) Vதொடங்கு(buffer *TUserDatagramProtocolHeaderBuffer) {
...
type IUserDatagramProtocolHandler interface {
	HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, அளவு uint16)
}

type TUserDatagramProtocolHandler struct {
}

func (self *TUserDatagramProtocolHandler) Vதொடங்கு(backend TInternetProtocolProvider) {
}
func (self *TUserDatagramProtocolHandler) HandleUserDatagramProtocolMessage(socket *TUserDatagramProtocolSocket, data uintptr, அளவு uint16) {
}

type IUserDatagramProtocolSocket interface {
	HandleUserDatagramProtocolMessage(data uintptr, அளவு uint16)
}
...
func (self *TUserDatagramProtocolSocket) Vதொடங்கு(p_udpProvider TUserDatagramProtocolProvider, p_udpHandler IUserDatagramProtocolHandler) {
...
func (self *TUserDatagramProtocolSocket) HandleUserDatagramProtocolMessage(data uintptr, அளவு uint16) {
	if udpHandler != nil {
		udpHandler.HandleUserDatagramProtocolMessage(self, data, அளவு)
	}
}
func (self *TUserDatagramProtocolSocket) Send(p_data []byte, அளவு uint16) {
	var buffer [4096]byte
	for i := 0; i < int(அளவு); i++ {
		buffer[i] = p_data[i]
	}
	var data = uintptr(Pointer(&buffer))
	udpProvider.Send(self, data, அளவு)
}
...
func (self *TUserDatagramProtocolProvider) Vதொடங்கு(p_ipProvider TInternetProtocolProvider, p_ipHandler IInternetProtocolHandler) {
	ipHandler = p_ipHandler
	ipHandler.Vதொடங்கு(p_ipProvider, p_ipHandler, 0x11)
	numSockets = 0
	freePort = 1024
}
func (self *TUserDatagramProtocolProvider) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, internetprotocolPayload uintptr, அளவு uint32) bool {
	if அளவு < udpHeaderSize {
		return false
	}
	...
	if socket != nil {
		socket.HandleUserDatagramProtocolMessage(internetprotocolPayload+uintptr(udpHeaderSize), uint16(அளவு-udpHeaderSize))
	}
...
func (self *TUserDatagramProtocolProvider) Connect(ip uint32, port uint16) *TUserDatagramProtocolSocket {
	...
	var socket = (*TUserDatagramProtocolSocket)(memoryManager.Vநினைவகத்தை_ஒதுக்கு(50))

	if socket != nil {

		socket.Vதொடங்கு(*self, nil)
...
func (self *TUserDatagramProtocolProvider) Send(socket *TUserDatagramProtocolSocket, p_data uintptr, அளவு uint16) {
	var totalLength = uint32(அளவு) + udpHeaderSize
