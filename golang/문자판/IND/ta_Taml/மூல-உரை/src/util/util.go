...
func PrintBytes(dataPointer uintptr, அளவு int) {
	var buffer [4 * 1024]byte = *(*([4 * 1024]byte))(Pointer(dataPointer))
	var i int
	util콘솔.M출력([]byte("util["))
	for i = 0; i < அளவு; i++ {
		util콘솔.MHex출력(buffer[i])
	}
	util콘솔.M출력([]byte("]"))
}
...
func PrintTest(params ...interface{}) {
	for _, param := range params {
		util콘솔.M출력([]byte(TypeOf(param).Name()))
...
func GetUintptrArrayFromPtr(முகவரிச்_சுட்டி_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{முகவரிச்_சுட்டி_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(முகவரிச்_சுட்டி_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(முகவரிச்_சுட்டி_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(முகவரிச்_சுட்டி_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&முகவரிச்_சுட்டி_2))
	return *(*func())(Pointer(&code1ptr))
}
