...
func (self *TSyscall) Vதொடங்கு(manager *TInterruptManager) {
	initFileDescriptors()
	...
	self.TInterruptHandler.Vதொடங்கு(0x80, uintptr(Pointer(manager)), addr)
}
...
func sysRead(fd int32, addr uint32, count uint32) int32 {
	...
	if entry.pos >= entry.அளவு {
		return 0
	}
	remaining := entry.அளவு - entry.pos
	if count > remaining {
		count = remaining
	}
	buf := GetBytesFromPtr(uintptr(addr), int(count), int(count))
...
func readVFSFile(entry *openFileDescription, dst []byte, count uint32) int32 {
	...
	tmpPtr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(entry.அளவு)
	if tmpPtr == nil {
		return EINVAL
	}
	tmp := GetBytesFromPtr(uintptr(tmpPtr), int(entry.அளவு), int(entry.அளவு))
	readFile(entry.name[:entry.nameLen], tmp)
	copy(dst[:count], tmp[entry.pos:entry.pos+count])
	entry.pos += count
	memoryManager.Vநினைவகத்தை_விடுவி(tmpPtr)
	return int32(count)
}
...
func sysGetcwd(bufferAddr uint32, அளவு uint32) int32 {
	if bufferAddr == 0 {
		return EFAULT
	}
	if அளவு < 2 {
	...
	buffer := GetBytesFromPtr(uintptr(bufferAddr), int(அளவு), int(அளவு))
	buffer[0] = '/'
	buffer[1] = 0
	return 2
}

func fillPosixStat(statAddr uint32, mode uint32, அளவு uint32, inode uint32) int32 {
	if statAddr == 0 {
		return EFAULT
	}
	...
	*stat = posixStat{}
	...
	stat.Mode = mode
	stat.Nlink = 1
	stat.Vஅளவு = int32(அளவு)
	stat.Blksize = 512
	stat.Blocks = int32((அளவு + 511) / 512)
	return 0
}
...
func sysStat(pathAddr uint32, statAddr uint32) int32 {
	...
	return fillPosixStat(statAddr, sIfreg|0444, அளவு, inode)
}
...
func sysFstat(fd int32, statAddr uint32) int32 {
	...
	case fdKindRootDir:
		return fillPosixStat(statAddr, sIfdir|0555, 0, 1)
	case fdKindFat:
		return fillPosixStat(statAddr, sIfreg|0444, entry.அளவு, uint32(fd+2))
	case fdKindSocket:
...
func socketSendTo(fd int32, bufferAddress uint32, length uint32, destinationAddress uint32, destinationLength uint32) int32 {
	...
	*packet = socketPacket{used: true, அளவு: length, source: socket.local}
	if length != 0 {
...
func sysExecve(cpu *TCPUState, pathAddr uint32) int32 {
	...
	memoryManager := &mem.TMemoryManager{}
	filePtr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(அளவு)
	if filePtr == nil {
		return EINVAL
	}
	data := GetBytesFromPtr(uintptr(filePtr), int(அளவு), int(அளவு))
	readFile(name[:nameLen], data)
	if அளவு < 52 || data[0] != 0x7F || data[1] != 'E' || data[2] != 'L' || data[3] != 'F' {
		memoryManager.Vநினைவகத்தை_விடுவி(filePtr)
		return ENOEXEC
	}
	loader := Elf{}
...
func sysFork(cpu *TCPUState) int32 {
	...
	memoryManager := &mem.TMemoryManager{}
	threadPtr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(uint32(Sizeof(Tசெயலிழை{})))
	stackPtr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(THREAD_STACK_SIZE)
	childPageDir := CloneAddressSpaceCOW(getCR3())
	if threadPtr == nil || stackPtr == nil || childPageDir == 0 {
		discardProcess(pid)
		return EINVAL
	}
	child := (*Tசெயலிழை)(threadPtr)
...
func fileSize(filename []byte) uint32 {
	...
	அளவு := bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()
	return அளவு
}
