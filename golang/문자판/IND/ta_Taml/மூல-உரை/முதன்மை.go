...
func GetFileSize(filename []byte) uint32 {
	...
	var அளவு uint32 = bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()

	return அளவு
}

func Vகோப்பைப்_படி(filename []byte, data []byte) {
	var ata0s = TAdvancedTechnologyAttachment{}
	ata0s.Vதொடங்கு(false, 0x1F0)
	ata0s.Identify()
	...
	bios := TBiosParameterBlock32{}
	bios.Vபடி(&ata0s, partition.MBR.PrimaryPartition[0], filename, data)
...
func memorytest(y int) {
	...
	allocated := uint32(uintptr(memoryManager.Vநினைவகத்தை_ஒதுக்கு(1024)))
	console.MUint32출력XY(allocated, 10, uint16(y))
	if y == 11 {
		memoryManager.Vநினைவகத்தை_விடுவி(Pointer(uintptr(allocated)))
	}
}
...
func KKernelEntry(PageDirEntry uintptr, stacktop uintptr, stackbottom uintptr) {
	...
	memoryManager := &TMemoryManager{}
	memoryManager.Vதொடங்கு(0, MAX_QUEUE_SIZE)

	paging := &Paging{}
	paging.Vதொடங்கு(PageDirEntry, 0x500000, memoryManager)
	...
	InterruptManager.Vதொடங்கு(0x20, 공용서술자테이블, 작업관리자)

	paging.PageFault(InterruptManager)

	DriverManager := TDriverManager{}
	DriverManager.Vதொடங்கு()

	threadHelper := &TThreadHelper{}
	threadHelper.Vதொடங்கு(memoryManager)

	processHelper := ProcessHelper{}
	processHelper.Vதொடங்கு(memoryManager, PageDirEntry)

	sche := &Scheduler{}
	sche.Vதொடங்கு(InterruptManager, memoryManager, tss)

	sysCall := &TSyscall{}
	sysCall.Vதொடங்கு(InterruptManager)
	...
	அளவு = GetFileSize(linkerFile)
	linkerAddr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(அளவு)
	linkerData := GetBytesFromPtr(uintptr(linkerAddr), int(அளவு), int(அளவு))
	Vகோப்பைப்_படி(linkerFile, linkerData)
	...
	linkMap := LinkMap{}
	linkMap.Vதொடங்கு(memoryManager)

	var lib1File []byte = ([]byte)("LIB1")
	அளவு = GetFileSize(lib1File)

	lib1Addr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(அளவு)
	lib1Data := GetBytesFromPtr(uintptr(lib1Addr), int(அளவு), int(அளவு))
	Vகோப்பைப்_படி(lib1File, lib1Data)

	lib1elf := Elf{}
	lib1elf.Parse(lib1Data[:], uint32(PageDirEntry))
	memoryManager.Vநினைவகத்தை_விடுவி(lib1Addr)

	linkMap.Vபட்டியலின்_முடிவில்_சேர்(uintptr(lib1elf.Dynamic))

	var lib2File []byte = ([]byte)("LIB2")
	அளவு = GetFileSize(lib2File)

	lib2Addr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(அளவு)
	lib2Data := GetBytesFromPtr(uintptr(lib2Addr), int(அளவு), int(அளவு))
	Vகோப்பைப்_படி(lib2File, lib2Data)

	lib2elf := Elf{}
	lib2elf.Parse(lib2Data[:], uint32(PageDirEntry))
	memoryManager.Vநினைவகத்தை_விடுவி(lib2Addr)

	linkMap.Vபட்டியலின்_முடிவில்_சேர்(uintptr(lib2elf.Dynamic))
	...
	அளவு = GetFileSize(user1file)
	user1addr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(அளவு)
	user1data := GetBytesFromPtr(uintptr(user1addr), int(அளவு), int(அளவு))
	Vகோப்பைப்_படி(user1file, user1data)
	...
	P1LinkMap.Vபட்டியலின்_முடிவில்_சேர்(uintptr(elf2.Dynamic))

	memoryManager.Vநினைவகத்தை_விடுவி(user1addr)

	var code1Ptr *uintptr
	var func1Val func()

	code1Ptr = (*uintptr)(memoryManager.Vநினைவகத்தை_ஒதுக்கு(4))
	*code1Ptr = uintptr(linkerEntry)
	...
	thr2 := (*Tசெயலிழை)(proc2.Threads.GetAt(0))
	...
	அளவு = GetFileSize(user2file)
	user2addr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(அளவு)
	user2data := GetBytesFromPtr(uintptr(user2addr), int(அளவு), int(அளவு))
	Vகோப்பைப்_படி(user2file, user2data)
	...
	P2LinkMap.Vபட்டியலின்_முடிவில்_சேர்(uintptr(elf3.Dynamic))

	memoryManager.Vநினைவகத்தை_விடுவி(user2addr)

	var code2Ptr *uintptr
	var func2Val func()

	code2Ptr = (*uintptr)(memoryManager.Vநினைவகத்தை_ஒதுக்கு(4))
	*code2Ptr = uintptr(linkerEntry)
	...
	thr3 := (*Tசெயலிழை)(proc3.Threads.GetAt(0))
	...
	அளவு = GetFileSize(user3file)
	user3addr := memoryManager.Vநினைவகத்தை_ஒதுக்கு(அளவு)
	user3data := GetBytesFromPtr(uintptr(user3addr), int(அளவு), int(அளவு))
	Vகோப்பைப்_படி(user3file, user3data)
	...
	P3LinkMap.Vபட்டியலின்_முடிவில்_சேர்(uintptr(elf4.Dynamic))

	memoryManager.Vநினைவகத்தை_விடுவி(user3addr)

	var code3Ptr *uintptr
	var func3Val func()

	code3Ptr = (*uintptr)(memoryManager.Vநினைவகத்தை_ஒதுக்கு(4))
	*code3Ptr = uintptr(linkerEntry)
	...
	thr4 := (*Tசெயலிழை)(proc4.Threads.GetAt(0))
	...
	pciController.Vதொடங்கு(myPCIControllerHandler)
