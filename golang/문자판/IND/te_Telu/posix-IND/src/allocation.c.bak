...
void *malloc(size_t పరిమాణం)
{
    struct heap_block *cursor = heap_head, *last_block = NULL, *raw;
    size_t total, padding;
    void *address;
    if (!పరిమాణం) పరిమాణం = 1;
    if (పరిమాణం > (size_t)INT_MAX - 2 * sizeof(struct heap_block)) { errno = ENOMEM; return NULL; }
    పరిమాణం = (పరిమాణం + 15U) & ~15U;
    while (cursor) {
        if (cursor->available && cursor->capacity >= పరిమాణం) {
            if (cursor->capacity - పరిమాణం >= sizeof(struct heap_block) + 16U) {
                raw = (struct heap_block *)((unsigned char *)(cursor + 1) + పరిమాణం);
                raw->capacity = cursor->capacity - పరిమాణం - sizeof(*raw);
                raw->next_block = cursor->next_block;
                raw->available = 1;
                cursor->next_block = raw;
                cursor->capacity = పరిమాణం;
            }
            cursor->available = 0;
            return cursor + 1;
        }
    ...
    total = padding + sizeof(struct heap_block) + పరిమాణం;
    ...
    raw->capacity = పరిమాణం; raw->next_block = NULL; raw->available = 0;
...
void *realloc(void *address, size_t పరిమాణం)
{
    void *destination;
    struct heap_block *cursor;
    if (!address) return malloc(పరిమాణం);
    /* Documented zero-size choice: retain a valid, freeable minimum block. */
    if (!పరిమాణం) పరిమాణం = 1;
    cursor = (struct heap_block *)address - 1;
    if (పరిమాణం <= cursor->capacity) return address;
    destination = malloc(పరిమాణం);
