...
ssize_t చదువు(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_చదువు, fd, buf, count));
}

ssize_t రాయి(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_రాయి, fd, buf, count));
}
...
char *getcwd(char *buf, size_t పరిమాణం)
{
    long result = __syscall_result(SC2(SYS_getcwd, buf, పరిమాణం));
