...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, పరిమాణం int) bool
	Send(dataPointer uintptr, పరిమాణం uint32)
}
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, పరిమాణం int) bool {
	넷카드콘솔.M출력XY(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawDataHandler) Send(dataPointer uintptr, పరిమాణం uint32) {
	넷카드콘솔.M출력XY(([]byte)("TRawDataSend"), 10, 10)
	rawDataBackend.Send(dataPointer, పరిమాణం)
}
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Vప్రారంభించు(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, పరిమాణం uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + పరిమాణం - 1

	for i := 0; i < int(పరిమాణం); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(పరిమాణం)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-పరిమాణం)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var పరిమాణం uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if పరిమాణం > 64 {
				పరిమాణం -= 4
			}

			콘솔.M출력([]byte(" size : ["))
			콘솔.MUint32출력(పరిమాణం)
			콘솔.M출력([]byte("]"))

			var buffer [4096]byte = *(*([4096]byte))(Pointer(uintptr(recvBufferDescr[currentRecvBuffer].address)))
			var చిరునామా_సూచిక uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(చిరునామా_సూచిక, int(పరిమాణం)) {

					콘솔.M출력XY(([]byte)("self.Send"), 0, 22)

					self.Send(చిరునామా_సూచిక, పరిమాణం)
				}
			}
