...
func (self *TARPMessage) Vప్రారంభించు(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, పరిమాణం int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(పరిమాణం))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, పరిమాణం uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, పరిమాణం)
}
...
func (self *TARPProvider) Vప్రారంభించు(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vప్రారంభించు(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, పరిమాణం uint32) bool {

	if పరిమాణం < arpMesgSize {
		return false
	}
	var arpBuffer *TARPMessageBuffer = (*TARPMessageBuffer)(Pointer(dataPointer))
	var arp TARPMessage = TARPMessage{}
	arp.Vప్రారంభించు(arpBuffer)
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var చిరునామా_సూచిక uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, చిరునామా_సూచిక, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, పరిమాణం uint32) {
