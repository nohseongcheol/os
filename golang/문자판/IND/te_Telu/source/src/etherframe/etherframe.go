...
func (self *TEtherFrameHeader) Vప్రారంభించు(buffer TEtherFrameHeaderBuffer) {
...
type IEtherFrameHandler interface {
	Vప్రారంభించు(backend TEtherFrameProvider)
	SetHandler(handler IEtherFrameHandler, etherType uint16)
	OnEtherFrameReceived(dataPointer uintptr, పరిమాణం int) bool
	Send(dstMAC_BE uint64, dataPointer uintptr, పరిమాణం uint32)
	SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, పరిమాణం uint32)
...
func (self *TEtherFrameHandler) Vప్రారంభించు(backend TEtherFrameProvider) {
	Backend = backend
}
...
func (self *TEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, పరిమాణం int) bool {
	이더넷콘솔.M출력(([]byte)("OnEtherFrameReceived"))
	return false
}
func (self *TEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, పరిమాణం uint32) {
	Backend.SendFrame(dstMAC_BE, frame.etherType_BE, dataPointer, పరిమాణం)
}
func (self *TEtherFrameHandler) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, పరిమాణం uint32) {
	Backend.SendFrame(dstMAC_BE, etherType_BE, dataPointer, పరిమాణం)
}
...
func (self *TEtherFrameRawDataHandler) Vప్రారంభించు(p_provider TEtherFrameProvider, p_backend Tamd_am79c973) {
	provider = p_provider
	provider.Vప్రారంభించు(p_backend)
}
func (self *TEtherFrameRawDataHandler) OnRawDataReceived(dataPointer uintptr, పరిమాణం int) bool {
	return provider.OnRawDataReceived(dataPointer, పరిమాణం)
}
func (self *TEtherFrameRawDataHandler) Send(dataPointer uintptr, పరిమాణం uint32) {
	provider.Send(dataPointer, పరిమాణం)
}
...
func (self *TEtherFrameProvider) Vప్రారంభించు(backend Tamd_am79c973) {
...
func (self *TEtherFrameProvider) OnRawDataReceived(dataPointer uintptr, పరిమాణం int) bool {
	...
	if frame.dstMAC_BE == 0x0000FFFFFFFFFFFF || Uint48_R(frame.dstMAC_BE) == self.GetMACAddress() {
		if handlers[frame.etherType_BE] != nil {
			...
			var చిరునామా_సూచిక uintptr = uintptr(Pointer(dataPointer)) + uintptr(frameHeaderSize)
			sendBack = handlers[frame.etherType_BE].OnEtherFrameReceived(చిరునామా_సూచిక, పరిమాణం-frameHeaderSize)
...
func (self *TEtherFrameProvider) Send(dataPointer uintptr, పరిమాణం uint32) {
	self.netCard.Send(dataPointer, పరిమాణం)
}
func (self *TEtherFrameProvider) SendFrame(dstMAC_BE uint64, etherType_BE uint16, dataPointer uintptr, పరిమాణం uint32) {
	...
	var చిరునామా_సూచిక uintptr = uintptr(Pointer(&buffer2))

	self.netCard.Send(చిరునామా_సూచిక, పరిమాణం+uint32(frameHeaderSize))
