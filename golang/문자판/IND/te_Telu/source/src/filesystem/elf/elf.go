	...
	if elfHeader.e_shnum != 0 {
		...
		for i := uint32(0); i < uint32(elfHeader.e_shnum); i++ {
			...
			if sectHeader.sh_addr > 0x1000 {
				పరిమాణం := sectHeader.sh_size
				if sectHeader.sh_type == 8 {
					ZeroBlockInPageDir(sectHeader.sh_addr, పరిమాణం, PageDirEntry)
				} else {
					dst := GetBytesFromPtr(uintptr(sectHeader.sh_addr), int(పరిమాణం), int(పరిమాణం))
					SetBlockInPageDir(sectValue, dst, పరిమాణం, PageDirEntry)
				}
			}
		...
		if textPtr != nil {
			memoryManager.Vజ్ఞాపకస్థలాన్ని_విడుదల_చేయి(textPtr)
		}
