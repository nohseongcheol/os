...
func (self *TBiosParameterBlock32) Vప్రారంభించు(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	bpbPtr := memoryManager.Vజ్ఞాపకస్థలాన్ని_కేటాయించు(90)
	bpbBytes := GetBytesFromPtr(uintptr(bpbPtr), 90, 90)
	var partitionOffset = partEntry.Start_lba

	hd.Read28(partitionOffset, &bpbBytes, 90)

	var bpb = TBiosParameterBlock32{}
	bpb.Vప్రారంభించు(bpbBytes)
	...
	direntPtr := memoryManager.Vజ్ఞాపకస్థలాన్ని_కేటాయించు(512)
	direntBytes := GetBytesFromPtr(uintptr(direntPtr), 512, 512)
	...
	for i := 0; i < 16; i++ {
		...
		dirent[i].Vప్రారంభించు(buf3)

		if dirent[i].name[0] == 0x00 {
			break
		}

		if dirent[i].పరిమాణం >= 0xFFFFFFFF {
			continue
		}
		...
		memoryManager.Vజ్ఞాపకస్థలాన్ని_విడుదల_చేయి(bpbPtr)
		memoryManager.Vజ్ఞాపకస్థలాన్ని_విడుదల_చేయి(direntPtr)
		return dirent[i].పరిమాణం
	}
	memoryManager.Vజ్ఞాపకస్థలాన్ని_విడుదల_చేయి(bpbPtr)
	memoryManager.Vజ్ఞాపకస్థలాన్ని_విడుదల_చేయి(direntPtr)
	return 0
}
func (self *TBiosParameterBlock32) Vచదువు(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
	...
	for i := 0; i < 16; i++ {
		...
		var SIZE = int32(dirent[i].పరిమాణం)
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].పరిమాణం > 512 {
					buf3 = buffer[:512]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, 512)

				} else {
					buf3 = buffer[:dirent[i].పరిమాణం]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].పరిమాణం))
				}

				copy(data[int32(dirent[i].పరిమాణం)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vప్రారంభించు(data [32]byte) {
	...
	self.పరిమాణం = Uint32_R(ArrayToUint32(buf))
}
