...
func (self *TPartitionTableEntry) Vప్రారంభించు(data [16]byte) {
...
func (self *TMSDOSPartitionTable) ReadPartitions(hd *TAdvancedTechnologyAttachment) {
	...
	self.MBR.PrimaryPartition[0].Vప్రారంభించు(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[1].Vప్రారంభించు(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[2].Vప్రారంభించు(buf3)
	i += 16

	copy(buf3[:16], partitionBytes[i:i+16])
	self.MBR.PrimaryPartition[3].Vప్రారంభించు(buf3)
	i += 16
