...
func (self *TInternetControlMessageProtocolMessage) Vప్రారంభించు(buffer TInternetControlMessageProtocolMessageBuffer) {
...
func (self *TICMPHandler) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, dataPointer uintptr, పరిమాణం uint32) bool {
	return icmp.OnInternetProtocolReceived(srcIP_BE, dstIP_BE, dataPointer, పరిమాణం)
}
...
func (self *TInternetControlMessageProtocol) Vప్రారంభించు(backend TInternetProtocolProvider, handler IInternetProtocolHandler) {
	ipHandler = handler
	ipHandler.Vప్రారంభించు(backend, handler, 0x01)
	icmp = self
}
func (self *TInternetControlMessageProtocol) OnInternetProtocolReceived(srcIP_BE uint32, dstIP_BE uint32, dataPointer uintptr, పరిమాణం uint32) bool {
	if పరిమాణం < uint32(icmpSize) {
		return false
	}
...
func (self *TInternetControlMessageProtocol) SendEchoRequest(ip_be uint32) bool {
	...
	var buffer = (*TInternetControlMessageProtocolMessageBuffer)(memoryManager.Vజ్ఞాపకస్థలాన్ని_కేటాయించు(1024))
