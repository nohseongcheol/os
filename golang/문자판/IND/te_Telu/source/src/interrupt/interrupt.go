...
func (self *TInterruptHandler) Vప్రారంభించు(InterruptNumber uint8, InterruptManager uintptr, func_addr uintptr) {
...
func (self *TInterruptManager) Vప్రారంభించు(hardwareInterruptOffset uint16, globalDescriptorTable *T공용서술자테이블, taskManager *T작업관리자) {
	...
	పరిమాణం := (*uint16)(Pointer(&idtPointer[0]))
	(*పరిమాణం) = (uint16)(Sizeof(idtData) - 1)
