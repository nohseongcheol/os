...
func (self *TCOWFrameManager) Vప్రారంభించు(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
	self.frameCount = frameCount
	refBytes := frameCount * uint32(unsafe.Sizeof(uint16(0)))
	refPtr := mem.Vజ్ఞాపకస్థలాన్ని_కేటాయించు(refBytes)
	if refPtr == nil {
		self.refs = nil
		self.frameCount = 0
		return false
...
func (self *Paging) Vప్రారంభించు(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vప్రారంభించు(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vప్రారంభించు(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, పరిమాణం uint32, pageDir uint32) {
	if పరిమాణం == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), పరిమాణం)

	for i := uint32(0); i < పరిమాణం; i++ {
		dst[i] = src[i]
	}
	setCR3(oldCR3)
}

func ZeroBlockInPageDir(addr uint32, పరిమాణం uint32, pageDir uint32) {
	if పరిమాణం == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, పరిమాణం)
	dst := GetBytesFromPtr(uintptr(addr), int(పరిమాణం), int(పరిమాణం))
	for i := uint32(0); i < పరిమాణం; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, పరిమాణం uint32) bool {
	if పరిమాణం == 0 {
		return true
	}
	last := addr + పరిమాణం - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, పరిమాణం uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, పరిమాణం)
	setCR3(oldCR3)
	return ok
}
