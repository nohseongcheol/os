...
func (self *PhysicalMemoryManager) Vకేటాయింపు_బిట్_తిప్పు(bit uint32) uint32 {
	self.memoryArray[bit/32] = self.memoryArray[bit/32] ^ (1 << (bit % 32))
	return self.memoryArray[bit/32]
}
...
func (self *PhysicalMemoryManager) FirstFreeSize(పరిమాణం uint32) uint32 {
	if పరిమాణం == 0 {
		return 0xffffffff
	}
	if పరిమాణం == 1 {
		...
		if self.memoryArray[i] != 0xffffffff {
			for j := uint32(0); j < 32; j++ {
				...
				if self.memoryArray[j]&bit == 0 {
					...
					var free uint32 = 0
					for count := uint32(0); count <= పరిమాణం; count++ {
...
func (self *PhysicalMemoryManager) Vప్రారంభించు(పరిమాణం uint32, bitmap []uint32) {
	self.memorySize = పరిమాణం
	self.memoryArray = bitmap
	self.maximumBlocks = పరిమాణం / BLOCK_SIZE
