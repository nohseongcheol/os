...
type SchedulerData struct {
	...
	currentThread	*Tనిర్వహణ_ధార
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vప్రారంభించు() {
...
func (self *SchedulerData) GetNextReadyThread() *Tనిర్వహణ_ధార {
	if list.Vపరిమాణం <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vపరిమాణం; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vపరిమాణం {
			currentThreadIndex = 0
		}
		thread := (*Tనిర్వహణ_ధార)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tనిర్వహణ_ధార) {
	if thread == nil {
		return
	}
	list.Vజాబితా_చివర_చేర్చు(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tనిర్వహణ_ధార) {
	if thread == nil {
		return
	}
	list.Vజాబితా_చివర_చేర్చు(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tనిర్వహణ_ధార) {
...
func (self *Scheduler) Vప్రారంభించు(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vప్రారంభించు()
	sche_data.tss = tss
	initPIT(pitFrequency)

	list = LinkedList{}
	list.Vప్రారంభించు(mem)
	콘솔.M출력("list:")
	...
	self.TInterruptHandler.Vప్రారంభించు(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vపరిమాణం > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vపరిమాణం; i++ {
		thread := (*Tనిర్వహణ_ధార)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tనిర్వహణ_ధార) {
