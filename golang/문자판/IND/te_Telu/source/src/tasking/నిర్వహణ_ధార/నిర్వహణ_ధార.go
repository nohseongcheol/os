...
func (self *TThreadHelper) Vప్రారంభించు(mem *mem.TMemoryManager) {
	self.mem = mem
	콘솔.M출력XY(([]byte)("thread:"), 1, 14)
}
func (self *TThreadHelper) CreateFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) Tనిర్వహణ_ధార {
	result := Tనిర్వహణ_ధార{}

	result.Stack = uint32(uintptr(self.mem.Vజ్ఞాపకస్థలాన్ని_కేటాయించు(THREAD_STACK_SIZE)))
	if result.Stack == 0 {
		return result
	}
	콘솔.M출력(([]byte)("[mem:"))
...
func (self *TThreadHelper) CreatePtrFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) *Tనిర్వహణ_ధార {
	result := (*Tనిర్వహణ_ధార)(self.mem.Vజ్ఞాపకస్థలాన్ని_కేటాయించు(uint32(Sizeof(Tనిర్వహణ_ధార{}))))
	if result == nil {
		return nil
	}
	*result = self.CreateFromFunction(entrypoint, PageDirEntry, isKernel)
	if result.CpuState == nil {
		self.mem.Vజ్ఞాపకస్థలాన్ని_విడుదల_చేయి(Pointer(result))
		return nil
	}
	return result
}
