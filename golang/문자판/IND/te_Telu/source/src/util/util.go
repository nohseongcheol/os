...
func PrintBytes(dataPointer uintptr, పరిమాణం int) {
...
func GetUintptrArrayFromPtr(చిరునామా_సూచిక_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{చిరునామా_సూచిక_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(చిరునామా_సూచిక_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(చిరునామా_సూచిక_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(చిరునామా_సూచిక_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&చిరునామా_సూచిక_2))
