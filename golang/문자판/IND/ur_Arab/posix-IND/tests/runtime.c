...
{
    (void)لکھنا(STDOUT_FILENO, text, text_length(text));
}
...
static void test_open_and_io(void)
{
    ...
    if (فائل_کا_وصف_کنندہ >= 0) {
        report("read-bytes", پڑھنا(فائل_کا_وصف_کنندہ, bytes, 2) == 2 &&
        ...
        report("close-file", close(فائل_کا_وصف_کنندہ) == 0);
        errno = 0;
        report("closed-fd", failed_with(close(فائل_کا_وصف_کنندہ), EBADF));
    }
    ...
    if (root >= 0) {
        ...
        errno = 0;
        report("read-directory", پڑھنا(root, bytes, 1) == -1 && errno == EISDIR);
        (void)close(root);
    }

    report("fstat-character", fstat(STDOUT_FILENO, &st) == 0 && S_ISCHR(st.st_mode));
    report("write", لکھنا(STDOUT_FILENO, "", 0) == 0);
    report("read-zero", پڑھنا(STDIN_FILENO, bytes, 0) == 0);
    errno = 0;
    report("read-ebadf", پڑھنا(STDOUT_FILENO, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
    report("write-ebadf", لکھنا(-1, bytes, 1) == -1 && errno == EBADF);
    errno = 0;
...
static void test_dup_and_fcntl(void)
{
    ...
    char second;
    struct stat st;
    int فائل_کا_وصف_کنندہ = open("/USER2", O_RDONLY);
    int copy;
    int high;
    int target;
    ...
    if (copy >= 0) {
        report("dup-shared-offset", پڑھنا(فائل_کا_وصف_کنندہ, &first, 1) == 1 && پڑھنا(copy, &second, 1) == 1 &&
               (unsigned char)first == 0x7f && second == 'E');
        report("dup-close-original", close(فائل_کا_وصف_کنندہ) == 0 && fstat(copy, &st) == 0);
...
void posix_test_process(void)
{
    ...
    if (child > 0 && reap_nohang(child, &حالت) == child) {
        report("fork-fd-isolation", پڑھنا(فائل_کا_وصف_کنندہ, &byte, 1) == 1 && (unsigned char)byte == 0x7f);
    } else {
