...
type IRawDataHandler interface {
	OnRawDataReceived(dataPointer uintptr, حجم int) bool
	Send(dataPointer uintptr, حجم uint32)
}
...
func (self *TRawDataHandler) OnRawDataReceived(dataPointer uintptr, حجم int) bool {
	넷카드콘솔.M출력XY(([]byte)("TRawDataRecv"), 1, 20)
	return true
}

func (self *TRawDataHandler) Send(dataPointer uintptr, حجم uint32) {
	넷카드콘솔.M출력XY(([]byte)("TRawDataSend"), 10, 10)
	rawDataBackend.Send(dataPointer, حجم)
}
...
func (self *Tamd_am79c973) InitDriver(interrupt *TInterruptManager, deviceDescriptor TPeripheralComponentInterconnectDeviceDescriptor, handler IRawDataHandler) {
	...
	self.Vآغاز_کرنا(uint8(0x20+deviceDescriptor.Interrupt), uintptr(Pointer(interrupt)), addr)
...
func (self *Tamd_am79c973) Send(dataPointer uintptr, حجم uint32) {
	...
	var dst uint32 = sendBufferDescr[sendDescriptor].address + حجم - 1

	for i := 0; i < int(حجم); i++ {

		*(*byte)(Pointer(uintptr(dst))) = src[int(حجم)-i-1]
	...
	sendBufferDescr[sendDescriptor].flags = 0x8300F000 | uint32((-حجم)&0xFFF)
...
func (self *Tamd_am79c973) Receive() {
	...
	for ; (recvBufferDescr[currentRecvBuffer].flags & 0x80000000) == 0; currentRecvBuffer = (currentRecvBuffer + 1) % 8 {
		...
		if !(recvBufferDescr[currentRecvBuffer].flags&0x40000000 != 0) && ((recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) {
			var حجم uint32 = recvBufferDescr[currentRecvBuffer].flags & 0xFFF
			if حجم > 64 {
				حجم -= 4
			}
			...
			var پتے_کا_حوالہ uintptr = uintptr(Pointer(&buffer))
			if iRawDataHandler != nil {
				if iRawDataHandler.OnRawDataReceived(پتے_کا_حوالہ, int(حجم)) {

					콘솔.M출력XY(([]byte)("self.Send"), 0, 22)

					self.Send(پتے_کا_حوالہ, حجم)
				}
			}
