...
func (self *TARPMessage) Vآغاز_کرنا(buffer *TARPMessageBuffer) {
...
func (self *TARPEtherFrameHandler) OnEtherFrameReceived(dataPointer uintptr, حجم int) bool {
	arp콘솔.M출력XY([]byte("arp recv:"), 0, 23)
	return arpProvider.OnEtherFrameReceived(dataPointer, uint32(حجم))

}
func (self *TARPEtherFrameHandler) Send(dstMAC_BE uint64, dataPointer uintptr, حجم uint32) {
	arp콘솔.M출력XY([]byte("arp send:"), 0, 24)
	var etherType_BE = Uint16_R(0x0806)
	self.TEtherFrameHandler.SendFrame(dstMAC_BE, etherType_BE, dataPointer, حجم)
}
...
func (self *TARPProvider) Vآغاز_کرنا(backend TEtherFrameProvider, userhandler IEtherFrameHandler) {

	handler = userhandler
	handler.Vآغاز_کرنا(backend)
...
func (self *TARPProvider) OnEtherFrameReceived(dataPointer uintptr, حجم uint32) bool {
...
func (self *TARPProvider) BroadcastMacAddress(IP_BE uint32) {
	...
	var پتے_کا_حوالہ uintptr = uintptr(Pointer(&arpBuffer))
	handler.Send(arp.dstMAC, پتے_کا_حوالہ, arpMesgSize)
}
...
func (self *TARPProvider) TestPrint(data *[]byte, حجم uint32) {
