	...
	if elfHeader.e_shnum != 0 {
		...
		for i := uint32(0); i < uint32(elfHeader.e_shnum); i++ {
			...
			if sectHeader.sh_addr > 0x1000 {
				حجم := sectHeader.sh_size
				if sectHeader.sh_type == 8 {
					ZeroBlockInPageDir(sectHeader.sh_addr, حجم, PageDirEntry)
				} else {
					dst := GetBytesFromPtr(uintptr(sectHeader.sh_addr), int(حجم), int(حجم))
					SetBlockInPageDir(sectValue, dst, حجم, PageDirEntry)
				}
			}
		...
		if textPtr != nil {
			memoryManager.Vحافظہ_آزاد_کرنا(textPtr)
		}
