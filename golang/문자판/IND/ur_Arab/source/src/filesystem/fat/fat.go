...
func (self *TBiosParameterBlock32) Vآغاز_کرنا(data []byte) {
...
func (self *TBiosParameterBlock32) Len(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte) uint32 {
	...
	bpbPtr := memoryManager.Vحافظہ_مختص_کرنا(90)
	...
	direntPtr := memoryManager.Vحافظہ_مختص_کرنا(512)
	...
	for i := 0; i < 16; i++ {
		...
		memoryManager.Vحافظہ_آزاد_کرنا(bpbPtr)
		memoryManager.Vحافظہ_آزاد_کرنا(direntPtr)
		return dirent[i].حجم
	}
	memoryManager.Vحافظہ_آزاد_کرنا(bpbPtr)
	memoryManager.Vحافظہ_آزاد_کرنا(direntPtr)
...
func (self *TBiosParameterBlock32) Vپڑھنا(hd *TAdvancedTechnologyAttachment, partEntry TPartitionTableEntry, filename []byte, data []byte) {
		...
		for SIZE > 0 {
			...
			for ; SIZE > 0; SIZE -= 512 {
				...
				if dirent[i].حجم > 512 {
					...
					buf3 = buffer[:dirent[i].حجم]
					hd.Read28(fileSector+uint32(sectorOffset), &buf3, int(dirent[i].حجم))
				}

				copy(data[int32(dirent[i].حجم)-SIZE:], buf3)
...
func (self *TDirectoryEntryFat32) Vآغاز_کرنا(data [32]byte) {
	...
	self.حجم = Uint32_R(ArrayToUint32(buf))
}
