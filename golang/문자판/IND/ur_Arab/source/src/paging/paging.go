...
func (self *TCOWFrameManager) Vآغاز_کرنا(mem *TMemoryManager, frameCount uint32) bool {
	self.mem = mem
	self.frameCount = frameCount
	refBytes := frameCount * uint32(unsafe.Sizeof(uint16(0)))
	refPtr := mem.Vحافظہ_مختص_کرنا(refBytes)
	if refPtr == nil {
		self.refs = nil
		self.frameCount = 0
		return false
...
func (self *Paging) Vآغاز_کرنا(pageDirEntry uintptr, pageTableEntry uint32, memoryManager *TMemoryManager) {
	...
	cowFrameManager.Vآغاز_کرنا(memoryManager, (512*1024*1024)>>12)
...
func (self *Paging) PageFault(manager *TInterruptManager) {
	...
	self.TInterruptHandler.Vآغاز_کرنا(0xE, uintptr(unsafe.Pointer(manager)), addr)
}
...
func SetBlockInPageDir(src []byte, dst []byte, حجم uint32, pageDir uint32) {
	if حجم == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, uint32(uintptr(unsafe.Pointer(&dst[0]))), حجم)
...
func ZeroBlockInPageDir(addr uint32, حجم uint32, pageDir uint32) {
	if حجم == 0 || pageDir == 0 {
		return
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	makeRangePrivateWritableCurrent(pageDir, addr, حجم)
	dst := GetBytesFromPtr(uintptr(addr), int(حجم), int(حجم))
	for i := uint32(0); i < حجم; i++ {
		dst[i] = 0
	}
	setCR3(oldCR3)
...
func makeRangePrivateWritableCurrent(pageDir uint32, addr uint32, حجم uint32) bool {
	if حجم == 0 {
		return true
	}
	last := addr + حجم - 1
...
func MakeRangePrivateWritable(pageDir uint32, addr uint32, حجم uint32) bool {
	if pageDir == 0 {
		return false
	}
	oldCR3 := getCR3()
	setCR3(pageDir)
	ok := makeRangePrivateWritableCurrent(pageDir, addr, حجم)
	setCR3(oldCR3)
	return ok
}
