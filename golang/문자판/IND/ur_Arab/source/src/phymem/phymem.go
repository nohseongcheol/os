...
func (self *PhysicalMemoryManager) Vتخصیص_کا_بٹ_الٹنا(bit uint32) uint32 {
	self.memoryArray[bit/32] = self.memoryArray[bit/32] ^ (1 << (bit % 32))
	return self.memoryArray[bit/32]
}
...
func (self *PhysicalMemoryManager) FirstFreeSize(حجم uint32) uint32 {
	if حجم == 0 {
		return 0xffffffff
	}
	if حجم == 1 {
...
func (self *PhysicalMemoryManager) Vآغاز_کرنا(حجم uint32, bitmap []uint32) {
	self.memorySize = حجم
	self.memoryArray = bitmap
	self.maximumBlocks = حجم / BLOCK_SIZE
