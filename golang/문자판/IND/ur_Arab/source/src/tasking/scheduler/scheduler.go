...
type SchedulerData struct {
	...
	currentThread	*Tعملی_دھارا
	tss		*TSSEntry
}

var sche_data SchedulerData = SchedulerData{}

func (self *SchedulerData) Vآغاز_کرنا() {
...
func (self *SchedulerData) GetNextReadyThread() *Tعملی_دھارا {
	if list.Vحجم <= 0 {
		return nil
	}
	...
	for checked := 0; checked < list.Vحجم; checked++ {
		currentThreadIndex++
		if currentThreadIndex >= list.Vحجم {
			currentThreadIndex = 0
		}
		thread := (*Tعملی_دھارا)(list.GetAt(currentThreadIndex))
...
func (self *Scheduler) AddThread(thread *Tعملی_دھارا) {
	if thread == nil {
		return
	}
	list.Vفہرست_کے_آخر_میں_شامل_کرنا(uintptr(Pointer(thread)))
}
func AddRunnableThread(thread *Tعملی_دھارا) {
	if thread == nil {
		return
	}
	list.Vفہرست_کے_آخر_میں_شامل_کرنا(uintptr(Pointer(thread)))
...
func (self *Scheduler) RemoveThread(thread *Tعملی_دھارا) {
...
func (self *Scheduler) Vآغاز_کرنا(manager *TInterruptManager, mem *mem.TMemoryManager, tss *TSSEntry) {
	sche_data.Vآغاز_کرنا()
	sche_data.tss = tss
	...
	self.TInterruptHandler.Vآغاز_کرنا(0x20, uintptr(Pointer(manager)), addr)
}
...
func handleInterrupt(esp uint32) uint32 {
	...
	if sche_data.tickCount == sche_data.frequency {
		...
		if list.Vحجم > 0 && sche_data.Enabled == true {
...
func StopCurrentThread(cpustate *TCPUState) *TCPUState {
	...
	for i := 0; i < list.Vحجم; i++ {
		thread := (*Tعملی_دھارا)(list.GetAt(i))
...
func InitialThreadUserJump(thread *Tعملی_دھارا) {
