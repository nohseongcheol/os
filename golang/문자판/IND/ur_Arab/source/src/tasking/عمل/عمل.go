...
func (self *Process) Vآغاز_کرنا(mem *mem.TMemoryManager) {
	self.ThreadList = LinkedList{}
	self.Threads = &self.ThreadList
	self.Threads.Vآغاز_کرنا(mem)
}
...
func (self *ProcessHelper) Vآغاز_کرنا(mem *mem.TMemoryManager, kernelPageDirEntry uintptr) {
	self.mem = mem
	self.processes = LinkedList{}
	self.processes.Vآغاز_کرنا(self.mem)
...
func (self *ProcessHelper) Create(entryPoint func(), threadHelper *TThreadHelper, PageDirEntry uint32, isKernel bool) Process {
	process := (*Process)(self.mem.Vحافظہ_مختص_کرنا(uint32(Sizeof(Process{}))))
	if process == nil {
		return Process{}
	}
	process.Vآغاز_کرنا(self.mem)
	...
	if mainThread != nil {
		...
		process.Threads.Vفہرست_کے_آخر_میں_شامل_کرنا(uintptr(Pointer(mainThread)))
	}

	self.processes.Vفہرست_کے_آخر_میں_شامل_کرنا(uintptr(Pointer(process)))
...
func (self *ProcessHelper) Spawn(entryPoint func(), threadHelper *TThreadHelper, scheduler *Scheduler, PageDirEntry uint32, isKernel bool) Process {
	...
	if process.Threads != nil && process.Threads.Vحجم > 0 {
		thread := (*Tعملی_دھارا)(process.Threads.GetAt(0))
