...
func (self *TThreadHelper) Vآغاز_کرنا(mem *mem.TMemoryManager) {
	self.mem = mem
	콘솔.M출력XY(([]byte)("thread:"), 1, 14)
}
func (self *TThreadHelper) CreateFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) Tعملی_دھارا {
	result := Tعملی_دھارا{}

	result.Stack = uint32(uintptr(self.mem.Vحافظہ_مختص_کرنا(THREAD_STACK_SIZE)))
	if result.Stack == 0 {
		return result
	}
	콘솔.M출력(([]byte)("[mem:"))
...
func (self *TThreadHelper) CreatePtrFromFunction(entrypoint func(), PageDirEntry uint32, isKernel bool) *Tعملی_دھارا {
	result := (*Tعملی_دھارا)(self.mem.Vحافظہ_مختص_کرنا(uint32(Sizeof(Tعملی_دھارا{}))))
	if result == nil {
		return nil
	}
	*result = self.CreateFromFunction(entrypoint, PageDirEntry, isKernel)
	if result.CpuState == nil {
		self.mem.Vحافظہ_آزاد_کرنا(Pointer(result))
		return nil
	}
	return result
}
