...
func PrintBytes(dataPointer uintptr, حجم int) {
...
func GetUintptrArrayFromPtr(پتے_کا_حوالہ_2 uintptr, len int, cap int) []uintptr {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
	}{پتے_کا_حوالہ_2, len, cap}
	return *(*[]uintptr)(Pointer(&sl))
}
func GetUint32ArrayFromPtr(پتے_کا_حوالہ_2 uintptr, len int, cap int) []uint32 {
	len = len * 4
	cap = cap * 4
	var sl = struct {
		addr	uintptr
...
func GetBytesFromPtr(پتے_کا_حوالہ_2 uintptr, len int, cap int) []byte {
	var sl = struct {
		addr	uintptr
		len	int
		cap	int
...
func GetFuncPtrFromPtr(پتے_کا_حوالہ_2 uintptr) func() {
	code1ptr := uintptr(Pointer(&پتے_کا_حوالہ_2))
