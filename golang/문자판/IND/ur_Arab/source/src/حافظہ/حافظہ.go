...
func (self *TMemoryManager) Vآغاز_کرنا(start uint32, حجم uint32) {

	ActiveMemoryManager = self

	memoryChunkSize = uint32(Sizeof(TMemoryChunk{}))

	if حجم < memoryChunkSize {
		first = nil
	} else {
...
func (self *TMemoryManager) Vحافظہ_مختص_کرنا(حجم uint32) Pointer {
	...
	if result.حجم >= (حجم + memoryChunkSize + 1) {

		var temp *TMemoryChunk
		temp = (*TMemoryChunk)(Pointer(uintptr(uint32(uintptr(Pointer(result))) + memoryChunkSize + حجم)))

		temp.allocated = false
		temp.حجم = result.حجم - حجم - memoryChunkSize
		temp.prev = result
...
func (self *TMemoryManager) AlignedMalloc(حجم uint32) (Pointer, uint32) {
	...
	حجم += diff
	if result.حجم-حجم >= memoryChunkSize+1 {
		temp := (*TMemoryChunk)(Pointer(uintptr(Pointer(result)) + uintptr(memoryChunkSize) + uintptr(حجم)))
		temp.allocated = false
		temp.حجم = result.حجم - حجم - memoryChunkSize
		temp.prev = result
		temp.next = result.next
...
func (self *TMemoryManager) Vحافظہ_آزاد_کرنا(پتے_کا_حوالہ_2 Pointer) {
	var chunk *TMemoryChunk = (*TMemoryChunk)(Pointer(uintptr(پتے_کا_حوالہ_2) - uintptr(memoryChunkSize)))
	chunk.allocated = false

	if chunk.prev != nil && !chunk.prev.allocated {
		chunk.prev.next = chunk.next
		chunk.prev.حجم += chunk.حجم + memoryChunkSize
		if chunk.next != nil {
	...
	if chunk.next != nil && !chunk.next.allocated {
		chunk.حجم += chunk.next.حجم + memoryChunkSize
...
func New(حجم int) Pointer {
	if ActiveMemoryManager == nil {
		return nil
	}
	return ActiveMemoryManager.Vحافظہ_مختص_کرنا(uint32(حجم))
}
func Delete(پتے_کا_حوالہ_2 Pointer) {
	if ActiveMemoryManager != nil {
		ActiveMemoryManager.Vحافظہ_آزاد_کرنا(پتے_کا_حوالہ_2)
	}
}
