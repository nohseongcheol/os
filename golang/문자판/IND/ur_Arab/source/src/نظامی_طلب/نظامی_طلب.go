...
func (self *TSyscall) Vآغاز_کرنا(manager *TInterruptManager) {
	initFileDescriptors()
	...
	self.TInterruptHandler.Vآغاز_کرنا(0x80, uintptr(Pointer(manager)), addr)
}
...
func sysRead(fd int32, addr uint32, count uint32) int32 {
	...
	if entry.pos >= entry.حجم {
		return 0
	}
	remaining := entry.حجم - entry.pos
	if count > remaining {
		count = remaining
	}
	buf := GetBytesFromPtr(uintptr(addr), int(count), int(count))
...
func readVFSFile(entry *openFileDescription, dst []byte, count uint32) int32 {
	...
	tmpPtr := memoryManager.Vحافظہ_مختص_کرنا(entry.حجم)
	if tmpPtr == nil {
		return EINVAL
	}
	tmp := GetBytesFromPtr(uintptr(tmpPtr), int(entry.حجم), int(entry.حجم))
	readFile(entry.name[:entry.nameLen], tmp)
	copy(dst[:count], tmp[entry.pos:entry.pos+count])
	entry.pos += count
	memoryManager.Vحافظہ_آزاد_کرنا(tmpPtr)
	return int32(count)
}
...
func sysGetcwd(bufferAddr uint32, حجم uint32) int32 {
	if bufferAddr == 0 {
		return EFAULT
	}
	if حجم < 2 {
	...
	buffer := GetBytesFromPtr(uintptr(bufferAddr), int(حجم), int(حجم))
	buffer[0] = '/'
	buffer[1] = 0
	return 2
}

func fillPosixStat(statAddr uint32, mode uint32, حجم uint32, inode uint32) int32 {
	if statAddr == 0 {
		return EFAULT
	}
...
func sysStat(pathAddr uint32, statAddr uint32) int32 {
	...
	return fillPosixStat(statAddr, sIfreg|0444, حجم, inode)
}
...
func sysFstat(fd int32, statAddr uint32) int32 {
	...
	case fdKindRootDir:
		return fillPosixStat(statAddr, sIfdir|0555, 0, 1)
	case fdKindFat:
		return fillPosixStat(statAddr, sIfreg|0444, entry.حجم, uint32(fd+2))
	case fdKindSocket:
...
func socketSendTo(fd int32, bufferAddress uint32, length uint32, destinationAddress uint32, destinationLength uint32) int32 {
	...
	*packet = socketPacket{used: true, حجم: length, source: socket.local}
	if length != 0 {
...
func sysExecve(cpu *TCPUState, pathAddr uint32) int32 {
	...
	memoryManager := &mem.TMemoryManager{}
	filePtr := memoryManager.Vحافظہ_مختص_کرنا(حجم)
	if filePtr == nil {
		return EINVAL
	}
	data := GetBytesFromPtr(uintptr(filePtr), int(حجم), int(حجم))
	readFile(name[:nameLen], data)
	if حجم < 52 || data[0] != 0x7F || data[1] != 'E' || data[2] != 'L' || data[3] != 'F' {
		memoryManager.Vحافظہ_آزاد_کرنا(filePtr)
		return ENOEXEC
	}
	loader := Elf{}
...
func sysFork(cpu *TCPUState) int32 {
	...
	memoryManager := &mem.TMemoryManager{}
	threadPtr := memoryManager.Vحافظہ_مختص_کرنا(uint32(Sizeof(Tعملی_دھارا{})))
	stackPtr := memoryManager.Vحافظہ_مختص_کرنا(THREAD_STACK_SIZE)
	childPageDir := CloneAddressSpaceCOW(getCR3())
	if threadPtr == nil || stackPtr == nil || childPageDir == 0 {
		discardProcess(pid)
		return EINVAL
	}
	child := (*Tعملی_دھارا)(threadPtr)
...
func fileSize(filename []byte) uint32 {
	...
	حجم := bios.Len(&ata0s, partition.MBR.PrimaryPartition[0], filename)
	ata0s.Flush()
	return حجم
}
