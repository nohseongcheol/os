...
GLOBL ·ゲートHandlers<>(SB), NOPTR, $NUM_IDT_ENTRIES*8
...
TEXT ·Hトッテワリコミ(SB),NOSPLIT,$0-10
	...
	LEAQ ·ワリコミゲートentry(SB), SI // SI points to entry for trap 0
check_next_entry:
	TESTB CX, CX
	JZ update_idt_entry
...
#define INT_ENTRY_WITH_CODE(num) \
	SUBQ $16, SP;                          \
	MOVQ R15, 0(SP);                       \
	MOVQ ·ゲートHandlers<>+8*num(SB), R15;   \
	MOVQ R15, 8(SP);                       \
	LEAQ ·dispatchワリコミ(SB), R15;      \
	XCHGQ R15, 0(SP);                      \
	BYTE $0xc3;                            \
...
//--------------------------- -----------------------------------------
TEXT ·dispatchワリコミ(SB),NOSPLIT,$0
...
TEXT ·ワリコミゲートentry(SB),NOSPLIT,$0
