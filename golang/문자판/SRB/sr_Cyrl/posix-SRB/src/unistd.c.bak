...
ssize_t читати(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_читати, fd, buf, count));
}

ssize_t писати(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_писати, fd, buf, count));
}

int затворити(int fd)
{
    return (int)__syscall_result(SC1(SYS_затворити, fd));
}
