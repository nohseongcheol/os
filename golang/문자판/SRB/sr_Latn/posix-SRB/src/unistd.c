...
ssize_t čitati(int fd, void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_čitati, fd, buf, count));
}

ssize_t pisati(int fd, const void *buf, size_t count)
{
    return (ssize_t)__syscall_result(SC3(SYS_pisati, fd, buf, count));
}

int zatvoriti(int fd)
{
    return (int)__syscall_result(SC1(SYS_zatvoriti, fd));
}
