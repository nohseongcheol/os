...
type T고급기술결합 struct{
	...
	에러포트 T바이트입출력단자
	섹터갯수포트 T바이트입출력단자
	논리블록주소_낮은포트 T바이트입출력단자 	// logical block address 
	논리블록주소_중간포트 T바이트입출력단자 	// logical block address
	논리블록주소_높은포트 T바이트입출력단자 	// logical block address
	장치입출력단자 T바이트입출력단자
	명령입출력단자 T바이트입출력단자
	제어입출력단자 T바이트입출력단자
}
func (자신 *T고급기술결합) MTest(){
}
func (자신 *T고급기술결합) M초기화(마스터 bool, 포트기준 uint16){
        ...
        자신.장치입출력단자.M초기화(포트기준+0x6)
        자신.명령입출력단자.M초기화(포트기준+0x7)
        자신.제어입출력단자.M초기화(포트기준+0x8)
...
func (자신 *T고급기술결합)Identify() {
	...
	var 상태 uint8 = 자신.명령입출력단자.M읽기()
	if 상태 == 0xFF {
		단말기.M출력("Invalid Status")
		return
	}
	...
	상태 = 자신.명령입출력단자.M읽기()
	if 상태 == 0x00 {
		단말기.M출력("HDD Does not exist, ignoreign ")
		return
	}
	...
	단말기.M출력XY("ata ok", 10, 22)

}
func (자신 *T고급기술결합)M읽기28(섹터 uint32, 자료 *[]byte, 갯수 int) {
	var 단말기 = T단말기{}
	if (섹터 & 0xF0000000)!=0 {
		단말기.M출력(([]byte)("ata read error "))
		return
	}
	...
	if 자신.마스터 {
		자신.장치입출력단자.M쓰기(uint8(0xE0 | uint8((섹터 & 0x0F000000) >> 24)))
	}else {
		자신.장치입출력단자.M쓰기(uint8(0xF0 | uint8((섹터 & 0x0F000000) >> 24)))
	}
	자신.에러포트.M쓰기(0)
	자신.섹터갯수포트.M쓰기(1)
	...
	단말기.M출력XY("Reading ATA Drive:", 1, 24)
...
func (자신 *T고급기술결합) M쓰기28(섹터번호 uint32, 자료 []byte, 갯수 uint32){
	...
	if 갯수 > 512 {
        ...
        if 자신.마스터 {
                자신.장치입출력단자.M쓰기(uint8(0xE0 | uint8((섹터번호 & 0x0F000000) >> 24)))
        }else {
                자신.장치입출력단자.M쓰기(uint8(0xF0 | uint8((섹터번호 & 0x0F000000) >> 24)))
        }
	...
	var 단말기 = T단말기{}
        단말기.M출력(([]byte)("Writing to ATA Drive:"))
