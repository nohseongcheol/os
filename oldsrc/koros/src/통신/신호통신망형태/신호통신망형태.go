...
type I신호통신망형태_처리기 interface{
	M신호통신망형태_받는동시(자료주소 uintptr, 크기 uint32) bool
	M신호보내기(목적지_매체주소_역순 uint64, 신호통신유형_역순 uint16, 자료주소 uintptr, 크기 uint32) // Frame 보내기..
}
/////////////////////////////////////////////////////////////////////////////
type T신호통신망형태_처리기_자료 struct{
	신호통신망형태_머리말 T신호통신망형태_머리말		// EthernetFrame header
	후단부 T신호통신망형태_제공자				// EthernetFrame Provider
}
var 처리기_자료 T신호통신망형태_처리기_자료
...
func (자신 *T신호통신망형태_처리기) M처리기설정(p_처리기 I신호통신망형태_처리기, p_etherType uint16){
	자료.신호통신망형태_처리기들[p_etherType] = p_처리기
}
func (자신 *T신호통신망형태_처리기)M신호통신망형태_받는동시(자료주소 uintptr, 크기 int32) bool{
	신호통신단말기.M출력("OnEtherFrameReceived")
	return false
}
func (자신 *T신호통신망형태_처리기)M신호보내기(목적지_매체주소_역순 uint64, 신호통신유형_역순 uint16, 자료주소 uintptr, 크기 uint32) {
	처리기_자료.후단부.M신호보내기(목적지_매체주소_역순, 신호통신유형_역순, 자료주소, 크기)	
}
func (자신 *T신호통신망형태_처리기)M매체주소_갖기() uint64{
	return 처리기_자료.후단부.M매체주소_갖기()
}
func (자신 *T신호통신망형태_처리기)M상호통신망주소_갖기() uint64{
	return 처리기_자료.후단부.M상호통신망주소_갖기()
}
/////////////////////////////////////////////////////////////////////////////
...
func (자신 *T신호통신망형태_제공자)M초기화(p_후단부 Tamd_am79c973) {
	자료.넷카드 = p_후단부
	for i:=0; i<65535; i++ {
		자료.신호통신망형태_처리기들[i] = nil
	}
}
func (자신 *T신호통신망형태_제공자)M원시자료_받은동시(자료주소 uintptr, 크기 uint32) bool{
	...
	if 신호통신망형태_머리말.목적지_매체주소_역순 == 0x0000FFFFFFFFFFFF || Uint48_R(신호통신망형태_머리말.목적지_매체주소_역순) == 자신.M매체주소_갖기(){
		if 자료.신호통신망형태_처리기들[신호통신망형태_머리말.신호통신유형_역순] != nil {
			var 주소 uintptr = uintptr(Pointer(자료주소)) + uintptr(신호통신망형태_머리말_크기)	
			sendBack = 자료.신호통신망형태_처리기들[신호통신망형태_머리말.신호통신유형_역순].M신호통신망형태_받는동시(주소, 크기 - 신호통신망형태_머리말_크기)
		}
	}
...
func (자신 *T신호통신망형태_제공자)M보내기(자료주소 uintptr, 크기 uint32){
	자료.넷카드.M보내기(자료주소, 크기)
}
func (자신 *T신호통신망형태_제공자)M신호보내기(목적지_매체주소_역순 uint64, 신호통신유형_역순 uint16, 자료주소 uintptr, 크기 uint32){
	...
	신호통신망형태_머리말.출발지_매체주소_역순 = Uint48_R(자료.넷카드.M매체주소_갖기())
	...
	자료.넷카드.M보내기(주소, 크기+uint32(신호통신망형태_머리말_크기))
	
}
func (자신 *T신호통신망형태_제공자)M매체주소_갖기() uint64{
	return 자료.넷카드.M매체주소_갖기()
}
func (자신 *T신호통신망형태_제공자)M상호통신망주소_갖기() uint64{
	return 자료.넷카드.M논리주소_갖기()
}
