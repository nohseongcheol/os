...
type T사용자자료단위규약_접속연결점 struct{
	...
	P사용자자료단위규약_제공자 T사용자자료단위규약_제공자
	P사용자자료단위규약_처리기 I사용자자료단위규약_처리기
}

func (자신 *T사용자자료단위규약_접속연결점) M초기화(p_사용자자료단위규약_제공자 T사용자자료단위규약_제공자){ 
	자신.P사용자자료단위규약_제공자 = p_사용자자료단위규약_제공자
	/*
	if 자신.P사용자자료단위규약_처리기 == nil  && p_사용자자료단위규약_처리기 != nil{
		자신.P사용자자료단위규약_처리기 = p_사용자자료단위규약_처리기
	}
	*/
	자신.신호수신대기중 = false
}
func (자신 *T사용자자료단위규약_접속연결점) 사용자자료단위규약_메시지_처리하기(data uintptr, 크기 uint16){
	...
	if 자신.P사용자자료단위규약_처리기 != nil {
		자신.P사용자자료단위규약_처리기.사용자자료단위규약_메시지_처리하기(자신, data, 크기)
	}
}
func (자신 *T사용자자료단위규약_접속연결점) M보내기(p_자료 []byte, 크기 uint16){
	...
	자신.P사용자자료단위규약_제공자.M보내기(자신, data, 크기)
}
func (자신 *T사용자자료단위규약_접속연결점) M연결끊기(){
	자신.P사용자자료단위규약_제공자.M연결끊기(자신)
}
///////////////////////////////////////////////////////////////////////////
type T사용자자료단위규약_제공자_자료 struct{
	접속연결점들 [65535] T사용자자료단위규약_접속연결점
	접속연결점들_갯수 int
	여유입출구번호 uint16 // freePort
}
var 자료 T사용자자료단위규약_제공자_자료

type T사용자자료단위규약_제공자 struct{
	T상호통신망규약_처리기
}
func (자신 *T사용자자료단위규약_제공자) M초기화(p_ipProvider T상호통신망규약_제공자){
	자신.T상호통신망규약_처리기.M초기화(p_ipProvider, 자신, 0x11)
	자료.접속연결점들_갯수 = 0
	자료.여유입출구번호 = 1024
}
func (자신 *T사용자자료단위규약_제공자) M상호통신망규약_받은동시(출발지IP주소_역순 uint32, 목적지IP주소_역순 uint32, 상호통신망규약_탑재자료 uintptr, 크기 uint32) bool{
	if 크기 < 사용자규약_머리글_크기 {
		return false
	}
	
	var 임시저장공간 *T사용자자료단위규약_머리말_임시저장공간 = (*T사용자자료단위규약_머리말_임시저장공간)(Pointer(상호통신망규약_탑재자료))
	...
	for i:=0; i<자료.접속연결점들_갯수 && 접속연결점==nil; i++ {

		if 자료.접속연결점들[i].자신입출구번호 == 머리말.목적지입출구번호 && 
		   자료.접속연결점들[i].자신IP주소 == 목적지IP주소_역순 && 
		   자료.접속연결점들[i].신호수신대기중 == true 	{

			접속연결점 = &자료.접속연결점들[i]
			접속연결점.신호수신대기중 = false
			접속연결점.원격입출구번호 = 머리말.출발지입출구번호
			접속연결점.원격IP주소 = 출발지IP주소_역순

		}else if 자료.접속연결점들[i].자신입출구번호 == 머리말.목적지입출구번호 && 
		  자료.접속연결점들[i].자신IP주소 == 목적지IP주소_역순 && 
		  자료.접속연결점들[i].원격입출구번호 == 머리말.출발지입출구번호 && 
		  자료.접속연결점들[i].원격IP주소 == 출발지IP주소_역순	{
	...
	if 접속연결점 != nil {
		접속연결점.사용자자료단위규약_메시지_처리하기(상호통신망규약_탑재자료 + uintptr(사용자규약_머리글_크기), uint16(크기 - 사용자규약_머리글_크기))
	}
...
func (자신 *T사용자자료단위규약_제공자) M연결하기(상호통신망_주소 uint32, 입출구번호 uint16) *T사용자자료단위규약_접속연결점{
	...
	var 접속연결점 = (*T사용자자료단위규약_접속연결점)(기억공간관리자.MM할당(50))
...
func (자신 *T사용자자료단위규약_제공자) M수신대기(입출구번호 uint16) *T사용자자료단위규약_접속연결점{
	var 기억공간관리자 = &T기억공간관리자{}
	var 접속연결점 = (*T사용자자료단위규약_접속연결점)(기억공간관리자.MM할당(50))
	if 접속연결점 != nil {
		...
		접속연결점.자신입출구번호 = 접속연결점.자신입출구번호

		//단말기.M출력XY(접속연결점.자신입출구번호, 5, 19)
	}
	return 접속연결점
}
func (자신 *T사용자자료단위규약_제공자) M연결끊기(접속연결점 *T사용자자료단위규약_접속연결점){
	for i:=0; i<자료.접속연결점들_갯수 && 접속연결점==nil; i++ {
		if 자료.접속연결점들[i] == *접속연결점 {
			자료.접속연결점들_갯수--
			자료.접속연결점들[i] = 자료.접속연결점들[자료.접속연결점들_갯수]
			break
		}
	}
}
func (자신 *T사용자자료단위규약_제공자) M보내기(접속연결점 *T사용자자료단위규약_접속연결점, p_자료 uintptr, 크기 uint16){
	...
	자신.T상호통신망규약_처리기.M패킷보내기(접속연결점.원격IP주소, 0x11, 자료주소, 전체길이)
	
}
func (자신 *T사용자자료단위규약_제공자) M결합하기(접속연결점 *T사용자자료단위규약_접속연결점, 처리기 *T사용자자료단위규약_처리기){
	접속연결점.P사용자자료단위규약_처리기 = 처리기
}
