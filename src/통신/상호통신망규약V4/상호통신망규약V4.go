...
func (자신 *T상호통신망규약_제공자) M신호통신망형태_받는동시(신호통신망형태_탑재자료 uintptr, 크기 uint32) bool{

	if 크기 < uint32(상호통신망규약_머리말_크기) {
		return false
	}
	
	var 임시저장공간 *T상호통신망규약V4메시지_임시저장공간 = (*T상호통신망규약V4메시지_임시저장공간)(Pointer(신호통신망형태_탑재자료))
	...
	if 메시지.목적지IP주소 == uint32(자신.M상호통신망주소_갖기()) {
		...
		if 자료.상호통신망규약_처리기들[메시지.규약] != nil {
			sendBack  = 자료.상호통신망규약_처리기들[메시지.규약].M상호통신망규약_받은동시(메시지.출발지IP주소, 
									메시지.목적지IP주소, 
									신호통신망형태_탑재자료 + uintptr(4*메시지.머리말_길이), 
	...
	if(sendBack) {
		...
		메시지.검사합 = 자신.M검사합((*([4096]uint16))(Pointer(신호통신망형태_탑재자료)), uint32(4*메시지.머리말_길이))
		메시지.M임시저장공간_설정(임시저장공간)
